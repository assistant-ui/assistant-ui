...
**Blocks are spaced by the surface.** The tree now sits in a `[data-aui="root"]` element that carries the vertical rhythm. `renderGenerativeUI` is unchanged and still returns exactly what it is given, so embedding a single node in your own layout works as before.
