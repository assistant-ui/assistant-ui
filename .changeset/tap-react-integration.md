...
- Also exports `resource`, `withKey`, `createResourceRoot`, `flushResourcesSync`, the experimental `useReducerWithDerivedState` (a `getDerivedStateFromProps` replacement) and `createResourceContext` / `withContextProvider` context API, the `Resource` / `ContravariantResource` / `ResourceElement` types, and `peekResourceFiber()` (a non-throwing probe for the resource render context).
