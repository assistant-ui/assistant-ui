...
- In dev mode (`NODE_ENV !== "production"`), `@assistant-ui/core` now warns once via `console.warn` when it detects that another version of `@assistant-ui/core` is already loaded in the same JavaScript runtime. Mismatched transitive versions are a common source of subtle bugs (lost tool registrations, broken context lookups, failed `instanceof` checks — see issue #4101). The warning points users at `npx assistant-ui doctor`.
