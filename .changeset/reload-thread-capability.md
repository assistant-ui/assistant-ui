...
`react-google-adk` has a remote session load path but does not register the capability yet, so reloads there take the remount fallback; porting the same `onReloadThread` wiring is a follow-up.
