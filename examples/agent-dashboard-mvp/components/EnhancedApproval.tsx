...
function ApprovalContent({ className }: { className?: string }) {
  ...
  const formatInput = () => {
    ...
    if ((toolName === "Read" || toolName === "Write") && input.path) {
  ...
  const getActionPreview = () => {
    ...
    switch (toolName) {
      ...
      case "Edit":
        return `Edit file: ${input.path || input.file_path}`;
      case "Glob":
        return `Search for files matching: ${input.pattern}`;
      case "Grep":
        return `Search for: ${input.pattern || input.query}`;
      default:
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1 text-muted-foreground text-xs">
            ...
            <span>
              ...
              approve or <kbd className="rounded bg-muted px-1.5 py-0.5">N</kbd>{" "}
              to deny
            </span>
          </div>
...
export function EnhancedApproval({
  ...
  return (
    <ApprovalPrimitive.Root approvalId={approvalId}>
      <ApprovalContent className={className} />
