...
The proxy accepts same-origin browser requests so the example works without exposing `LANGCHAIN_API_KEY` to the client. Origin checks are not user authentication. Before deploying with a cloud API key, require your application session in the proxy route and apply a durable rate limit.
