...
- `server/api/chat.post.ts` runs `streamText` and returns the AI SDK UI message stream, the same server shape as the Next.js templates.
...
The first `pnpm dev` (or `pnpm build`) also generates `.nuxt/`, which `tsconfig.json` extends.
