...
- The bridge owns the tools it registers: it only ever unregisters names it registered. The pre-registration collision check needs a *synchronous* `getTools()`; the spec's asynchronous `getTools()` cannot be consulted before `registerTool` returns, so on those browsers the check never fires. Without it — and without enumeration at all — the bridge relies on `registerTool` throwing or rejecting for duplicates, warns, and drops its own registration; an implementation that silently accepts duplicates can still end up with the last registration winning.
