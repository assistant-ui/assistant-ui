var jn =
  typeof globalThis < "u"
    ? globalThis
    : typeof window < "u"
      ? window
      : typeof global < "u"
        ? global
        : typeof self < "u"
          ? self
          : {};
function kv(h) {
  return h && h.__esModule && Object.prototype.hasOwnProperty.call(h, "default")
    ? h.default
    : h;
}
var Pc = { exports: {} },
  bu = {}; /**
 * @license React
 * react-jsx-runtime.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var ed;
function $v() {
  if (ed) return bu;
  ed = 1;
  var h = Symbol.for("react.transitional.element"),
    p = Symbol.for("react.fragment");
  function _(s, N, q) {
    var V = null;
    if (
      (q !== void 0 && (V = "" + q),
      N.key !== void 0 && (V = "" + N.key),
      "key" in N)
    ) {
      q = {};
      for (var el in N) el !== "key" && (q[el] = N[el]);
    } else q = N;
    return (
      (N = q.ref),
      { $$typeof: h, type: s, key: V, ref: N !== void 0 ? N : null, props: q }
    );
  }
  return (bu.Fragment = p), (bu.jsx = _), (bu.jsxs = _), bu;
}
var ud;
function Wv() {
  return ud || ((ud = 1), (Pc.exports = $v())), Pc.exports;
}
var L = Wv(),
  lf = { exports: {} },
  K = {}; /**
 * @license React
 * react.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var nd;
function Fv() {
  if (nd) return K;
  nd = 1;
  var h = Symbol.for("react.transitional.element"),
    p = Symbol.for("react.portal"),
    _ = Symbol.for("react.fragment"),
    s = Symbol.for("react.strict_mode"),
    N = Symbol.for("react.profiler"),
    q = Symbol.for("react.consumer"),
    V = Symbol.for("react.context"),
    el = Symbol.for("react.forward_ref"),
    O = Symbol.for("react.suspense"),
    b = Symbol.for("react.memo"),
    x = Symbol.for("react.lazy"),
    $ = Symbol.iterator;
  function J(o) {
    return o === null || typeof o != "object"
      ? null
      : ((o = ($ && o[$]) || o["@@iterator"]),
        typeof o == "function" ? o : null);
  }
  var bl = {
      isMounted: function () {
        return !1;
      },
      enqueueForceUpdate: function () {},
      enqueueReplaceState: function () {},
      enqueueSetState: function () {},
    },
    Tl = Object.assign,
    zl = {};
  function dl(o, A, U) {
    (this.props = o),
      (this.context = A),
      (this.refs = zl),
      (this.updater = U || bl);
  }
  (dl.prototype.isReactComponent = {}),
    (dl.prototype.setState = function (o, A) {
      if (typeof o != "object" && typeof o != "function" && o != null)
        throw Error(
          "takes an object of state variables to update or a function which returns an object of state variables.",
        );
      this.updater.enqueueSetState(this, o, A, "setState");
    }),
    (dl.prototype.forceUpdate = function (o) {
      this.updater.enqueueForceUpdate(this, o, "forceUpdate");
    });
  function Ol() {}
  Ol.prototype = dl.prototype;
  function Ml(o, A, U) {
    (this.props = o),
      (this.context = A),
      (this.refs = zl),
      (this.updater = U || bl);
  }
  var vl = (Ml.prototype = new Ol());
  (vl.constructor = Ml), Tl(vl, dl.prototype), (vl.isPureReactComponent = !0);
  var pl = Array.isArray,
    R = { H: null, A: null, T: null, S: null, V: null },
    X = Object.prototype.hasOwnProperty;
  function ul(o, A, U, D, j, P) {
    return (
      (U = P.ref),
      { $$typeof: h, type: o, key: A, ref: U !== void 0 ? U : null, props: P }
    );
  }
  function cl(o, A) {
    return ul(o.type, A, void 0, void 0, void 0, o.props);
  }
  function El(o) {
    return typeof o == "object" && o !== null && o.$$typeof === h;
  }
  function Ul(o) {
    var A = { "=": "=0", ":": "=2" };
    return (
      "$" +
      o.replace(/[=:]/g, function (U) {
        return A[U];
      })
    );
  }
  var Hl = /\/+/g;
  function fl(o, A) {
    return typeof o == "object" && o !== null && o.key != null
      ? Ul("" + o.key)
      : A.toString(36);
  }
  function $l() {}
  function et(o) {
    switch (o.status) {
      case "fulfilled":
        return o.value;
      case "rejected":
        throw o.reason;
      default:
        switch (
          (typeof o.status == "string"
            ? o.then($l, $l)
            : ((o.status = "pending"),
              o.then(
                function (A) {
                  o.status === "pending" &&
                    ((o.status = "fulfilled"), (o.value = A));
                },
                function (A) {
                  o.status === "pending" &&
                    ((o.status = "rejected"), (o.reason = A));
                },
              )),
          o.status)
        ) {
          case "fulfilled":
            return o.value;
          case "rejected":
            throw o.reason;
        }
    }
    throw o;
  }
  function Yl(o, A, U, D, j) {
    var P = typeof o;
    (P === "undefined" || P === "boolean") && (o = null);
    var Q = !1;
    if (o === null) Q = !0;
    else
      switch (P) {
        case "bigint":
        case "string":
        case "number":
          Q = !0;
          break;
        case "object":
          switch (o.$$typeof) {
            case h:
            case p:
              Q = !0;
              break;
            case x:
              return (Q = o._init), Yl(Q(o._payload), A, U, D, j);
          }
      }
    if (Q)
      return (
        (j = j(o)),
        (Q = D === "" ? "." + fl(o, 0) : D),
        pl(j)
          ? ((U = ""),
            Q != null && (U = Q.replace(Hl, "$&/") + "/"),
            Yl(j, A, U, "", function (Fl) {
              return Fl;
            }))
          : j != null &&
            (El(j) &&
              (j = cl(
                j,
                U +
                  (j.key == null || (o && o.key === j.key)
                    ? ""
                    : ("" + j.key).replace(Hl, "$&/") + "/") +
                  Q,
              )),
            A.push(j)),
        1
      );
    Q = 0;
    var tl = D === "" ? "." : D + ":";
    if (pl(o))
      for (var al = 0; al < o.length; al++)
        (D = o[al]), (P = tl + fl(D, al)), (Q += Yl(D, A, U, P, j));
    else if (((al = J(o)), typeof al == "function"))
      for (o = al.call(o), al = 0; !(D = o.next()).done; )
        (D = D.value), (P = tl + fl(D, al++)), (Q += Yl(D, A, U, P, j));
    else if (P === "object") {
      if (typeof o.then == "function") return Yl(et(o), A, U, D, j);
      throw (
        ((A = String(o)),
        Error(
          "Objects are not valid as a React child (found: " +
            (A === "[object Object]"
              ? "object with keys {" + Object.keys(o).join(", ") + "}"
              : A) +
            "). If you meant to render a collection of children, use an array instead.",
        ))
      );
    }
    return Q;
  }
  function S(o, A, U) {
    if (o == null) return o;
    var D = [],
      j = 0;
    return (
      Yl(o, D, "", "", function (P) {
        return A.call(U, P, j++);
      }),
      D
    );
  }
  function M(o) {
    if (o._status === -1) {
      var A = o._result;
      (A = A()),
        A.then(
          function (U) {
            (o._status === 0 || o._status === -1) &&
              ((o._status = 1), (o._result = U));
          },
          function (U) {
            (o._status === 0 || o._status === -1) &&
              ((o._status = 2), (o._result = U));
          },
        ),
        o._status === -1 && ((o._status = 0), (o._result = A));
    }
    if (o._status === 1) return o._result.default;
    throw o._result;
  }
  var Y =
    typeof reportError == "function"
      ? reportError
      : function (o) {
          if (
            typeof window == "object" &&
            typeof window.ErrorEvent == "function"
          ) {
            var A = new window.ErrorEvent("error", {
              bubbles: !0,
              cancelable: !0,
              message:
                typeof o == "object" &&
                o !== null &&
                typeof o.message == "string"
                  ? String(o.message)
                  : String(o),
              error: o,
            });
            if (!window.dispatchEvent(A)) return;
          } else if (
            typeof process == "object" &&
            typeof process.emit == "function"
          ) {
            process.emit("uncaughtException", o);
            return;
          }
          console.error(o);
        };
  function sl() {}
  return (
    (K.Children = {
      map: S,
      forEach: function (o, A, U) {
        S(
          o,
          function () {
            A.apply(this, arguments);
          },
          U,
        );
      },
      count: function (o) {
        var A = 0;
        return (
          S(o, function () {
            A++;
          }),
          A
        );
      },
      toArray: function (o) {
        return (
          S(o, function (A) {
            return A;
          }) || []
        );
      },
      only: function (o) {
        if (!El(o))
          throw Error(
            "React.Children.only expected to receive a single React element child.",
          );
        return o;
      },
    }),
    (K.Component = dl),
    (K.Fragment = _),
    (K.Profiler = N),
    (K.PureComponent = Ml),
    (K.StrictMode = s),
    (K.Suspense = O),
    (K.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE = R),
    (K.__COMPILER_RUNTIME = {
      __proto__: null,
      c: function (o) {
        return R.H.useMemoCache(o);
      },
    }),
    (K.cache = function (o) {
      return function () {
        return o.apply(null, arguments);
      };
    }),
    (K.cloneElement = function (o, A, U) {
      if (o == null)
        throw Error(
          "The argument must be a React element, but you passed " + o + ".",
        );
      var D = Tl({}, o.props),
        j = o.key,
        P = void 0;
      if (A != null)
        for (Q in (A.ref !== void 0 && (P = void 0),
        A.key !== void 0 && (j = "" + A.key),
        A))
          !X.call(A, Q) ||
            Q === "key" ||
            Q === "__self" ||
            Q === "__source" ||
            (Q === "ref" && A.ref === void 0) ||
            (D[Q] = A[Q]);
      var Q = arguments.length - 2;
      if (Q === 1) D.children = U;
      else if (1 < Q) {
        for (var tl = Array(Q), al = 0; al < Q; al++)
          tl[al] = arguments[al + 2];
        D.children = tl;
      }
      return ul(o.type, j, void 0, void 0, P, D);
    }),
    (K.createContext = function (o) {
      return (
        (o = {
          $$typeof: V,
          _currentValue: o,
          _currentValue2: o,
          _threadCount: 0,
          Provider: null,
          Consumer: null,
        }),
        (o.Provider = o),
        (o.Consumer = { $$typeof: q, _context: o }),
        o
      );
    }),
    (K.createElement = function (o, A, U) {
      var D,
        j = {},
        P = null;
      if (A != null)
        for (D in (A.key !== void 0 && (P = "" + A.key), A))
          X.call(A, D) &&
            D !== "key" &&
            D !== "__self" &&
            D !== "__source" &&
            (j[D] = A[D]);
      var Q = arguments.length - 2;
      if (Q === 1) j.children = U;
      else if (1 < Q) {
        for (var tl = Array(Q), al = 0; al < Q; al++)
          tl[al] = arguments[al + 2];
        j.children = tl;
      }
      if (o && o.defaultProps)
        for (D in ((Q = o.defaultProps), Q)) j[D] === void 0 && (j[D] = Q[D]);
      return ul(o, P, void 0, void 0, null, j);
    }),
    (K.createRef = function () {
      return { current: null };
    }),
    (K.forwardRef = function (o) {
      return { $$typeof: el, render: o };
    }),
    (K.isValidElement = El),
    (K.lazy = function (o) {
      return { $$typeof: x, _payload: { _status: -1, _result: o }, _init: M };
    }),
    (K.memo = function (o, A) {
      return { $$typeof: b, type: o, compare: A === void 0 ? null : A };
    }),
    (K.startTransition = function (o) {
      var A = R.T,
        U = {};
      R.T = U;
      try {
        var D = o(),
          j = R.S;
        j !== null && j(U, D),
          typeof D == "object" &&
            D !== null &&
            typeof D.then == "function" &&
            D.then(sl, Y);
      } catch (P) {
        Y(P);
      } finally {
        R.T = A;
      }
    }),
    (K.unstable_useCacheRefresh = function () {
      return R.H.useCacheRefresh();
    }),
    (K.use = function (o) {
      return R.H.use(o);
    }),
    (K.useActionState = function (o, A, U) {
      return R.H.useActionState(o, A, U);
    }),
    (K.useCallback = function (o, A) {
      return R.H.useCallback(o, A);
    }),
    (K.useContext = function (o) {
      return R.H.useContext(o);
    }),
    (K.useDebugValue = function () {}),
    (K.useDeferredValue = function (o, A) {
      return R.H.useDeferredValue(o, A);
    }),
    (K.useEffect = function (o, A, U) {
      var D = R.H;
      if (typeof U == "function")
        throw Error(
          "useEffect CRUD overload is not enabled in this build of React.",
        );
      return D.useEffect(o, A);
    }),
    (K.useId = function () {
      return R.H.useId();
    }),
    (K.useImperativeHandle = function (o, A, U) {
      return R.H.useImperativeHandle(o, A, U);
    }),
    (K.useInsertionEffect = function (o, A) {
      return R.H.useInsertionEffect(o, A);
    }),
    (K.useLayoutEffect = function (o, A) {
      return R.H.useLayoutEffect(o, A);
    }),
    (K.useMemo = function (o, A) {
      return R.H.useMemo(o, A);
    }),
    (K.useOptimistic = function (o, A) {
      return R.H.useOptimistic(o, A);
    }),
    (K.useReducer = function (o, A, U) {
      return R.H.useReducer(o, A, U);
    }),
    (K.useRef = function (o) {
      return R.H.useRef(o);
    }),
    (K.useState = function (o) {
      return R.H.useState(o);
    }),
    (K.useSyncExternalStore = function (o, A, U) {
      return R.H.useSyncExternalStore(o, A, U);
    }),
    (K.useTransition = function () {
      return R.H.useTransition();
    }),
    (K.version = "19.1.1"),
    K
  );
}
var id;
function sf() {
  return id || ((id = 1), (lf.exports = Fv())), lf.exports;
}
var I = sf();
const Iv = kv(I);
var tf = { exports: {} },
  Tu = {},
  af = { exports: {} },
  ef = {}; /**
 * @license React
 * scheduler.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var cd;
function Pv() {
  return (
    cd ||
      ((cd = 1),
      (function (h) {
        function p(S, M) {
          var Y = S.length;
          S.push(M);
          for (; 0 < Y; ) {
            var sl = (Y - 1) >>> 1,
              o = S[sl];
            if (0 < N(o, M)) (S[sl] = M), (S[Y] = o), (Y = sl);
            else break;
          }
        }
        function _(S) {
          return S.length === 0 ? null : S[0];
        }
        function s(S) {
          if (S.length === 0) return null;
          var M = S[0],
            Y = S.pop();
          if (Y !== M) {
            S[0] = Y;
            for (var sl = 0, o = S.length, A = o >>> 1; sl < A; ) {
              var U = 2 * (sl + 1) - 1,
                D = S[U],
                j = U + 1,
                P = S[j];
              if (0 > N(D, Y))
                j < o && 0 > N(P, D)
                  ? ((S[sl] = P), (S[j] = Y), (sl = j))
                  : ((S[sl] = D), (S[U] = Y), (sl = U));
              else if (j < o && 0 > N(P, Y)) (S[sl] = P), (S[j] = Y), (sl = j);
              else break;
            }
          }
          return M;
        }
        function N(S, M) {
          var Y = S.sortIndex - M.sortIndex;
          return Y !== 0 ? Y : S.id - M.id;
        }
        if (
          ((h.unstable_now = void 0),
          typeof performance == "object" &&
            typeof performance.now == "function")
        ) {
          var q = performance;
          h.unstable_now = function () {
            return q.now();
          };
        } else {
          var V = Date,
            el = V.now();
          h.unstable_now = function () {
            return V.now() - el;
          };
        }
        var O = [],
          b = [],
          x = 1,
          $ = null,
          J = 3,
          bl = !1,
          Tl = !1,
          zl = !1,
          dl = !1,
          Ol = typeof setTimeout == "function" ? setTimeout : null,
          Ml = typeof clearTimeout == "function" ? clearTimeout : null,
          vl = typeof setImmediate < "u" ? setImmediate : null;
        function pl(S) {
          for (var M = _(b); M !== null; ) {
            if (M.callback === null) s(b);
            else if (M.startTime <= S)
              s(b), (M.sortIndex = M.expirationTime), p(O, M);
            else break;
            M = _(b);
          }
        }
        function R(S) {
          if (((zl = !1), pl(S), !Tl))
            if (_(O) !== null) (Tl = !0), X || ((X = !0), fl());
            else {
              var M = _(b);
              M !== null && Yl(R, M.startTime - S);
            }
        }
        var X = !1,
          ul = -1,
          cl = 5,
          El = -1;
        function Ul() {
          return dl ? !0 : !(h.unstable_now() - El < cl);
        }
        function Hl() {
          if (((dl = !1), X)) {
            var S = h.unstable_now();
            El = S;
            var M = !0;
            try {
              l: {
                (Tl = !1), zl && ((zl = !1), Ml(ul), (ul = -1)), (bl = !0);
                var Y = J;
                try {
                  t: {
                    for (
                      pl(S), $ = _(O);
                      $ !== null && !($.expirationTime > S && Ul());
                    ) {
                      var sl = $.callback;
                      if (typeof sl == "function") {
                        ($.callback = null), (J = $.priorityLevel);
                        var o = sl($.expirationTime <= S);
                        if (((S = h.unstable_now()), typeof o == "function")) {
                          ($.callback = o), pl(S), (M = !0);
                          break t;
                        }
                        $ === _(O) && s(O), pl(S);
                      } else s(O);
                      $ = _(O);
                    }
                    if ($ !== null) M = !0;
                    else {
                      var A = _(b);
                      A !== null && Yl(R, A.startTime - S), (M = !1);
                    }
                  }
                  break l;
                } finally {
                  ($ = null), (J = Y), (bl = !1);
                }
                M = void 0;
              }
            } finally {
              M ? fl() : (X = !1);
            }
          }
        }
        var fl;
        if (typeof vl == "function")
          fl = function () {
            vl(Hl);
          };
        else if (typeof MessageChannel < "u") {
          var $l = new MessageChannel(),
            et = $l.port2;
          ($l.port1.onmessage = Hl),
            (fl = function () {
              et.postMessage(null);
            });
        } else
          fl = function () {
            Ol(Hl, 0);
          };
        function Yl(S, M) {
          ul = Ol(function () {
            S(h.unstable_now());
          }, M);
        }
        (h.unstable_IdlePriority = 5),
          (h.unstable_ImmediatePriority = 1),
          (h.unstable_LowPriority = 4),
          (h.unstable_NormalPriority = 3),
          (h.unstable_Profiling = null),
          (h.unstable_UserBlockingPriority = 2),
          (h.unstable_cancelCallback = function (S) {
            S.callback = null;
          }),
          (h.unstable_forceFrameRate = function (S) {
            0 > S || 125 < S
              ? console.error(
                  "forceFrameRate takes a positive int between 0 and 125, forcing frame rates higher than 125 fps is not supported",
                )
              : (cl = 0 < S ? Math.floor(1e3 / S) : 5);
          }),
          (h.unstable_getCurrentPriorityLevel = function () {
            return J;
          }),
          (h.unstable_next = function (S) {
            switch (J) {
              case 1:
              case 2:
              case 3:
                var M = 3;
                break;
              default:
                M = J;
            }
            var Y = J;
            J = M;
            try {
              return S();
            } finally {
              J = Y;
            }
          }),
          (h.unstable_requestPaint = function () {
            dl = !0;
          }),
          (h.unstable_runWithPriority = function (S, M) {
            switch (S) {
              case 1:
              case 2:
              case 3:
              case 4:
              case 5:
                break;
              default:
                S = 3;
            }
            var Y = J;
            J = S;
            try {
              return M();
            } finally {
              J = Y;
            }
          }),
          (h.unstable_scheduleCallback = function (S, M, Y) {
            var sl = h.unstable_now();
            switch (
              (typeof Y == "object" && Y !== null
                ? ((Y = Y.delay),
                  (Y = typeof Y == "number" && 0 < Y ? sl + Y : sl))
                : (Y = sl),
              S)
            ) {
              case 1:
                var o = -1;
                break;
              case 2:
                o = 250;
                break;
              case 5:
                o = 1073741823;
                break;
              case 4:
                o = 1e4;
                break;
              default:
                o = 5e3;
            }
            return (
              (o = Y + o),
              (S = {
                id: x++,
                callback: M,
                priorityLevel: S,
                startTime: Y,
                expirationTime: o,
                sortIndex: -1,
              }),
              Y > sl
                ? ((S.sortIndex = Y),
                  p(b, S),
                  _(O) === null &&
                    S === _(b) &&
                    (zl ? (Ml(ul), (ul = -1)) : (zl = !0), Yl(R, Y - sl)))
                : ((S.sortIndex = o),
                  p(O, S),
                  Tl || bl || ((Tl = !0), X || ((X = !0), fl()))),
              S
            );
          }),
          (h.unstable_shouldYield = Ul),
          (h.unstable_wrapCallback = function (S) {
            var M = J;
            return function () {
              var Y = J;
              J = M;
              try {
                return S.apply(this, arguments);
              } finally {
                J = Y;
              }
            };
          });
      })(ef)),
    ef
  );
}
var fd;
function ly() {
  return fd || ((fd = 1), (af.exports = Pv())), af.exports;
}
var uf = { exports: {} },
  kl = {}; /**
 * @license React
 * react-dom.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var sd;
function ty() {
  if (sd) return kl;
  sd = 1;
  var h = sf();
  function p(O) {
    var b = "https://react.dev/errors/" + O;
    if (1 < arguments.length) {
      b += "?args[]=" + encodeURIComponent(arguments[1]);
      for (var x = 2; x < arguments.length; x++)
        b += "&args[]=" + encodeURIComponent(arguments[x]);
    }
    return (
      "Minified React error #" +
      O +
      "; visit " +
      b +
      " for the full message or use the non-minified dev environment for full errors and additional helpful warnings."
    );
  }
  function _() {}
  var s = {
      d: {
        f: _,
        r: function () {
          throw Error(p(522));
        },
        D: _,
        C: _,
        L: _,
        m: _,
        X: _,
        S: _,
        M: _,
      },
      p: 0,
      findDOMNode: null,
    },
    N = Symbol.for("react.portal");
  function q(O, b, x) {
    var $ =
      3 < arguments.length && arguments[3] !== void 0 ? arguments[3] : null;
    return {
      $$typeof: N,
      key: $ == null ? null : "" + $,
      children: O,
      containerInfo: b,
      implementation: x,
    };
  }
  var V = h.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE;
  function el(O, b) {
    if (O === "font") return "";
    if (typeof b == "string") return b === "use-credentials" ? b : "";
  }
  return (
    (kl.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE = s),
    (kl.createPortal = function (O, b) {
      var x =
        2 < arguments.length && arguments[2] !== void 0 ? arguments[2] : null;
      if (!b || (b.nodeType !== 1 && b.nodeType !== 9 && b.nodeType !== 11))
        throw Error(p(299));
      return q(O, b, null, x);
    }),
    (kl.flushSync = function (O) {
      var b = V.T,
        x = s.p;
      try {
        if (((V.T = null), (s.p = 2), O)) return O();
      } finally {
        (V.T = b), (s.p = x), s.d.f();
      }
    }),
    (kl.preconnect = function (O, b) {
      typeof O == "string" &&
        (b
          ? ((b = b.crossOrigin),
            (b =
              typeof b == "string"
                ? b === "use-credentials"
                  ? b
                  : ""
                : void 0))
          : (b = null),
        s.d.C(O, b));
    }),
    (kl.prefetchDNS = function (O) {
      typeof O == "string" && s.d.D(O);
    }),
    (kl.preinit = function (O, b) {
      if (typeof O == "string" && b && typeof b.as == "string") {
        var x = b.as,
          $ = el(x, b.crossOrigin),
          J = typeof b.integrity == "string" ? b.integrity : void 0,
          bl = typeof b.fetchPriority == "string" ? b.fetchPriority : void 0;
        x === "style"
          ? s.d.S(O, typeof b.precedence == "string" ? b.precedence : void 0, {
              crossOrigin: $,
              integrity: J,
              fetchPriority: bl,
            })
          : x === "script" &&
            s.d.X(O, {
              crossOrigin: $,
              integrity: J,
              fetchPriority: bl,
              nonce: typeof b.nonce == "string" ? b.nonce : void 0,
            });
      }
    }),
    (kl.preinitModule = function (O, b) {
      if (typeof O == "string")
        if (typeof b == "object" && b !== null) {
          if (b.as == null || b.as === "script") {
            var x = el(b.as, b.crossOrigin);
            s.d.M(O, {
              crossOrigin: x,
              integrity: typeof b.integrity == "string" ? b.integrity : void 0,
              nonce: typeof b.nonce == "string" ? b.nonce : void 0,
            });
          }
        } else b == null && s.d.M(O);
    }),
    (kl.preload = function (O, b) {
      if (
        typeof O == "string" &&
        typeof b == "object" &&
        b !== null &&
        typeof b.as == "string"
      ) {
        var x = b.as,
          $ = el(x, b.crossOrigin);
        s.d.L(O, x, {
          crossOrigin: $,
          integrity: typeof b.integrity == "string" ? b.integrity : void 0,
          nonce: typeof b.nonce == "string" ? b.nonce : void 0,
          type: typeof b.type == "string" ? b.type : void 0,
          fetchPriority:
            typeof b.fetchPriority == "string" ? b.fetchPriority : void 0,
          referrerPolicy:
            typeof b.referrerPolicy == "string" ? b.referrerPolicy : void 0,
          imageSrcSet:
            typeof b.imageSrcSet == "string" ? b.imageSrcSet : void 0,
          imageSizes: typeof b.imageSizes == "string" ? b.imageSizes : void 0,
          media: typeof b.media == "string" ? b.media : void 0,
        });
      }
    }),
    (kl.preloadModule = function (O, b) {
      if (typeof O == "string")
        if (b) {
          var x = el(b.as, b.crossOrigin);
          s.d.m(O, {
            as: typeof b.as == "string" && b.as !== "script" ? b.as : void 0,
            crossOrigin: x,
            integrity: typeof b.integrity == "string" ? b.integrity : void 0,
          });
        } else s.d.m(O);
    }),
    (kl.requestFormReset = function (O) {
      s.d.r(O);
    }),
    (kl.unstable_batchedUpdates = function (O, b) {
      return O(b);
    }),
    (kl.useFormState = function (O, b, x) {
      return V.H.useFormState(O, b, x);
    }),
    (kl.useFormStatus = function () {
      return V.H.useHostTransitionStatus();
    }),
    (kl.version = "19.1.1"),
    kl
  );
}
var od;
function ay() {
  if (od) return uf.exports;
  od = 1;
  function h() {
    if (
      !(
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ > "u" ||
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE != "function"
      )
    )
      try {
        __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE(h);
      } catch (p) {
        console.error(p);
      }
  }
  return h(), (uf.exports = ty()), uf.exports;
} /**
 * @license React
 * react-dom-client.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var rd;
function ey() {
  if (rd) return Tu;
  rd = 1;
  var h = ly(),
    p = sf(),
    _ = ay();
  function s(l) {
    var t = "https://react.dev/errors/" + l;
    if (1 < arguments.length) {
      t += "?args[]=" + encodeURIComponent(arguments[1]);
      for (var a = 2; a < arguments.length; a++)
        t += "&args[]=" + encodeURIComponent(arguments[a]);
    }
    return (
      "Minified React error #" +
      l +
      "; visit " +
      t +
      " for the full message or use the non-minified dev environment for full errors and additional helpful warnings."
    );
  }
  function N(l) {
    return !(!l || (l.nodeType !== 1 && l.nodeType !== 9 && l.nodeType !== 11));
  }
  function q(l) {
    var t = l,
      a = l;
    if (l.alternate) for (; t.return; ) t = t.return;
    else {
      l = t;
      do (t = l), (t.flags & 4098) !== 0 && (a = t.return), (l = t.return);
      while (l);
    }
    return t.tag === 3 ? a : null;
  }
  function V(l) {
    if (l.tag === 13) {
      var t = l.memoizedState;
      if (
        (t === null && ((l = l.alternate), l !== null && (t = l.memoizedState)),
        t !== null)
      )
        return t.dehydrated;
    }
    return null;
  }
  function el(l) {
    if (q(l) !== l) throw Error(s(188));
  }
  function O(l) {
    var t = l.alternate;
    if (!t) {
      if (((t = q(l)), t === null)) throw Error(s(188));
      return t !== l ? null : l;
    }
    for (var a = l, e = t; ; ) {
      var u = a.return;
      if (u === null) break;
      var n = u.alternate;
      if (n === null) {
        if (((e = u.return), e !== null)) {
          a = e;
          continue;
        }
        break;
      }
      if (u.child === n.child) {
        for (n = u.child; n; ) {
          if (n === a) return el(u), l;
          if (n === e) return el(u), t;
          n = n.sibling;
        }
        throw Error(s(188));
      }
      if (a.return !== e.return) (a = u), (e = n);
      else {
        for (var i = !1, c = u.child; c; ) {
          if (c === a) {
            (i = !0), (a = u), (e = n);
            break;
          }
          if (c === e) {
            (i = !0), (e = u), (a = n);
            break;
          }
          c = c.sibling;
        }
        if (!i) {
          for (c = n.child; c; ) {
            if (c === a) {
              (i = !0), (a = n), (e = u);
              break;
            }
            if (c === e) {
              (i = !0), (e = n), (a = u);
              break;
            }
            c = c.sibling;
          }
          if (!i) throw Error(s(189));
        }
      }
      if (a.alternate !== e) throw Error(s(190));
    }
    if (a.tag !== 3) throw Error(s(188));
    return a.stateNode.current === a ? l : t;
  }
  function b(l) {
    var t = l.tag;
    if (t === 5 || t === 26 || t === 27 || t === 6) return l;
    for (l = l.child; l !== null; ) {
      if (((t = b(l)), t !== null)) return t;
      l = l.sibling;
    }
    return null;
  }
  var x = Object.assign,
    $ = Symbol.for("react.element"),
    J = Symbol.for("react.transitional.element"),
    bl = Symbol.for("react.portal"),
    Tl = Symbol.for("react.fragment"),
    zl = Symbol.for("react.strict_mode"),
    dl = Symbol.for("react.profiler"),
    Ol = Symbol.for("react.provider"),
    Ml = Symbol.for("react.consumer"),
    vl = Symbol.for("react.context"),
    pl = Symbol.for("react.forward_ref"),
    R = Symbol.for("react.suspense"),
    X = Symbol.for("react.suspense_list"),
    ul = Symbol.for("react.memo"),
    cl = Symbol.for("react.lazy"),
    El = Symbol.for("react.activity"),
    Ul = Symbol.for("react.memo_cache_sentinel"),
    Hl = Symbol.iterator;
  function fl(l) {
    return l === null || typeof l != "object"
      ? null
      : ((l = (Hl && l[Hl]) || l["@@iterator"]),
        typeof l == "function" ? l : null);
  }
  var $l = Symbol.for("react.client.reference");
  function et(l) {
    if (l == null) return null;
    if (typeof l == "function")
      return l.$$typeof === $l ? null : l.displayName || l.name || null;
    if (typeof l == "string") return l;
    switch (l) {
      case Tl:
        return "Fragment";
      case dl:
        return "Profiler";
      case zl:
        return "StrictMode";
      case R:
        return "Suspense";
      case X:
        return "SuspenseList";
      case El:
        return "Activity";
    }
    if (typeof l == "object")
      switch (l.$$typeof) {
        case bl:
          return "Portal";
        case vl:
          return (l.displayName || "Context") + ".Provider";
        case Ml:
          return (l._context.displayName || "Context") + ".Consumer";
        case pl:
          var t = l.render;
          return (
            (l = l.displayName),
            l ||
              ((l = t.displayName || t.name || ""),
              (l = l !== "" ? "ForwardRef(" + l + ")" : "ForwardRef")),
            l
          );
        case ul:
          return (
            (t = l.displayName || null), t !== null ? t : et(l.type) || "Memo"
          );
        case cl:
          (t = l._payload), (l = l._init);
          try {
            return et(l(t));
          } catch {}
      }
    return null;
  }
  var Yl = Array.isArray,
    S = p.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE,
    M = _.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE,
    Y = { pending: !1, data: null, method: null, action: null },
    sl = [],
    o = -1;
  function A(l) {
    return { current: l };
  }
  function U(l) {
    0 > o || ((l.current = sl[o]), (sl[o] = null), o--);
  }
  function D(l, t) {
    o++, (sl[o] = l.current), (l.current = t);
  }
  var j = A(null),
    P = A(null),
    Q = A(null),
    tl = A(null);
  function al(l, t) {
    switch ((D(Q, t), D(P, l), D(j, null), t.nodeType)) {
      case 9:
      case 11:
        l = (l = t.documentElement) && (l = l.namespaceURI) ? Nr(l) : 0;
        break;
      default:
        if (((l = t.tagName), (t = t.namespaceURI)))
          (t = Nr(t)), (l = xr(t, l));
        else
          switch (l) {
            case "svg":
              l = 1;
              break;
            case "math":
              l = 2;
              break;
            default:
              l = 0;
          }
    }
    U(j), D(j, l);
  }
  function Fl() {
    U(j), U(P), U(Q);
  }
  function qa(l) {
    l.memoizedState !== null && D(tl, l);
    var t = j.current,
      a = xr(t, l.type);
    t !== a && (D(P, l), D(j, a));
  }
  function zu(l) {
    P.current === l && (U(j), U(P)),
      tl.current === l && (U(tl), (yu._currentValue = Y));
  }
  var Gn = Object.prototype.hasOwnProperty,
    Xn = h.unstable_scheduleCallback,
    Qn = h.unstable_cancelCallback,
    Od = h.unstable_shouldYield,
    Md = h.unstable_requestPaint,
    _t = h.unstable_now,
    Dd = h.unstable_getCurrentPriorityLevel,
    of = h.unstable_ImmediatePriority,
    rf = h.unstable_UserBlockingPriority,
    Eu = h.unstable_NormalPriority,
    Rd = h.unstable_LowPriority,
    df = h.unstable_IdlePriority,
    Ud = h.log,
    Nd = h.unstable_setDisableYieldValue,
    Ee = null,
    ut = null;
  function Jt(l) {
    if (
      (typeof Ud == "function" && Nd(l),
      ut && typeof ut.setStrictMode == "function")
    )
      try {
        ut.setStrictMode(Ee, l);
      } catch {}
  }
  var nt = Math.clz32 ? Math.clz32 : qd,
    xd = Math.log,
    Hd = Math.LN2;
  function qd(l) {
    return (l >>>= 0), l === 0 ? 32 : (31 - ((xd(l) / Hd) | 0)) | 0;
  }
  var Au = 256,
    pu = 4194304;
  function ma(l) {
    var t = l & 42;
    if (t !== 0) return t;
    switch (l & -l) {
      case 1:
        return 1;
      case 2:
        return 2;
      case 4:
        return 4;
      case 8:
        return 8;
      case 16:
        return 16;
      case 32:
        return 32;
      case 64:
        return 64;
      case 128:
        return 128;
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
        return l & 4194048;
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        return l & 62914560;
      case 67108864:
        return 67108864;
      case 134217728:
        return 134217728;
      case 268435456:
        return 268435456;
      case 536870912:
        return 536870912;
      case 1073741824:
        return 0;
      default:
        return l;
    }
  }
  function _u(l, t, a) {
    var e = l.pendingLanes;
    if (e === 0) return 0;
    var u = 0,
      n = l.suspendedLanes,
      i = l.pingedLanes;
    l = l.warmLanes;
    var c = e & 134217727;
    return (
      c !== 0
        ? ((e = c & ~n),
          e !== 0
            ? (u = ma(e))
            : ((i &= c),
              i !== 0
                ? (u = ma(i))
                : a || ((a = c & ~l), a !== 0 && (u = ma(a)))))
        : ((c = e & ~n),
          c !== 0
            ? (u = ma(c))
            : i !== 0
              ? (u = ma(i))
              : a || ((a = e & ~l), a !== 0 && (u = ma(a)))),
      u === 0
        ? 0
        : t !== 0 &&
            t !== u &&
            (t & n) === 0 &&
            ((n = u & -u),
            (a = t & -t),
            n >= a || (n === 32 && (a & 4194048) !== 0))
          ? t
          : u
    );
  }
  function Ae(l, t) {
    return (l.pendingLanes & ~(l.suspendedLanes & ~l.pingedLanes) & t) === 0;
  }
  function Bd(l, t) {
    switch (l) {
      case 1:
      case 2:
      case 4:
      case 8:
      case 64:
        return t + 250;
      case 16:
      case 32:
      case 128:
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
        return t + 5e3;
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        return -1;
      case 67108864:
      case 134217728:
      case 268435456:
      case 536870912:
      case 1073741824:
        return -1;
      default:
        return -1;
    }
  }
  function vf() {
    var l = Au;
    return (Au <<= 1), (Au & 4194048) === 0 && (Au = 256), l;
  }
  function yf() {
    var l = pu;
    return (pu <<= 1), (pu & 62914560) === 0 && (pu = 4194304), l;
  }
  function Zn(l) {
    for (var t = [], a = 0; 31 > a; a++) t.push(l);
    return t;
  }
  function pe(l, t) {
    (l.pendingLanes |= t),
      t !== 268435456 &&
        ((l.suspendedLanes = 0), (l.pingedLanes = 0), (l.warmLanes = 0));
  }
  function Yd(l, t, a, e, u, n) {
    var i = l.pendingLanes;
    (l.pendingLanes = a),
      (l.suspendedLanes = 0),
      (l.pingedLanes = 0),
      (l.warmLanes = 0),
      (l.expiredLanes &= a),
      (l.entangledLanes &= a),
      (l.errorRecoveryDisabledLanes &= a),
      (l.shellSuspendCounter = 0);
    var c = l.entanglements,
      f = l.expirationTimes,
      y = l.hiddenUpdates;
    for (a = i & ~a; 0 < a; ) {
      var T = 31 - nt(a),
        E = 1 << T;
      (c[T] = 0), (f[T] = -1);
      var m = y[T];
      if (m !== null)
        for (y[T] = null, T = 0; T < m.length; T++) {
          var g = m[T];
          g !== null && (g.lane &= -536870913);
        }
      a &= ~E;
    }
    e !== 0 && hf(l, e, 0),
      n !== 0 && u === 0 && l.tag !== 0 && (l.suspendedLanes |= n & ~(i & ~t));
  }
  function hf(l, t, a) {
    (l.pendingLanes |= t), (l.suspendedLanes &= ~t);
    var e = 31 - nt(t);
    (l.entangledLanes |= t),
      (l.entanglements[e] = l.entanglements[e] | 1073741824 | (a & 4194090));
  }
  function mf(l, t) {
    var a = (l.entangledLanes |= t);
    for (l = l.entanglements; a; ) {
      var e = 31 - nt(a),
        u = 1 << e;
      (u & t) | (l[e] & t) && (l[e] |= t), (a &= ~u);
    }
  }
  function Ln(l) {
    switch (l) {
      case 2:
        l = 1;
        break;
      case 8:
        l = 4;
        break;
      case 32:
        l = 16;
        break;
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        l = 128;
        break;
      case 268435456:
        l = 134217728;
        break;
      default:
        l = 0;
    }
    return l;
  }
  function Vn(l) {
    return (
      (l &= -l),
      2 < l ? (8 < l ? ((l & 134217727) !== 0 ? 32 : 268435456) : 8) : 2
    );
  }
  function gf() {
    var l = M.p;
    return l !== 0 ? l : ((l = window.event), l === void 0 ? 32 : Fr(l.type));
  }
  function jd(l, t) {
    var a = M.p;
    try {
      return (M.p = l), t();
    } finally {
      M.p = a;
    }
  }
  var wt = Math.random().toString(36).slice(2),
    Jl = "__reactFiber$" + wt,
    Il = "__reactProps$" + wt,
    Ba = "__reactContainer$" + wt,
    Kn = "__reactEvents$" + wt,
    Cd = "__reactListeners$" + wt,
    Gd = "__reactHandles$" + wt,
    Sf = "__reactResources$" + wt,
    _e = "__reactMarker$" + wt;
  function Jn(l) {
    delete l[Jl], delete l[Il], delete l[Kn], delete l[Cd], delete l[Gd];
  }
  function Ya(l) {
    var t = l[Jl];
    if (t) return t;
    for (var a = l.parentNode; a; ) {
      if ((t = a[Ba] || a[Jl])) {
        if (
          ((a = t.alternate),
          t.child !== null || (a !== null && a.child !== null))
        )
          for (l = Yr(l); l !== null; ) {
            if ((a = l[Jl])) return a;
            l = Yr(l);
          }
        return t;
      }
      (l = a), (a = l.parentNode);
    }
    return null;
  }
  function ja(l) {
    if ((l = l[Jl] || l[Ba])) {
      var t = l.tag;
      if (t === 5 || t === 6 || t === 13 || t === 26 || t === 27 || t === 3)
        return l;
    }
    return null;
  }
  function Oe(l) {
    var t = l.tag;
    if (t === 5 || t === 26 || t === 27 || t === 6) return l.stateNode;
    throw Error(s(33));
  }
  function Ca(l) {
    var t = l[Sf];
    return (
      t ||
        (t = l[Sf] =
          { hoistableStyles: new Map(), hoistableScripts: new Map() }),
      t
    );
  }
  function Gl(l) {
    l[_e] = !0;
  }
  var bf = new Set(),
    Tf = {};
  function ga(l, t) {
    Ga(l, t), Ga(l + "Capture", t);
  }
  function Ga(l, t) {
    for (Tf[l] = t, l = 0; l < t.length; l++) bf.add(t[l]);
  }
  var Xd =
      /^[:A-Z_a-z\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u02FF\u0370-\u037D\u037F-\u1FFF\u200C-\u200D\u2070-\u218F\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD][:A-Z_a-z\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u02FF\u0370-\u037D\u037F-\u1FFF\u200C-\u200D\u2070-\u218F\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD\-.0-9\u00B7\u0300-\u036F\u203F-\u2040]*$/,
    zf = {},
    Ef = {};
  function Qd(l) {
    return Gn.call(Ef, l)
      ? !0
      : Gn.call(zf, l)
        ? !1
        : Xd.test(l)
          ? (Ef[l] = !0)
          : ((zf[l] = !0), !1);
  }
  function Ou(l, t, a) {
    if (Qd(t))
      if (a === null) l.removeAttribute(t);
      else {
        switch (typeof a) {
          case "undefined":
          case "function":
          case "symbol":
            l.removeAttribute(t);
            return;
          case "boolean":
            var e = t.toLowerCase().slice(0, 5);
            if (e !== "data-" && e !== "aria-") {
              l.removeAttribute(t);
              return;
            }
        }
        l.setAttribute(t, "" + a);
      }
  }
  function Mu(l, t, a) {
    if (a === null) l.removeAttribute(t);
    else {
      switch (typeof a) {
        case "undefined":
        case "function":
        case "symbol":
        case "boolean":
          l.removeAttribute(t);
          return;
      }
      l.setAttribute(t, "" + a);
    }
  }
  function Nt(l, t, a, e) {
    if (e === null) l.removeAttribute(a);
    else {
      switch (typeof e) {
        case "undefined":
        case "function":
        case "symbol":
        case "boolean":
          l.removeAttribute(a);
          return;
      }
      l.setAttributeNS(t, a, "" + e);
    }
  }
  var wn, Af;
  function Xa(l) {
    if (wn === void 0)
      try {
        throw Error();
      } catch (a) {
        var t = a.stack.trim().match(/\n( *(at )?)/);
        (wn = (t && t[1]) || ""),
          (Af =
            -1 <
            a.stack.indexOf(`
    at`)
              ? " (<anonymous>)"
              : -1 < a.stack.indexOf("@")
                ? "@unknown:0:0"
                : "");
      }
    return (
      `
` +
      wn +
      l +
      Af
    );
  }
  var kn = !1;
  function $n(l, t) {
    if (!l || kn) return "";
    kn = !0;
    var a = Error.prepareStackTrace;
    Error.prepareStackTrace = void 0;
    try {
      var e = {
        DetermineComponentFrameRoot: function () {
          try {
            if (t) {
              var E = function () {
                throw Error();
              };
              if (
                (Object.defineProperty(E.prototype, "props", {
                  set: function () {
                    throw Error();
                  },
                }),
                typeof Reflect == "object" && Reflect.construct)
              ) {
                try {
                  Reflect.construct(E, []);
                } catch (g) {
                  var m = g;
                }
                Reflect.construct(l, [], E);
              } else {
                try {
                  E.call();
                } catch (g) {
                  m = g;
                }
                l.call(E.prototype);
              }
            } else {
              try {
                throw Error();
              } catch (g) {
                m = g;
              }
              (E = l()) &&
                typeof E.catch == "function" &&
                E.catch(function () {});
            }
          } catch (g) {
            if (g && m && typeof g.stack == "string") return [g.stack, m.stack];
          }
          return [null, null];
        },
      };
      e.DetermineComponentFrameRoot.displayName = "DetermineComponentFrameRoot";
      var u = Object.getOwnPropertyDescriptor(
        e.DetermineComponentFrameRoot,
        "name",
      );
      u &&
        u.configurable &&
        Object.defineProperty(e.DetermineComponentFrameRoot, "name", {
          value: "DetermineComponentFrameRoot",
        });
      var n = e.DetermineComponentFrameRoot(),
        i = n[0],
        c = n[1];
      if (i && c) {
        var f = i.split(`
`),
          y = c.split(`
`);
        for (
          u = e = 0;
          e < f.length && !f[e].includes("DetermineComponentFrameRoot");
        )
          e++;
        for (; u < y.length && !y[u].includes("DetermineComponentFrameRoot"); )
          u++;
        if (e === f.length || u === y.length)
          for (
            e = f.length - 1, u = y.length - 1;
            1 <= e && 0 <= u && f[e] !== y[u];
          )
            u--;
        for (; 1 <= e && 0 <= u; e--, u--)
          if (f[e] !== y[u]) {
            if (e !== 1 || u !== 1)
              do
                if ((e--, u--, 0 > u || f[e] !== y[u])) {
                  var T =
                    `
` + f[e].replace(" at new ", " at ");
                  return (
                    l.displayName &&
                      T.includes("<anonymous>") &&
                      (T = T.replace("<anonymous>", l.displayName)),
                    T
                  );
                }
              while (1 <= e && 0 <= u);
            break;
          }
      }
    } finally {
      (kn = !1), (Error.prepareStackTrace = a);
    }
    return (a = l ? l.displayName || l.name : "") ? Xa(a) : "";
  }
  function Zd(l) {
    switch (l.tag) {
      case 26:
      case 27:
      case 5:
        return Xa(l.type);
      case 16:
        return Xa("Lazy");
      case 13:
        return Xa("Suspense");
      case 19:
        return Xa("SuspenseList");
      case 0:
      case 15:
        return $n(l.type, !1);
      case 11:
        return $n(l.type.render, !1);
      case 1:
        return $n(l.type, !0);
      case 31:
        return Xa("Activity");
      default:
        return "";
    }
  }
  function pf(l) {
    try {
      var t = "";
      do (t += Zd(l)), (l = l.return);
      while (l);
      return t;
    } catch (a) {
      return (
        `
Error generating stack: ` +
        a.message +
        `
` +
        a.stack
      );
    }
  }
  function vt(l) {
    switch (typeof l) {
      case "bigint":
      case "boolean":
      case "number":
      case "string":
      case "undefined":
        return l;
      case "object":
        return l;
      default:
        return "";
    }
  }
  function _f(l) {
    var t = l.type;
    return (
      (l = l.nodeName) &&
      l.toLowerCase() === "input" &&
      (t === "checkbox" || t === "radio")
    );
  }
  function Ld(l) {
    var t = _f(l) ? "checked" : "value",
      a = Object.getOwnPropertyDescriptor(l.constructor.prototype, t),
      e = "" + l[t];
    if (
      !l.hasOwnProperty(t) &&
      typeof a < "u" &&
      typeof a.get == "function" &&
      typeof a.set == "function"
    ) {
      var u = a.get,
        n = a.set;
      return (
        Object.defineProperty(l, t, {
          configurable: !0,
          get: function () {
            return u.call(this);
          },
          set: function (i) {
            (e = "" + i), n.call(this, i);
          },
        }),
        Object.defineProperty(l, t, { enumerable: a.enumerable }),
        {
          getValue: function () {
            return e;
          },
          setValue: function (i) {
            e = "" + i;
          },
          stopTracking: function () {
            (l._valueTracker = null), delete l[t];
          },
        }
      );
    }
  }
  function Du(l) {
    l._valueTracker || (l._valueTracker = Ld(l));
  }
  function Of(l) {
    if (!l) return !1;
    var t = l._valueTracker;
    if (!t) return !0;
    var a = t.getValue(),
      e = "";
    return (
      l && (e = _f(l) ? (l.checked ? "true" : "false") : l.value),
      (l = e),
      l !== a ? (t.setValue(l), !0) : !1
    );
  }
  function Ru(l) {
    if (
      ((l = l || (typeof document < "u" ? document : void 0)), typeof l > "u")
    )
      return null;
    try {
      return l.activeElement || l.body;
    } catch {
      return l.body;
    }
  }
  var Vd = /[\n"\\]/g;
  function yt(l) {
    return l.replace(Vd, function (t) {
      return "\\" + t.charCodeAt(0).toString(16) + " ";
    });
  }
  function Wn(l, t, a, e, u, n, i, c) {
    (l.name = ""),
      i != null &&
      typeof i != "function" &&
      typeof i != "symbol" &&
      typeof i != "boolean"
        ? (l.type = i)
        : l.removeAttribute("type"),
      t != null
        ? i === "number"
          ? ((t === 0 && l.value === "") || l.value != t) &&
            (l.value = "" + vt(t))
          : l.value !== "" + vt(t) && (l.value = "" + vt(t))
        : (i !== "submit" && i !== "reset") || l.removeAttribute("value"),
      t != null
        ? Fn(l, i, vt(t))
        : a != null
          ? Fn(l, i, vt(a))
          : e != null && l.removeAttribute("value"),
      u == null && n != null && (l.defaultChecked = !!n),
      u != null &&
        (l.checked = u && typeof u != "function" && typeof u != "symbol"),
      c != null &&
      typeof c != "function" &&
      typeof c != "symbol" &&
      typeof c != "boolean"
        ? (l.name = "" + vt(c))
        : l.removeAttribute("name");
  }
  function Mf(l, t, a, e, u, n, i, c) {
    if (
      (n != null &&
        typeof n != "function" &&
        typeof n != "symbol" &&
        typeof n != "boolean" &&
        (l.type = n),
      t != null || a != null)
    ) {
      if (!((n !== "submit" && n !== "reset") || t != null)) return;
      (a = a != null ? "" + vt(a) : ""),
        (t = t != null ? "" + vt(t) : a),
        c || t === l.value || (l.value = t),
        (l.defaultValue = t);
    }
    (e = e ?? u),
      (e = typeof e != "function" && typeof e != "symbol" && !!e),
      (l.checked = c ? l.checked : !!e),
      (l.defaultChecked = !!e),
      i != null &&
        typeof i != "function" &&
        typeof i != "symbol" &&
        typeof i != "boolean" &&
        (l.name = i);
  }
  function Fn(l, t, a) {
    (t === "number" && Ru(l.ownerDocument) === l) ||
      l.defaultValue === "" + a ||
      (l.defaultValue = "" + a);
  }
  function Qa(l, t, a, e) {
    if (((l = l.options), t)) {
      t = {};
      for (var u = 0; u < a.length; u++) t["$" + a[u]] = !0;
      for (a = 0; a < l.length; a++)
        (u = t.hasOwnProperty("$" + l[a].value)),
          l[a].selected !== u && (l[a].selected = u),
          u && e && (l[a].defaultSelected = !0);
    } else {
      for (a = "" + vt(a), t = null, u = 0; u < l.length; u++) {
        if (l[u].value === a) {
          (l[u].selected = !0), e && (l[u].defaultSelected = !0);
          return;
        }
        t !== null || l[u].disabled || (t = l[u]);
      }
      t !== null && (t.selected = !0);
    }
  }
  function Df(l, t, a) {
    if (
      t != null &&
      ((t = "" + vt(t)), t !== l.value && (l.value = t), a == null)
    ) {
      l.defaultValue !== t && (l.defaultValue = t);
      return;
    }
    l.defaultValue = a != null ? "" + vt(a) : "";
  }
  function Rf(l, t, a, e) {
    if (t == null) {
      if (e != null) {
        if (a != null) throw Error(s(92));
        if (Yl(e)) {
          if (1 < e.length) throw Error(s(93));
          e = e[0];
        }
        a = e;
      }
      a == null && (a = ""), (t = a);
    }
    (a = vt(t)),
      (l.defaultValue = a),
      (e = l.textContent),
      e === a && e !== "" && e !== null && (l.value = e);
  }
  function Za(l, t) {
    if (t) {
      var a = l.firstChild;
      if (a && a === l.lastChild && a.nodeType === 3) {
        a.nodeValue = t;
        return;
      }
    }
    l.textContent = t;
  }
  var Kd = new Set(
    "animationIterationCount aspectRatio borderImageOutset borderImageSlice borderImageWidth boxFlex boxFlexGroup boxOrdinalGroup columnCount columns flex flexGrow flexPositive flexShrink flexNegative flexOrder gridArea gridRow gridRowEnd gridRowSpan gridRowStart gridColumn gridColumnEnd gridColumnSpan gridColumnStart fontWeight lineClamp lineHeight opacity order orphans scale tabSize widows zIndex zoom fillOpacity floodOpacity stopOpacity strokeDasharray strokeDashoffset strokeMiterlimit strokeOpacity strokeWidth MozAnimationIterationCount MozBoxFlex MozBoxFlexGroup MozLineClamp msAnimationIterationCount msFlex msZoom msFlexGrow msFlexNegative msFlexOrder msFlexPositive msFlexShrink msGridColumn msGridColumnSpan msGridRow msGridRowSpan WebkitAnimationIterationCount WebkitBoxFlex WebKitBoxFlexGroup WebkitBoxOrdinalGroup WebkitColumnCount WebkitColumns WebkitFlex WebkitFlexGrow WebkitFlexPositive WebkitFlexShrink WebkitLineClamp".split(
      " ",
    ),
  );
  function Uf(l, t, a) {
    var e = t.indexOf("--") === 0;
    a == null || typeof a == "boolean" || a === ""
      ? e
        ? l.setProperty(t, "")
        : t === "float"
          ? (l.cssFloat = "")
          : (l[t] = "")
      : e
        ? l.setProperty(t, a)
        : typeof a != "number" || a === 0 || Kd.has(t)
          ? t === "float"
            ? (l.cssFloat = a)
            : (l[t] = ("" + a).trim())
          : (l[t] = a + "px");
  }
  function Nf(l, t, a) {
    if (t != null && typeof t != "object") throw Error(s(62));
    if (((l = l.style), a != null)) {
      for (var e in a)
        !a.hasOwnProperty(e) ||
          (t != null && t.hasOwnProperty(e)) ||
          (e.indexOf("--") === 0
            ? l.setProperty(e, "")
            : e === "float"
              ? (l.cssFloat = "")
              : (l[e] = ""));
      for (var u in t)
        (e = t[u]), t.hasOwnProperty(u) && a[u] !== e && Uf(l, u, e);
    } else for (var n in t) t.hasOwnProperty(n) && Uf(l, n, t[n]);
  }
  function In(l) {
    if (l.indexOf("-") === -1) return !1;
    switch (l) {
      case "annotation-xml":
      case "color-profile":
      case "font-face":
      case "font-face-src":
      case "font-face-uri":
      case "font-face-format":
      case "font-face-name":
      case "missing-glyph":
        return !1;
      default:
        return !0;
    }
  }
  var Jd = new Map([
      ["acceptCharset", "accept-charset"],
      ["htmlFor", "for"],
      ["httpEquiv", "http-equiv"],
      ["crossOrigin", "crossorigin"],
      ["accentHeight", "accent-height"],
      ["alignmentBaseline", "alignment-baseline"],
      ["arabicForm", "arabic-form"],
      ["baselineShift", "baseline-shift"],
      ["capHeight", "cap-height"],
      ["clipPath", "clip-path"],
      ["clipRule", "clip-rule"],
      ["colorInterpolation", "color-interpolation"],
      ["colorInterpolationFilters", "color-interpolation-filters"],
      ["colorProfile", "color-profile"],
      ["colorRendering", "color-rendering"],
      ["dominantBaseline", "dominant-baseline"],
      ["enableBackground", "enable-background"],
      ["fillOpacity", "fill-opacity"],
      ["fillRule", "fill-rule"],
      ["floodColor", "flood-color"],
      ["floodOpacity", "flood-opacity"],
      ["fontFamily", "font-family"],
      ["fontSize", "font-size"],
      ["fontSizeAdjust", "font-size-adjust"],
      ["fontStretch", "font-stretch"],
      ["fontStyle", "font-style"],
      ["fontVariant", "font-variant"],
      ["fontWeight", "font-weight"],
      ["glyphName", "glyph-name"],
      ["glyphOrientationHorizontal", "glyph-orientation-horizontal"],
      ["glyphOrientationVertical", "glyph-orientation-vertical"],
      ["horizAdvX", "horiz-adv-x"],
      ["horizOriginX", "horiz-origin-x"],
      ["imageRendering", "image-rendering"],
      ["letterSpacing", "letter-spacing"],
      ["lightingColor", "lighting-color"],
      ["markerEnd", "marker-end"],
      ["markerMid", "marker-mid"],
      ["markerStart", "marker-start"],
      ["overlinePosition", "overline-position"],
      ["overlineThickness", "overline-thickness"],
      ["paintOrder", "paint-order"],
      ["panose-1", "panose-1"],
      ["pointerEvents", "pointer-events"],
      ["renderingIntent", "rendering-intent"],
      ["shapeRendering", "shape-rendering"],
      ["stopColor", "stop-color"],
      ["stopOpacity", "stop-opacity"],
      ["strikethroughPosition", "strikethrough-position"],
      ["strikethroughThickness", "strikethrough-thickness"],
      ["strokeDasharray", "stroke-dasharray"],
      ["strokeDashoffset", "stroke-dashoffset"],
      ["strokeLinecap", "stroke-linecap"],
      ["strokeLinejoin", "stroke-linejoin"],
      ["strokeMiterlimit", "stroke-miterlimit"],
      ["strokeOpacity", "stroke-opacity"],
      ["strokeWidth", "stroke-width"],
      ["textAnchor", "text-anchor"],
      ["textDecoration", "text-decoration"],
      ["textRendering", "text-rendering"],
      ["transformOrigin", "transform-origin"],
      ["underlinePosition", "underline-position"],
      ["underlineThickness", "underline-thickness"],
      ["unicodeBidi", "unicode-bidi"],
      ["unicodeRange", "unicode-range"],
      ["unitsPerEm", "units-per-em"],
      ["vAlphabetic", "v-alphabetic"],
      ["vHanging", "v-hanging"],
      ["vIdeographic", "v-ideographic"],
      ["vMathematical", "v-mathematical"],
      ["vectorEffect", "vector-effect"],
      ["vertAdvY", "vert-adv-y"],
      ["vertOriginX", "vert-origin-x"],
      ["vertOriginY", "vert-origin-y"],
      ["wordSpacing", "word-spacing"],
      ["writingMode", "writing-mode"],
      ["xmlnsXlink", "xmlns:xlink"],
      ["xHeight", "x-height"],
    ]),
    wd =
      /^[\u0000-\u001F ]*j[\r\n\t]*a[\r\n\t]*v[\r\n\t]*a[\r\n\t]*s[\r\n\t]*c[\r\n\t]*r[\r\n\t]*i[\r\n\t]*p[\r\n\t]*t[\r\n\t]*:/i;
  function Uu(l) {
    return wd.test("" + l)
      ? "javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')"
      : l;
  }
  var Pn = null;
  function li(l) {
    return (
      (l = l.target || l.srcElement || window),
      l.correspondingUseElement && (l = l.correspondingUseElement),
      l.nodeType === 3 ? l.parentNode : l
    );
  }
  var La = null,
    Va = null;
  function xf(l) {
    var t = ja(l);
    if (t && (l = t.stateNode)) {
      var a = l[Il] || null;
      switch (((l = t.stateNode), t.type)) {
        case "input":
          if (
            (Wn(
              l,
              a.value,
              a.defaultValue,
              a.defaultValue,
              a.checked,
              a.defaultChecked,
              a.type,
              a.name,
            ),
            (t = a.name),
            a.type === "radio" && t != null)
          ) {
            for (a = l; a.parentNode; ) a = a.parentNode;
            for (
              a = a.querySelectorAll(
                'input[name="' + yt("" + t) + '"][type="radio"]',
              ),
                t = 0;
              t < a.length;
              t++
            ) {
              var e = a[t];
              if (e !== l && e.form === l.form) {
                var u = e[Il] || null;
                if (!u) throw Error(s(90));
                Wn(
                  e,
                  u.value,
                  u.defaultValue,
                  u.defaultValue,
                  u.checked,
                  u.defaultChecked,
                  u.type,
                  u.name,
                );
              }
            }
            for (t = 0; t < a.length; t++)
              (e = a[t]), e.form === l.form && Of(e);
          }
          break;
        case "textarea":
          Df(l, a.value, a.defaultValue);
          break;
        case "select":
          (t = a.value), t != null && Qa(l, !!a.multiple, t, !1);
      }
    }
  }
  var ti = !1;
  function Hf(l, t, a) {
    if (ti) return l(t, a);
    ti = !0;
    try {
      var e = l(t);
      return e;
    } finally {
      if (
        ((ti = !1),
        (La !== null || Va !== null) &&
          (mn(), La && ((t = La), (l = Va), (Va = La = null), xf(t), l)))
      )
        for (t = 0; t < l.length; t++) xf(l[t]);
    }
  }
  function Me(l, t) {
    var a = l.stateNode;
    if (a === null) return null;
    var e = a[Il] || null;
    if (e === null) return null;
    a = e[t];
    switch (t) {
      case "onClick":
      case "onClickCapture":
      case "onDoubleClick":
      case "onDoubleClickCapture":
      case "onMouseDown":
      case "onMouseDownCapture":
      case "onMouseMove":
      case "onMouseMoveCapture":
      case "onMouseUp":
      case "onMouseUpCapture":
      case "onMouseEnter":
        (e = !e.disabled) ||
          ((l = l.type),
          (e = !(
            l === "button" ||
            l === "input" ||
            l === "select" ||
            l === "textarea"
          ))),
          (l = !e);
        break;
      default:
        l = !1;
    }
    if (l) return null;
    if (a && typeof a != "function") throw Error(s(231, t, typeof a));
    return a;
  }
  var xt = !(
      typeof window > "u" ||
      typeof window.document > "u" ||
      typeof window.document.createElement > "u"
    ),
    ai = !1;
  if (xt)
    try {
      var De = {};
      Object.defineProperty(De, "passive", {
        get: function () {
          ai = !0;
        },
      }),
        window.addEventListener("test", De, De),
        window.removeEventListener("test", De, De);
    } catch {
      ai = !1;
    }
  var kt = null,
    ei = null,
    Nu = null;
  function qf() {
    if (Nu) return Nu;
    var l,
      t = ei,
      a = t.length,
      e,
      u = "value" in kt ? kt.value : kt.textContent,
      n = u.length;
    for (l = 0; l < a && t[l] === u[l]; l++);
    var i = a - l;
    for (e = 1; e <= i && t[a - e] === u[n - e]; e++);
    return (Nu = u.slice(l, 1 < e ? 1 - e : void 0));
  }
  function xu(l) {
    var t = l.keyCode;
    return (
      "charCode" in l
        ? ((l = l.charCode), l === 0 && t === 13 && (l = 13))
        : (l = t),
      l === 10 && (l = 13),
      32 <= l || l === 13 ? l : 0
    );
  }
  function Hu() {
    return !0;
  }
  function Bf() {
    return !1;
  }
  function Pl(l) {
    function t(a, e, u, n, i) {
      (this._reactName = a),
        (this._targetInst = u),
        (this.type = e),
        (this.nativeEvent = n),
        (this.target = i),
        (this.currentTarget = null);
      for (var c in l)
        l.hasOwnProperty(c) && ((a = l[c]), (this[c] = a ? a(n) : n[c]));
      return (
        (this.isDefaultPrevented = (
          n.defaultPrevented != null
            ? n.defaultPrevented
            : n.returnValue === !1
        )
          ? Hu
          : Bf),
        (this.isPropagationStopped = Bf),
        this
      );
    }
    return (
      x(t.prototype, {
        preventDefault: function () {
          this.defaultPrevented = !0;
          var a = this.nativeEvent;
          a &&
            (a.preventDefault
              ? a.preventDefault()
              : typeof a.returnValue != "unknown" && (a.returnValue = !1),
            (this.isDefaultPrevented = Hu));
        },
        stopPropagation: function () {
          var a = this.nativeEvent;
          a &&
            (a.stopPropagation
              ? a.stopPropagation()
              : typeof a.cancelBubble != "unknown" && (a.cancelBubble = !0),
            (this.isPropagationStopped = Hu));
        },
        persist: function () {},
        isPersistent: Hu,
      }),
      t
    );
  }
  var Sa = {
      eventPhase: 0,
      bubbles: 0,
      cancelable: 0,
      timeStamp: function (l) {
        return l.timeStamp || Date.now();
      },
      defaultPrevented: 0,
      isTrusted: 0,
    },
    qu = Pl(Sa),
    Re = x({}, Sa, { view: 0, detail: 0 }),
    kd = Pl(Re),
    ui,
    ni,
    Ue,
    Bu = x({}, Re, {
      screenX: 0,
      screenY: 0,
      clientX: 0,
      clientY: 0,
      pageX: 0,
      pageY: 0,
      ctrlKey: 0,
      shiftKey: 0,
      altKey: 0,
      metaKey: 0,
      getModifierState: ci,
      button: 0,
      buttons: 0,
      relatedTarget: function (l) {
        return l.relatedTarget === void 0
          ? l.fromElement === l.srcElement
            ? l.toElement
            : l.fromElement
          : l.relatedTarget;
      },
      movementX: function (l) {
        return "movementX" in l
          ? l.movementX
          : (l !== Ue &&
              (Ue && l.type === "mousemove"
                ? ((ui = l.screenX - Ue.screenX), (ni = l.screenY - Ue.screenY))
                : (ni = ui = 0),
              (Ue = l)),
            ui);
      },
      movementY: function (l) {
        return "movementY" in l ? l.movementY : ni;
      },
    }),
    Yf = Pl(Bu),
    $d = x({}, Bu, { dataTransfer: 0 }),
    Wd = Pl($d),
    Fd = x({}, Re, { relatedTarget: 0 }),
    ii = Pl(Fd),
    Id = x({}, Sa, { animationName: 0, elapsedTime: 0, pseudoElement: 0 }),
    Pd = Pl(Id),
    l0 = x({}, Sa, {
      clipboardData: function (l) {
        return "clipboardData" in l ? l.clipboardData : window.clipboardData;
      },
    }),
    t0 = Pl(l0),
    a0 = x({}, Sa, { data: 0 }),
    jf = Pl(a0),
    e0 = {
      Esc: "Escape",
      Spacebar: " ",
      Left: "ArrowLeft",
      Up: "ArrowUp",
      Right: "ArrowRight",
      Down: "ArrowDown",
      Del: "Delete",
      Win: "OS",
      Menu: "ContextMenu",
      Apps: "ContextMenu",
      Scroll: "ScrollLock",
      MozPrintableKey: "Unidentified",
    },
    u0 = {
      8: "Backspace",
      9: "Tab",
      12: "Clear",
      13: "Enter",
      16: "Shift",
      17: "Control",
      18: "Alt",
      19: "Pause",
      20: "CapsLock",
      27: "Escape",
      32: " ",
      33: "PageUp",
      34: "PageDown",
      35: "End",
      36: "Home",
      37: "ArrowLeft",
      38: "ArrowUp",
      39: "ArrowRight",
      40: "ArrowDown",
      45: "Insert",
      46: "Delete",
      112: "F1",
      113: "F2",
      114: "F3",
      115: "F4",
      116: "F5",
      117: "F6",
      118: "F7",
      119: "F8",
      120: "F9",
      121: "F10",
      122: "F11",
      123: "F12",
      144: "NumLock",
      145: "ScrollLock",
      224: "Meta",
    },
    n0 = {
      Alt: "altKey",
      Control: "ctrlKey",
      Meta: "metaKey",
      Shift: "shiftKey",
    };
  function i0(l) {
    var t = this.nativeEvent;
    return t.getModifierState
      ? t.getModifierState(l)
      : (l = n0[l])
        ? !!t[l]
        : !1;
  }
  function ci() {
    return i0;
  }
  var c0 = x({}, Re, {
      key: function (l) {
        if (l.key) {
          var t = e0[l.key] || l.key;
          if (t !== "Unidentified") return t;
        }
        return l.type === "keypress"
          ? ((l = xu(l)), l === 13 ? "Enter" : String.fromCharCode(l))
          : l.type === "keydown" || l.type === "keyup"
            ? u0[l.keyCode] || "Unidentified"
            : "";
      },
      code: 0,
      location: 0,
      ctrlKey: 0,
      shiftKey: 0,
      altKey: 0,
      metaKey: 0,
      repeat: 0,
      locale: 0,
      getModifierState: ci,
      charCode: function (l) {
        return l.type === "keypress" ? xu(l) : 0;
      },
      keyCode: function (l) {
        return l.type === "keydown" || l.type === "keyup" ? l.keyCode : 0;
      },
      which: function (l) {
        return l.type === "keypress"
          ? xu(l)
          : l.type === "keydown" || l.type === "keyup"
            ? l.keyCode
            : 0;
      },
    }),
    f0 = Pl(c0),
    s0 = x({}, Bu, {
      pointerId: 0,
      width: 0,
      height: 0,
      pressure: 0,
      tangentialPressure: 0,
      tiltX: 0,
      tiltY: 0,
      twist: 0,
      pointerType: 0,
      isPrimary: 0,
    }),
    Cf = Pl(s0),
    o0 = x({}, Re, {
      touches: 0,
      targetTouches: 0,
      changedTouches: 0,
      altKey: 0,
      metaKey: 0,
      ctrlKey: 0,
      shiftKey: 0,
      getModifierState: ci,
    }),
    r0 = Pl(o0),
    d0 = x({}, Sa, { propertyName: 0, elapsedTime: 0, pseudoElement: 0 }),
    v0 = Pl(d0),
    y0 = x({}, Bu, {
      deltaX: function (l) {
        return "deltaX" in l
          ? l.deltaX
          : "wheelDeltaX" in l
            ? -l.wheelDeltaX
            : 0;
      },
      deltaY: function (l) {
        return "deltaY" in l
          ? l.deltaY
          : "wheelDeltaY" in l
            ? -l.wheelDeltaY
            : "wheelDelta" in l
              ? -l.wheelDelta
              : 0;
      },
      deltaZ: 0,
      deltaMode: 0,
    }),
    h0 = Pl(y0),
    m0 = x({}, Sa, { newState: 0, oldState: 0 }),
    g0 = Pl(m0),
    S0 = [9, 13, 27, 32],
    fi = xt && "CompositionEvent" in window,
    Ne = null;
  xt && "documentMode" in document && (Ne = document.documentMode);
  var b0 = xt && "TextEvent" in window && !Ne,
    Gf = xt && (!fi || (Ne && 8 < Ne && 11 >= Ne)),
    Xf = " ",
    Qf = !1;
  function Zf(l, t) {
    switch (l) {
      case "keyup":
        return S0.indexOf(t.keyCode) !== -1;
      case "keydown":
        return t.keyCode !== 229;
      case "keypress":
      case "mousedown":
      case "focusout":
        return !0;
      default:
        return !1;
    }
  }
  function Lf(l) {
    return (l = l.detail), typeof l == "object" && "data" in l ? l.data : null;
  }
  var Ka = !1;
  function T0(l, t) {
    switch (l) {
      case "compositionend":
        return Lf(t);
      case "keypress":
        return t.which !== 32 ? null : ((Qf = !0), Xf);
      case "textInput":
        return (l = t.data), l === Xf && Qf ? null : l;
      default:
        return null;
    }
  }
  function z0(l, t) {
    if (Ka)
      return l === "compositionend" || (!fi && Zf(l, t))
        ? ((l = qf()), (Nu = ei = kt = null), (Ka = !1), l)
        : null;
    switch (l) {
      case "paste":
        return null;
      case "keypress":
        if (!(t.ctrlKey || t.altKey || t.metaKey) || (t.ctrlKey && t.altKey)) {
          if (t.char && 1 < t.char.length) return t.char;
          if (t.which) return String.fromCharCode(t.which);
        }
        return null;
      case "compositionend":
        return Gf && t.locale !== "ko" ? null : t.data;
      default:
        return null;
    }
  }
  var E0 = {
    color: !0,
    date: !0,
    datetime: !0,
    "datetime-local": !0,
    email: !0,
    month: !0,
    number: !0,
    password: !0,
    range: !0,
    search: !0,
    tel: !0,
    text: !0,
    time: !0,
    url: !0,
    week: !0,
  };
  function Vf(l) {
    var t = l && l.nodeName && l.nodeName.toLowerCase();
    return t === "input" ? !!E0[l.type] : t === "textarea";
  }
  function Kf(l, t, a, e) {
    La ? (Va ? Va.push(e) : (Va = [e])) : (La = e),
      (t = En(t, "onChange")),
      0 < t.length &&
        ((a = new qu("onChange", "change", null, a, e)),
        l.push({ event: a, listeners: t }));
  }
  var xe = null,
    He = null;
  function A0(l) {
    Or(l, 0);
  }
  function Yu(l) {
    var t = Oe(l);
    if (Of(t)) return l;
  }
  function Jf(l, t) {
    if (l === "change") return t;
  }
  var wf = !1;
  if (xt) {
    var si;
    if (xt) {
      var oi = "oninput" in document;
      if (!oi) {
        var kf = document.createElement("div");
        kf.setAttribute("oninput", "return;"),
          (oi = typeof kf.oninput == "function");
      }
      si = oi;
    } else si = !1;
    wf = si && (!document.documentMode || 9 < document.documentMode);
  }
  function $f() {
    xe && (xe.detachEvent("onpropertychange", Wf), (He = xe = null));
  }
  function Wf(l) {
    if (l.propertyName === "value" && Yu(He)) {
      var t = [];
      Kf(t, He, l, li(l)), Hf(A0, t);
    }
  }
  function p0(l, t, a) {
    l === "focusin"
      ? ($f(), (xe = t), (He = a), xe.attachEvent("onpropertychange", Wf))
      : l === "focusout" && $f();
  }
  function _0(l) {
    if (l === "selectionchange" || l === "keyup" || l === "keydown")
      return Yu(He);
  }
  function O0(l, t) {
    if (l === "click") return Yu(t);
  }
  function M0(l, t) {
    if (l === "input" || l === "change") return Yu(t);
  }
  function D0(l, t) {
    return (l === t && (l !== 0 || 1 / l === 1 / t)) || (l !== l && t !== t);
  }
  var it = typeof Object.is == "function" ? Object.is : D0;
  function qe(l, t) {
    if (it(l, t)) return !0;
    if (
      typeof l != "object" ||
      l === null ||
      typeof t != "object" ||
      t === null
    )
      return !1;
    var a = Object.keys(l),
      e = Object.keys(t);
    if (a.length !== e.length) return !1;
    for (e = 0; e < a.length; e++) {
      var u = a[e];
      if (!Gn.call(t, u) || !it(l[u], t[u])) return !1;
    }
    return !0;
  }
  function Ff(l) {
    for (; l && l.firstChild; ) l = l.firstChild;
    return l;
  }
  function If(l, t) {
    var a = Ff(l);
    l = 0;
    for (var e; a; ) {
      if (a.nodeType === 3) {
        if (((e = l + a.textContent.length), l <= t && e >= t))
          return { node: a, offset: t - l };
        l = e;
      }
      l: {
        for (; a; ) {
          if (a.nextSibling) {
            a = a.nextSibling;
            break l;
          }
          a = a.parentNode;
        }
        a = void 0;
      }
      a = Ff(a);
    }
  }
  function Pf(l, t) {
    return l && t
      ? l === t
        ? !0
        : l && l.nodeType === 3
          ? !1
          : t && t.nodeType === 3
            ? Pf(l, t.parentNode)
            : "contains" in l
              ? l.contains(t)
              : l.compareDocumentPosition
                ? !!(l.compareDocumentPosition(t) & 16)
                : !1
      : !1;
  }
  function ls(l) {
    l =
      l != null &&
      l.ownerDocument != null &&
      l.ownerDocument.defaultView != null
        ? l.ownerDocument.defaultView
        : window;
    for (var t = Ru(l.document); t instanceof l.HTMLIFrameElement; ) {
      try {
        var a = typeof t.contentWindow.location.href == "string";
      } catch {
        a = !1;
      }
      if (a) l = t.contentWindow;
      else break;
      t = Ru(l.document);
    }
    return t;
  }
  function ri(l) {
    var t = l && l.nodeName && l.nodeName.toLowerCase();
    return (
      t &&
      ((t === "input" &&
        (l.type === "text" ||
          l.type === "search" ||
          l.type === "tel" ||
          l.type === "url" ||
          l.type === "password")) ||
        t === "textarea" ||
        l.contentEditable === "true")
    );
  }
  var R0 = xt && "documentMode" in document && 11 >= document.documentMode,
    Ja = null,
    di = null,
    Be = null,
    vi = !1;
  function ts(l, t, a) {
    var e =
      a.window === a ? a.document : a.nodeType === 9 ? a : a.ownerDocument;
    vi ||
      Ja == null ||
      Ja !== Ru(e) ||
      ((e = Ja),
      "selectionStart" in e && ri(e)
        ? (e = { start: e.selectionStart, end: e.selectionEnd })
        : ((e = (
            (e.ownerDocument && e.ownerDocument.defaultView) ||
            window
          ).getSelection()),
          (e = {
            anchorNode: e.anchorNode,
            anchorOffset: e.anchorOffset,
            focusNode: e.focusNode,
            focusOffset: e.focusOffset,
          })),
      (Be && qe(Be, e)) ||
        ((Be = e),
        (e = En(di, "onSelect")),
        0 < e.length &&
          ((t = new qu("onSelect", "select", null, t, a)),
          l.push({ event: t, listeners: e }),
          (t.target = Ja))));
  }
  function ba(l, t) {
    var a = {};
    return (
      (a[l.toLowerCase()] = t.toLowerCase()),
      (a["Webkit" + l] = "webkit" + t),
      (a["Moz" + l] = "moz" + t),
      a
    );
  }
  var wa = {
      animationend: ba("Animation", "AnimationEnd"),
      animationiteration: ba("Animation", "AnimationIteration"),
      animationstart: ba("Animation", "AnimationStart"),
      transitionrun: ba("Transition", "TransitionRun"),
      transitionstart: ba("Transition", "TransitionStart"),
      transitioncancel: ba("Transition", "TransitionCancel"),
      transitionend: ba("Transition", "TransitionEnd"),
    },
    yi = {},
    as = {};
  xt &&
    ((as = document.createElement("div").style),
    "AnimationEvent" in window ||
      (delete wa.animationend.animation,
      delete wa.animationiteration.animation,
      delete wa.animationstart.animation),
    "TransitionEvent" in window || delete wa.transitionend.transition);
  function Ta(l) {
    if (yi[l]) return yi[l];
    if (!wa[l]) return l;
    var t = wa[l],
      a;
    for (a in t) if (t.hasOwnProperty(a) && a in as) return (yi[l] = t[a]);
    return l;
  }
  var es = Ta("animationend"),
    us = Ta("animationiteration"),
    ns = Ta("animationstart"),
    U0 = Ta("transitionrun"),
    N0 = Ta("transitionstart"),
    x0 = Ta("transitioncancel"),
    is = Ta("transitionend"),
    cs = new Map(),
    hi =
      "abort auxClick beforeToggle cancel canPlay canPlayThrough click close contextMenu copy cut drag dragEnd dragEnter dragExit dragLeave dragOver dragStart drop durationChange emptied encrypted ended error gotPointerCapture input invalid keyDown keyPress keyUp load loadedData loadedMetadata loadStart lostPointerCapture mouseDown mouseMove mouseOut mouseOver mouseUp paste pause play playing pointerCancel pointerDown pointerMove pointerOut pointerOver pointerUp progress rateChange reset resize seeked seeking stalled submit suspend timeUpdate touchCancel touchEnd touchStart volumeChange scroll toggle touchMove waiting wheel".split(
        " ",
      );
  hi.push("scrollEnd");
  function Et(l, t) {
    cs.set(l, t), ga(t, [l]);
  }
  var fs = new WeakMap();
  function ht(l, t) {
    if (typeof l == "object" && l !== null) {
      var a = fs.get(l);
      return a !== void 0
        ? a
        : ((t = { value: l, source: t, stack: pf(t) }), fs.set(l, t), t);
    }
    return { value: l, source: t, stack: pf(t) };
  }
  var mt = [],
    ka = 0,
    mi = 0;
  function ju() {
    for (var l = ka, t = (mi = ka = 0); t < l; ) {
      var a = mt[t];
      mt[t++] = null;
      var e = mt[t];
      mt[t++] = null;
      var u = mt[t];
      mt[t++] = null;
      var n = mt[t];
      if (((mt[t++] = null), e !== null && u !== null)) {
        var i = e.pending;
        i === null ? (u.next = u) : ((u.next = i.next), (i.next = u)),
          (e.pending = u);
      }
      n !== 0 && ss(a, u, n);
    }
  }
  function Cu(l, t, a, e) {
    (mt[ka++] = l),
      (mt[ka++] = t),
      (mt[ka++] = a),
      (mt[ka++] = e),
      (mi |= e),
      (l.lanes |= e),
      (l = l.alternate),
      l !== null && (l.lanes |= e);
  }
  function gi(l, t, a, e) {
    return Cu(l, t, a, e), Gu(l);
  }
  function $a(l, t) {
    return Cu(l, null, null, t), Gu(l);
  }
  function ss(l, t, a) {
    l.lanes |= a;
    var e = l.alternate;
    e !== null && (e.lanes |= a);
    for (var u = !1, n = l.return; n !== null; )
      (n.childLanes |= a),
        (e = n.alternate),
        e !== null && (e.childLanes |= a),
        n.tag === 22 &&
          ((l = n.stateNode), l === null || l._visibility & 1 || (u = !0)),
        (l = n),
        (n = n.return);
    return l.tag === 3
      ? ((n = l.stateNode),
        u &&
          t !== null &&
          ((u = 31 - nt(a)),
          (l = n.hiddenUpdates),
          (e = l[u]),
          e === null ? (l[u] = [t]) : e.push(t),
          (t.lane = a | 536870912)),
        n)
      : null;
  }
  function Gu(l) {
    if (50 < iu) throw ((iu = 0), (Ac = null), Error(s(185)));
    for (var t = l.return; t !== null; ) (l = t), (t = l.return);
    return l.tag === 3 ? l.stateNode : null;
  }
  var Wa = {};
  function H0(l, t, a, e) {
    (this.tag = l),
      (this.key = a),
      (this.sibling =
        this.child =
        this.return =
        this.stateNode =
        this.type =
        this.elementType =
          null),
      (this.index = 0),
      (this.refCleanup = this.ref = null),
      (this.pendingProps = t),
      (this.dependencies =
        this.memoizedState =
        this.updateQueue =
        this.memoizedProps =
          null),
      (this.mode = e),
      (this.subtreeFlags = this.flags = 0),
      (this.deletions = null),
      (this.childLanes = this.lanes = 0),
      (this.alternate = null);
  }
  function ct(l, t, a, e) {
    return new H0(l, t, a, e);
  }
  function Si(l) {
    return (l = l.prototype), !(!l || !l.isReactComponent);
  }
  function Ht(l, t) {
    var a = l.alternate;
    return (
      a === null
        ? ((a = ct(l.tag, t, l.key, l.mode)),
          (a.elementType = l.elementType),
          (a.type = l.type),
          (a.stateNode = l.stateNode),
          (a.alternate = l),
          (l.alternate = a))
        : ((a.pendingProps = t),
          (a.type = l.type),
          (a.flags = 0),
          (a.subtreeFlags = 0),
          (a.deletions = null)),
      (a.flags = l.flags & 65011712),
      (a.childLanes = l.childLanes),
      (a.lanes = l.lanes),
      (a.child = l.child),
      (a.memoizedProps = l.memoizedProps),
      (a.memoizedState = l.memoizedState),
      (a.updateQueue = l.updateQueue),
      (t = l.dependencies),
      (a.dependencies =
        t === null ? null : { lanes: t.lanes, firstContext: t.firstContext }),
      (a.sibling = l.sibling),
      (a.index = l.index),
      (a.ref = l.ref),
      (a.refCleanup = l.refCleanup),
      a
    );
  }
  function os(l, t) {
    l.flags &= 65011714;
    var a = l.alternate;
    return (
      a === null
        ? ((l.childLanes = 0),
          (l.lanes = t),
          (l.child = null),
          (l.subtreeFlags = 0),
          (l.memoizedProps = null),
          (l.memoizedState = null),
          (l.updateQueue = null),
          (l.dependencies = null),
          (l.stateNode = null))
        : ((l.childLanes = a.childLanes),
          (l.lanes = a.lanes),
          (l.child = a.child),
          (l.subtreeFlags = 0),
          (l.deletions = null),
          (l.memoizedProps = a.memoizedProps),
          (l.memoizedState = a.memoizedState),
          (l.updateQueue = a.updateQueue),
          (l.type = a.type),
          (t = a.dependencies),
          (l.dependencies =
            t === null
              ? null
              : { lanes: t.lanes, firstContext: t.firstContext })),
      l
    );
  }
  function Xu(l, t, a, e, u, n) {
    var i = 0;
    if (((e = l), typeof l == "function")) Si(l) && (i = 1);
    else if (typeof l == "string")
      i = Bv(l, a, j.current)
        ? 26
        : l === "html" || l === "head" || l === "body"
          ? 27
          : 5;
    else
      l: switch (l) {
        case El:
          return (l = ct(31, a, t, u)), (l.elementType = El), (l.lanes = n), l;
        case Tl:
          return za(a.children, u, n, t);
        case zl:
          (i = 8), (u |= 24);
          break;
        case dl:
          return (
            (l = ct(12, a, t, u | 2)), (l.elementType = dl), (l.lanes = n), l
          );
        case R:
          return (l = ct(13, a, t, u)), (l.elementType = R), (l.lanes = n), l;
        case X:
          return (l = ct(19, a, t, u)), (l.elementType = X), (l.lanes = n), l;
        default:
          if (typeof l == "object" && l !== null)
            switch (l.$$typeof) {
              case Ol:
              case vl:
                i = 10;
                break l;
              case Ml:
                i = 9;
                break l;
              case pl:
                i = 11;
                break l;
              case ul:
                i = 14;
                break l;
              case cl:
                (i = 16), (e = null);
                break l;
            }
          (i = 29),
            (a = Error(s(130, l === null ? "null" : typeof l, ""))),
            (e = null);
      }
    return (
      (t = ct(i, a, t, u)), (t.elementType = l), (t.type = e), (t.lanes = n), t
    );
  }
  function za(l, t, a, e) {
    return (l = ct(7, l, e, t)), (l.lanes = a), l;
  }
  function bi(l, t, a) {
    return (l = ct(6, l, null, t)), (l.lanes = a), l;
  }
  function Ti(l, t, a) {
    return (
      (t = ct(4, l.children !== null ? l.children : [], l.key, t)),
      (t.lanes = a),
      (t.stateNode = {
        containerInfo: l.containerInfo,
        pendingChildren: null,
        implementation: l.implementation,
      }),
      t
    );
  }
  var Fa = [],
    Ia = 0,
    Qu = null,
    Zu = 0,
    gt = [],
    St = 0,
    Ea = null,
    qt = 1,
    Bt = "";
  function Aa(l, t) {
    (Fa[Ia++] = Zu), (Fa[Ia++] = Qu), (Qu = l), (Zu = t);
  }
  function rs(l, t, a) {
    (gt[St++] = qt), (gt[St++] = Bt), (gt[St++] = Ea), (Ea = l);
    var e = qt;
    l = Bt;
    var u = 32 - nt(e) - 1;
    (e &= ~(1 << u)), (a += 1);
    var n = 32 - nt(t) + u;
    if (30 < n) {
      var i = u - (u % 5);
      (n = (e & ((1 << i) - 1)).toString(32)),
        (e >>= i),
        (u -= i),
        (qt = (1 << (32 - nt(t) + u)) | (a << u) | e),
        (Bt = n + l);
    } else (qt = (1 << n) | (a << u) | e), (Bt = l);
  }
  function zi(l) {
    l.return !== null && (Aa(l, 1), rs(l, 1, 0));
  }
  function Ei(l) {
    for (; l === Qu; )
      (Qu = Fa[--Ia]), (Fa[Ia] = null), (Zu = Fa[--Ia]), (Fa[Ia] = null);
    for (; l === Ea; )
      (Ea = gt[--St]),
        (gt[St] = null),
        (Bt = gt[--St]),
        (gt[St] = null),
        (qt = gt[--St]),
        (gt[St] = null);
  }
  var Wl = null,
    Dl = null,
    il = !1,
    pa = null,
    Ot = !1,
    Ai = Error(s(519));
  function _a(l) {
    var t = Error(s(418, ""));
    throw (Ce(ht(t, l)), Ai);
  }
  function ds(l) {
    var t = l.stateNode,
      a = l.type,
      e = l.memoizedProps;
    switch (((t[Jl] = l), (t[Il] = e), a)) {
      case "dialog":
        F("cancel", t), F("close", t);
        break;
      case "iframe":
      case "object":
      case "embed":
        F("load", t);
        break;
      case "video":
      case "audio":
        for (a = 0; a < fu.length; a++) F(fu[a], t);
        break;
      case "source":
        F("error", t);
        break;
      case "img":
      case "image":
      case "link":
        F("error", t), F("load", t);
        break;
      case "details":
        F("toggle", t);
        break;
      case "input":
        F("invalid", t),
          Mf(
            t,
            e.value,
            e.defaultValue,
            e.checked,
            e.defaultChecked,
            e.type,
            e.name,
            !0,
          ),
          Du(t);
        break;
      case "select":
        F("invalid", t);
        break;
      case "textarea":
        F("invalid", t), Rf(t, e.value, e.defaultValue, e.children), Du(t);
    }
    (a = e.children),
      (typeof a != "string" && typeof a != "number" && typeof a != "bigint") ||
      t.textContent === "" + a ||
      e.suppressHydrationWarning === !0 ||
      Ur(t.textContent, a)
        ? (e.popover != null && (F("beforetoggle", t), F("toggle", t)),
          e.onScroll != null && F("scroll", t),
          e.onScrollEnd != null && F("scrollend", t),
          e.onClick != null && (t.onclick = An),
          (t = !0))
        : (t = !1),
      t || _a(l);
  }
  function vs(l) {
    for (Wl = l.return; Wl; )
      switch (Wl.tag) {
        case 5:
        case 13:
          Ot = !1;
          return;
        case 27:
        case 3:
          Ot = !0;
          return;
        default:
          Wl = Wl.return;
      }
  }
  function Ye(l) {
    if (l !== Wl) return !1;
    if (!il) return vs(l), (il = !0), !1;
    var t = l.tag,
      a;
    if (
      ((a = t !== 3 && t !== 27) &&
        ((a = t === 5) &&
          ((a = l.type),
          (a =
            !(a !== "form" && a !== "button") || Gc(l.type, l.memoizedProps))),
        (a = !a)),
      a && Dl && _a(l),
      vs(l),
      t === 13)
    ) {
      if (((l = l.memoizedState), (l = l !== null ? l.dehydrated : null), !l))
        throw Error(s(317));
      l: {
        for (l = l.nextSibling, t = 0; l; ) {
          if (l.nodeType === 8)
            if (((a = l.data), a === "/$")) {
              if (t === 0) {
                Dl = pt(l.nextSibling);
                break l;
              }
              t--;
            } else (a !== "$" && a !== "$!" && a !== "$?") || t++;
          l = l.nextSibling;
        }
        Dl = null;
      }
    } else
      t === 27
        ? ((t = Dl), oa(l.type) ? ((l = Lc), (Lc = null), (Dl = l)) : (Dl = t))
        : (Dl = Wl ? pt(l.stateNode.nextSibling) : null);
    return !0;
  }
  function je() {
    (Dl = Wl = null), (il = !1);
  }
  function ys() {
    var l = pa;
    return (
      l !== null &&
        (at === null ? (at = l) : at.push.apply(at, l), (pa = null)),
      l
    );
  }
  function Ce(l) {
    pa === null ? (pa = [l]) : pa.push(l);
  }
  var pi = A(null),
    Oa = null,
    Yt = null;
  function $t(l, t, a) {
    D(pi, t._currentValue), (t._currentValue = a);
  }
  function jt(l) {
    (l._currentValue = pi.current), U(pi);
  }
  function _i(l, t, a) {
    for (; l !== null; ) {
      var e = l.alternate;
      if (
        ((l.childLanes & t) !== t
          ? ((l.childLanes |= t), e !== null && (e.childLanes |= t))
          : e !== null && (e.childLanes & t) !== t && (e.childLanes |= t),
        l === a)
      )
        break;
      l = l.return;
    }
  }
  function Oi(l, t, a, e) {
    var u = l.child;
    for (u !== null && (u.return = l); u !== null; ) {
      var n = u.dependencies;
      if (n !== null) {
        var i = u.child;
        n = n.firstContext;
        l: for (; n !== null; ) {
          var c = n;
          n = u;
          for (var f = 0; f < t.length; f++)
            if (c.context === t[f]) {
              (n.lanes |= a),
                (c = n.alternate),
                c !== null && (c.lanes |= a),
                _i(n.return, a, l),
                e || (i = null);
              break l;
            }
          n = c.next;
        }
      } else if (u.tag === 18) {
        if (((i = u.return), i === null)) throw Error(s(341));
        (i.lanes |= a),
          (n = i.alternate),
          n !== null && (n.lanes |= a),
          _i(i, a, l),
          (i = null);
      } else i = u.child;
      if (i !== null) i.return = u;
      else
        for (i = u; i !== null; ) {
          if (i === l) {
            i = null;
            break;
          }
          if (((u = i.sibling), u !== null)) {
            (u.return = i.return), (i = u);
            break;
          }
          i = i.return;
        }
      u = i;
    }
  }
  function Ge(l, t, a, e) {
    l = null;
    for (var u = t, n = !1; u !== null; ) {
      if (!n) {
        if ((u.flags & 524288) !== 0) n = !0;
        else if ((u.flags & 262144) !== 0) break;
      }
      if (u.tag === 10) {
        var i = u.alternate;
        if (i === null) throw Error(s(387));
        if (((i = i.memoizedProps), i !== null)) {
          var c = u.type;
          it(u.pendingProps.value, i.value) ||
            (l !== null ? l.push(c) : (l = [c]));
        }
      } else if (u === tl.current) {
        if (((i = u.alternate), i === null)) throw Error(s(387));
        i.memoizedState.memoizedState !== u.memoizedState.memoizedState &&
          (l !== null ? l.push(yu) : (l = [yu]));
      }
      u = u.return;
    }
    l !== null && Oi(t, l, a, e), (t.flags |= 262144);
  }
  function Lu(l) {
    for (l = l.firstContext; l !== null; ) {
      if (!it(l.context._currentValue, l.memoizedValue)) return !0;
      l = l.next;
    }
    return !1;
  }
  function Ma(l) {
    (Oa = l),
      (Yt = null),
      (l = l.dependencies),
      l !== null && (l.firstContext = null);
  }
  function wl(l) {
    return hs(Oa, l);
  }
  function Vu(l, t) {
    return Oa === null && Ma(l), hs(l, t);
  }
  function hs(l, t) {
    var a = t._currentValue;
    if (((t = { context: t, memoizedValue: a, next: null }), Yt === null)) {
      if (l === null) throw Error(s(308));
      (Yt = t),
        (l.dependencies = { lanes: 0, firstContext: t }),
        (l.flags |= 524288);
    } else Yt = Yt.next = t;
    return a;
  }
  var q0 =
      typeof AbortController < "u"
        ? AbortController
        : function () {
            var l = [],
              t = (this.signal = {
                aborted: !1,
                addEventListener: function (a, e) {
                  l.push(e);
                },
              });
            this.abort = function () {
              (t.aborted = !0),
                l.forEach(function (a) {
                  return a();
                });
            };
          },
    B0 = h.unstable_scheduleCallback,
    Y0 = h.unstable_NormalPriority,
    jl = {
      $$typeof: vl,
      Consumer: null,
      Provider: null,
      _currentValue: null,
      _currentValue2: null,
      _threadCount: 0,
    };
  function Mi() {
    return { controller: new q0(), data: new Map(), refCount: 0 };
  }
  function Xe(l) {
    l.refCount--,
      l.refCount === 0 &&
        B0(Y0, function () {
          l.controller.abort();
        });
  }
  var Qe = null,
    Di = 0,
    Pa = 0,
    le = null;
  function j0(l, t) {
    if (Qe === null) {
      var a = (Qe = []);
      (Di = 0),
        (Pa = Uc()),
        (le = {
          status: "pending",
          value: void 0,
          then: function (e) {
            a.push(e);
          },
        });
    }
    return Di++, t.then(ms, ms), t;
  }
  function ms() {
    if (--Di === 0 && Qe !== null) {
      le !== null && (le.status = "fulfilled");
      var l = Qe;
      (Qe = null), (Pa = 0), (le = null);
      for (var t = 0; t < l.length; t++) (0, l[t])();
    }
  }
  function C0(l, t) {
    var a = [],
      e = {
        status: "pending",
        value: null,
        reason: null,
        then: function (u) {
          a.push(u);
        },
      };
    return (
      l.then(
        function () {
          (e.status = "fulfilled"), (e.value = t);
          for (var u = 0; u < a.length; u++) (0, a[u])(t);
        },
        function (u) {
          for (e.status = "rejected", e.reason = u, u = 0; u < a.length; u++)
            (0, a[u])(void 0);
        },
      ),
      e
    );
  }
  var gs = S.S;
  S.S = function (l, t) {
    typeof t == "object" &&
      t !== null &&
      typeof t.then == "function" &&
      j0(l, t),
      gs !== null && gs(l, t);
  };
  var Da = A(null);
  function Ri() {
    var l = Da.current;
    return l !== null ? l : Sl.pooledCache;
  }
  function Ku(l, t) {
    t === null ? D(Da, Da.current) : D(Da, t.pool);
  }
  function Ss() {
    var l = Ri();
    return l === null ? null : { parent: jl._currentValue, pool: l };
  }
  var Ze = Error(s(460)),
    bs = Error(s(474)),
    Ju = Error(s(542)),
    Ui = { then: function () {} };
  function Ts(l) {
    return (l = l.status), l === "fulfilled" || l === "rejected";
  }
  function wu() {}
  function zs(l, t, a) {
    switch (
      ((a = l[a]),
      a === void 0 ? l.push(t) : a !== t && (t.then(wu, wu), (t = a)),
      t.status)
    ) {
      case "fulfilled":
        return t.value;
      case "rejected":
        throw ((l = t.reason), As(l), l);
      default:
        if (typeof t.status == "string") t.then(wu, wu);
        else {
          if (((l = Sl), l !== null && 100 < l.shellSuspendCounter))
            throw Error(s(482));
          (l = t),
            (l.status = "pending"),
            l.then(
              function (e) {
                if (t.status === "pending") {
                  var u = t;
                  (u.status = "fulfilled"), (u.value = e);
                }
              },
              function (e) {
                if (t.status === "pending") {
                  var u = t;
                  (u.status = "rejected"), (u.reason = e);
                }
              },
            );
        }
        switch (t.status) {
          case "fulfilled":
            return t.value;
          case "rejected":
            throw ((l = t.reason), As(l), l);
        }
        throw ((Le = t), Ze);
    }
  }
  var Le = null;
  function Es() {
    if (Le === null) throw Error(s(459));
    var l = Le;
    return (Le = null), l;
  }
  function As(l) {
    if (l === Ze || l === Ju) throw Error(s(483));
  }
  var Wt = !1;
  function Ni(l) {
    l.updateQueue = {
      baseState: l.memoizedState,
      firstBaseUpdate: null,
      lastBaseUpdate: null,
      shared: { pending: null, lanes: 0, hiddenCallbacks: null },
      callbacks: null,
    };
  }
  function xi(l, t) {
    (l = l.updateQueue),
      t.updateQueue === l &&
        (t.updateQueue = {
          baseState: l.baseState,
          firstBaseUpdate: l.firstBaseUpdate,
          lastBaseUpdate: l.lastBaseUpdate,
          shared: l.shared,
          callbacks: null,
        });
  }
  function Ft(l) {
    return { lane: l, tag: 0, payload: null, callback: null, next: null };
  }
  function It(l, t, a) {
    var e = l.updateQueue;
    if (e === null) return null;
    if (((e = e.shared), (ol & 2) !== 0)) {
      var u = e.pending;
      return (
        u === null ? (t.next = t) : ((t.next = u.next), (u.next = t)),
        (e.pending = t),
        (t = Gu(l)),
        ss(l, null, a),
        t
      );
    }
    return Cu(l, e, t, a), Gu(l);
  }
  function Ve(l, t, a) {
    if (
      ((t = t.updateQueue), t !== null && ((t = t.shared), (a & 4194048) !== 0))
    ) {
      var e = t.lanes;
      (e &= l.pendingLanes), (a |= e), (t.lanes = a), mf(l, a);
    }
  }
  function Hi(l, t) {
    var a = l.updateQueue,
      e = l.alternate;
    if (e !== null && ((e = e.updateQueue), a === e)) {
      var u = null,
        n = null;
      if (((a = a.firstBaseUpdate), a !== null)) {
        do {
          var i = {
            lane: a.lane,
            tag: a.tag,
            payload: a.payload,
            callback: null,
            next: null,
          };
          n === null ? (u = n = i) : (n = n.next = i), (a = a.next);
        } while (a !== null);
        n === null ? (u = n = t) : (n = n.next = t);
      } else u = n = t;
      (a = {
        baseState: e.baseState,
        firstBaseUpdate: u,
        lastBaseUpdate: n,
        shared: e.shared,
        callbacks: e.callbacks,
      }),
        (l.updateQueue = a);
      return;
    }
    (l = a.lastBaseUpdate),
      l === null ? (a.firstBaseUpdate = t) : (l.next = t),
      (a.lastBaseUpdate = t);
  }
  var qi = !1;
  function Ke() {
    if (qi) {
      var l = le;
      if (l !== null) throw l;
    }
  }
  function Je(l, t, a, e) {
    qi = !1;
    var u = l.updateQueue;
    Wt = !1;
    var n = u.firstBaseUpdate,
      i = u.lastBaseUpdate,
      c = u.shared.pending;
    if (c !== null) {
      u.shared.pending = null;
      var f = c,
        y = f.next;
      (f.next = null), i === null ? (n = y) : (i.next = y), (i = f);
      var T = l.alternate;
      T !== null &&
        ((T = T.updateQueue),
        (c = T.lastBaseUpdate),
        c !== i &&
          (c === null ? (T.firstBaseUpdate = y) : (c.next = y),
          (T.lastBaseUpdate = f)));
    }
    if (n !== null) {
      var E = u.baseState;
      (i = 0), (T = y = f = null), (c = n);
      do {
        var m = c.lane & -536870913,
          g = m !== c.lane;
        if (g ? (ll & m) === m : (e & m) === m) {
          m !== 0 && m === Pa && (qi = !0),
            T !== null &&
              (T = T.next =
                {
                  lane: 0,
                  tag: c.tag,
                  payload: c.payload,
                  callback: null,
                  next: null,
                });
          l: {
            var Z = l,
              C = c;
            m = t;
            var ml = a;
            switch (C.tag) {
              case 1:
                if (((Z = C.payload), typeof Z == "function")) {
                  E = Z.call(ml, E, m);
                  break l;
                }
                E = Z;
                break l;
              case 3:
                Z.flags = (Z.flags & -65537) | 128;
              case 0:
                if (
                  ((Z = C.payload),
                  (m = typeof Z == "function" ? Z.call(ml, E, m) : Z),
                  m == null)
                )
                  break l;
                E = x({}, E, m);
                break l;
              case 2:
                Wt = !0;
            }
          }
          (m = c.callback),
            m !== null &&
              ((l.flags |= 64),
              g && (l.flags |= 8192),
              (g = u.callbacks),
              g === null ? (u.callbacks = [m]) : g.push(m));
        } else
          (g = {
            lane: m,
            tag: c.tag,
            payload: c.payload,
            callback: c.callback,
            next: null,
          }),
            T === null ? ((y = T = g), (f = E)) : (T = T.next = g),
            (i |= m);
        if (((c = c.next), c === null)) {
          if (((c = u.shared.pending), c === null)) break;
          (g = c),
            (c = g.next),
            (g.next = null),
            (u.lastBaseUpdate = g),
            (u.shared.pending = null);
        }
      } while (!0);
      T === null && (f = E),
        (u.baseState = f),
        (u.firstBaseUpdate = y),
        (u.lastBaseUpdate = T),
        n === null && (u.shared.lanes = 0),
        (ia |= i),
        (l.lanes = i),
        (l.memoizedState = E);
    }
  }
  function ps(l, t) {
    if (typeof l != "function") throw Error(s(191, l));
    l.call(t);
  }
  function _s(l, t) {
    var a = l.callbacks;
    if (a !== null)
      for (l.callbacks = null, l = 0; l < a.length; l++) ps(a[l], t);
  }
  var te = A(null),
    ku = A(0);
  function Os(l, t) {
    (l = Vt), D(ku, l), D(te, t), (Vt = l | t.baseLanes);
  }
  function Bi() {
    D(ku, Vt), D(te, te.current);
  }
  function Yi() {
    (Vt = ku.current), U(te), U(ku);
  }
  var Pt = 0,
    w = null,
    yl = null,
    ql = null,
    $u = !1,
    ae = !1,
    Ra = !1,
    Wu = 0,
    we = 0,
    ee = null,
    G0 = 0;
  function Nl() {
    throw Error(s(321));
  }
  function ji(l, t) {
    if (t === null) return !1;
    for (var a = 0; a < t.length && a < l.length; a++)
      if (!it(l[a], t[a])) return !1;
    return !0;
  }
  function Ci(l, t, a, e, u, n) {
    return (
      (Pt = n),
      (w = t),
      (t.memoizedState = null),
      (t.updateQueue = null),
      (t.lanes = 0),
      (S.H = l === null || l.memoizedState === null ? so : oo),
      (Ra = !1),
      (n = a(e, u)),
      (Ra = !1),
      ae && (n = Ds(t, a, e, u)),
      Ms(l),
      n
    );
  }
  function Ms(l) {
    S.H = an;
    var t = yl !== null && yl.next !== null;
    if (((Pt = 0), (ql = yl = w = null), ($u = !1), (we = 0), (ee = null), t))
      throw Error(s(300));
    l === null ||
      Xl ||
      ((l = l.dependencies), l !== null && Lu(l) && (Xl = !0));
  }
  function Ds(l, t, a, e) {
    w = l;
    var u = 0;
    do {
      if ((ae && (ee = null), (we = 0), (ae = !1), 25 <= u))
        throw Error(s(301));
      if (((u += 1), (ql = yl = null), l.updateQueue != null)) {
        var n = l.updateQueue;
        (n.lastEffect = null),
          (n.events = null),
          (n.stores = null),
          n.memoCache != null && (n.memoCache.index = 0);
      }
      (S.H = J0), (n = t(a, e));
    } while (ae);
    return n;
  }
  function X0() {
    var l = S.H,
      t = l.useState()[0];
    return (
      (t = typeof t.then == "function" ? ke(t) : t),
      (l = l.useState()[0]),
      (yl !== null ? yl.memoizedState : null) !== l && (w.flags |= 1024),
      t
    );
  }
  function Gi() {
    var l = Wu !== 0;
    return (Wu = 0), l;
  }
  function Xi(l, t, a) {
    (t.updateQueue = l.updateQueue), (t.flags &= -2053), (l.lanes &= ~a);
  }
  function Qi(l) {
    if ($u) {
      for (l = l.memoizedState; l !== null; ) {
        var t = l.queue;
        t !== null && (t.pending = null), (l = l.next);
      }
      $u = !1;
    }
    (Pt = 0), (ql = yl = w = null), (ae = !1), (we = Wu = 0), (ee = null);
  }
  function lt() {
    var l = {
      memoizedState: null,
      baseState: null,
      baseQueue: null,
      queue: null,
      next: null,
    };
    return ql === null ? (w.memoizedState = ql = l) : (ql = ql.next = l), ql;
  }
  function Bl() {
    if (yl === null) {
      var l = w.alternate;
      l = l !== null ? l.memoizedState : null;
    } else l = yl.next;
    var t = ql === null ? w.memoizedState : ql.next;
    if (t !== null) (ql = t), (yl = l);
    else {
      if (l === null)
        throw w.alternate === null ? Error(s(467)) : Error(s(310));
      (yl = l),
        (l = {
          memoizedState: yl.memoizedState,
          baseState: yl.baseState,
          baseQueue: yl.baseQueue,
          queue: yl.queue,
          next: null,
        }),
        ql === null ? (w.memoizedState = ql = l) : (ql = ql.next = l);
    }
    return ql;
  }
  function Zi() {
    return { lastEffect: null, events: null, stores: null, memoCache: null };
  }
  function ke(l) {
    var t = we;
    return (
      (we += 1),
      ee === null && (ee = []),
      (l = zs(ee, l, t)),
      (t = w),
      (ql === null ? t.memoizedState : ql.next) === null &&
        ((t = t.alternate),
        (S.H = t === null || t.memoizedState === null ? so : oo)),
      l
    );
  }
  function Fu(l) {
    if (l !== null && typeof l == "object") {
      if (typeof l.then == "function") return ke(l);
      if (l.$$typeof === vl) return wl(l);
    }
    throw Error(s(438, String(l)));
  }
  function Li(l) {
    var t = null,
      a = w.updateQueue;
    if ((a !== null && (t = a.memoCache), t == null)) {
      var e = w.alternate;
      e !== null &&
        ((e = e.updateQueue),
        e !== null &&
          ((e = e.memoCache),
          e != null &&
            (t = {
              data: e.data.map(function (u) {
                return u.slice();
              }),
              index: 0,
            })));
    }
    if (
      (t == null && (t = { data: [], index: 0 }),
      a === null && ((a = Zi()), (w.updateQueue = a)),
      (a.memoCache = t),
      (a = t.data[t.index]),
      a === void 0)
    )
      for (a = t.data[t.index] = Array(l), e = 0; e < l; e++) a[e] = Ul;
    return t.index++, a;
  }
  function Ct(l, t) {
    return typeof t == "function" ? t(l) : t;
  }
  function Iu(l) {
    var t = Bl();
    return Vi(t, yl, l);
  }
  function Vi(l, t, a) {
    var e = l.queue;
    if (e === null) throw Error(s(311));
    e.lastRenderedReducer = a;
    var u = l.baseQueue,
      n = e.pending;
    if (n !== null) {
      if (u !== null) {
        var i = u.next;
        (u.next = n.next), (n.next = i);
      }
      (t.baseQueue = u = n), (e.pending = null);
    }
    if (((n = l.baseState), u === null)) l.memoizedState = n;
    else {
      t = u.next;
      var c = (i = null),
        f = null,
        y = t,
        T = !1;
      do {
        var E = y.lane & -536870913;
        if (E !== y.lane ? (ll & E) === E : (Pt & E) === E) {
          var m = y.revertLane;
          if (m === 0)
            f !== null &&
              (f = f.next =
                {
                  lane: 0,
                  revertLane: 0,
                  action: y.action,
                  hasEagerState: y.hasEagerState,
                  eagerState: y.eagerState,
                  next: null,
                }),
              E === Pa && (T = !0);
          else if ((Pt & m) === m) {
            (y = y.next), m === Pa && (T = !0);
            continue;
          } else
            (E = {
              lane: 0,
              revertLane: y.revertLane,
              action: y.action,
              hasEagerState: y.hasEagerState,
              eagerState: y.eagerState,
              next: null,
            }),
              f === null ? ((c = f = E), (i = n)) : (f = f.next = E),
              (w.lanes |= m),
              (ia |= m);
          (E = y.action),
            Ra && a(n, E),
            (n = y.hasEagerState ? y.eagerState : a(n, E));
        } else
          (m = {
            lane: E,
            revertLane: y.revertLane,
            action: y.action,
            hasEagerState: y.hasEagerState,
            eagerState: y.eagerState,
            next: null,
          }),
            f === null ? ((c = f = m), (i = n)) : (f = f.next = m),
            (w.lanes |= E),
            (ia |= E);
        y = y.next;
      } while (y !== null && y !== t);
      if (
        (f === null ? (i = n) : (f.next = c),
        !it(n, l.memoizedState) && ((Xl = !0), T && ((a = le), a !== null)))
      )
        throw a;
      (l.memoizedState = n),
        (l.baseState = i),
        (l.baseQueue = f),
        (e.lastRenderedState = n);
    }
    return u === null && (e.lanes = 0), [l.memoizedState, e.dispatch];
  }
  function Ki(l) {
    var t = Bl(),
      a = t.queue;
    if (a === null) throw Error(s(311));
    a.lastRenderedReducer = l;
    var e = a.dispatch,
      u = a.pending,
      n = t.memoizedState;
    if (u !== null) {
      a.pending = null;
      var i = (u = u.next);
      do (n = l(n, i.action)), (i = i.next);
      while (i !== u);
      it(n, t.memoizedState) || (Xl = !0),
        (t.memoizedState = n),
        t.baseQueue === null && (t.baseState = n),
        (a.lastRenderedState = n);
    }
    return [n, e];
  }
  function Rs(l, t, a) {
    var e = w,
      u = Bl(),
      n = il;
    if (n) {
      if (a === void 0) throw Error(s(407));
      a = a();
    } else a = t();
    var i = !it((yl || u).memoizedState, a);
    i && ((u.memoizedState = a), (Xl = !0)), (u = u.queue);
    var c = xs.bind(null, e, u, l);
    if (
      ($e(2048, 8, c, [l]),
      u.getSnapshot !== t || i || (ql !== null && ql.memoizedState.tag & 1))
    ) {
      if (
        ((e.flags |= 2048),
        ue(9, Pu(), Ns.bind(null, e, u, a, t), null),
        Sl === null)
      )
        throw Error(s(349));
      n || (Pt & 124) !== 0 || Us(e, t, a);
    }
    return a;
  }
  function Us(l, t, a) {
    (l.flags |= 16384),
      (l = { getSnapshot: t, value: a }),
      (t = w.updateQueue),
      t === null
        ? ((t = Zi()), (w.updateQueue = t), (t.stores = [l]))
        : ((a = t.stores), a === null ? (t.stores = [l]) : a.push(l));
  }
  function Ns(l, t, a, e) {
    (t.value = a), (t.getSnapshot = e), Hs(t) && qs(l);
  }
  function xs(l, t, a) {
    return a(function () {
      Hs(t) && qs(l);
    });
  }
  function Hs(l) {
    var t = l.getSnapshot;
    l = l.value;
    try {
      var a = t();
      return !it(l, a);
    } catch {
      return !0;
    }
  }
  function qs(l) {
    var t = $a(l, 2);
    t !== null && dt(t, l, 2);
  }
  function Ji(l) {
    var t = lt();
    if (typeof l == "function") {
      var a = l;
      if (((l = a()), Ra)) {
        Jt(!0);
        try {
          a();
        } finally {
          Jt(!1);
        }
      }
    }
    return (
      (t.memoizedState = t.baseState = l),
      (t.queue = {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: Ct,
        lastRenderedState: l,
      }),
      t
    );
  }
  function Bs(l, t, a, e) {
    return (l.baseState = a), Vi(l, yl, typeof e == "function" ? e : Ct);
  }
  function Q0(l, t, a, e, u) {
    if (tn(l)) throw Error(s(485));
    if (((l = t.action), l !== null)) {
      var n = {
        payload: u,
        action: l,
        next: null,
        isTransition: !0,
        status: "pending",
        value: null,
        reason: null,
        listeners: [],
        then: function (i) {
          n.listeners.push(i);
        },
      };
      S.T !== null ? a(!0) : (n.isTransition = !1),
        e(n),
        (a = t.pending),
        a === null
          ? ((n.next = t.pending = n), Ys(t, n))
          : ((n.next = a.next), (t.pending = a.next = n));
    }
  }
  function Ys(l, t) {
    var a = t.action,
      e = t.payload,
      u = l.state;
    if (t.isTransition) {
      var n = S.T,
        i = {};
      S.T = i;
      try {
        var c = a(u, e),
          f = S.S;
        f !== null && f(i, c), js(l, t, c);
      } catch (y) {
        wi(l, t, y);
      } finally {
        S.T = n;
      }
    } else
      try {
        (n = a(u, e)), js(l, t, n);
      } catch (y) {
        wi(l, t, y);
      }
  }
  function js(l, t, a) {
    a !== null && typeof a == "object" && typeof a.then == "function"
      ? a.then(
          function (e) {
            Cs(l, t, e);
          },
          function (e) {
            return wi(l, t, e);
          },
        )
      : Cs(l, t, a);
  }
  function Cs(l, t, a) {
    (t.status = "fulfilled"),
      (t.value = a),
      Gs(t),
      (l.state = a),
      (t = l.pending),
      t !== null &&
        ((a = t.next),
        a === t ? (l.pending = null) : ((a = a.next), (t.next = a), Ys(l, a)));
  }
  function wi(l, t, a) {
    var e = l.pending;
    if (((l.pending = null), e !== null)) {
      e = e.next;
      do (t.status = "rejected"), (t.reason = a), Gs(t), (t = t.next);
      while (t !== e);
    }
    l.action = null;
  }
  function Gs(l) {
    l = l.listeners;
    for (var t = 0; t < l.length; t++) (0, l[t])();
  }
  function Xs(l, t) {
    return t;
  }
  function Qs(l, t) {
    if (il) {
      var a = Sl.formState;
      if (a !== null) {
        l: {
          var e = w;
          if (il) {
            if (Dl) {
              t: {
                for (var u = Dl, n = Ot; u.nodeType !== 8; ) {
                  if (!n) {
                    u = null;
                    break t;
                  }
                  if (((u = pt(u.nextSibling)), u === null)) {
                    u = null;
                    break t;
                  }
                }
                (n = u.data), (u = n === "F!" || n === "F" ? u : null);
              }
              if (u) {
                (Dl = pt(u.nextSibling)), (e = u.data === "F!");
                break l;
              }
            }
            _a(e);
          }
          e = !1;
        }
        e && (t = a[0]);
      }
    }
    return (
      (a = lt()),
      (a.memoizedState = a.baseState = t),
      (e = {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: Xs,
        lastRenderedState: t,
      }),
      (a.queue = e),
      (a = io.bind(null, w, e)),
      (e.dispatch = a),
      (e = Ji(!1)),
      (n = Ii.bind(null, w, !1, e.queue)),
      (e = lt()),
      (u = { state: t, dispatch: null, action: l, pending: null }),
      (e.queue = u),
      (a = Q0.bind(null, w, u, n, a)),
      (u.dispatch = a),
      (e.memoizedState = l),
      [t, a, !1]
    );
  }
  function Zs(l) {
    var t = Bl();
    return Ls(t, yl, l);
  }
  function Ls(l, t, a) {
    if (
      ((t = Vi(l, t, Xs)[0]),
      (l = Iu(Ct)[0]),
      typeof t == "object" && t !== null && typeof t.then == "function")
    )
      try {
        var e = ke(t);
      } catch (i) {
        throw i === Ze ? Ju : i;
      }
    else e = t;
    t = Bl();
    var u = t.queue,
      n = u.dispatch;
    return (
      a !== t.memoizedState &&
        ((w.flags |= 2048), ue(9, Pu(), Z0.bind(null, u, a), null)),
      [e, n, l]
    );
  }
  function Z0(l, t) {
    l.action = t;
  }
  function Vs(l) {
    var t = Bl(),
      a = yl;
    if (a !== null) return Ls(t, a, l);
    Bl(), (t = t.memoizedState), (a = Bl());
    var e = a.queue.dispatch;
    return (a.memoizedState = l), [t, e, !1];
  }
  function ue(l, t, a, e) {
    return (
      (l = { tag: l, create: a, deps: e, inst: t, next: null }),
      (t = w.updateQueue),
      t === null && ((t = Zi()), (w.updateQueue = t)),
      (a = t.lastEffect),
      a === null
        ? (t.lastEffect = l.next = l)
        : ((e = a.next), (a.next = l), (l.next = e), (t.lastEffect = l)),
      l
    );
  }
  function Pu() {
    return { destroy: void 0, resource: void 0 };
  }
  function Ks() {
    return Bl().memoizedState;
  }
  function ln(l, t, a, e) {
    var u = lt();
    (e = e === void 0 ? null : e),
      (w.flags |= l),
      (u.memoizedState = ue(1 | t, Pu(), a, e));
  }
  function $e(l, t, a, e) {
    var u = Bl();
    e = e === void 0 ? null : e;
    var n = u.memoizedState.inst;
    yl !== null && e !== null && ji(e, yl.memoizedState.deps)
      ? (u.memoizedState = ue(t, n, a, e))
      : ((w.flags |= l), (u.memoizedState = ue(1 | t, n, a, e)));
  }
  function Js(l, t) {
    ln(8390656, 8, l, t);
  }
  function ws(l, t) {
    $e(2048, 8, l, t);
  }
  function ks(l, t) {
    return $e(4, 2, l, t);
  }
  function $s(l, t) {
    return $e(4, 4, l, t);
  }
  function Ws(l, t) {
    if (typeof t == "function") {
      l = l();
      var a = t(l);
      return function () {
        typeof a == "function" ? a() : t(null);
      };
    }
    if (t != null)
      return (
        (l = l()),
        (t.current = l),
        function () {
          t.current = null;
        }
      );
  }
  function Fs(l, t, a) {
    (a = a != null ? a.concat([l]) : null), $e(4, 4, Ws.bind(null, t, l), a);
  }
  function ki() {}
  function Is(l, t) {
    var a = Bl();
    t = t === void 0 ? null : t;
    var e = a.memoizedState;
    return t !== null && ji(t, e[1]) ? e[0] : ((a.memoizedState = [l, t]), l);
  }
  function Ps(l, t) {
    var a = Bl();
    t = t === void 0 ? null : t;
    var e = a.memoizedState;
    if (t !== null && ji(t, e[1])) return e[0];
    if (((e = l()), Ra)) {
      Jt(!0);
      try {
        l();
      } finally {
        Jt(!1);
      }
    }
    return (a.memoizedState = [e, t]), e;
  }
  function $i(l, t, a) {
    return a === void 0 || (Pt & 1073741824) !== 0
      ? (l.memoizedState = t)
      : ((l.memoizedState = a), (l = er()), (w.lanes |= l), (ia |= l), a);
  }
  function lo(l, t, a, e) {
    return it(a, t)
      ? a
      : te.current !== null
        ? ((l = $i(l, a, e)), it(l, t) || (Xl = !0), l)
        : (Pt & 42) === 0
          ? ((Xl = !0), (l.memoizedState = a))
          : ((l = er()), (w.lanes |= l), (ia |= l), t);
  }
  function to(l, t, a, e, u) {
    var n = M.p;
    M.p = n !== 0 && 8 > n ? n : 8;
    var i = S.T,
      c = {};
    (S.T = c), Ii(l, !1, t, a);
    try {
      var f = u(),
        y = S.S;
      if (
        (y !== null && y(c, f),
        f !== null && typeof f == "object" && typeof f.then == "function")
      ) {
        var T = C0(f, e);
        We(l, t, T, rt(l));
      } else We(l, t, e, rt(l));
    } catch (E) {
      We(l, t, { then: function () {}, status: "rejected", reason: E }, rt());
    } finally {
      (M.p = n), (S.T = i);
    }
  }
  function L0() {}
  function Wi(l, t, a, e) {
    if (l.tag !== 5) throw Error(s(476));
    var u = ao(l).queue;
    to(
      l,
      u,
      t,
      Y,
      a === null
        ? L0
        : function () {
            return eo(l), a(e);
          },
    );
  }
  function ao(l) {
    var t = l.memoizedState;
    if (t !== null) return t;
    t = {
      memoizedState: Y,
      baseState: Y,
      baseQueue: null,
      queue: {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: Ct,
        lastRenderedState: Y,
      },
      next: null,
    };
    var a = {};
    return (
      (t.next = {
        memoizedState: a,
        baseState: a,
        baseQueue: null,
        queue: {
          pending: null,
          lanes: 0,
          dispatch: null,
          lastRenderedReducer: Ct,
          lastRenderedState: a,
        },
        next: null,
      }),
      (l.memoizedState = t),
      (l = l.alternate),
      l !== null && (l.memoizedState = t),
      t
    );
  }
  function eo(l) {
    var t = ao(l).next.queue;
    We(l, t, {}, rt());
  }
  function Fi() {
    return wl(yu);
  }
  function uo() {
    return Bl().memoizedState;
  }
  function no() {
    return Bl().memoizedState;
  }
  function V0(l) {
    for (var t = l.return; t !== null; ) {
      switch (t.tag) {
        case 24:
        case 3:
          var a = rt();
          l = Ft(a);
          var e = It(t, l, a);
          e !== null && (dt(e, t, a), Ve(e, t, a)),
            (t = { cache: Mi() }),
            (l.payload = t);
          return;
      }
      t = t.return;
    }
  }
  function K0(l, t, a) {
    var e = rt();
    (a = {
      lane: e,
      revertLane: 0,
      action: a,
      hasEagerState: !1,
      eagerState: null,
      next: null,
    }),
      tn(l)
        ? co(t, a)
        : ((a = gi(l, t, a, e)), a !== null && (dt(a, l, e), fo(a, t, e)));
  }
  function io(l, t, a) {
    var e = rt();
    We(l, t, a, e);
  }
  function We(l, t, a, e) {
    var u = {
      lane: e,
      revertLane: 0,
      action: a,
      hasEagerState: !1,
      eagerState: null,
      next: null,
    };
    if (tn(l)) co(t, u);
    else {
      var n = l.alternate;
      if (
        l.lanes === 0 &&
        (n === null || n.lanes === 0) &&
        ((n = t.lastRenderedReducer), n !== null)
      )
        try {
          var i = t.lastRenderedState,
            c = n(i, a);
          if (((u.hasEagerState = !0), (u.eagerState = c), it(c, i)))
            return Cu(l, t, u, 0), Sl === null && ju(), !1;
        } catch {
        } finally {
        }
      if (((a = gi(l, t, u, e)), a !== null))
        return dt(a, l, e), fo(a, t, e), !0;
    }
    return !1;
  }
  function Ii(l, t, a, e) {
    if (
      ((e = {
        lane: 2,
        revertLane: Uc(),
        action: e,
        hasEagerState: !1,
        eagerState: null,
        next: null,
      }),
      tn(l))
    ) {
      if (t) throw Error(s(479));
    } else (t = gi(l, a, e, 2)), t !== null && dt(t, l, 2);
  }
  function tn(l) {
    var t = l.alternate;
    return l === w || (t !== null && t === w);
  }
  function co(l, t) {
    ae = $u = !0;
    var a = l.pending;
    a === null ? (t.next = t) : ((t.next = a.next), (a.next = t)),
      (l.pending = t);
  }
  function fo(l, t, a) {
    if ((a & 4194048) !== 0) {
      var e = t.lanes;
      (e &= l.pendingLanes), (a |= e), (t.lanes = a), mf(l, a);
    }
  }
  var an = {
      readContext: wl,
      use: Fu,
      useCallback: Nl,
      useContext: Nl,
      useEffect: Nl,
      useImperativeHandle: Nl,
      useLayoutEffect: Nl,
      useInsertionEffect: Nl,
      useMemo: Nl,
      useReducer: Nl,
      useRef: Nl,
      useState: Nl,
      useDebugValue: Nl,
      useDeferredValue: Nl,
      useTransition: Nl,
      useSyncExternalStore: Nl,
      useId: Nl,
      useHostTransitionStatus: Nl,
      useFormState: Nl,
      useActionState: Nl,
      useOptimistic: Nl,
      useMemoCache: Nl,
      useCacheRefresh: Nl,
    },
    so = {
      readContext: wl,
      use: Fu,
      useCallback: function (l, t) {
        return (lt().memoizedState = [l, t === void 0 ? null : t]), l;
      },
      useContext: wl,
      useEffect: Js,
      useImperativeHandle: function (l, t, a) {
        (a = a != null ? a.concat([l]) : null),
          ln(4194308, 4, Ws.bind(null, t, l), a);
      },
      useLayoutEffect: function (l, t) {
        return ln(4194308, 4, l, t);
      },
      useInsertionEffect: function (l, t) {
        ln(4, 2, l, t);
      },
      useMemo: function (l, t) {
        var a = lt();
        t = t === void 0 ? null : t;
        var e = l();
        if (Ra) {
          Jt(!0);
          try {
            l();
          } finally {
            Jt(!1);
          }
        }
        return (a.memoizedState = [e, t]), e;
      },
      useReducer: function (l, t, a) {
        var e = lt();
        if (a !== void 0) {
          var u = a(t);
          if (Ra) {
            Jt(!0);
            try {
              a(t);
            } finally {
              Jt(!1);
            }
          }
        } else u = t;
        return (
          (e.memoizedState = e.baseState = u),
          (l = {
            pending: null,
            lanes: 0,
            dispatch: null,
            lastRenderedReducer: l,
            lastRenderedState: u,
          }),
          (e.queue = l),
          (l = l.dispatch = K0.bind(null, w, l)),
          [e.memoizedState, l]
        );
      },
      useRef: function (l) {
        var t = lt();
        return (l = { current: l }), (t.memoizedState = l);
      },
      useState: function (l) {
        l = Ji(l);
        var t = l.queue,
          a = io.bind(null, w, t);
        return (t.dispatch = a), [l.memoizedState, a];
      },
      useDebugValue: ki,
      useDeferredValue: function (l, t) {
        var a = lt();
        return $i(a, l, t);
      },
      useTransition: function () {
        var l = Ji(!1);
        return (
          (l = to.bind(null, w, l.queue, !0, !1)),
          (lt().memoizedState = l),
          [!1, l]
        );
      },
      useSyncExternalStore: function (l, t, a) {
        var e = w,
          u = lt();
        if (il) {
          if (a === void 0) throw Error(s(407));
          a = a();
        } else {
          if (((a = t()), Sl === null)) throw Error(s(349));
          (ll & 124) !== 0 || Us(e, t, a);
        }
        u.memoizedState = a;
        var n = { value: a, getSnapshot: t };
        return (
          (u.queue = n),
          Js(xs.bind(null, e, n, l), [l]),
          (e.flags |= 2048),
          ue(9, Pu(), Ns.bind(null, e, n, a, t), null),
          a
        );
      },
      useId: function () {
        var l = lt(),
          t = Sl.identifierPrefix;
        if (il) {
          var a = Bt,
            e = qt;
          (a = (e & ~(1 << (32 - nt(e) - 1))).toString(32) + a),
            (t = "«" + t + "R" + a),
            (a = Wu++),
            0 < a && (t += "H" + a.toString(32)),
            (t += "»");
        } else (a = G0++), (t = "«" + t + "r" + a.toString(32) + "»");
        return (l.memoizedState = t);
      },
      useHostTransitionStatus: Fi,
      useFormState: Qs,
      useActionState: Qs,
      useOptimistic: function (l) {
        var t = lt();
        t.memoizedState = t.baseState = l;
        var a = {
          pending: null,
          lanes: 0,
          dispatch: null,
          lastRenderedReducer: null,
          lastRenderedState: null,
        };
        return (
          (t.queue = a), (t = Ii.bind(null, w, !0, a)), (a.dispatch = t), [l, t]
        );
      },
      useMemoCache: Li,
      useCacheRefresh: function () {
        return (lt().memoizedState = V0.bind(null, w));
      },
    },
    oo = {
      readContext: wl,
      use: Fu,
      useCallback: Is,
      useContext: wl,
      useEffect: ws,
      useImperativeHandle: Fs,
      useInsertionEffect: ks,
      useLayoutEffect: $s,
      useMemo: Ps,
      useReducer: Iu,
      useRef: Ks,
      useState: function () {
        return Iu(Ct);
      },
      useDebugValue: ki,
      useDeferredValue: function (l, t) {
        var a = Bl();
        return lo(a, yl.memoizedState, l, t);
      },
      useTransition: function () {
        var l = Iu(Ct)[0],
          t = Bl().memoizedState;
        return [typeof l == "boolean" ? l : ke(l), t];
      },
      useSyncExternalStore: Rs,
      useId: uo,
      useHostTransitionStatus: Fi,
      useFormState: Zs,
      useActionState: Zs,
      useOptimistic: function (l, t) {
        var a = Bl();
        return Bs(a, yl, l, t);
      },
      useMemoCache: Li,
      useCacheRefresh: no,
    },
    J0 = {
      readContext: wl,
      use: Fu,
      useCallback: Is,
      useContext: wl,
      useEffect: ws,
      useImperativeHandle: Fs,
      useInsertionEffect: ks,
      useLayoutEffect: $s,
      useMemo: Ps,
      useReducer: Ki,
      useRef: Ks,
      useState: function () {
        return Ki(Ct);
      },
      useDebugValue: ki,
      useDeferredValue: function (l, t) {
        var a = Bl();
        return yl === null ? $i(a, l, t) : lo(a, yl.memoizedState, l, t);
      },
      useTransition: function () {
        var l = Ki(Ct)[0],
          t = Bl().memoizedState;
        return [typeof l == "boolean" ? l : ke(l), t];
      },
      useSyncExternalStore: Rs,
      useId: uo,
      useHostTransitionStatus: Fi,
      useFormState: Vs,
      useActionState: Vs,
      useOptimistic: function (l, t) {
        var a = Bl();
        return yl !== null
          ? Bs(a, yl, l, t)
          : ((a.baseState = l), [l, a.queue.dispatch]);
      },
      useMemoCache: Li,
      useCacheRefresh: no,
    },
    ne = null,
    Fe = 0;
  function en(l) {
    var t = Fe;
    return (Fe += 1), ne === null && (ne = []), zs(ne, l, t);
  }
  function Ie(l, t) {
    (t = t.props.ref), (l.ref = t !== void 0 ? t : null);
  }
  function un(l, t) {
    throw t.$$typeof === $
      ? Error(s(525))
      : ((l = Object.prototype.toString.call(t)),
        Error(
          s(
            31,
            l === "[object Object]"
              ? "object with keys {" + Object.keys(t).join(", ") + "}"
              : l,
          ),
        ));
  }
  function ro(l) {
    var t = l._init;
    return t(l._payload);
  }
  function vo(l) {
    function t(d, r) {
      if (l) {
        var v = d.deletions;
        v === null ? ((d.deletions = [r]), (d.flags |= 16)) : v.push(r);
      }
    }
    function a(d, r) {
      if (!l) return null;
      for (; r !== null; ) t(d, r), (r = r.sibling);
      return null;
    }
    function e(d) {
      for (var r = new Map(); d !== null; )
        d.key !== null ? r.set(d.key, d) : r.set(d.index, d), (d = d.sibling);
      return r;
    }
    function u(d, r) {
      return (d = Ht(d, r)), (d.index = 0), (d.sibling = null), d;
    }
    function n(d, r, v) {
      return (
        (d.index = v),
        l
          ? ((v = d.alternate),
            v !== null
              ? ((v = v.index), v < r ? ((d.flags |= 67108866), r) : v)
              : ((d.flags |= 67108866), r))
          : ((d.flags |= 1048576), r)
      );
    }
    function i(d) {
      return l && d.alternate === null && (d.flags |= 67108866), d;
    }
    function c(d, r, v, z) {
      return r === null || r.tag !== 6
        ? ((r = bi(v, d.mode, z)), (r.return = d), r)
        : ((r = u(r, v)), (r.return = d), r);
    }
    function f(d, r, v, z) {
      var H = v.type;
      return H === Tl
        ? T(d, r, v.props.children, z, v.key)
        : r !== null &&
            (r.elementType === H ||
              (typeof H == "object" &&
                H !== null &&
                H.$$typeof === cl &&
                ro(H) === r.type))
          ? ((r = u(r, v.props)), Ie(r, v), (r.return = d), r)
          : ((r = Xu(v.type, v.key, v.props, null, d.mode, z)),
            Ie(r, v),
            (r.return = d),
            r);
    }
    function y(d, r, v, z) {
      return r === null ||
        r.tag !== 4 ||
        r.stateNode.containerInfo !== v.containerInfo ||
        r.stateNode.implementation !== v.implementation
        ? ((r = Ti(v, d.mode, z)), (r.return = d), r)
        : ((r = u(r, v.children || [])), (r.return = d), r);
    }
    function T(d, r, v, z, H) {
      return r === null || r.tag !== 7
        ? ((r = za(v, d.mode, z, H)), (r.return = d), r)
        : ((r = u(r, v)), (r.return = d), r);
    }
    function E(d, r, v) {
      if (
        (typeof r == "string" && r !== "") ||
        typeof r == "number" ||
        typeof r == "bigint"
      )
        return (r = bi("" + r, d.mode, v)), (r.return = d), r;
      if (typeof r == "object" && r !== null) {
        switch (r.$$typeof) {
          case J:
            return (
              (v = Xu(r.type, r.key, r.props, null, d.mode, v)),
              Ie(v, r),
              (v.return = d),
              v
            );
          case bl:
            return (r = Ti(r, d.mode, v)), (r.return = d), r;
          case cl:
            var z = r._init;
            return (r = z(r._payload)), E(d, r, v);
        }
        if (Yl(r) || fl(r))
          return (r = za(r, d.mode, v, null)), (r.return = d), r;
        if (typeof r.then == "function") return E(d, en(r), v);
        if (r.$$typeof === vl) return E(d, Vu(d, r), v);
        un(d, r);
      }
      return null;
    }
    function m(d, r, v, z) {
      var H = r !== null ? r.key : null;
      if (
        (typeof v == "string" && v !== "") ||
        typeof v == "number" ||
        typeof v == "bigint"
      )
        return H !== null ? null : c(d, r, "" + v, z);
      if (typeof v == "object" && v !== null) {
        switch (v.$$typeof) {
          case J:
            return v.key === H ? f(d, r, v, z) : null;
          case bl:
            return v.key === H ? y(d, r, v, z) : null;
          case cl:
            return (H = v._init), (v = H(v._payload)), m(d, r, v, z);
        }
        if (Yl(v) || fl(v)) return H !== null ? null : T(d, r, v, z, null);
        if (typeof v.then == "function") return m(d, r, en(v), z);
        if (v.$$typeof === vl) return m(d, r, Vu(d, v), z);
        un(d, v);
      }
      return null;
    }
    function g(d, r, v, z, H) {
      if (
        (typeof z == "string" && z !== "") ||
        typeof z == "number" ||
        typeof z == "bigint"
      )
        return (d = d.get(v) || null), c(r, d, "" + z, H);
      if (typeof z == "object" && z !== null) {
        switch (z.$$typeof) {
          case J:
            return (
              (d = d.get(z.key === null ? v : z.key) || null), f(r, d, z, H)
            );
          case bl:
            return (
              (d = d.get(z.key === null ? v : z.key) || null), y(r, d, z, H)
            );
          case cl:
            var k = z._init;
            return (z = k(z._payload)), g(d, r, v, z, H);
        }
        if (Yl(z) || fl(z)) return (d = d.get(v) || null), T(r, d, z, H, null);
        if (typeof z.then == "function") return g(d, r, v, en(z), H);
        if (z.$$typeof === vl) return g(d, r, v, Vu(r, z), H);
        un(r, z);
      }
      return null;
    }
    function Z(d, r, v, z) {
      for (
        var H = null, k = null, B = r, G = (r = 0), Zl = null;
        B !== null && G < v.length;
        G++
      ) {
        B.index > G ? ((Zl = B), (B = null)) : (Zl = B.sibling);
        var nl = m(d, B, v[G], z);
        if (nl === null) {
          B === null && (B = Zl);
          break;
        }
        l && B && nl.alternate === null && t(d, B),
          (r = n(nl, r, G)),
          k === null ? (H = nl) : (k.sibling = nl),
          (k = nl),
          (B = Zl);
      }
      if (G === v.length) return a(d, B), il && Aa(d, G), H;
      if (B === null) {
        for (; G < v.length; G++)
          (B = E(d, v[G], z)),
            B !== null &&
              ((r = n(B, r, G)),
              k === null ? (H = B) : (k.sibling = B),
              (k = B));
        return il && Aa(d, G), H;
      }
      for (B = e(B); G < v.length; G++)
        (Zl = g(B, d, G, v[G], z)),
          Zl !== null &&
            (l &&
              Zl.alternate !== null &&
              B.delete(Zl.key === null ? G : Zl.key),
            (r = n(Zl, r, G)),
            k === null ? (H = Zl) : (k.sibling = Zl),
            (k = Zl));
      return (
        l &&
          B.forEach(function (ha) {
            return t(d, ha);
          }),
        il && Aa(d, G),
        H
      );
    }
    function C(d, r, v, z) {
      if (v == null) throw Error(s(151));
      for (
        var H = null, k = null, B = r, G = (r = 0), Zl = null, nl = v.next();
        B !== null && !nl.done;
        G++, nl = v.next()
      ) {
        B.index > G ? ((Zl = B), (B = null)) : (Zl = B.sibling);
        var ha = m(d, B, nl.value, z);
        if (ha === null) {
          B === null && (B = Zl);
          break;
        }
        l && B && ha.alternate === null && t(d, B),
          (r = n(ha, r, G)),
          k === null ? (H = ha) : (k.sibling = ha),
          (k = ha),
          (B = Zl);
      }
      if (nl.done) return a(d, B), il && Aa(d, G), H;
      if (B === null) {
        for (; !nl.done; G++, nl = v.next())
          (nl = E(d, nl.value, z)),
            nl !== null &&
              ((r = n(nl, r, G)),
              k === null ? (H = nl) : (k.sibling = nl),
              (k = nl));
        return il && Aa(d, G), H;
      }
      for (B = e(B); !nl.done; G++, nl = v.next())
        (nl = g(B, d, G, nl.value, z)),
          nl !== null &&
            (l &&
              nl.alternate !== null &&
              B.delete(nl.key === null ? G : nl.key),
            (r = n(nl, r, G)),
            k === null ? (H = nl) : (k.sibling = nl),
            (k = nl));
      return (
        l &&
          B.forEach(function (wv) {
            return t(d, wv);
          }),
        il && Aa(d, G),
        H
      );
    }
    function ml(d, r, v, z) {
      if (
        (typeof v == "object" &&
          v !== null &&
          v.type === Tl &&
          v.key === null &&
          (v = v.props.children),
        typeof v == "object" && v !== null)
      ) {
        switch (v.$$typeof) {
          case J:
            l: {
              for (var H = v.key; r !== null; ) {
                if (r.key === H) {
                  if (((H = v.type), H === Tl)) {
                    if (r.tag === 7) {
                      a(d, r.sibling),
                        (z = u(r, v.props.children)),
                        (z.return = d),
                        (d = z);
                      break l;
                    }
                  } else if (
                    r.elementType === H ||
                    (typeof H == "object" &&
                      H !== null &&
                      H.$$typeof === cl &&
                      ro(H) === r.type)
                  ) {
                    a(d, r.sibling),
                      (z = u(r, v.props)),
                      Ie(z, v),
                      (z.return = d),
                      (d = z);
                    break l;
                  }
                  a(d, r);
                  break;
                } else t(d, r);
                r = r.sibling;
              }
              v.type === Tl
                ? ((z = za(v.props.children, d.mode, z, v.key)),
                  (z.return = d),
                  (d = z))
                : ((z = Xu(v.type, v.key, v.props, null, d.mode, z)),
                  Ie(z, v),
                  (z.return = d),
                  (d = z));
            }
            return i(d);
          case bl:
            l: {
              for (H = v.key; r !== null; ) {
                if (r.key === H)
                  if (
                    r.tag === 4 &&
                    r.stateNode.containerInfo === v.containerInfo &&
                    r.stateNode.implementation === v.implementation
                  ) {
                    a(d, r.sibling),
                      (z = u(r, v.children || [])),
                      (z.return = d),
                      (d = z);
                    break l;
                  } else {
                    a(d, r);
                    break;
                  }
                else t(d, r);
                r = r.sibling;
              }
              (z = Ti(v, d.mode, z)), (z.return = d), (d = z);
            }
            return i(d);
          case cl:
            return (H = v._init), (v = H(v._payload)), ml(d, r, v, z);
        }
        if (Yl(v)) return Z(d, r, v, z);
        if (fl(v)) {
          if (((H = fl(v)), typeof H != "function")) throw Error(s(150));
          return (v = H.call(v)), C(d, r, v, z);
        }
        if (typeof v.then == "function") return ml(d, r, en(v), z);
        if (v.$$typeof === vl) return ml(d, r, Vu(d, v), z);
        un(d, v);
      }
      return (typeof v == "string" && v !== "") ||
        typeof v == "number" ||
        typeof v == "bigint"
        ? ((v = "" + v),
          r !== null && r.tag === 6
            ? (a(d, r.sibling), (z = u(r, v)), (z.return = d), (d = z))
            : (a(d, r), (z = bi(v, d.mode, z)), (z.return = d), (d = z)),
          i(d))
        : a(d, r);
    }
    return function (d, r, v, z) {
      try {
        Fe = 0;
        var H = ml(d, r, v, z);
        return (ne = null), H;
      } catch (B) {
        if (B === Ze || B === Ju) throw B;
        var k = ct(29, B, null, d.mode);
        return (k.lanes = z), (k.return = d), k;
      } finally {
      }
    };
  }
  var ie = vo(!0),
    yo = vo(!1),
    bt = A(null),
    Mt = null;
  function la(l) {
    var t = l.alternate;
    D(Cl, Cl.current & 1),
      D(bt, l),
      Mt === null &&
        (t === null || te.current !== null || t.memoizedState !== null) &&
        (Mt = l);
  }
  function ho(l) {
    if (l.tag === 22) {
      if ((D(Cl, Cl.current), D(bt, l), Mt === null)) {
        var t = l.alternate;
        t !== null && t.memoizedState !== null && (Mt = l);
      }
    } else ta();
  }
  function ta() {
    D(Cl, Cl.current), D(bt, bt.current);
  }
  function Gt(l) {
    U(bt), Mt === l && (Mt = null), U(Cl);
  }
  var Cl = A(0);
  function nn(l) {
    for (var t = l; t !== null; ) {
      if (t.tag === 13) {
        var a = t.memoizedState;
        if (
          a !== null &&
          ((a = a.dehydrated), a === null || a.data === "$?" || Zc(a))
        )
          return t;
      } else if (t.tag === 19 && t.memoizedProps.revealOrder !== void 0) {
        if ((t.flags & 128) !== 0) return t;
      } else if (t.child !== null) {
        (t.child.return = t), (t = t.child);
        continue;
      }
      if (t === l) break;
      for (; t.sibling === null; ) {
        if (t.return === null || t.return === l) return null;
        t = t.return;
      }
      (t.sibling.return = t.return), (t = t.sibling);
    }
    return null;
  }
  function Pi(l, t, a, e) {
    (t = l.memoizedState),
      (a = a(e, t)),
      (a = a == null ? t : x({}, t, a)),
      (l.memoizedState = a),
      l.lanes === 0 && (l.updateQueue.baseState = a);
  }
  var lc = {
    enqueueSetState: function (l, t, a) {
      l = l._reactInternals;
      var e = rt(),
        u = Ft(e);
      (u.payload = t),
        a != null && (u.callback = a),
        (t = It(l, u, e)),
        t !== null && (dt(t, l, e), Ve(t, l, e));
    },
    enqueueReplaceState: function (l, t, a) {
      l = l._reactInternals;
      var e = rt(),
        u = Ft(e);
      (u.tag = 1),
        (u.payload = t),
        a != null && (u.callback = a),
        (t = It(l, u, e)),
        t !== null && (dt(t, l, e), Ve(t, l, e));
    },
    enqueueForceUpdate: function (l, t) {
      l = l._reactInternals;
      var a = rt(),
        e = Ft(a);
      (e.tag = 2),
        t != null && (e.callback = t),
        (t = It(l, e, a)),
        t !== null && (dt(t, l, a), Ve(t, l, a));
    },
  };
  function mo(l, t, a, e, u, n, i) {
    return (
      (l = l.stateNode),
      typeof l.shouldComponentUpdate == "function"
        ? l.shouldComponentUpdate(e, n, i)
        : t.prototype && t.prototype.isPureReactComponent
          ? !qe(a, e) || !qe(u, n)
          : !0
    );
  }
  function go(l, t, a, e) {
    (l = t.state),
      typeof t.componentWillReceiveProps == "function" &&
        t.componentWillReceiveProps(a, e),
      typeof t.UNSAFE_componentWillReceiveProps == "function" &&
        t.UNSAFE_componentWillReceiveProps(a, e),
      t.state !== l && lc.enqueueReplaceState(t, t.state, null);
  }
  function Ua(l, t) {
    var a = t;
    if ("ref" in t) {
      a = {};
      for (var e in t) e !== "ref" && (a[e] = t[e]);
    }
    if ((l = l.defaultProps)) {
      a === t && (a = x({}, a));
      for (var u in l) a[u] === void 0 && (a[u] = l[u]);
    }
    return a;
  }
  var cn =
    typeof reportError == "function"
      ? reportError
      : function (l) {
          if (
            typeof window == "object" &&
            typeof window.ErrorEvent == "function"
          ) {
            var t = new window.ErrorEvent("error", {
              bubbles: !0,
              cancelable: !0,
              message:
                typeof l == "object" &&
                l !== null &&
                typeof l.message == "string"
                  ? String(l.message)
                  : String(l),
              error: l,
            });
            if (!window.dispatchEvent(t)) return;
          } else if (
            typeof process == "object" &&
            typeof process.emit == "function"
          ) {
            process.emit("uncaughtException", l);
            return;
          }
          console.error(l);
        };
  function So(l) {
    cn(l);
  }
  function bo(l) {
    console.error(l);
  }
  function To(l) {
    cn(l);
  }
  function fn(l, t) {
    try {
      var a = l.onUncaughtError;
      a(t.value, { componentStack: t.stack });
    } catch (e) {
      setTimeout(function () {
        throw e;
      });
    }
  }
  function zo(l, t, a) {
    try {
      var e = l.onCaughtError;
      e(a.value, {
        componentStack: a.stack,
        errorBoundary: t.tag === 1 ? t.stateNode : null,
      });
    } catch (u) {
      setTimeout(function () {
        throw u;
      });
    }
  }
  function tc(l, t, a) {
    return (
      (a = Ft(a)),
      (a.tag = 3),
      (a.payload = { element: null }),
      (a.callback = function () {
        fn(l, t);
      }),
      a
    );
  }
  function Eo(l) {
    return (l = Ft(l)), (l.tag = 3), l;
  }
  function Ao(l, t, a, e) {
    var u = a.type.getDerivedStateFromError;
    if (typeof u == "function") {
      var n = e.value;
      (l.payload = function () {
        return u(n);
      }),
        (l.callback = function () {
          zo(t, a, e);
        });
    }
    var i = a.stateNode;
    i !== null &&
      typeof i.componentDidCatch == "function" &&
      (l.callback = function () {
        zo(t, a, e),
          typeof u != "function" &&
            (ca === null ? (ca = new Set([this])) : ca.add(this));
        var c = e.stack;
        this.componentDidCatch(e.value, {
          componentStack: c !== null ? c : "",
        });
      });
  }
  function w0(l, t, a, e, u) {
    if (
      ((a.flags |= 32768),
      e !== null && typeof e == "object" && typeof e.then == "function")
    ) {
      if (
        ((t = a.alternate),
        t !== null && Ge(t, a, u, !0),
        (a = bt.current),
        a !== null)
      ) {
        switch (a.tag) {
          case 13:
            return (
              Mt === null ? _c() : a.alternate === null && Rl === 0 && (Rl = 3),
              (a.flags &= -257),
              (a.flags |= 65536),
              (a.lanes = u),
              e === Ui
                ? (a.flags |= 16384)
                : ((t = a.updateQueue),
                  t === null ? (a.updateQueue = new Set([e])) : t.add(e),
                  Mc(l, e, u)),
              !1
            );
          case 22:
            return (
              (a.flags |= 65536),
              e === Ui
                ? (a.flags |= 16384)
                : ((t = a.updateQueue),
                  t === null
                    ? ((t = {
                        transitions: null,
                        markerInstances: null,
                        retryQueue: new Set([e]),
                      }),
                      (a.updateQueue = t))
                    : ((a = t.retryQueue),
                      a === null ? (t.retryQueue = new Set([e])) : a.add(e)),
                  Mc(l, e, u)),
              !1
            );
        }
        throw Error(s(435, a.tag));
      }
      return Mc(l, e, u), _c(), !1;
    }
    if (il)
      return (
        (t = bt.current),
        t !== null
          ? ((t.flags & 65536) === 0 && (t.flags |= 256),
            (t.flags |= 65536),
            (t.lanes = u),
            e !== Ai && ((l = Error(s(422), { cause: e })), Ce(ht(l, a))))
          : (e !== Ai && ((t = Error(s(423), { cause: e })), Ce(ht(t, a))),
            (l = l.current.alternate),
            (l.flags |= 65536),
            (u &= -u),
            (l.lanes |= u),
            (e = ht(e, a)),
            (u = tc(l.stateNode, e, u)),
            Hi(l, u),
            Rl !== 4 && (Rl = 2)),
        !1
      );
    var n = Error(s(520), { cause: e });
    if (
      ((n = ht(n, a)),
      nu === null ? (nu = [n]) : nu.push(n),
      Rl !== 4 && (Rl = 2),
      t === null)
    )
      return !0;
    (e = ht(e, a)), (a = t);
    do {
      switch (a.tag) {
        case 3:
          return (
            (a.flags |= 65536),
            (l = u & -u),
            (a.lanes |= l),
            (l = tc(a.stateNode, e, l)),
            Hi(a, l),
            !1
          );
        case 1:
          if (
            ((t = a.type),
            (n = a.stateNode),
            (a.flags & 128) === 0 &&
              (typeof t.getDerivedStateFromError == "function" ||
                (n !== null &&
                  typeof n.componentDidCatch == "function" &&
                  (ca === null || !ca.has(n)))))
          )
            return (
              (a.flags |= 65536),
              (u &= -u),
              (a.lanes |= u),
              (u = Eo(u)),
              Ao(u, l, a, e),
              Hi(a, u),
              !1
            );
      }
      a = a.return;
    } while (a !== null);
    return !1;
  }
  var po = Error(s(461)),
    Xl = !1;
  function Ll(l, t, a, e) {
    t.child = l === null ? yo(t, null, a, e) : ie(t, l.child, a, e);
  }
  function _o(l, t, a, e, u) {
    a = a.render;
    var n = t.ref;
    if ("ref" in e) {
      var i = {};
      for (var c in e) c !== "ref" && (i[c] = e[c]);
    } else i = e;
    return (
      Ma(t),
      (e = Ci(l, t, a, i, n, u)),
      (c = Gi()),
      l !== null && !Xl
        ? (Xi(l, t, u), Xt(l, t, u))
        : (il && c && zi(t), (t.flags |= 1), Ll(l, t, e, u), t.child)
    );
  }
  function Oo(l, t, a, e, u) {
    if (l === null) {
      var n = a.type;
      return typeof n == "function" &&
        !Si(n) &&
        n.defaultProps === void 0 &&
        a.compare === null
        ? ((t.tag = 15), (t.type = n), Mo(l, t, n, e, u))
        : ((l = Xu(a.type, null, e, t, t.mode, u)),
          (l.ref = t.ref),
          (l.return = t),
          (t.child = l));
    }
    if (((n = l.child), !sc(l, u))) {
      var i = n.memoizedProps;
      if (
        ((a = a.compare), (a = a !== null ? a : qe), a(i, e) && l.ref === t.ref)
      )
        return Xt(l, t, u);
    }
    return (
      (t.flags |= 1),
      (l = Ht(n, e)),
      (l.ref = t.ref),
      (l.return = t),
      (t.child = l)
    );
  }
  function Mo(l, t, a, e, u) {
    if (l !== null) {
      var n = l.memoizedProps;
      if (qe(n, e) && l.ref === t.ref)
        if (((Xl = !1), (t.pendingProps = e = n), sc(l, u)))
          (l.flags & 131072) !== 0 && (Xl = !0);
        else return (t.lanes = l.lanes), Xt(l, t, u);
    }
    return ac(l, t, a, e, u);
  }
  function Do(l, t, a) {
    var e = t.pendingProps,
      u = e.children,
      n = l !== null ? l.memoizedState : null;
    if (e.mode === "hidden") {
      if ((t.flags & 128) !== 0) {
        if (((e = n !== null ? n.baseLanes | a : a), l !== null)) {
          for (u = t.child = l.child, n = 0; u !== null; )
            (n = n | u.lanes | u.childLanes), (u = u.sibling);
          t.childLanes = n & ~e;
        } else (t.childLanes = 0), (t.child = null);
        return Ro(l, t, e, a);
      }
      if ((a & 536870912) !== 0)
        (t.memoizedState = { baseLanes: 0, cachePool: null }),
          l !== null && Ku(t, n !== null ? n.cachePool : null),
          n !== null ? Os(t, n) : Bi(),
          ho(t);
      else
        return (
          (t.lanes = t.childLanes = 536870912),
          Ro(l, t, n !== null ? n.baseLanes | a : a, a)
        );
    } else
      n !== null
        ? (Ku(t, n.cachePool), Os(t, n), ta(), (t.memoizedState = null))
        : (l !== null && Ku(t, null), Bi(), ta());
    return Ll(l, t, u, a), t.child;
  }
  function Ro(l, t, a, e) {
    var u = Ri();
    return (
      (u = u === null ? null : { parent: jl._currentValue, pool: u }),
      (t.memoizedState = { baseLanes: a, cachePool: u }),
      l !== null && Ku(t, null),
      Bi(),
      ho(t),
      l !== null && Ge(l, t, e, !0),
      null
    );
  }
  function sn(l, t) {
    var a = t.ref;
    if (a === null) l !== null && l.ref !== null && (t.flags |= 4194816);
    else {
      if (typeof a != "function" && typeof a != "object") throw Error(s(284));
      (l === null || l.ref !== a) && (t.flags |= 4194816);
    }
  }
  function ac(l, t, a, e, u) {
    return (
      Ma(t),
      (a = Ci(l, t, a, e, void 0, u)),
      (e = Gi()),
      l !== null && !Xl
        ? (Xi(l, t, u), Xt(l, t, u))
        : (il && e && zi(t), (t.flags |= 1), Ll(l, t, a, u), t.child)
    );
  }
  function Uo(l, t, a, e, u, n) {
    return (
      Ma(t),
      (t.updateQueue = null),
      (a = Ds(t, e, a, u)),
      Ms(l),
      (e = Gi()),
      l !== null && !Xl
        ? (Xi(l, t, n), Xt(l, t, n))
        : (il && e && zi(t), (t.flags |= 1), Ll(l, t, a, n), t.child)
    );
  }
  function No(l, t, a, e, u) {
    if ((Ma(t), t.stateNode === null)) {
      var n = Wa,
        i = a.contextType;
      typeof i == "object" && i !== null && (n = wl(i)),
        (n = new a(e, n)),
        (t.memoizedState =
          n.state !== null && n.state !== void 0 ? n.state : null),
        (n.updater = lc),
        (t.stateNode = n),
        (n._reactInternals = t),
        (n = t.stateNode),
        (n.props = e),
        (n.state = t.memoizedState),
        (n.refs = {}),
        Ni(t),
        (i = a.contextType),
        (n.context = typeof i == "object" && i !== null ? wl(i) : Wa),
        (n.state = t.memoizedState),
        (i = a.getDerivedStateFromProps),
        typeof i == "function" && (Pi(t, a, i, e), (n.state = t.memoizedState)),
        typeof a.getDerivedStateFromProps == "function" ||
          typeof n.getSnapshotBeforeUpdate == "function" ||
          (typeof n.UNSAFE_componentWillMount != "function" &&
            typeof n.componentWillMount != "function") ||
          ((i = n.state),
          typeof n.componentWillMount == "function" && n.componentWillMount(),
          typeof n.UNSAFE_componentWillMount == "function" &&
            n.UNSAFE_componentWillMount(),
          i !== n.state && lc.enqueueReplaceState(n, n.state, null),
          Je(t, e, n, u),
          Ke(),
          (n.state = t.memoizedState)),
        typeof n.componentDidMount == "function" && (t.flags |= 4194308),
        (e = !0);
    } else if (l === null) {
      n = t.stateNode;
      var c = t.memoizedProps,
        f = Ua(a, c);
      n.props = f;
      var y = n.context,
        T = a.contextType;
      (i = Wa), typeof T == "object" && T !== null && (i = wl(T));
      var E = a.getDerivedStateFromProps;
      (T =
        typeof E == "function" ||
        typeof n.getSnapshotBeforeUpdate == "function"),
        (c = t.pendingProps !== c),
        T ||
          (typeof n.UNSAFE_componentWillReceiveProps != "function" &&
            typeof n.componentWillReceiveProps != "function") ||
          ((c || y !== i) && go(t, n, e, i)),
        (Wt = !1);
      var m = t.memoizedState;
      (n.state = m),
        Je(t, e, n, u),
        Ke(),
        (y = t.memoizedState),
        c || m !== y || Wt
          ? (typeof E == "function" && (Pi(t, a, E, e), (y = t.memoizedState)),
            (f = Wt || mo(t, a, f, e, m, y, i))
              ? (T ||
                  (typeof n.UNSAFE_componentWillMount != "function" &&
                    typeof n.componentWillMount != "function") ||
                  (typeof n.componentWillMount == "function" &&
                    n.componentWillMount(),
                  typeof n.UNSAFE_componentWillMount == "function" &&
                    n.UNSAFE_componentWillMount()),
                typeof n.componentDidMount == "function" &&
                  (t.flags |= 4194308))
              : (typeof n.componentDidMount == "function" &&
                  (t.flags |= 4194308),
                (t.memoizedProps = e),
                (t.memoizedState = y)),
            (n.props = e),
            (n.state = y),
            (n.context = i),
            (e = f))
          : (typeof n.componentDidMount == "function" && (t.flags |= 4194308),
            (e = !1));
    } else {
      (n = t.stateNode),
        xi(l, t),
        (i = t.memoizedProps),
        (T = Ua(a, i)),
        (n.props = T),
        (E = t.pendingProps),
        (m = n.context),
        (y = a.contextType),
        (f = Wa),
        typeof y == "object" && y !== null && (f = wl(y)),
        (c = a.getDerivedStateFromProps),
        (y =
          typeof c == "function" ||
          typeof n.getSnapshotBeforeUpdate == "function") ||
          (typeof n.UNSAFE_componentWillReceiveProps != "function" &&
            typeof n.componentWillReceiveProps != "function") ||
          ((i !== E || m !== f) && go(t, n, e, f)),
        (Wt = !1),
        (m = t.memoizedState),
        (n.state = m),
        Je(t, e, n, u),
        Ke();
      var g = t.memoizedState;
      i !== E ||
      m !== g ||
      Wt ||
      (l !== null && l.dependencies !== null && Lu(l.dependencies))
        ? (typeof c == "function" && (Pi(t, a, c, e), (g = t.memoizedState)),
          (T =
            Wt ||
            mo(t, a, T, e, m, g, f) ||
            (l !== null && l.dependencies !== null && Lu(l.dependencies)))
            ? (y ||
                (typeof n.UNSAFE_componentWillUpdate != "function" &&
                  typeof n.componentWillUpdate != "function") ||
                (typeof n.componentWillUpdate == "function" &&
                  n.componentWillUpdate(e, g, f),
                typeof n.UNSAFE_componentWillUpdate == "function" &&
                  n.UNSAFE_componentWillUpdate(e, g, f)),
              typeof n.componentDidUpdate == "function" && (t.flags |= 4),
              typeof n.getSnapshotBeforeUpdate == "function" &&
                (t.flags |= 1024))
            : (typeof n.componentDidUpdate != "function" ||
                (i === l.memoizedProps && m === l.memoizedState) ||
                (t.flags |= 4),
              typeof n.getSnapshotBeforeUpdate != "function" ||
                (i === l.memoizedProps && m === l.memoizedState) ||
                (t.flags |= 1024),
              (t.memoizedProps = e),
              (t.memoizedState = g)),
          (n.props = e),
          (n.state = g),
          (n.context = f),
          (e = T))
        : (typeof n.componentDidUpdate != "function" ||
            (i === l.memoizedProps && m === l.memoizedState) ||
            (t.flags |= 4),
          typeof n.getSnapshotBeforeUpdate != "function" ||
            (i === l.memoizedProps && m === l.memoizedState) ||
            (t.flags |= 1024),
          (e = !1));
    }
    return (
      (n = e),
      sn(l, t),
      (e = (t.flags & 128) !== 0),
      n || e
        ? ((n = t.stateNode),
          (a =
            e && typeof a.getDerivedStateFromError != "function"
              ? null
              : n.render()),
          (t.flags |= 1),
          l !== null && e
            ? ((t.child = ie(t, l.child, null, u)),
              (t.child = ie(t, null, a, u)))
            : Ll(l, t, a, u),
          (t.memoizedState = n.state),
          (l = t.child))
        : (l = Xt(l, t, u)),
      l
    );
  }
  function xo(l, t, a, e) {
    return je(), (t.flags |= 256), Ll(l, t, a, e), t.child;
  }
  var ec = {
    dehydrated: null,
    treeContext: null,
    retryLane: 0,
    hydrationErrors: null,
  };
  function uc(l) {
    return { baseLanes: l, cachePool: Ss() };
  }
  function nc(l, t, a) {
    return (l = l !== null ? l.childLanes & ~a : 0), t && (l |= Tt), l;
  }
  function Ho(l, t, a) {
    var e = t.pendingProps,
      u = !1,
      n = (t.flags & 128) !== 0,
      i;
    if (
      ((i = n) ||
        (i =
          l !== null && l.memoizedState === null ? !1 : (Cl.current & 2) !== 0),
      i && ((u = !0), (t.flags &= -129)),
      (i = (t.flags & 32) !== 0),
      (t.flags &= -33),
      l === null)
    ) {
      if (il) {
        if ((u ? la(t) : ta(), il)) {
          var c = Dl,
            f;
          if ((f = c)) {
            l: {
              for (f = c, c = Ot; f.nodeType !== 8; ) {
                if (!c) {
                  c = null;
                  break l;
                }
                if (((f = pt(f.nextSibling)), f === null)) {
                  c = null;
                  break l;
                }
              }
              c = f;
            }
            c !== null
              ? ((t.memoizedState = {
                  dehydrated: c,
                  treeContext: Ea !== null ? { id: qt, overflow: Bt } : null,
                  retryLane: 536870912,
                  hydrationErrors: null,
                }),
                (f = ct(18, null, null, 0)),
                (f.stateNode = c),
                (f.return = t),
                (t.child = f),
                (Wl = t),
                (Dl = null),
                (f = !0))
              : (f = !1);
          }
          f || _a(t);
        }
        if (
          ((c = t.memoizedState),
          c !== null && ((c = c.dehydrated), c !== null))
        )
          return Zc(c) ? (t.lanes = 32) : (t.lanes = 536870912), null;
        Gt(t);
      }
      return (
        (c = e.children),
        (e = e.fallback),
        u
          ? (ta(),
            (u = t.mode),
            (c = on({ mode: "hidden", children: c }, u)),
            (e = za(e, u, a, null)),
            (c.return = t),
            (e.return = t),
            (c.sibling = e),
            (t.child = c),
            (u = t.child),
            (u.memoizedState = uc(a)),
            (u.childLanes = nc(l, i, a)),
            (t.memoizedState = ec),
            e)
          : (la(t), ic(t, c))
      );
    }
    if (
      ((f = l.memoizedState), f !== null && ((c = f.dehydrated), c !== null))
    ) {
      if (n)
        t.flags & 256
          ? (la(t), (t.flags &= -257), (t = cc(l, t, a)))
          : t.memoizedState !== null
            ? (ta(), (t.child = l.child), (t.flags |= 128), (t = null))
            : (ta(),
              (u = e.fallback),
              (c = t.mode),
              (e = on({ mode: "visible", children: e.children }, c)),
              (u = za(u, c, a, null)),
              (u.flags |= 2),
              (e.return = t),
              (u.return = t),
              (e.sibling = u),
              (t.child = e),
              ie(t, l.child, null, a),
              (e = t.child),
              (e.memoizedState = uc(a)),
              (e.childLanes = nc(l, i, a)),
              (t.memoizedState = ec),
              (t = u));
      else if ((la(t), Zc(c))) {
        if (((i = c.nextSibling && c.nextSibling.dataset), i)) var y = i.dgst;
        (i = y),
          (e = Error(s(419))),
          (e.stack = ""),
          (e.digest = i),
          Ce({ value: e, source: null, stack: null }),
          (t = cc(l, t, a));
      } else if (
        (Xl || Ge(l, t, a, !1), (i = (a & l.childLanes) !== 0), Xl || i)
      ) {
        if (
          ((i = Sl),
          i !== null &&
            ((e = a & -a),
            (e = (e & 42) !== 0 ? 1 : Ln(e)),
            (e = (e & (i.suspendedLanes | a)) !== 0 ? 0 : e),
            e !== 0 && e !== f.retryLane))
        )
          throw ((f.retryLane = e), $a(l, e), dt(i, l, e), po);
        c.data === "$?" || _c(), (t = cc(l, t, a));
      } else
        c.data === "$?"
          ? ((t.flags |= 192), (t.child = l.child), (t = null))
          : ((l = f.treeContext),
            (Dl = pt(c.nextSibling)),
            (Wl = t),
            (il = !0),
            (pa = null),
            (Ot = !1),
            l !== null &&
              ((gt[St++] = qt),
              (gt[St++] = Bt),
              (gt[St++] = Ea),
              (qt = l.id),
              (Bt = l.overflow),
              (Ea = t)),
            (t = ic(t, e.children)),
            (t.flags |= 4096));
      return t;
    }
    return u
      ? (ta(),
        (u = e.fallback),
        (c = t.mode),
        (f = l.child),
        (y = f.sibling),
        (e = Ht(f, { mode: "hidden", children: e.children })),
        (e.subtreeFlags = f.subtreeFlags & 65011712),
        y !== null ? (u = Ht(y, u)) : ((u = za(u, c, a, null)), (u.flags |= 2)),
        (u.return = t),
        (e.return = t),
        (e.sibling = u),
        (t.child = e),
        (e = u),
        (u = t.child),
        (c = l.child.memoizedState),
        c === null
          ? (c = uc(a))
          : ((f = c.cachePool),
            f !== null
              ? ((y = jl._currentValue),
                (f = f.parent !== y ? { parent: y, pool: y } : f))
              : (f = Ss()),
            (c = { baseLanes: c.baseLanes | a, cachePool: f })),
        (u.memoizedState = c),
        (u.childLanes = nc(l, i, a)),
        (t.memoizedState = ec),
        e)
      : (la(t),
        (a = l.child),
        (l = a.sibling),
        (a = Ht(a, { mode: "visible", children: e.children })),
        (a.return = t),
        (a.sibling = null),
        l !== null &&
          ((i = t.deletions),
          i === null ? ((t.deletions = [l]), (t.flags |= 16)) : i.push(l)),
        (t.child = a),
        (t.memoizedState = null),
        a);
  }
  function ic(l, t) {
    return (
      (t = on({ mode: "visible", children: t }, l.mode)),
      (t.return = l),
      (l.child = t)
    );
  }
  function on(l, t) {
    return (
      (l = ct(22, l, null, t)),
      (l.lanes = 0),
      (l.stateNode = {
        _visibility: 1,
        _pendingMarkers: null,
        _retryCache: null,
        _transitions: null,
      }),
      l
    );
  }
  function cc(l, t, a) {
    return (
      ie(t, l.child, null, a),
      (l = ic(t, t.pendingProps.children)),
      (l.flags |= 2),
      (t.memoizedState = null),
      l
    );
  }
  function qo(l, t, a) {
    l.lanes |= t;
    var e = l.alternate;
    e !== null && (e.lanes |= t), _i(l.return, t, a);
  }
  function fc(l, t, a, e, u) {
    var n = l.memoizedState;
    n === null
      ? (l.memoizedState = {
          isBackwards: t,
          rendering: null,
          renderingStartTime: 0,
          last: e,
          tail: a,
          tailMode: u,
        })
      : ((n.isBackwards = t),
        (n.rendering = null),
        (n.renderingStartTime = 0),
        (n.last = e),
        (n.tail = a),
        (n.tailMode = u));
  }
  function Bo(l, t, a) {
    var e = t.pendingProps,
      u = e.revealOrder,
      n = e.tail;
    if ((Ll(l, t, e.children, a), (e = Cl.current), (e & 2) !== 0))
      (e = (e & 1) | 2), (t.flags |= 128);
    else {
      if (l !== null && (l.flags & 128) !== 0)
        l: for (l = t.child; l !== null; ) {
          if (l.tag === 13) l.memoizedState !== null && qo(l, a, t);
          else if (l.tag === 19) qo(l, a, t);
          else if (l.child !== null) {
            (l.child.return = l), (l = l.child);
            continue;
          }
          if (l === t) break;
          for (; l.sibling === null; ) {
            if (l.return === null || l.return === t) break l;
            l = l.return;
          }
          (l.sibling.return = l.return), (l = l.sibling);
        }
      e &= 1;
    }
    switch ((D(Cl, e), u)) {
      case "forwards":
        for (a = t.child, u = null; a !== null; )
          (l = a.alternate),
            l !== null && nn(l) === null && (u = a),
            (a = a.sibling);
        (a = u),
          a === null
            ? ((u = t.child), (t.child = null))
            : ((u = a.sibling), (a.sibling = null)),
          fc(t, !1, u, a, n);
        break;
      case "backwards":
        for (a = null, u = t.child, t.child = null; u !== null; ) {
          if (((l = u.alternate), l !== null && nn(l) === null)) {
            t.child = u;
            break;
          }
          (l = u.sibling), (u.sibling = a), (a = u), (u = l);
        }
        fc(t, !0, a, null, n);
        break;
      case "together":
        fc(t, !1, null, null, void 0);
        break;
      default:
        t.memoizedState = null;
    }
    return t.child;
  }
  function Xt(l, t, a) {
    if (
      (l !== null && (t.dependencies = l.dependencies),
      (ia |= t.lanes),
      (a & t.childLanes) === 0)
    )
      if (l !== null) {
        if ((Ge(l, t, a, !1), (a & t.childLanes) === 0)) return null;
      } else return null;
    if (l !== null && t.child !== l.child) throw Error(s(153));
    if (t.child !== null) {
      for (
        l = t.child, a = Ht(l, l.pendingProps), t.child = a, a.return = t;
        l.sibling !== null;
      )
        (l = l.sibling),
          (a = a.sibling = Ht(l, l.pendingProps)),
          (a.return = t);
      a.sibling = null;
    }
    return t.child;
  }
  function sc(l, t) {
    return (l.lanes & t) !== 0
      ? !0
      : ((l = l.dependencies), !!(l !== null && Lu(l)));
  }
  function k0(l, t, a) {
    switch (t.tag) {
      case 3:
        al(t, t.stateNode.containerInfo),
          $t(t, jl, l.memoizedState.cache),
          je();
        break;
      case 27:
      case 5:
        qa(t);
        break;
      case 4:
        al(t, t.stateNode.containerInfo);
        break;
      case 10:
        $t(t, t.type, t.memoizedProps.value);
        break;
      case 13:
        var e = t.memoizedState;
        if (e !== null)
          return e.dehydrated !== null
            ? (la(t), (t.flags |= 128), null)
            : (a & t.child.childLanes) !== 0
              ? Ho(l, t, a)
              : (la(t), (l = Xt(l, t, a)), l !== null ? l.sibling : null);
        la(t);
        break;
      case 19:
        var u = (l.flags & 128) !== 0;
        if (
          ((e = (a & t.childLanes) !== 0),
          e || (Ge(l, t, a, !1), (e = (a & t.childLanes) !== 0)),
          u)
        ) {
          if (e) return Bo(l, t, a);
          t.flags |= 128;
        }
        if (
          ((u = t.memoizedState),
          u !== null &&
            ((u.rendering = null), (u.tail = null), (u.lastEffect = null)),
          D(Cl, Cl.current),
          e)
        )
          break;
        return null;
      case 22:
      case 23:
        return (t.lanes = 0), Do(l, t, a);
      case 24:
        $t(t, jl, l.memoizedState.cache);
    }
    return Xt(l, t, a);
  }
  function Yo(l, t, a) {
    if (l !== null)
      if (l.memoizedProps !== t.pendingProps) Xl = !0;
      else {
        if (!sc(l, a) && (t.flags & 128) === 0) return (Xl = !1), k0(l, t, a);
        Xl = (l.flags & 131072) !== 0;
      }
    else (Xl = !1), il && (t.flags & 1048576) !== 0 && rs(t, Zu, t.index);
    switch (((t.lanes = 0), t.tag)) {
      case 16:
        l: {
          l = t.pendingProps;
          var e = t.elementType,
            u = e._init;
          if (((e = u(e._payload)), (t.type = e), typeof e == "function"))
            Si(e)
              ? ((l = Ua(e, l)), (t.tag = 1), (t = No(null, t, e, l, a)))
              : ((t.tag = 0), (t = ac(null, t, e, l, a)));
          else {
            if (e != null) {
              if (((u = e.$$typeof), u === pl)) {
                (t.tag = 11), (t = _o(null, t, e, l, a));
                break l;
              } else if (u === ul) {
                (t.tag = 14), (t = Oo(null, t, e, l, a));
                break l;
              }
            }
            throw ((t = et(e) || e), Error(s(306, t, "")));
          }
        }
        return t;
      case 0:
        return ac(l, t, t.type, t.pendingProps, a);
      case 1:
        return (e = t.type), (u = Ua(e, t.pendingProps)), No(l, t, e, u, a);
      case 3:
        l: {
          if ((al(t, t.stateNode.containerInfo), l === null))
            throw Error(s(387));
          e = t.pendingProps;
          var n = t.memoizedState;
          (u = n.element), xi(l, t), Je(t, e, null, a);
          var i = t.memoizedState;
          if (
            ((e = i.cache),
            $t(t, jl, e),
            e !== n.cache && Oi(t, [jl], a, !0),
            Ke(),
            (e = i.element),
            n.isDehydrated)
          )
            if (
              ((n = { element: e, isDehydrated: !1, cache: i.cache }),
              (t.updateQueue.baseState = n),
              (t.memoizedState = n),
              t.flags & 256)
            ) {
              t = xo(l, t, e, a);
              break l;
            } else if (e !== u) {
              (u = ht(Error(s(424)), t)), Ce(u), (t = xo(l, t, e, a));
              break l;
            } else {
              switch (((l = t.stateNode.containerInfo), l.nodeType)) {
                case 9:
                  l = l.body;
                  break;
                default:
                  l = l.nodeName === "HTML" ? l.ownerDocument.body : l;
              }
              for (
                Dl = pt(l.firstChild),
                  Wl = t,
                  il = !0,
                  pa = null,
                  Ot = !0,
                  a = yo(t, null, e, a),
                  t.child = a;
                a;
              )
                (a.flags = (a.flags & -3) | 4096), (a = a.sibling);
            }
          else {
            if ((je(), e === u)) {
              t = Xt(l, t, a);
              break l;
            }
            Ll(l, t, e, a);
          }
          t = t.child;
        }
        return t;
      case 26:
        return (
          sn(l, t),
          l === null
            ? (a = Xr(t.type, null, t.pendingProps, null))
              ? (t.memoizedState = a)
              : il ||
                ((a = t.type),
                (l = t.pendingProps),
                (e = pn(Q.current).createElement(a)),
                (e[Jl] = t),
                (e[Il] = l),
                Kl(e, a, l),
                Gl(e),
                (t.stateNode = e))
            : (t.memoizedState = Xr(
                t.type,
                l.memoizedProps,
                t.pendingProps,
                l.memoizedState,
              )),
          null
        );
      case 27:
        return (
          qa(t),
          l === null &&
            il &&
            ((e = t.stateNode = jr(t.type, t.pendingProps, Q.current)),
            (Wl = t),
            (Ot = !0),
            (u = Dl),
            oa(t.type) ? ((Lc = u), (Dl = pt(e.firstChild))) : (Dl = u)),
          Ll(l, t, t.pendingProps.children, a),
          sn(l, t),
          l === null && (t.flags |= 4194304),
          t.child
        );
      case 5:
        return (
          l === null &&
            il &&
            ((u = e = Dl) &&
              ((e = Ev(e, t.type, t.pendingProps, Ot)),
              e !== null
                ? ((t.stateNode = e),
                  (Wl = t),
                  (Dl = pt(e.firstChild)),
                  (Ot = !1),
                  (u = !0))
                : (u = !1)),
            u || _a(t)),
          qa(t),
          (u = t.type),
          (n = t.pendingProps),
          (i = l !== null ? l.memoizedProps : null),
          (e = n.children),
          Gc(u, n) ? (e = null) : i !== null && Gc(u, i) && (t.flags |= 32),
          t.memoizedState !== null &&
            ((u = Ci(l, t, X0, null, null, a)), (yu._currentValue = u)),
          sn(l, t),
          Ll(l, t, e, a),
          t.child
        );
      case 6:
        return (
          l === null &&
            il &&
            ((l = a = Dl) &&
              ((a = Av(a, t.pendingProps, Ot)),
              a !== null
                ? ((t.stateNode = a), (Wl = t), (Dl = null), (l = !0))
                : (l = !1)),
            l || _a(t)),
          null
        );
      case 13:
        return Ho(l, t, a);
      case 4:
        return (
          al(t, t.stateNode.containerInfo),
          (e = t.pendingProps),
          l === null ? (t.child = ie(t, null, e, a)) : Ll(l, t, e, a),
          t.child
        );
      case 11:
        return _o(l, t, t.type, t.pendingProps, a);
      case 7:
        return Ll(l, t, t.pendingProps, a), t.child;
      case 8:
        return Ll(l, t, t.pendingProps.children, a), t.child;
      case 12:
        return Ll(l, t, t.pendingProps.children, a), t.child;
      case 10:
        return (
          (e = t.pendingProps),
          $t(t, t.type, e.value),
          Ll(l, t, e.children, a),
          t.child
        );
      case 9:
        return (
          (u = t.type._context),
          (e = t.pendingProps.children),
          Ma(t),
          (u = wl(u)),
          (e = e(u)),
          (t.flags |= 1),
          Ll(l, t, e, a),
          t.child
        );
      case 14:
        return Oo(l, t, t.type, t.pendingProps, a);
      case 15:
        return Mo(l, t, t.type, t.pendingProps, a);
      case 19:
        return Bo(l, t, a);
      case 31:
        return (
          (e = t.pendingProps),
          (a = t.mode),
          (e = { mode: e.mode, children: e.children }),
          l === null
            ? ((a = on(e, a)),
              (a.ref = t.ref),
              (t.child = a),
              (a.return = t),
              (t = a))
            : ((a = Ht(l.child, e)),
              (a.ref = t.ref),
              (t.child = a),
              (a.return = t),
              (t = a)),
          t
        );
      case 22:
        return Do(l, t, a);
      case 24:
        return (
          Ma(t),
          (e = wl(jl)),
          l === null
            ? ((u = Ri()),
              u === null &&
                ((u = Sl),
                (n = Mi()),
                (u.pooledCache = n),
                n.refCount++,
                n !== null && (u.pooledCacheLanes |= a),
                (u = n)),
              (t.memoizedState = { parent: e, cache: u }),
              Ni(t),
              $t(t, jl, u))
            : ((l.lanes & a) !== 0 && (xi(l, t), Je(t, null, null, a), Ke()),
              (u = l.memoizedState),
              (n = t.memoizedState),
              u.parent !== e
                ? ((u = { parent: e, cache: e }),
                  (t.memoizedState = u),
                  t.lanes === 0 &&
                    (t.memoizedState = t.updateQueue.baseState = u),
                  $t(t, jl, e))
                : ((e = n.cache),
                  $t(t, jl, e),
                  e !== u.cache && Oi(t, [jl], a, !0))),
          Ll(l, t, t.pendingProps.children, a),
          t.child
        );
      case 29:
        throw t.pendingProps;
    }
    throw Error(s(156, t.tag));
  }
  function Qt(l) {
    l.flags |= 4;
  }
  function jo(l, t) {
    if (t.type !== "stylesheet" || (t.state.loading & 4) !== 0)
      l.flags &= -16777217;
    else if (((l.flags |= 16777216), !Kr(t))) {
      if (
        ((t = bt.current),
        t !== null &&
          ((ll & 4194048) === ll
            ? Mt !== null
            : ((ll & 62914560) !== ll && (ll & 536870912) === 0) || t !== Mt))
      )
        throw ((Le = Ui), bs);
      l.flags |= 8192;
    }
  }
  function rn(l, t) {
    t !== null && (l.flags |= 4),
      l.flags & 16384 &&
        ((t = l.tag !== 22 ? yf() : 536870912), (l.lanes |= t), (oe |= t));
  }
  function Pe(l, t) {
    if (!il)
      switch (l.tailMode) {
        case "hidden":
          t = l.tail;
          for (var a = null; t !== null; )
            t.alternate !== null && (a = t), (t = t.sibling);
          a === null ? (l.tail = null) : (a.sibling = null);
          break;
        case "collapsed":
          a = l.tail;
          for (var e = null; a !== null; )
            a.alternate !== null && (e = a), (a = a.sibling);
          e === null
            ? t || l.tail === null
              ? (l.tail = null)
              : (l.tail.sibling = null)
            : (e.sibling = null);
      }
  }
  function _l(l) {
    var t = l.alternate !== null && l.alternate.child === l.child,
      a = 0,
      e = 0;
    if (t)
      for (var u = l.child; u !== null; )
        (a |= u.lanes | u.childLanes),
          (e |= u.subtreeFlags & 65011712),
          (e |= u.flags & 65011712),
          (u.return = l),
          (u = u.sibling);
    else
      for (u = l.child; u !== null; )
        (a |= u.lanes | u.childLanes),
          (e |= u.subtreeFlags),
          (e |= u.flags),
          (u.return = l),
          (u = u.sibling);
    return (l.subtreeFlags |= e), (l.childLanes = a), t;
  }
  function $0(l, t, a) {
    var e = t.pendingProps;
    switch ((Ei(t), t.tag)) {
      case 31:
      case 16:
      case 15:
      case 0:
      case 11:
      case 7:
      case 8:
      case 12:
      case 9:
      case 14:
        return _l(t), null;
      case 1:
        return _l(t), null;
      case 3:
        return (
          (a = t.stateNode),
          (e = null),
          l !== null && (e = l.memoizedState.cache),
          t.memoizedState.cache !== e && (t.flags |= 2048),
          jt(jl),
          Fl(),
          a.pendingContext &&
            ((a.context = a.pendingContext), (a.pendingContext = null)),
          (l === null || l.child === null) &&
            (Ye(t)
              ? Qt(t)
              : l === null ||
                (l.memoizedState.isDehydrated && (t.flags & 256) === 0) ||
                ((t.flags |= 1024), ys())),
          _l(t),
          null
        );
      case 26:
        return (
          (a = t.memoizedState),
          l === null
            ? (Qt(t),
              a !== null ? (_l(t), jo(t, a)) : (_l(t), (t.flags &= -16777217)))
            : a
              ? a !== l.memoizedState
                ? (Qt(t), _l(t), jo(t, a))
                : (_l(t), (t.flags &= -16777217))
              : (l.memoizedProps !== e && Qt(t), _l(t), (t.flags &= -16777217)),
          null
        );
      case 27:
        zu(t), (a = Q.current);
        var u = t.type;
        if (l !== null && t.stateNode != null) l.memoizedProps !== e && Qt(t);
        else {
          if (!e) {
            if (t.stateNode === null) throw Error(s(166));
            return _l(t), null;
          }
          (l = j.current),
            Ye(t) ? ds(t) : ((l = jr(u, e, a)), (t.stateNode = l), Qt(t));
        }
        return _l(t), null;
      case 5:
        if ((zu(t), (a = t.type), l !== null && t.stateNode != null))
          l.memoizedProps !== e && Qt(t);
        else {
          if (!e) {
            if (t.stateNode === null) throw Error(s(166));
            return _l(t), null;
          }
          if (((l = j.current), Ye(t))) ds(t);
          else {
            switch (((u = pn(Q.current)), l)) {
              case 1:
                l = u.createElementNS("http://www.w3.org/2000/svg", a);
                break;
              case 2:
                l = u.createElementNS("http://www.w3.org/1998/Math/MathML", a);
                break;
              default:
                switch (a) {
                  case "svg":
                    l = u.createElementNS("http://www.w3.org/2000/svg", a);
                    break;
                  case "math":
                    l = u.createElementNS(
                      "http://www.w3.org/1998/Math/MathML",
                      a,
                    );
                    break;
                  case "script":
                    (l = u.createElement("div")),
                      (l.innerHTML = "<script></script>"),
                      (l = l.removeChild(l.firstChild));
                    break;
                  case "select":
                    (l =
                      typeof e.is == "string"
                        ? u.createElement("select", { is: e.is })
                        : u.createElement("select")),
                      e.multiple
                        ? (l.multiple = !0)
                        : e.size && (l.size = e.size);
                    break;
                  default:
                    l =
                      typeof e.is == "string"
                        ? u.createElement(a, { is: e.is })
                        : u.createElement(a);
                }
            }
            (l[Jl] = t), (l[Il] = e);
            l: for (u = t.child; u !== null; ) {
              if (u.tag === 5 || u.tag === 6) l.appendChild(u.stateNode);
              else if (u.tag !== 4 && u.tag !== 27 && u.child !== null) {
                (u.child.return = u), (u = u.child);
                continue;
              }
              if (u === t) break;
              for (; u.sibling === null; ) {
                if (u.return === null || u.return === t) break l;
                u = u.return;
              }
              (u.sibling.return = u.return), (u = u.sibling);
            }
            t.stateNode = l;
            switch ((Kl(l, a, e), a)) {
              case "button":
              case "input":
              case "select":
              case "textarea":
                l = !!e.autoFocus;
                break;
              case "img":
                l = !0;
                break;
              default:
                l = !1;
            }
            l && Qt(t);
          }
        }
        return _l(t), (t.flags &= -16777217), null;
      case 6:
        if (l && t.stateNode != null) l.memoizedProps !== e && Qt(t);
        else {
          if (typeof e != "string" && t.stateNode === null) throw Error(s(166));
          if (((l = Q.current), Ye(t))) {
            if (
              ((l = t.stateNode),
              (a = t.memoizedProps),
              (e = null),
              (u = Wl),
              u !== null)
            )
              switch (u.tag) {
                case 27:
                case 5:
                  e = u.memoizedProps;
              }
            (l[Jl] = t),
              (l = !!(
                l.nodeValue === a ||
                (e !== null && e.suppressHydrationWarning === !0) ||
                Ur(l.nodeValue, a)
              )),
              l || _a(t);
          } else (l = pn(l).createTextNode(e)), (l[Jl] = t), (t.stateNode = l);
        }
        return _l(t), null;
      case 13:
        if (
          ((e = t.memoizedState),
          l === null ||
            (l.memoizedState !== null && l.memoizedState.dehydrated !== null))
        ) {
          if (((u = Ye(t)), e !== null && e.dehydrated !== null)) {
            if (l === null) {
              if (!u) throw Error(s(318));
              if (
                ((u = t.memoizedState),
                (u = u !== null ? u.dehydrated : null),
                !u)
              )
                throw Error(s(317));
              u[Jl] = t;
            } else
              je(),
                (t.flags & 128) === 0 && (t.memoizedState = null),
                (t.flags |= 4);
            _l(t), (u = !1);
          } else
            (u = ys()),
              l !== null &&
                l.memoizedState !== null &&
                (l.memoizedState.hydrationErrors = u),
              (u = !0);
          if (!u) return t.flags & 256 ? (Gt(t), t) : (Gt(t), null);
        }
        if ((Gt(t), (t.flags & 128) !== 0)) return (t.lanes = a), t;
        if (
          ((a = e !== null), (l = l !== null && l.memoizedState !== null), a)
        ) {
          (e = t.child),
            (u = null),
            e.alternate !== null &&
              e.alternate.memoizedState !== null &&
              e.alternate.memoizedState.cachePool !== null &&
              (u = e.alternate.memoizedState.cachePool.pool);
          var n = null;
          e.memoizedState !== null &&
            e.memoizedState.cachePool !== null &&
            (n = e.memoizedState.cachePool.pool),
            n !== u && (e.flags |= 2048);
        }
        return (
          a !== l && a && (t.child.flags |= 8192),
          rn(t, t.updateQueue),
          _l(t),
          null
        );
      case 4:
        return Fl(), l === null && qc(t.stateNode.containerInfo), _l(t), null;
      case 10:
        return jt(t.type), _l(t), null;
      case 19:
        if ((U(Cl), (u = t.memoizedState), u === null)) return _l(t), null;
        if (((e = (t.flags & 128) !== 0), (n = u.rendering), n === null))
          if (e) Pe(u, !1);
          else {
            if (Rl !== 0 || (l !== null && (l.flags & 128) !== 0))
              for (l = t.child; l !== null; ) {
                if (((n = nn(l)), n !== null)) {
                  for (
                    t.flags |= 128,
                      Pe(u, !1),
                      l = n.updateQueue,
                      t.updateQueue = l,
                      rn(t, l),
                      t.subtreeFlags = 0,
                      l = a,
                      a = t.child;
                    a !== null;
                  )
                    os(a, l), (a = a.sibling);
                  return D(Cl, (Cl.current & 1) | 2), t.child;
                }
                l = l.sibling;
              }
            u.tail !== null &&
              _t() > yn &&
              ((t.flags |= 128), (e = !0), Pe(u, !1), (t.lanes = 4194304));
          }
        else {
          if (!e)
            if (((l = nn(n)), l !== null)) {
              if (
                ((t.flags |= 128),
                (e = !0),
                (l = l.updateQueue),
                (t.updateQueue = l),
                rn(t, l),
                Pe(u, !0),
                u.tail === null &&
                  u.tailMode === "hidden" &&
                  !n.alternate &&
                  !il)
              )
                return _l(t), null;
            } else
              2 * _t() - u.renderingStartTime > yn &&
                a !== 536870912 &&
                ((t.flags |= 128), (e = !0), Pe(u, !1), (t.lanes = 4194304));
          u.isBackwards
            ? ((n.sibling = t.child), (t.child = n))
            : ((l = u.last),
              l !== null ? (l.sibling = n) : (t.child = n),
              (u.last = n));
        }
        return u.tail !== null
          ? ((t = u.tail),
            (u.rendering = t),
            (u.tail = t.sibling),
            (u.renderingStartTime = _t()),
            (t.sibling = null),
            (l = Cl.current),
            D(Cl, e ? (l & 1) | 2 : l & 1),
            t)
          : (_l(t), null);
      case 22:
      case 23:
        return (
          Gt(t),
          Yi(),
          (e = t.memoizedState !== null),
          l !== null
            ? (l.memoizedState !== null) !== e && (t.flags |= 8192)
            : e && (t.flags |= 8192),
          e
            ? (a & 536870912) !== 0 &&
              (t.flags & 128) === 0 &&
              (_l(t), t.subtreeFlags & 6 && (t.flags |= 8192))
            : _l(t),
          (a = t.updateQueue),
          a !== null && rn(t, a.retryQueue),
          (a = null),
          l !== null &&
            l.memoizedState !== null &&
            l.memoizedState.cachePool !== null &&
            (a = l.memoizedState.cachePool.pool),
          (e = null),
          t.memoizedState !== null &&
            t.memoizedState.cachePool !== null &&
            (e = t.memoizedState.cachePool.pool),
          e !== a && (t.flags |= 2048),
          l !== null && U(Da),
          null
        );
      case 24:
        return (
          (a = null),
          l !== null && (a = l.memoizedState.cache),
          t.memoizedState.cache !== a && (t.flags |= 2048),
          jt(jl),
          _l(t),
          null
        );
      case 25:
        return null;
      case 30:
        return null;
    }
    throw Error(s(156, t.tag));
  }
  function W0(l, t) {
    switch ((Ei(t), t.tag)) {
      case 1:
        return (
          (l = t.flags), l & 65536 ? ((t.flags = (l & -65537) | 128), t) : null
        );
      case 3:
        return (
          jt(jl),
          Fl(),
          (l = t.flags),
          (l & 65536) !== 0 && (l & 128) === 0
            ? ((t.flags = (l & -65537) | 128), t)
            : null
        );
      case 26:
      case 27:
      case 5:
        return zu(t), null;
      case 13:
        if (
          (Gt(t), (l = t.memoizedState), l !== null && l.dehydrated !== null)
        ) {
          if (t.alternate === null) throw Error(s(340));
          je();
        }
        return (
          (l = t.flags), l & 65536 ? ((t.flags = (l & -65537) | 128), t) : null
        );
      case 19:
        return U(Cl), null;
      case 4:
        return Fl(), null;
      case 10:
        return jt(t.type), null;
      case 22:
      case 23:
        return (
          Gt(t),
          Yi(),
          l !== null && U(Da),
          (l = t.flags),
          l & 65536 ? ((t.flags = (l & -65537) | 128), t) : null
        );
      case 24:
        return jt(jl), null;
      case 25:
        return null;
      default:
        return null;
    }
  }
  function Co(l, t) {
    switch ((Ei(t), t.tag)) {
      case 3:
        jt(jl), Fl();
        break;
      case 26:
      case 27:
      case 5:
        zu(t);
        break;
      case 4:
        Fl();
        break;
      case 13:
        Gt(t);
        break;
      case 19:
        U(Cl);
        break;
      case 10:
        jt(t.type);
        break;
      case 22:
      case 23:
        Gt(t), Yi(), l !== null && U(Da);
        break;
      case 24:
        jt(jl);
    }
  }
  function lu(l, t) {
    try {
      var a = t.updateQueue,
        e = a !== null ? a.lastEffect : null;
      if (e !== null) {
        var u = e.next;
        a = u;
        do {
          if ((a.tag & l) === l) {
            e = void 0;
            var n = a.create,
              i = a.inst;
            (e = n()), (i.destroy = e);
          }
          a = a.next;
        } while (a !== u);
      }
    } catch (c) {
      gl(t, t.return, c);
    }
  }
  function aa(l, t, a) {
    try {
      var e = t.updateQueue,
        u = e !== null ? e.lastEffect : null;
      if (u !== null) {
        var n = u.next;
        e = n;
        do {
          if ((e.tag & l) === l) {
            var i = e.inst,
              c = i.destroy;
            if (c !== void 0) {
              (i.destroy = void 0), (u = t);
              var f = a,
                y = c;
              try {
                y();
              } catch (T) {
                gl(u, f, T);
              }
            }
          }
          e = e.next;
        } while (e !== n);
      }
    } catch (T) {
      gl(t, t.return, T);
    }
  }
  function Go(l) {
    var t = l.updateQueue;
    if (t !== null) {
      var a = l.stateNode;
      try {
        _s(t, a);
      } catch (e) {
        gl(l, l.return, e);
      }
    }
  }
  function Xo(l, t, a) {
    (a.props = Ua(l.type, l.memoizedProps)), (a.state = l.memoizedState);
    try {
      a.componentWillUnmount();
    } catch (e) {
      gl(l, t, e);
    }
  }
  function tu(l, t) {
    try {
      var a = l.ref;
      if (a !== null) {
        switch (l.tag) {
          case 26:
          case 27:
          case 5:
            var e = l.stateNode;
            break;
          case 30:
            e = l.stateNode;
            break;
          default:
            e = l.stateNode;
        }
        typeof a == "function" ? (l.refCleanup = a(e)) : (a.current = e);
      }
    } catch (u) {
      gl(l, t, u);
    }
  }
  function Dt(l, t) {
    var a = l.ref,
      e = l.refCleanup;
    if (a !== null)
      if (typeof e == "function")
        try {
          e();
        } catch (u) {
          gl(l, t, u);
        } finally {
          (l.refCleanup = null),
            (l = l.alternate),
            l != null && (l.refCleanup = null);
        }
      else if (typeof a == "function")
        try {
          a(null);
        } catch (u) {
          gl(l, t, u);
        }
      else a.current = null;
  }
  function Qo(l) {
    var t = l.type,
      a = l.memoizedProps,
      e = l.stateNode;
    try {
      switch (t) {
        case "button":
        case "input":
        case "select":
        case "textarea":
          a.autoFocus && e.focus();
          break;
        case "img":
          a.src ? (e.src = a.src) : a.srcSet && (e.srcset = a.srcSet);
      }
    } catch (u) {
      gl(l, l.return, u);
    }
  }
  function oc(l, t, a) {
    try {
      var e = l.stateNode;
      gv(e, l.type, a, t), (e[Il] = t);
    } catch (u) {
      gl(l, l.return, u);
    }
  }
  function Zo(l) {
    return (
      l.tag === 5 ||
      l.tag === 3 ||
      l.tag === 26 ||
      (l.tag === 27 && oa(l.type)) ||
      l.tag === 4
    );
  }
  function rc(l) {
    l: for (;;) {
      for (; l.sibling === null; ) {
        if (l.return === null || Zo(l.return)) return null;
        l = l.return;
      }
      for (
        l.sibling.return = l.return, l = l.sibling;
        l.tag !== 5 && l.tag !== 6 && l.tag !== 18;
      ) {
        if (
          (l.tag === 27 && oa(l.type)) ||
          l.flags & 2 ||
          l.child === null ||
          l.tag === 4
        )
          continue l;
        (l.child.return = l), (l = l.child);
      }
      if (!(l.flags & 2)) return l.stateNode;
    }
  }
  function dc(l, t, a) {
    var e = l.tag;
    if (e === 5 || e === 6)
      (l = l.stateNode),
        t
          ? (a.nodeType === 9
              ? a.body
              : a.nodeName === "HTML"
                ? a.ownerDocument.body
                : a
            ).insertBefore(l, t)
          : ((t =
              a.nodeType === 9
                ? a.body
                : a.nodeName === "HTML"
                  ? a.ownerDocument.body
                  : a),
            t.appendChild(l),
            (a = a._reactRootContainer),
            a != null || t.onclick !== null || (t.onclick = An));
    else if (
      e !== 4 &&
      (e === 27 && oa(l.type) && ((a = l.stateNode), (t = null)),
      (l = l.child),
      l !== null)
    )
      for (dc(l, t, a), l = l.sibling; l !== null; )
        dc(l, t, a), (l = l.sibling);
  }
  function dn(l, t, a) {
    var e = l.tag;
    if (e === 5 || e === 6)
      (l = l.stateNode), t ? a.insertBefore(l, t) : a.appendChild(l);
    else if (
      e !== 4 &&
      (e === 27 && oa(l.type) && (a = l.stateNode), (l = l.child), l !== null)
    )
      for (dn(l, t, a), l = l.sibling; l !== null; )
        dn(l, t, a), (l = l.sibling);
  }
  function Lo(l) {
    var t = l.stateNode,
      a = l.memoizedProps;
    try {
      for (var e = l.type, u = t.attributes; u.length; )
        t.removeAttributeNode(u[0]);
      Kl(t, e, a), (t[Jl] = l), (t[Il] = a);
    } catch (n) {
      gl(l, l.return, n);
    }
  }
  var Zt = !1,
    xl = !1,
    vc = !1,
    Vo = typeof WeakSet == "function" ? WeakSet : Set,
    Ql = null;
  function F0(l, t) {
    if (((l = l.containerInfo), (jc = Un), (l = ls(l)), ri(l))) {
      if ("selectionStart" in l)
        var a = { start: l.selectionStart, end: l.selectionEnd };
      else
        l: {
          a = ((a = l.ownerDocument) && a.defaultView) || window;
          var e = a.getSelection && a.getSelection();
          if (e && e.rangeCount !== 0) {
            a = e.anchorNode;
            var u = e.anchorOffset,
              n = e.focusNode;
            e = e.focusOffset;
            try {
              a.nodeType, n.nodeType;
            } catch {
              a = null;
              break l;
            }
            var i = 0,
              c = -1,
              f = -1,
              y = 0,
              T = 0,
              E = l,
              m = null;
            t: for (;;) {
              for (
                var g;
                E !== a || (u !== 0 && E.nodeType !== 3) || (c = i + u),
                  E !== n || (e !== 0 && E.nodeType !== 3) || (f = i + e),
                  E.nodeType === 3 && (i += E.nodeValue.length),
                  (g = E.firstChild) !== null;
              )
                (m = E), (E = g);
              for (;;) {
                if (E === l) break t;
                if (
                  (m === a && ++y === u && (c = i),
                  m === n && ++T === e && (f = i),
                  (g = E.nextSibling) !== null)
                )
                  break;
                (E = m), (m = E.parentNode);
              }
              E = g;
            }
            a = c === -1 || f === -1 ? null : { start: c, end: f };
          } else a = null;
        }
      a = a || { start: 0, end: 0 };
    } else a = null;
    for (
      Cc = { focusedElem: l, selectionRange: a }, Un = !1, Ql = t;
      Ql !== null;
    )
      if (
        ((t = Ql), (l = t.child), (t.subtreeFlags & 1024) !== 0 && l !== null)
      )
        (l.return = t), (Ql = l);
      else
        for (; Ql !== null; ) {
          switch (((t = Ql), (n = t.alternate), (l = t.flags), t.tag)) {
            case 0:
              break;
            case 11:
            case 15:
              break;
            case 1:
              if ((l & 1024) !== 0 && n !== null) {
                (l = void 0),
                  (a = t),
                  (u = n.memoizedProps),
                  (n = n.memoizedState),
                  (e = a.stateNode);
                try {
                  var Z = Ua(a.type, u, a.elementType === a.type);
                  (l = e.getSnapshotBeforeUpdate(Z, n)),
                    (e.__reactInternalSnapshotBeforeUpdate = l);
                } catch (C) {
                  gl(a, a.return, C);
                }
              }
              break;
            case 3:
              if ((l & 1024) !== 0) {
                if (
                  ((l = t.stateNode.containerInfo), (a = l.nodeType), a === 9)
                )
                  Qc(l);
                else if (a === 1)
                  switch (l.nodeName) {
                    case "HEAD":
                    case "HTML":
                    case "BODY":
                      Qc(l);
                      break;
                    default:
                      l.textContent = "";
                  }
              }
              break;
            case 5:
            case 26:
            case 27:
            case 6:
            case 4:
            case 17:
              break;
            default:
              if ((l & 1024) !== 0) throw Error(s(163));
          }
          if (((l = t.sibling), l !== null)) {
            (l.return = t.return), (Ql = l);
            break;
          }
          Ql = t.return;
        }
  }
  function Ko(l, t, a) {
    var e = a.flags;
    switch (a.tag) {
      case 0:
      case 11:
      case 15:
        ea(l, a), e & 4 && lu(5, a);
        break;
      case 1:
        if ((ea(l, a), e & 4))
          if (((l = a.stateNode), t === null))
            try {
              l.componentDidMount();
            } catch (i) {
              gl(a, a.return, i);
            }
          else {
            var u = Ua(a.type, t.memoizedProps);
            t = t.memoizedState;
            try {
              l.componentDidUpdate(u, t, l.__reactInternalSnapshotBeforeUpdate);
            } catch (i) {
              gl(a, a.return, i);
            }
          }
        e & 64 && Go(a), e & 512 && tu(a, a.return);
        break;
      case 3:
        if ((ea(l, a), e & 64 && ((l = a.updateQueue), l !== null))) {
          if (((t = null), a.child !== null))
            switch (a.child.tag) {
              case 27:
              case 5:
                t = a.child.stateNode;
                break;
              case 1:
                t = a.child.stateNode;
            }
          try {
            _s(l, t);
          } catch (i) {
            gl(a, a.return, i);
          }
        }
        break;
      case 27:
        t === null && e & 4 && Lo(a);
      case 26:
      case 5:
        ea(l, a), t === null && e & 4 && Qo(a), e & 512 && tu(a, a.return);
        break;
      case 12:
        ea(l, a);
        break;
      case 13:
        ea(l, a),
          e & 4 && ko(l, a),
          e & 64 &&
            ((l = a.memoizedState),
            l !== null &&
              ((l = l.dehydrated),
              l !== null && ((a = iv.bind(null, a)), pv(l, a))));
        break;
      case 22:
        if (((e = a.memoizedState !== null || Zt), !e)) {
          (t = (t !== null && t.memoizedState !== null) || xl), (u = Zt);
          var n = xl;
          (Zt = e),
            (xl = t) && !n ? ua(l, a, (a.subtreeFlags & 8772) !== 0) : ea(l, a),
            (Zt = u),
            (xl = n);
        }
        break;
      case 30:
        break;
      default:
        ea(l, a);
    }
  }
  function Jo(l) {
    var t = l.alternate;
    t !== null && ((l.alternate = null), Jo(t)),
      (l.child = null),
      (l.deletions = null),
      (l.sibling = null),
      l.tag === 5 && ((t = l.stateNode), t !== null && Jn(t)),
      (l.stateNode = null),
      (l.return = null),
      (l.dependencies = null),
      (l.memoizedProps = null),
      (l.memoizedState = null),
      (l.pendingProps = null),
      (l.stateNode = null),
      (l.updateQueue = null);
  }
  var Al = null,
    tt = !1;
  function Lt(l, t, a) {
    for (a = a.child; a !== null; ) wo(l, t, a), (a = a.sibling);
  }
  function wo(l, t, a) {
    if (ut && typeof ut.onCommitFiberUnmount == "function")
      try {
        ut.onCommitFiberUnmount(Ee, a);
      } catch {}
    switch (a.tag) {
      case 26:
        xl || Dt(a, t),
          Lt(l, t, a),
          a.memoizedState
            ? a.memoizedState.count--
            : a.stateNode && ((a = a.stateNode), a.parentNode.removeChild(a));
        break;
      case 27:
        xl || Dt(a, t);
        var e = Al,
          u = tt;
        oa(a.type) && ((Al = a.stateNode), (tt = !1)),
          Lt(l, t, a),
          ou(a.stateNode),
          (Al = e),
          (tt = u);
        break;
      case 5:
        xl || Dt(a, t);
      case 6:
        if (
          ((e = Al),
          (u = tt),
          (Al = null),
          Lt(l, t, a),
          (Al = e),
          (tt = u),
          Al !== null)
        )
          if (tt)
            try {
              (Al.nodeType === 9
                ? Al.body
                : Al.nodeName === "HTML"
                  ? Al.ownerDocument.body
                  : Al
              ).removeChild(a.stateNode);
            } catch (n) {
              gl(a, t, n);
            }
          else
            try {
              Al.removeChild(a.stateNode);
            } catch (n) {
              gl(a, t, n);
            }
        break;
      case 18:
        Al !== null &&
          (tt
            ? ((l = Al),
              Br(
                l.nodeType === 9
                  ? l.body
                  : l.nodeName === "HTML"
                    ? l.ownerDocument.body
                    : l,
                a.stateNode,
              ),
              Su(l))
            : Br(Al, a.stateNode));
        break;
      case 4:
        (e = Al),
          (u = tt),
          (Al = a.stateNode.containerInfo),
          (tt = !0),
          Lt(l, t, a),
          (Al = e),
          (tt = u);
        break;
      case 0:
      case 11:
      case 14:
      case 15:
        xl || aa(2, a, t), xl || aa(4, a, t), Lt(l, t, a);
        break;
      case 1:
        xl ||
          (Dt(a, t),
          (e = a.stateNode),
          typeof e.componentWillUnmount == "function" && Xo(a, t, e)),
          Lt(l, t, a);
        break;
      case 21:
        Lt(l, t, a);
        break;
      case 22:
        (xl = (e = xl) || a.memoizedState !== null), Lt(l, t, a), (xl = e);
        break;
      default:
        Lt(l, t, a);
    }
  }
  function ko(l, t) {
    if (
      t.memoizedState === null &&
      ((l = t.alternate),
      l !== null &&
        ((l = l.memoizedState), l !== null && ((l = l.dehydrated), l !== null)))
    )
      try {
        Su(l);
      } catch (a) {
        gl(t, t.return, a);
      }
  }
  function I0(l) {
    switch (l.tag) {
      case 13:
      case 19:
        var t = l.stateNode;
        return t === null && (t = l.stateNode = new Vo()), t;
      case 22:
        return (
          (l = l.stateNode),
          (t = l._retryCache),
          t === null && (t = l._retryCache = new Vo()),
          t
        );
      default:
        throw Error(s(435, l.tag));
    }
  }
  function yc(l, t) {
    var a = I0(l);
    t.forEach(function (e) {
      var u = cv.bind(null, l, e);
      a.has(e) || (a.add(e), e.then(u, u));
    });
  }
  function ft(l, t) {
    var a = t.deletions;
    if (a !== null)
      for (var e = 0; e < a.length; e++) {
        var u = a[e],
          n = l,
          i = t,
          c = i;
        l: for (; c !== null; ) {
          switch (c.tag) {
            case 27:
              if (oa(c.type)) {
                (Al = c.stateNode), (tt = !1);
                break l;
              }
              break;
            case 5:
              (Al = c.stateNode), (tt = !1);
              break l;
            case 3:
            case 4:
              (Al = c.stateNode.containerInfo), (tt = !0);
              break l;
          }
          c = c.return;
        }
        if (Al === null) throw Error(s(160));
        wo(n, i, u),
          (Al = null),
          (tt = !1),
          (n = u.alternate),
          n !== null && (n.return = null),
          (u.return = null);
      }
    if (t.subtreeFlags & 13878)
      for (t = t.child; t !== null; ) $o(t, l), (t = t.sibling);
  }
  var At = null;
  function $o(l, t) {
    var a = l.alternate,
      e = l.flags;
    switch (l.tag) {
      case 0:
      case 11:
      case 14:
      case 15:
        ft(t, l),
          st(l),
          e & 4 && (aa(3, l, l.return), lu(3, l), aa(5, l, l.return));
        break;
      case 1:
        ft(t, l),
          st(l),
          e & 512 && (xl || a === null || Dt(a, a.return)),
          e & 64 &&
            Zt &&
            ((l = l.updateQueue),
            l !== null &&
              ((e = l.callbacks),
              e !== null &&
                ((a = l.shared.hiddenCallbacks),
                (l.shared.hiddenCallbacks = a === null ? e : a.concat(e)))));
        break;
      case 26:
        var u = At;
        if (
          (ft(t, l),
          st(l),
          e & 512 && (xl || a === null || Dt(a, a.return)),
          e & 4)
        ) {
          var n = a !== null ? a.memoizedState : null;
          if (((e = l.memoizedState), a === null))
            if (e === null)
              if (l.stateNode === null) {
                l: {
                  (e = l.type),
                    (a = l.memoizedProps),
                    (u = u.ownerDocument || u);
                  t: switch (e) {
                    case "title":
                      (n = u.getElementsByTagName("title")[0]),
                        (!n ||
                          n[_e] ||
                          n[Jl] ||
                          n.namespaceURI === "http://www.w3.org/2000/svg" ||
                          n.hasAttribute("itemprop")) &&
                          ((n = u.createElement(e)),
                          u.head.insertBefore(
                            n,
                            u.querySelector("head > title"),
                          )),
                        Kl(n, e, a),
                        (n[Jl] = l),
                        Gl(n),
                        (e = n);
                      break l;
                    case "link":
                      var i = Lr("link", "href", u).get(e + (a.href || ""));
                      if (i) {
                        for (var c = 0; c < i.length; c++)
                          if (
                            ((n = i[c]),
                            n.getAttribute("href") ===
                              (a.href == null || a.href === ""
                                ? null
                                : a.href) &&
                              n.getAttribute("rel") ===
                                (a.rel == null ? null : a.rel) &&
                              n.getAttribute("title") ===
                                (a.title == null ? null : a.title) &&
                              n.getAttribute("crossorigin") ===
                                (a.crossOrigin == null ? null : a.crossOrigin))
                          ) {
                            i.splice(c, 1);
                            break t;
                          }
                      }
                      (n = u.createElement(e)),
                        Kl(n, e, a),
                        u.head.appendChild(n);
                      break;
                    case "meta":
                      if (
                        (i = Lr("meta", "content", u).get(
                          e + (a.content || ""),
                        ))
                      ) {
                        for (c = 0; c < i.length; c++)
                          if (
                            ((n = i[c]),
                            n.getAttribute("content") ===
                              (a.content == null ? null : "" + a.content) &&
                              n.getAttribute("name") ===
                                (a.name == null ? null : a.name) &&
                              n.getAttribute("property") ===
                                (a.property == null ? null : a.property) &&
                              n.getAttribute("http-equiv") ===
                                (a.httpEquiv == null ? null : a.httpEquiv) &&
                              n.getAttribute("charset") ===
                                (a.charSet == null ? null : a.charSet))
                          ) {
                            i.splice(c, 1);
                            break t;
                          }
                      }
                      (n = u.createElement(e)),
                        Kl(n, e, a),
                        u.head.appendChild(n);
                      break;
                    default:
                      throw Error(s(468, e));
                  }
                  (n[Jl] = l), Gl(n), (e = n);
                }
                l.stateNode = e;
              } else Vr(u, l.type, l.stateNode);
            else l.stateNode = Zr(u, e, l.memoizedProps);
          else
            n !== e
              ? (n === null
                  ? a.stateNode !== null &&
                    ((a = a.stateNode), a.parentNode.removeChild(a))
                  : n.count--,
                e === null
                  ? Vr(u, l.type, l.stateNode)
                  : Zr(u, e, l.memoizedProps))
              : e === null &&
                l.stateNode !== null &&
                oc(l, l.memoizedProps, a.memoizedProps);
        }
        break;
      case 27:
        ft(t, l),
          st(l),
          e & 512 && (xl || a === null || Dt(a, a.return)),
          a !== null && e & 4 && oc(l, l.memoizedProps, a.memoizedProps);
        break;
      case 5:
        if (
          (ft(t, l),
          st(l),
          e & 512 && (xl || a === null || Dt(a, a.return)),
          l.flags & 32)
        ) {
          u = l.stateNode;
          try {
            Za(u, "");
          } catch (g) {
            gl(l, l.return, g);
          }
        }
        e & 4 &&
          l.stateNode != null &&
          ((u = l.memoizedProps), oc(l, u, a !== null ? a.memoizedProps : u)),
          e & 1024 && (vc = !0);
        break;
      case 6:
        if ((ft(t, l), st(l), e & 4)) {
          if (l.stateNode === null) throw Error(s(162));
          (e = l.memoizedProps), (a = l.stateNode);
          try {
            a.nodeValue = e;
          } catch (g) {
            gl(l, l.return, g);
          }
        }
        break;
      case 3:
        if (
          ((Mn = null),
          (u = At),
          (At = _n(t.containerInfo)),
          ft(t, l),
          (At = u),
          st(l),
          e & 4 && a !== null && a.memoizedState.isDehydrated)
        )
          try {
            Su(t.containerInfo);
          } catch (g) {
            gl(l, l.return, g);
          }
        vc && ((vc = !1), Wo(l));
        break;
      case 4:
        (e = At),
          (At = _n(l.stateNode.containerInfo)),
          ft(t, l),
          st(l),
          (At = e);
        break;
      case 12:
        ft(t, l), st(l);
        break;
      case 13:
        ft(t, l),
          st(l),
          l.child.flags & 8192 &&
            (l.memoizedState !== null) !=
              (a !== null && a.memoizedState !== null) &&
            (Tc = _t()),
          e & 4 &&
            ((e = l.updateQueue),
            e !== null && ((l.updateQueue = null), yc(l, e)));
        break;
      case 22:
        u = l.memoizedState !== null;
        var f = a !== null && a.memoizedState !== null,
          y = Zt,
          T = xl;
        if (
          ((Zt = y || u),
          (xl = T || f),
          ft(t, l),
          (xl = T),
          (Zt = y),
          st(l),
          e & 8192)
        )
          l: for (
            t = l.stateNode,
              t._visibility = u ? t._visibility & -2 : t._visibility | 1,
              u && (a === null || f || Zt || xl || Na(l)),
              a = null,
              t = l;
            ;
          ) {
            if (t.tag === 5 || t.tag === 26) {
              if (a === null) {
                f = a = t;
                try {
                  if (((n = f.stateNode), u))
                    (i = n.style),
                      typeof i.setProperty == "function"
                        ? i.setProperty("display", "none", "important")
                        : (i.display = "none");
                  else {
                    c = f.stateNode;
                    var E = f.memoizedProps.style,
                      m =
                        E != null && E.hasOwnProperty("display")
                          ? E.display
                          : null;
                    c.style.display =
                      m == null || typeof m == "boolean" ? "" : ("" + m).trim();
                  }
                } catch (g) {
                  gl(f, f.return, g);
                }
              }
            } else if (t.tag === 6) {
              if (a === null) {
                f = t;
                try {
                  f.stateNode.nodeValue = u ? "" : f.memoizedProps;
                } catch (g) {
                  gl(f, f.return, g);
                }
              }
            } else if (
              ((t.tag !== 22 && t.tag !== 23) ||
                t.memoizedState === null ||
                t === l) &&
              t.child !== null
            ) {
              (t.child.return = t), (t = t.child);
              continue;
            }
            if (t === l) break;
            for (; t.sibling === null; ) {
              if (t.return === null || t.return === l) break l;
              a === t && (a = null), (t = t.return);
            }
            a === t && (a = null),
              (t.sibling.return = t.return),
              (t = t.sibling);
          }
        e & 4 &&
          ((e = l.updateQueue),
          e !== null &&
            ((a = e.retryQueue),
            a !== null && ((e.retryQueue = null), yc(l, a))));
        break;
      case 19:
        ft(t, l),
          st(l),
          e & 4 &&
            ((e = l.updateQueue),
            e !== null && ((l.updateQueue = null), yc(l, e)));
        break;
      case 30:
        break;
      case 21:
        break;
      default:
        ft(t, l), st(l);
    }
  }
  function st(l) {
    var t = l.flags;
    if (t & 2) {
      try {
        for (var a, e = l.return; e !== null; ) {
          if (Zo(e)) {
            a = e;
            break;
          }
          e = e.return;
        }
        if (a == null) throw Error(s(160));
        switch (a.tag) {
          case 27:
            var u = a.stateNode,
              n = rc(l);
            dn(l, n, u);
            break;
          case 5:
            var i = a.stateNode;
            a.flags & 32 && (Za(i, ""), (a.flags &= -33));
            var c = rc(l);
            dn(l, c, i);
            break;
          case 3:
          case 4:
            var f = a.stateNode.containerInfo,
              y = rc(l);
            dc(l, y, f);
            break;
          default:
            throw Error(s(161));
        }
      } catch (T) {
        gl(l, l.return, T);
      }
      l.flags &= -3;
    }
    t & 4096 && (l.flags &= -4097);
  }
  function Wo(l) {
    if (l.subtreeFlags & 1024)
      for (l = l.child; l !== null; ) {
        var t = l;
        Wo(t),
          t.tag === 5 && t.flags & 1024 && t.stateNode.reset(),
          (l = l.sibling);
      }
  }
  function ea(l, t) {
    if (t.subtreeFlags & 8772)
      for (t = t.child; t !== null; ) Ko(l, t.alternate, t), (t = t.sibling);
  }
  function Na(l) {
    for (l = l.child; l !== null; ) {
      var t = l;
      switch (t.tag) {
        case 0:
        case 11:
        case 14:
        case 15:
          aa(4, t, t.return), Na(t);
          break;
        case 1:
          Dt(t, t.return);
          var a = t.stateNode;
          typeof a.componentWillUnmount == "function" && Xo(t, t.return, a),
            Na(t);
          break;
        case 27:
          ou(t.stateNode);
        case 26:
        case 5:
          Dt(t, t.return), Na(t);
          break;
        case 22:
          t.memoizedState === null && Na(t);
          break;
        case 30:
          Na(t);
          break;
        default:
          Na(t);
      }
      l = l.sibling;
    }
  }
  function ua(l, t, a) {
    for (a = a && (t.subtreeFlags & 8772) !== 0, t = t.child; t !== null; ) {
      var e = t.alternate,
        u = l,
        n = t,
        i = n.flags;
      switch (n.tag) {
        case 0:
        case 11:
        case 15:
          ua(u, n, a), lu(4, n);
          break;
        case 1:
          if (
            (ua(u, n, a),
            (e = n),
            (u = e.stateNode),
            typeof u.componentDidMount == "function")
          )
            try {
              u.componentDidMount();
            } catch (y) {
              gl(e, e.return, y);
            }
          if (((e = n), (u = e.updateQueue), u !== null)) {
            var c = e.stateNode;
            try {
              var f = u.shared.hiddenCallbacks;
              if (f !== null)
                for (u.shared.hiddenCallbacks = null, u = 0; u < f.length; u++)
                  ps(f[u], c);
            } catch (y) {
              gl(e, e.return, y);
            }
          }
          a && i & 64 && Go(n), tu(n, n.return);
          break;
        case 27:
          Lo(n);
        case 26:
        case 5:
          ua(u, n, a), a && e === null && i & 4 && Qo(n), tu(n, n.return);
          break;
        case 12:
          ua(u, n, a);
          break;
        case 13:
          ua(u, n, a), a && i & 4 && ko(u, n);
          break;
        case 22:
          n.memoizedState === null && ua(u, n, a), tu(n, n.return);
          break;
        case 30:
          break;
        default:
          ua(u, n, a);
      }
      t = t.sibling;
    }
  }
  function hc(l, t) {
    var a = null;
    l !== null &&
      l.memoizedState !== null &&
      l.memoizedState.cachePool !== null &&
      (a = l.memoizedState.cachePool.pool),
      (l = null),
      t.memoizedState !== null &&
        t.memoizedState.cachePool !== null &&
        (l = t.memoizedState.cachePool.pool),
      l !== a && (l != null && l.refCount++, a != null && Xe(a));
  }
  function mc(l, t) {
    (l = null),
      t.alternate !== null && (l = t.alternate.memoizedState.cache),
      (t = t.memoizedState.cache),
      t !== l && (t.refCount++, l != null && Xe(l));
  }
  function Rt(l, t, a, e) {
    if (t.subtreeFlags & 10256)
      for (t = t.child; t !== null; ) Fo(l, t, a, e), (t = t.sibling);
  }
  function Fo(l, t, a, e) {
    var u = t.flags;
    switch (t.tag) {
      case 0:
      case 11:
      case 15:
        Rt(l, t, a, e), u & 2048 && lu(9, t);
        break;
      case 1:
        Rt(l, t, a, e);
        break;
      case 3:
        Rt(l, t, a, e),
          u & 2048 &&
            ((l = null),
            t.alternate !== null && (l = t.alternate.memoizedState.cache),
            (t = t.memoizedState.cache),
            t !== l && (t.refCount++, l != null && Xe(l)));
        break;
      case 12:
        if (u & 2048) {
          Rt(l, t, a, e), (l = t.stateNode);
          try {
            var n = t.memoizedProps,
              i = n.id,
              c = n.onPostCommit;
            typeof c == "function" &&
              c(
                i,
                t.alternate === null ? "mount" : "update",
                l.passiveEffectDuration,
                -0,
              );
          } catch (f) {
            gl(t, t.return, f);
          }
        } else Rt(l, t, a, e);
        break;
      case 13:
        Rt(l, t, a, e);
        break;
      case 23:
        break;
      case 22:
        (n = t.stateNode),
          (i = t.alternate),
          t.memoizedState !== null
            ? n._visibility & 2
              ? Rt(l, t, a, e)
              : au(l, t)
            : n._visibility & 2
              ? Rt(l, t, a, e)
              : ((n._visibility |= 2),
                ce(l, t, a, e, (t.subtreeFlags & 10256) !== 0)),
          u & 2048 && hc(i, t);
        break;
      case 24:
        Rt(l, t, a, e), u & 2048 && mc(t.alternate, t);
        break;
      default:
        Rt(l, t, a, e);
    }
  }
  function ce(l, t, a, e, u) {
    for (u = u && (t.subtreeFlags & 10256) !== 0, t = t.child; t !== null; ) {
      var n = l,
        i = t,
        c = a,
        f = e,
        y = i.flags;
      switch (i.tag) {
        case 0:
        case 11:
        case 15:
          ce(n, i, c, f, u), lu(8, i);
          break;
        case 23:
          break;
        case 22:
          var T = i.stateNode;
          i.memoizedState !== null
            ? T._visibility & 2
              ? ce(n, i, c, f, u)
              : au(n, i)
            : ((T._visibility |= 2), ce(n, i, c, f, u)),
            u && y & 2048 && hc(i.alternate, i);
          break;
        case 24:
          ce(n, i, c, f, u), u && y & 2048 && mc(i.alternate, i);
          break;
        default:
          ce(n, i, c, f, u);
      }
      t = t.sibling;
    }
  }
  function au(l, t) {
    if (t.subtreeFlags & 10256)
      for (t = t.child; t !== null; ) {
        var a = l,
          e = t,
          u = e.flags;
        switch (e.tag) {
          case 22:
            au(a, e), u & 2048 && hc(e.alternate, e);
            break;
          case 24:
            au(a, e), u & 2048 && mc(e.alternate, e);
            break;
          default:
            au(a, e);
        }
        t = t.sibling;
      }
  }
  var eu = 8192;
  function fe(l) {
    if (l.subtreeFlags & eu)
      for (l = l.child; l !== null; ) Io(l), (l = l.sibling);
  }
  function Io(l) {
    switch (l.tag) {
      case 26:
        fe(l),
          l.flags & eu &&
            l.memoizedState !== null &&
            jv(At, l.memoizedState, l.memoizedProps);
        break;
      case 5:
        fe(l);
        break;
      case 3:
      case 4:
        var t = At;
        (At = _n(l.stateNode.containerInfo)), fe(l), (At = t);
        break;
      case 22:
        l.memoizedState === null &&
          ((t = l.alternate),
          t !== null && t.memoizedState !== null
            ? ((t = eu), (eu = 16777216), fe(l), (eu = t))
            : fe(l));
        break;
      default:
        fe(l);
    }
  }
  function Po(l) {
    var t = l.alternate;
    if (t !== null && ((l = t.child), l !== null)) {
      t.child = null;
      do (t = l.sibling), (l.sibling = null), (l = t);
      while (l !== null);
    }
  }
  function uu(l) {
    var t = l.deletions;
    if ((l.flags & 16) !== 0) {
      if (t !== null)
        for (var a = 0; a < t.length; a++) {
          var e = t[a];
          (Ql = e), tr(e, l);
        }
      Po(l);
    }
    if (l.subtreeFlags & 10256)
      for (l = l.child; l !== null; ) lr(l), (l = l.sibling);
  }
  function lr(l) {
    switch (l.tag) {
      case 0:
      case 11:
      case 15:
        uu(l), l.flags & 2048 && aa(9, l, l.return);
        break;
      case 3:
        uu(l);
        break;
      case 12:
        uu(l);
        break;
      case 22:
        var t = l.stateNode;
        l.memoizedState !== null &&
        t._visibility & 2 &&
        (l.return === null || l.return.tag !== 13)
          ? ((t._visibility &= -3), vn(l))
          : uu(l);
        break;
      default:
        uu(l);
    }
  }
  function vn(l) {
    var t = l.deletions;
    if ((l.flags & 16) !== 0) {
      if (t !== null)
        for (var a = 0; a < t.length; a++) {
          var e = t[a];
          (Ql = e), tr(e, l);
        }
      Po(l);
    }
    for (l = l.child; l !== null; ) {
      switch (((t = l), t.tag)) {
        case 0:
        case 11:
        case 15:
          aa(8, t, t.return), vn(t);
          break;
        case 22:
          (a = t.stateNode),
            a._visibility & 2 && ((a._visibility &= -3), vn(t));
          break;
        default:
          vn(t);
      }
      l = l.sibling;
    }
  }
  function tr(l, t) {
    for (; Ql !== null; ) {
      var a = Ql;
      switch (a.tag) {
        case 0:
        case 11:
        case 15:
          aa(8, a, t);
          break;
        case 23:
        case 22:
          if (a.memoizedState !== null && a.memoizedState.cachePool !== null) {
            var e = a.memoizedState.cachePool.pool;
            e != null && e.refCount++;
          }
          break;
        case 24:
          Xe(a.memoizedState.cache);
      }
      if (((e = a.child), e !== null)) (e.return = a), (Ql = e);
      else
        for (a = l; Ql !== null; ) {
          e = Ql;
          var u = e.sibling,
            n = e.return;
          if ((Jo(e), e === a)) {
            Ql = null;
            break;
          }
          if (u !== null) {
            (u.return = n), (Ql = u);
            break;
          }
          Ql = n;
        }
    }
  }
  var P0 = {
      getCacheForType: function (l) {
        var t = wl(jl),
          a = t.data.get(l);
        return a === void 0 && ((a = l()), t.data.set(l, a)), a;
      },
    },
    lv = typeof WeakMap == "function" ? WeakMap : Map,
    ol = 0,
    Sl = null,
    W = null,
    ll = 0,
    rl = 0,
    ot = null,
    na = !1,
    se = !1,
    gc = !1,
    Vt = 0,
    Rl = 0,
    ia = 0,
    xa = 0,
    Sc = 0,
    Tt = 0,
    oe = 0,
    nu = null,
    at = null,
    bc = !1,
    Tc = 0,
    yn = 1 / 0,
    hn = null,
    ca = null,
    Vl = 0,
    fa = null,
    re = null,
    de = 0,
    zc = 0,
    Ec = null,
    ar = null,
    iu = 0,
    Ac = null;
  function rt() {
    if ((ol & 2) !== 0 && ll !== 0) return ll & -ll;
    if (S.T !== null) {
      var l = Pa;
      return l !== 0 ? l : Uc();
    }
    return gf();
  }
  function er() {
    Tt === 0 && (Tt = (ll & 536870912) === 0 || il ? vf() : 536870912);
    var l = bt.current;
    return l !== null && (l.flags |= 32), Tt;
  }
  function dt(l, t, a) {
    ((l === Sl && (rl === 2 || rl === 9)) || l.cancelPendingCommit !== null) &&
      (ve(l, 0), sa(l, ll, Tt, !1)),
      pe(l, a),
      ((ol & 2) === 0 || l !== Sl) &&
        (l === Sl &&
          ((ol & 2) === 0 && (xa |= a), Rl === 4 && sa(l, ll, Tt, !1)),
        Ut(l));
  }
  function ur(l, t, a) {
    if ((ol & 6) !== 0) throw Error(s(327));
    var e = (!a && (t & 124) === 0 && (t & l.expiredLanes) === 0) || Ae(l, t),
      u = e ? ev(l, t) : Oc(l, t, !0),
      n = e;
    do {
      if (u === 0) {
        se && !e && sa(l, t, 0, !1);
        break;
      } else {
        if (((a = l.current.alternate), n && !tv(a))) {
          (u = Oc(l, t, !1)), (n = !1);
          continue;
        }
        if (u === 2) {
          if (((n = t), l.errorRecoveryDisabledLanes & n)) var i = 0;
          else
            (i = l.pendingLanes & -536870913),
              (i = i !== 0 ? i : i & 536870912 ? 536870912 : 0);
          if (i !== 0) {
            t = i;
            l: {
              var c = l;
              u = nu;
              var f = c.current.memoizedState.isDehydrated;
              if ((f && (ve(c, i).flags |= 256), (i = Oc(c, i, !1)), i !== 2)) {
                if (gc && !f) {
                  (c.errorRecoveryDisabledLanes |= n), (xa |= n), (u = 4);
                  break l;
                }
                (n = at),
                  (at = u),
                  n !== null && (at === null ? (at = n) : at.push.apply(at, n));
              }
              u = i;
            }
            if (((n = !1), u !== 2)) continue;
          }
        }
        if (u === 1) {
          ve(l, 0), sa(l, t, 0, !0);
          break;
        }
        l: {
          switch (((e = l), (n = u), n)) {
            case 0:
            case 1:
              throw Error(s(345));
            case 4:
              if ((t & 4194048) !== t) break;
            case 6:
              sa(e, t, Tt, !na);
              break l;
            case 2:
              at = null;
              break;
            case 3:
            case 5:
              break;
            default:
              throw Error(s(329));
          }
          if ((t & 62914560) === t && ((u = Tc + 300 - _t()), 10 < u)) {
            if ((sa(e, t, Tt, !na), _u(e, 0, !0) !== 0)) break l;
            e.timeoutHandle = Hr(
              nr.bind(null, e, a, at, hn, bc, t, Tt, xa, oe, na, n, 2, -0, 0),
              u,
            );
            break l;
          }
          nr(e, a, at, hn, bc, t, Tt, xa, oe, na, n, 0, -0, 0);
        }
      }
      break;
    } while (!0);
    Ut(l);
  }
  function nr(l, t, a, e, u, n, i, c, f, y, T, E, m, g) {
    if (
      ((l.timeoutHandle = -1),
      (E = t.subtreeFlags),
      (E & 8192 || (E & 16785408) === 16785408) &&
        ((vu = { stylesheets: null, count: 0, unsuspend: Yv }),
        Io(t),
        (E = Cv()),
        E !== null))
    ) {
      (l.cancelPendingCommit = E(
        dr.bind(null, l, t, n, a, e, u, i, c, f, T, 1, m, g),
      )),
        sa(l, n, i, !y);
      return;
    }
    dr(l, t, n, a, e, u, i, c, f);
  }
  function tv(l) {
    for (var t = l; ; ) {
      var a = t.tag;
      if (
        (a === 0 || a === 11 || a === 15) &&
        t.flags & 16384 &&
        ((a = t.updateQueue), a !== null && ((a = a.stores), a !== null))
      )
        for (var e = 0; e < a.length; e++) {
          var u = a[e],
            n = u.getSnapshot;
          u = u.value;
          try {
            if (!it(n(), u)) return !1;
          } catch {
            return !1;
          }
        }
      if (((a = t.child), t.subtreeFlags & 16384 && a !== null))
        (a.return = t), (t = a);
      else {
        if (t === l) break;
        for (; t.sibling === null; ) {
          if (t.return === null || t.return === l) return !0;
          t = t.return;
        }
        (t.sibling.return = t.return), (t = t.sibling);
      }
    }
    return !0;
  }
  function sa(l, t, a, e) {
    (t &= ~Sc),
      (t &= ~xa),
      (l.suspendedLanes |= t),
      (l.pingedLanes &= ~t),
      e && (l.warmLanes |= t),
      (e = l.expirationTimes);
    for (var u = t; 0 < u; ) {
      var n = 31 - nt(u),
        i = 1 << n;
      (e[n] = -1), (u &= ~i);
    }
    a !== 0 && hf(l, a, t);
  }
  function mn() {
    return (ol & 6) === 0 ? (cu(0), !1) : !0;
  }
  function pc() {
    if (W !== null) {
      if (rl === 0) var l = W.return;
      else (l = W), (Yt = Oa = null), Qi(l), (ne = null), (Fe = 0), (l = W);
      for (; l !== null; ) Co(l.alternate, l), (l = l.return);
      W = null;
    }
  }
  function ve(l, t) {
    var a = l.timeoutHandle;
    a !== -1 && ((l.timeoutHandle = -1), bv(a)),
      (a = l.cancelPendingCommit),
      a !== null && ((l.cancelPendingCommit = null), a()),
      pc(),
      (Sl = l),
      (W = a = Ht(l.current, null)),
      (ll = t),
      (rl = 0),
      (ot = null),
      (na = !1),
      (se = Ae(l, t)),
      (gc = !1),
      (oe = Tt = Sc = xa = ia = Rl = 0),
      (at = nu = null),
      (bc = !1),
      (t & 8) !== 0 && (t |= t & 32);
    var e = l.entangledLanes;
    if (e !== 0)
      for (l = l.entanglements, e &= t; 0 < e; ) {
        var u = 31 - nt(e),
          n = 1 << u;
        (t |= l[u]), (e &= ~n);
      }
    return (Vt = t), ju(), a;
  }
  function ir(l, t) {
    (w = null),
      (S.H = an),
      t === Ze || t === Ju
        ? ((t = Es()), (rl = 3))
        : t === bs
          ? ((t = Es()), (rl = 4))
          : (rl =
              t === po
                ? 8
                : t !== null &&
                    typeof t == "object" &&
                    typeof t.then == "function"
                  ? 6
                  : 1),
      (ot = t),
      W === null && ((Rl = 1), fn(l, ht(t, l.current)));
  }
  function cr() {
    var l = S.H;
    return (S.H = an), l === null ? an : l;
  }
  function fr() {
    var l = S.A;
    return (S.A = P0), l;
  }
  function _c() {
    (Rl = 4),
      na || ((ll & 4194048) !== ll && bt.current !== null) || (se = !0),
      ((ia & 134217727) === 0 && (xa & 134217727) === 0) ||
        Sl === null ||
        sa(Sl, ll, Tt, !1);
  }
  function Oc(l, t, a) {
    var e = ol;
    ol |= 2;
    var u = cr(),
      n = fr();
    (Sl !== l || ll !== t) && ((hn = null), ve(l, t)), (t = !1);
    var i = Rl;
    l: do
      try {
        if (rl !== 0 && W !== null) {
          var c = W,
            f = ot;
          switch (rl) {
            case 8:
              pc(), (i = 6);
              break l;
            case 3:
            case 2:
            case 9:
            case 6:
              bt.current === null && (t = !0);
              var y = rl;
              if (((rl = 0), (ot = null), ye(l, c, f, y), a && se)) {
                i = 0;
                break l;
              }
              break;
            default:
              (y = rl), (rl = 0), (ot = null), ye(l, c, f, y);
          }
        }
        av(), (i = Rl);
        break;
      } catch (T) {
        ir(l, T);
      }
    while (!0);
    return (
      t && l.shellSuspendCounter++,
      (Yt = Oa = null),
      (ol = e),
      (S.H = u),
      (S.A = n),
      W === null && ((Sl = null), (ll = 0), ju()),
      i
    );
  }
  function av() {
    for (; W !== null; ) sr(W);
  }
  function ev(l, t) {
    var a = ol;
    ol |= 2;
    var e = cr(),
      u = fr();
    Sl !== l || ll !== t
      ? ((hn = null), (yn = _t() + 500), ve(l, t))
      : (se = Ae(l, t));
    l: do
      try {
        if (rl !== 0 && W !== null) {
          t = W;
          var n = ot;
          t: switch (rl) {
            case 1:
              (rl = 0), (ot = null), ye(l, t, n, 1);
              break;
            case 2:
            case 9:
              if (Ts(n)) {
                (rl = 0), (ot = null), or(t);
                break;
              }
              (t = function () {
                (rl !== 2 && rl !== 9) || Sl !== l || (rl = 7), Ut(l);
              }),
                n.then(t, t);
              break l;
            case 3:
              rl = 7;
              break l;
            case 4:
              rl = 5;
              break l;
            case 7:
              Ts(n)
                ? ((rl = 0), (ot = null), or(t))
                : ((rl = 0), (ot = null), ye(l, t, n, 7));
              break;
            case 5:
              var i = null;
              switch (W.tag) {
                case 26:
                  i = W.memoizedState;
                case 5:
                case 27:
                  var c = W;
                  if (!i || Kr(i)) {
                    (rl = 0), (ot = null);
                    var f = c.sibling;
                    if (f !== null) W = f;
                    else {
                      var y = c.return;
                      y !== null ? ((W = y), gn(y)) : (W = null);
                    }
                    break t;
                  }
              }
              (rl = 0), (ot = null), ye(l, t, n, 5);
              break;
            case 6:
              (rl = 0), (ot = null), ye(l, t, n, 6);
              break;
            case 8:
              pc(), (Rl = 6);
              break l;
            default:
              throw Error(s(462));
          }
        }
        uv();
        break;
      } catch (T) {
        ir(l, T);
      }
    while (!0);
    return (
      (Yt = Oa = null),
      (S.H = e),
      (S.A = u),
      (ol = a),
      W !== null ? 0 : ((Sl = null), (ll = 0), ju(), Rl)
    );
  }
  function uv() {
    for (; W !== null && !Od(); ) sr(W);
  }
  function sr(l) {
    var t = Yo(l.alternate, l, Vt);
    (l.memoizedProps = l.pendingProps), t === null ? gn(l) : (W = t);
  }
  function or(l) {
    var t = l,
      a = t.alternate;
    switch (t.tag) {
      case 15:
      case 0:
        t = Uo(a, t, t.pendingProps, t.type, void 0, ll);
        break;
      case 11:
        t = Uo(a, t, t.pendingProps, t.type.render, t.ref, ll);
        break;
      case 5:
        Qi(t);
      default:
        Co(a, t), (t = W = os(t, Vt)), (t = Yo(a, t, Vt));
    }
    (l.memoizedProps = l.pendingProps), t === null ? gn(l) : (W = t);
  }
  function ye(l, t, a, e) {
    (Yt = Oa = null), Qi(t), (ne = null), (Fe = 0);
    var u = t.return;
    try {
      if (w0(l, u, t, a, ll)) {
        (Rl = 1), fn(l, ht(a, l.current)), (W = null);
        return;
      }
    } catch (n) {
      if (u !== null) throw ((W = u), n);
      (Rl = 1), fn(l, ht(a, l.current)), (W = null);
      return;
    }
    t.flags & 32768
      ? (il || e === 1
          ? (l = !0)
          : se || (ll & 536870912) !== 0
            ? (l = !1)
            : ((na = l = !0),
              (e === 2 || e === 9 || e === 3 || e === 6) &&
                ((e = bt.current),
                e !== null && e.tag === 13 && (e.flags |= 16384))),
        rr(t, l))
      : gn(t);
  }
  function gn(l) {
    var t = l;
    do {
      if ((t.flags & 32768) !== 0) {
        rr(t, na);
        return;
      }
      l = t.return;
      var a = $0(t.alternate, t, Vt);
      if (a !== null) {
        W = a;
        return;
      }
      if (((t = t.sibling), t !== null)) {
        W = t;
        return;
      }
      W = t = l;
    } while (t !== null);
    Rl === 0 && (Rl = 5);
  }
  function rr(l, t) {
    do {
      var a = W0(l.alternate, l);
      if (a !== null) {
        (a.flags &= 32767), (W = a);
        return;
      }
      if (
        ((a = l.return),
        a !== null &&
          ((a.flags |= 32768), (a.subtreeFlags = 0), (a.deletions = null)),
        !t && ((l = l.sibling), l !== null))
      ) {
        W = l;
        return;
      }
      W = l = a;
    } while (l !== null);
    (Rl = 6), (W = null);
  }
  function dr(l, t, a, e, u, n, i, c, f) {
    l.cancelPendingCommit = null;
    do Sn();
    while (Vl !== 0);
    if ((ol & 6) !== 0) throw Error(s(327));
    if (t !== null) {
      if (t === l.current) throw Error(s(177));
      if (
        ((n = t.lanes | t.childLanes),
        (n |= mi),
        Yd(l, a, n, i, c, f),
        l === Sl && ((W = Sl = null), (ll = 0)),
        (re = t),
        (fa = l),
        (de = a),
        (zc = n),
        (Ec = u),
        (ar = e),
        (t.subtreeFlags & 10256) !== 0 || (t.flags & 10256) !== 0
          ? ((l.callbackNode = null),
            (l.callbackPriority = 0),
            fv(Eu, function () {
              return gr(), null;
            }))
          : ((l.callbackNode = null), (l.callbackPriority = 0)),
        (e = (t.flags & 13878) !== 0),
        (t.subtreeFlags & 13878) !== 0 || e)
      ) {
        (e = S.T), (S.T = null), (u = M.p), (M.p = 2), (i = ol), (ol |= 4);
        try {
          F0(l, t, a);
        } finally {
          (ol = i), (M.p = u), (S.T = e);
        }
      }
      (Vl = 1), vr(), yr(), hr();
    }
  }
  function vr() {
    if (Vl === 1) {
      Vl = 0;
      var l = fa,
        t = re,
        a = (t.flags & 13878) !== 0;
      if ((t.subtreeFlags & 13878) !== 0 || a) {
        (a = S.T), (S.T = null);
        var e = M.p;
        M.p = 2;
        var u = ol;
        ol |= 4;
        try {
          $o(t, l);
          var n = Cc,
            i = ls(l.containerInfo),
            c = n.focusedElem,
            f = n.selectionRange;
          if (
            i !== c &&
            c &&
            c.ownerDocument &&
            Pf(c.ownerDocument.documentElement, c)
          ) {
            if (f !== null && ri(c)) {
              var y = f.start,
                T = f.end;
              if ((T === void 0 && (T = y), "selectionStart" in c))
                (c.selectionStart = y),
                  (c.selectionEnd = Math.min(T, c.value.length));
              else {
                var E = c.ownerDocument || document,
                  m = (E && E.defaultView) || window;
                if (m.getSelection) {
                  var g = m.getSelection(),
                    Z = c.textContent.length,
                    C = Math.min(f.start, Z),
                    ml = f.end === void 0 ? C : Math.min(f.end, Z);
                  !g.extend && C > ml && ((i = ml), (ml = C), (C = i));
                  var d = If(c, C),
                    r = If(c, ml);
                  if (
                    d &&
                    r &&
                    (g.rangeCount !== 1 ||
                      g.anchorNode !== d.node ||
                      g.anchorOffset !== d.offset ||
                      g.focusNode !== r.node ||
                      g.focusOffset !== r.offset)
                  ) {
                    var v = E.createRange();
                    v.setStart(d.node, d.offset),
                      g.removeAllRanges(),
                      C > ml
                        ? (g.addRange(v), g.extend(r.node, r.offset))
                        : (v.setEnd(r.node, r.offset), g.addRange(v));
                  }
                }
              }
            }
            for (E = [], g = c; (g = g.parentNode); )
              g.nodeType === 1 &&
                E.push({ element: g, left: g.scrollLeft, top: g.scrollTop });
            for (
              typeof c.focus == "function" && c.focus(), c = 0;
              c < E.length;
              c++
            ) {
              var z = E[c];
              (z.element.scrollLeft = z.left), (z.element.scrollTop = z.top);
            }
          }
          (Un = !!jc), (Cc = jc = null);
        } finally {
          (ol = u), (M.p = e), (S.T = a);
        }
      }
      (l.current = t), (Vl = 2);
    }
  }
  function yr() {
    if (Vl === 2) {
      Vl = 0;
      var l = fa,
        t = re,
        a = (t.flags & 8772) !== 0;
      if ((t.subtreeFlags & 8772) !== 0 || a) {
        (a = S.T), (S.T = null);
        var e = M.p;
        M.p = 2;
        var u = ol;
        ol |= 4;
        try {
          Ko(l, t.alternate, t);
        } finally {
          (ol = u), (M.p = e), (S.T = a);
        }
      }
      Vl = 3;
    }
  }
  function hr() {
    if (Vl === 4 || Vl === 3) {
      (Vl = 0), Md();
      var l = fa,
        t = re,
        a = de,
        e = ar;
      (t.subtreeFlags & 10256) !== 0 || (t.flags & 10256) !== 0
        ? (Vl = 5)
        : ((Vl = 0), (re = fa = null), mr(l, l.pendingLanes));
      var u = l.pendingLanes;
      if (
        (u === 0 && (ca = null),
        Vn(a),
        (t = t.stateNode),
        ut && typeof ut.onCommitFiberRoot == "function")
      )
        try {
          ut.onCommitFiberRoot(Ee, t, void 0, (t.current.flags & 128) === 128);
        } catch {}
      if (e !== null) {
        (t = S.T), (u = M.p), (M.p = 2), (S.T = null);
        try {
          for (var n = l.onRecoverableError, i = 0; i < e.length; i++) {
            var c = e[i];
            n(c.value, { componentStack: c.stack });
          }
        } finally {
          (S.T = t), (M.p = u);
        }
      }
      (de & 3) !== 0 && Sn(),
        Ut(l),
        (u = l.pendingLanes),
        (a & 4194090) !== 0 && (u & 42) !== 0
          ? l === Ac
            ? iu++
            : ((iu = 0), (Ac = l))
          : (iu = 0),
        cu(0);
    }
  }
  function mr(l, t) {
    (l.pooledCacheLanes &= t) === 0 &&
      ((t = l.pooledCache), t != null && ((l.pooledCache = null), Xe(t)));
  }
  function Sn(l) {
    return vr(), yr(), hr(), gr();
  }
  function gr() {
    if (Vl !== 5) return !1;
    var l = fa,
      t = zc;
    zc = 0;
    var a = Vn(de),
      e = S.T,
      u = M.p;
    try {
      (M.p = 32 > a ? 32 : a), (S.T = null), (a = Ec), (Ec = null);
      var n = fa,
        i = de;
      if (((Vl = 0), (re = fa = null), (de = 0), (ol & 6) !== 0))
        throw Error(s(331));
      var c = ol;
      if (
        ((ol |= 4),
        lr(n.current),
        Fo(n, n.current, i, a),
        (ol = c),
        cu(0, !1),
        ut && typeof ut.onPostCommitFiberRoot == "function")
      )
        try {
          ut.onPostCommitFiberRoot(Ee, n);
        } catch {}
      return !0;
    } finally {
      (M.p = u), (S.T = e), mr(l, t);
    }
  }
  function Sr(l, t, a) {
    (t = ht(a, t)),
      (t = tc(l.stateNode, t, 2)),
      (l = It(l, t, 2)),
      l !== null && (pe(l, 2), Ut(l));
  }
  function gl(l, t, a) {
    if (l.tag === 3) Sr(l, l, a);
    else
      for (; t !== null; ) {
        if (t.tag === 3) {
          Sr(t, l, a);
          break;
        } else if (t.tag === 1) {
          var e = t.stateNode;
          if (
            typeof t.type.getDerivedStateFromError == "function" ||
            (typeof e.componentDidCatch == "function" &&
              (ca === null || !ca.has(e)))
          ) {
            (l = ht(a, l)),
              (a = Eo(2)),
              (e = It(t, a, 2)),
              e !== null && (Ao(a, e, t, l), pe(e, 2), Ut(e));
            break;
          }
        }
        t = t.return;
      }
  }
  function Mc(l, t, a) {
    var e = l.pingCache;
    if (e === null) {
      e = l.pingCache = new lv();
      var u = new Set();
      e.set(t, u);
    } else (u = e.get(t)), u === void 0 && ((u = new Set()), e.set(t, u));
    u.has(a) ||
      ((gc = !0), u.add(a), (l = nv.bind(null, l, t, a)), t.then(l, l));
  }
  function nv(l, t, a) {
    var e = l.pingCache;
    e !== null && e.delete(t),
      (l.pingedLanes |= l.suspendedLanes & a),
      (l.warmLanes &= ~a),
      Sl === l &&
        (ll & a) === a &&
        (Rl === 4 || (Rl === 3 && (ll & 62914560) === ll && 300 > _t() - Tc)
          ? (ol & 2) === 0 && ve(l, 0)
          : (Sc |= a),
        oe === ll && (oe = 0)),
      Ut(l);
  }
  function br(l, t) {
    t === 0 && (t = yf()), (l = $a(l, t)), l !== null && (pe(l, t), Ut(l));
  }
  function iv(l) {
    var t = l.memoizedState,
      a = 0;
    t !== null && (a = t.retryLane), br(l, a);
  }
  function cv(l, t) {
    var a = 0;
    switch (l.tag) {
      case 13:
        var e = l.stateNode,
          u = l.memoizedState;
        u !== null && (a = u.retryLane);
        break;
      case 19:
        e = l.stateNode;
        break;
      case 22:
        e = l.stateNode._retryCache;
        break;
      default:
        throw Error(s(314));
    }
    e !== null && e.delete(t), br(l, a);
  }
  function fv(l, t) {
    return Xn(l, t);
  }
  var bn = null,
    he = null,
    Dc = !1,
    Tn = !1,
    Rc = !1,
    Ha = 0;
  function Ut(l) {
    l !== he &&
      l.next === null &&
      (he === null ? (bn = he = l) : (he = he.next = l)),
      (Tn = !0),
      Dc || ((Dc = !0), ov());
  }
  function cu(l, t) {
    if (!Rc && Tn) {
      Rc = !0;
      do
        for (var a = !1, e = bn; e !== null; ) {
          if (l !== 0) {
            var u = e.pendingLanes;
            if (u === 0) var n = 0;
            else {
              var i = e.suspendedLanes,
                c = e.pingedLanes;
              (n = (1 << (31 - nt(42 | l) + 1)) - 1),
                (n &= u & ~(i & ~c)),
                (n = n & 201326741 ? (n & 201326741) | 1 : n ? n | 2 : 0);
            }
            n !== 0 && ((a = !0), Ar(e, n));
          } else
            (n = ll),
              (n = _u(
                e,
                e === Sl ? n : 0,
                e.cancelPendingCommit !== null || e.timeoutHandle !== -1,
              )),
              (n & 3) === 0 || Ae(e, n) || ((a = !0), Ar(e, n));
          e = e.next;
        }
      while (a);
      Rc = !1;
    }
  }
  function sv() {
    Tr();
  }
  function Tr() {
    Tn = Dc = !1;
    var l = 0;
    Ha !== 0 && (Sv() && (l = Ha), (Ha = 0));
    for (var t = _t(), a = null, e = bn; e !== null; ) {
      var u = e.next,
        n = zr(e, t);
      n === 0
        ? ((e.next = null),
          a === null ? (bn = u) : (a.next = u),
          u === null && (he = a))
        : ((a = e), (l !== 0 || (n & 3) !== 0) && (Tn = !0)),
        (e = u);
    }
    cu(l);
  }
  function zr(l, t) {
    for (
      var a = l.suspendedLanes,
        e = l.pingedLanes,
        u = l.expirationTimes,
        n = l.pendingLanes & -62914561;
      0 < n;
    ) {
      var i = 31 - nt(n),
        c = 1 << i,
        f = u[i];
      f === -1
        ? ((c & a) === 0 || (c & e) !== 0) && (u[i] = Bd(c, t))
        : f <= t && (l.expiredLanes |= c),
        (n &= ~c);
    }
    if (
      ((t = Sl),
      (a = ll),
      (a = _u(
        l,
        l === t ? a : 0,
        l.cancelPendingCommit !== null || l.timeoutHandle !== -1,
      )),
      (e = l.callbackNode),
      a === 0 ||
        (l === t && (rl === 2 || rl === 9)) ||
        l.cancelPendingCommit !== null)
    )
      return (
        e !== null && e !== null && Qn(e),
        (l.callbackNode = null),
        (l.callbackPriority = 0)
      );
    if ((a & 3) === 0 || Ae(l, a)) {
      if (((t = a & -a), t === l.callbackPriority)) return t;
      switch ((e !== null && Qn(e), Vn(a))) {
        case 2:
        case 8:
          a = rf;
          break;
        case 32:
          a = Eu;
          break;
        case 268435456:
          a = df;
          break;
        default:
          a = Eu;
      }
      return (
        (e = Er.bind(null, l)),
        (a = Xn(a, e)),
        (l.callbackPriority = t),
        (l.callbackNode = a),
        t
      );
    }
    return (
      e !== null && e !== null && Qn(e),
      (l.callbackPriority = 2),
      (l.callbackNode = null),
      2
    );
  }
  function Er(l, t) {
    if (Vl !== 0 && Vl !== 5)
      return (l.callbackNode = null), (l.callbackPriority = 0), null;
    var a = l.callbackNode;
    if (Sn() && l.callbackNode !== a) return null;
    var e = ll;
    return (
      (e = _u(
        l,
        l === Sl ? e : 0,
        l.cancelPendingCommit !== null || l.timeoutHandle !== -1,
      )),
      e === 0
        ? null
        : (ur(l, e, t),
          zr(l, _t()),
          l.callbackNode != null && l.callbackNode === a
            ? Er.bind(null, l)
            : null)
    );
  }
  function Ar(l, t) {
    if (Sn()) return null;
    ur(l, t, !0);
  }
  function ov() {
    Tv(function () {
      (ol & 6) !== 0 ? Xn(of, sv) : Tr();
    });
  }
  function Uc() {
    return Ha === 0 && (Ha = vf()), Ha;
  }
  function pr(l) {
    return l == null || typeof l == "symbol" || typeof l == "boolean"
      ? null
      : typeof l == "function"
        ? l
        : Uu("" + l);
  }
  function _r(l, t) {
    var a = t.ownerDocument.createElement("input");
    return (
      (a.name = t.name),
      (a.value = t.value),
      l.id && a.setAttribute("form", l.id),
      t.parentNode.insertBefore(a, t),
      (l = new FormData(l)),
      a.parentNode.removeChild(a),
      l
    );
  }
  function rv(l, t, a, e, u) {
    if (t === "submit" && a && a.stateNode === u) {
      var n = pr((u[Il] || null).action),
        i = e.submitter;
      i &&
        ((t = (t = i[Il] || null)
          ? pr(t.formAction)
          : i.getAttribute("formAction")),
        t !== null && ((n = t), (i = null)));
      var c = new qu("action", "action", null, e, u);
      l.push({
        event: c,
        listeners: [
          {
            instance: null,
            listener: function () {
              if (e.defaultPrevented) {
                if (Ha !== 0) {
                  var f = i ? _r(u, i) : new FormData(u);
                  Wi(
                    a,
                    { pending: !0, data: f, method: u.method, action: n },
                    null,
                    f,
                  );
                }
              } else
                typeof n == "function" &&
                  (c.preventDefault(),
                  (f = i ? _r(u, i) : new FormData(u)),
                  Wi(
                    a,
                    { pending: !0, data: f, method: u.method, action: n },
                    n,
                    f,
                  ));
            },
            currentTarget: u,
          },
        ],
      });
    }
  }
  for (var Nc = 0; Nc < hi.length; Nc++) {
    var xc = hi[Nc],
      dv = xc.toLowerCase(),
      vv = xc[0].toUpperCase() + xc.slice(1);
    Et(dv, "on" + vv);
  }
  Et(es, "onAnimationEnd"),
    Et(us, "onAnimationIteration"),
    Et(ns, "onAnimationStart"),
    Et("dblclick", "onDoubleClick"),
    Et("focusin", "onFocus"),
    Et("focusout", "onBlur"),
    Et(U0, "onTransitionRun"),
    Et(N0, "onTransitionStart"),
    Et(x0, "onTransitionCancel"),
    Et(is, "onTransitionEnd"),
    Ga("onMouseEnter", ["mouseout", "mouseover"]),
    Ga("onMouseLeave", ["mouseout", "mouseover"]),
    Ga("onPointerEnter", ["pointerout", "pointerover"]),
    Ga("onPointerLeave", ["pointerout", "pointerover"]),
    ga(
      "onChange",
      "change click focusin focusout input keydown keyup selectionchange".split(
        " ",
      ),
    ),
    ga(
      "onSelect",
      "focusout contextmenu dragend focusin keydown keyup mousedown mouseup selectionchange".split(
        " ",
      ),
    ),
    ga("onBeforeInput", ["compositionend", "keypress", "textInput", "paste"]),
    ga(
      "onCompositionEnd",
      "compositionend focusout keydown keypress keyup mousedown".split(" "),
    ),
    ga(
      "onCompositionStart",
      "compositionstart focusout keydown keypress keyup mousedown".split(" "),
    ),
    ga(
      "onCompositionUpdate",
      "compositionupdate focusout keydown keypress keyup mousedown".split(" "),
    );
  var fu =
      "abort canplay canplaythrough durationchange emptied encrypted ended error loadeddata loadedmetadata loadstart pause play playing progress ratechange resize seeked seeking stalled suspend timeupdate volumechange waiting".split(
        " ",
      ),
    yv = new Set(
      "beforetoggle cancel close invalid load scroll scrollend toggle"
        .split(" ")
        .concat(fu),
    );
  function Or(l, t) {
    t = (t & 4) !== 0;
    for (var a = 0; a < l.length; a++) {
      var e = l[a],
        u = e.event;
      e = e.listeners;
      l: {
        var n = void 0;
        if (t)
          for (var i = e.length - 1; 0 <= i; i--) {
            var c = e[i],
              f = c.instance,
              y = c.currentTarget;
            if (((c = c.listener), f !== n && u.isPropagationStopped()))
              break l;
            (n = c), (u.currentTarget = y);
            try {
              n(u);
            } catch (T) {
              cn(T);
            }
            (u.currentTarget = null), (n = f);
          }
        else
          for (i = 0; i < e.length; i++) {
            if (
              ((c = e[i]),
              (f = c.instance),
              (y = c.currentTarget),
              (c = c.listener),
              f !== n && u.isPropagationStopped())
            )
              break l;
            (n = c), (u.currentTarget = y);
            try {
              n(u);
            } catch (T) {
              cn(T);
            }
            (u.currentTarget = null), (n = f);
          }
      }
    }
  }
  function F(l, t) {
    var a = t[Kn];
    a === void 0 && (a = t[Kn] = new Set());
    var e = l + "__bubble";
    a.has(e) || (Mr(t, l, 2, !1), a.add(e));
  }
  function Hc(l, t, a) {
    var e = 0;
    t && (e |= 4), Mr(a, l, e, t);
  }
  var zn = "_reactListening" + Math.random().toString(36).slice(2);
  function qc(l) {
    if (!l[zn]) {
      (l[zn] = !0),
        bf.forEach(function (a) {
          a !== "selectionchange" && (yv.has(a) || Hc(a, !1, l), Hc(a, !0, l));
        });
      var t = l.nodeType === 9 ? l : l.ownerDocument;
      t === null || t[zn] || ((t[zn] = !0), Hc("selectionchange", !1, t));
    }
  }
  function Mr(l, t, a, e) {
    switch (Fr(t)) {
      case 2:
        var u = Qv;
        break;
      case 8:
        u = Zv;
        break;
      default:
        u = kc;
    }
    (a = u.bind(null, t, a, l)),
      (u = void 0),
      !ai ||
        (t !== "touchstart" && t !== "touchmove" && t !== "wheel") ||
        (u = !0),
      e
        ? u !== void 0
          ? l.addEventListener(t, a, { capture: !0, passive: u })
          : l.addEventListener(t, a, !0)
        : u !== void 0
          ? l.addEventListener(t, a, { passive: u })
          : l.addEventListener(t, a, !1);
  }
  function Bc(l, t, a, e, u) {
    var n = e;
    if ((t & 1) === 0 && (t & 2) === 0 && e !== null)
      l: for (;;) {
        if (e === null) return;
        var i = e.tag;
        if (i === 3 || i === 4) {
          var c = e.stateNode.containerInfo;
          if (c === u) break;
          if (i === 4)
            for (i = e.return; i !== null; ) {
              var f = i.tag;
              if ((f === 3 || f === 4) && i.stateNode.containerInfo === u)
                return;
              i = i.return;
            }
          for (; c !== null; ) {
            if (((i = Ya(c)), i === null)) return;
            if (((f = i.tag), f === 5 || f === 6 || f === 26 || f === 27)) {
              e = n = i;
              continue l;
            }
            c = c.parentNode;
          }
        }
        e = e.return;
      }
    Hf(function () {
      var y = n,
        T = li(a),
        E = [];
      l: {
        var m = cs.get(l);
        if (m !== void 0) {
          var g = qu,
            Z = l;
          switch (l) {
            case "keypress":
              if (xu(a) === 0) break l;
            case "keydown":
            case "keyup":
              g = f0;
              break;
            case "focusin":
              (Z = "focus"), (g = ii);
              break;
            case "focusout":
              (Z = "blur"), (g = ii);
              break;
            case "beforeblur":
            case "afterblur":
              g = ii;
              break;
            case "click":
              if (a.button === 2) break l;
            case "auxclick":
            case "dblclick":
            case "mousedown":
            case "mousemove":
            case "mouseup":
            case "mouseout":
            case "mouseover":
            case "contextmenu":
              g = Yf;
              break;
            case "drag":
            case "dragend":
            case "dragenter":
            case "dragexit":
            case "dragleave":
            case "dragover":
            case "dragstart":
            case "drop":
              g = Wd;
              break;
            case "touchcancel":
            case "touchend":
            case "touchmove":
            case "touchstart":
              g = r0;
              break;
            case es:
            case us:
            case ns:
              g = Pd;
              break;
            case is:
              g = v0;
              break;
            case "scroll":
            case "scrollend":
              g = kd;
              break;
            case "wheel":
              g = h0;
              break;
            case "copy":
            case "cut":
            case "paste":
              g = t0;
              break;
            case "gotpointercapture":
            case "lostpointercapture":
            case "pointercancel":
            case "pointerdown":
            case "pointermove":
            case "pointerout":
            case "pointerover":
            case "pointerup":
              g = Cf;
              break;
            case "toggle":
            case "beforetoggle":
              g = g0;
          }
          var C = (t & 4) !== 0,
            ml = !C && (l === "scroll" || l === "scrollend"),
            d = C ? (m !== null ? m + "Capture" : null) : m;
          C = [];
          for (var r = y, v; r !== null; ) {
            var z = r;
            if (
              ((v = z.stateNode),
              (z = z.tag),
              (z !== 5 && z !== 26 && z !== 27) ||
                v === null ||
                d === null ||
                ((z = Me(r, d)), z != null && C.push(su(r, z, v))),
              ml)
            )
              break;
            r = r.return;
          }
          0 < C.length &&
            ((m = new g(m, Z, null, a, T)), E.push({ event: m, listeners: C }));
        }
      }
      if ((t & 7) === 0) {
        l: {
          if (
            ((m = l === "mouseover" || l === "pointerover"),
            (g = l === "mouseout" || l === "pointerout"),
            m &&
              a !== Pn &&
              (Z = a.relatedTarget || a.fromElement) &&
              (Ya(Z) || Z[Ba]))
          )
            break l;
          if (
            (g || m) &&
            ((m =
              T.window === T
                ? T
                : (m = T.ownerDocument)
                  ? m.defaultView || m.parentWindow
                  : window),
            g
              ? ((Z = a.relatedTarget || a.toElement),
                (g = y),
                (Z = Z ? Ya(Z) : null),
                Z !== null &&
                  ((ml = q(Z)),
                  (C = Z.tag),
                  Z !== ml || (C !== 5 && C !== 27 && C !== 6)) &&
                  (Z = null))
              : ((g = null), (Z = y)),
            g !== Z)
          ) {
            if (
              ((C = Yf),
              (z = "onMouseLeave"),
              (d = "onMouseEnter"),
              (r = "mouse"),
              (l === "pointerout" || l === "pointerover") &&
                ((C = Cf),
                (z = "onPointerLeave"),
                (d = "onPointerEnter"),
                (r = "pointer")),
              (ml = g == null ? m : Oe(g)),
              (v = Z == null ? m : Oe(Z)),
              (m = new C(z, r + "leave", g, a, T)),
              (m.target = ml),
              (m.relatedTarget = v),
              (z = null),
              Ya(T) === y &&
                ((C = new C(d, r + "enter", Z, a, T)),
                (C.target = v),
                (C.relatedTarget = ml),
                (z = C)),
              (ml = z),
              g && Z)
            )
              t: {
                for (C = g, d = Z, r = 0, v = C; v; v = me(v)) r++;
                for (v = 0, z = d; z; z = me(z)) v++;
                for (; 0 < r - v; ) (C = me(C)), r--;
                for (; 0 < v - r; ) (d = me(d)), v--;
                for (; r--; ) {
                  if (C === d || (d !== null && C === d.alternate)) break t;
                  (C = me(C)), (d = me(d));
                }
                C = null;
              }
            else C = null;
            g !== null && Dr(E, m, g, C, !1),
              Z !== null && ml !== null && Dr(E, ml, Z, C, !0);
          }
        }
        l: {
          if (
            ((m = y ? Oe(y) : window),
            (g = m.nodeName && m.nodeName.toLowerCase()),
            g === "select" || (g === "input" && m.type === "file"))
          )
            var H = Jf;
          else if (Vf(m))
            if (wf) H = M0;
            else {
              H = _0;
              var k = p0;
            }
          else
            (g = m.nodeName),
              !g ||
              g.toLowerCase() !== "input" ||
              (m.type !== "checkbox" && m.type !== "radio")
                ? y && In(y.elementType) && (H = Jf)
                : (H = O0);
          if (H && (H = H(l, y))) {
            Kf(E, H, a, T);
            break l;
          }
          k && k(l, m, y),
            l === "focusout" &&
              y &&
              m.type === "number" &&
              y.memoizedProps.value != null &&
              Fn(m, "number", m.value);
        }
        switch (((k = y ? Oe(y) : window), l)) {
          case "focusin":
            (Vf(k) || k.contentEditable === "true") &&
              ((Ja = k), (di = y), (Be = null));
            break;
          case "focusout":
            Be = di = Ja = null;
            break;
          case "mousedown":
            vi = !0;
            break;
          case "contextmenu":
          case "mouseup":
          case "dragend":
            (vi = !1), ts(E, a, T);
            break;
          case "selectionchange":
            if (R0) break;
          case "keydown":
          case "keyup":
            ts(E, a, T);
        }
        var B;
        if (fi)
          l: {
            switch (l) {
              case "compositionstart":
                var G = "onCompositionStart";
                break l;
              case "compositionend":
                G = "onCompositionEnd";
                break l;
              case "compositionupdate":
                G = "onCompositionUpdate";
                break l;
            }
            G = void 0;
          }
        else
          Ka
            ? Zf(l, a) && (G = "onCompositionEnd")
            : l === "keydown" &&
              a.keyCode === 229 &&
              (G = "onCompositionStart");
        G &&
          (Gf &&
            a.locale !== "ko" &&
            (Ka || G !== "onCompositionStart"
              ? G === "onCompositionEnd" && Ka && (B = qf())
              : ((kt = T),
                (ei = "value" in kt ? kt.value : kt.textContent),
                (Ka = !0))),
          (k = En(y, G)),
          0 < k.length &&
            ((G = new jf(G, l, null, a, T)),
            E.push({ event: G, listeners: k }),
            B ? (G.data = B) : ((B = Lf(a)), B !== null && (G.data = B)))),
          (B = b0 ? T0(l, a) : z0(l, a)) &&
            ((G = En(y, "onBeforeInput")),
            0 < G.length &&
              ((k = new jf("onBeforeInput", "beforeinput", null, a, T)),
              E.push({ event: k, listeners: G }),
              (k.data = B))),
          rv(E, l, y, a, T);
      }
      Or(E, t);
    });
  }
  function su(l, t, a) {
    return { instance: l, listener: t, currentTarget: a };
  }
  function En(l, t) {
    for (var a = t + "Capture", e = []; l !== null; ) {
      var u = l,
        n = u.stateNode;
      if (
        ((u = u.tag),
        (u !== 5 && u !== 26 && u !== 27) ||
          n === null ||
          ((u = Me(l, a)),
          u != null && e.unshift(su(l, u, n)),
          (u = Me(l, t)),
          u != null && e.push(su(l, u, n))),
        l.tag === 3)
      )
        return e;
      l = l.return;
    }
    return [];
  }
  function me(l) {
    if (l === null) return null;
    do l = l.return;
    while (l && l.tag !== 5 && l.tag !== 27);
    return l || null;
  }
  function Dr(l, t, a, e, u) {
    for (var n = t._reactName, i = []; a !== null && a !== e; ) {
      var c = a,
        f = c.alternate,
        y = c.stateNode;
      if (((c = c.tag), f !== null && f === e)) break;
      (c !== 5 && c !== 26 && c !== 27) ||
        y === null ||
        ((f = y),
        u
          ? ((y = Me(a, n)), y != null && i.unshift(su(a, y, f)))
          : u || ((y = Me(a, n)), y != null && i.push(su(a, y, f)))),
        (a = a.return);
    }
    i.length !== 0 && l.push({ event: t, listeners: i });
  }
  var hv = /\r\n?/g,
    mv = /\u0000|\uFFFD/g;
  function Rr(l) {
    return (typeof l == "string" ? l : "" + l)
      .replace(
        hv,
        `
`,
      )
      .replace(mv, "");
  }
  function Ur(l, t) {
    return (t = Rr(t)), Rr(l) === t;
  }
  function An() {}
  function hl(l, t, a, e, u, n) {
    switch (a) {
      case "children":
        typeof e == "string"
          ? t === "body" || (t === "textarea" && e === "") || Za(l, e)
          : (typeof e == "number" || typeof e == "bigint") &&
            t !== "body" &&
            Za(l, "" + e);
        break;
      case "className":
        Mu(l, "class", e);
        break;
      case "tabIndex":
        Mu(l, "tabindex", e);
        break;
      case "dir":
      case "role":
      case "viewBox":
      case "width":
      case "height":
        Mu(l, a, e);
        break;
      case "style":
        Nf(l, e, n);
        break;
      case "data":
        if (t !== "object") {
          Mu(l, "data", e);
          break;
        }
      case "src":
      case "href":
        if (e === "" && (t !== "a" || a !== "href")) {
          l.removeAttribute(a);
          break;
        }
        if (
          e == null ||
          typeof e == "function" ||
          typeof e == "symbol" ||
          typeof e == "boolean"
        ) {
          l.removeAttribute(a);
          break;
        }
        (e = Uu("" + e)), l.setAttribute(a, e);
        break;
      case "action":
      case "formAction":
        if (typeof e == "function") {
          l.setAttribute(
            a,
            "javascript:throw new Error('A React form was unexpectedly submitted. If you called form.submit() manually, consider using form.requestSubmit() instead. If you\\'re trying to use event.stopPropagation() in a submit event handler, consider also calling event.preventDefault().')",
          );
          break;
        } else
          typeof n == "function" &&
            (a === "formAction"
              ? (t !== "input" && hl(l, t, "name", u.name, u, null),
                hl(l, t, "formEncType", u.formEncType, u, null),
                hl(l, t, "formMethod", u.formMethod, u, null),
                hl(l, t, "formTarget", u.formTarget, u, null))
              : (hl(l, t, "encType", u.encType, u, null),
                hl(l, t, "method", u.method, u, null),
                hl(l, t, "target", u.target, u, null)));
        if (e == null || typeof e == "symbol" || typeof e == "boolean") {
          l.removeAttribute(a);
          break;
        }
        (e = Uu("" + e)), l.setAttribute(a, e);
        break;
      case "onClick":
        e != null && (l.onclick = An);
        break;
      case "onScroll":
        e != null && F("scroll", l);
        break;
      case "onScrollEnd":
        e != null && F("scrollend", l);
        break;
      case "dangerouslySetInnerHTML":
        if (e != null) {
          if (typeof e != "object" || !("__html" in e)) throw Error(s(61));
          if (((a = e.__html), a != null)) {
            if (u.children != null) throw Error(s(60));
            l.innerHTML = a;
          }
        }
        break;
      case "multiple":
        l.multiple = e && typeof e != "function" && typeof e != "symbol";
        break;
      case "muted":
        l.muted = e && typeof e != "function" && typeof e != "symbol";
        break;
      case "suppressContentEditableWarning":
      case "suppressHydrationWarning":
      case "defaultValue":
      case "defaultChecked":
      case "innerHTML":
      case "ref":
        break;
      case "autoFocus":
        break;
      case "xlinkHref":
        if (
          e == null ||
          typeof e == "function" ||
          typeof e == "boolean" ||
          typeof e == "symbol"
        ) {
          l.removeAttribute("xlink:href");
          break;
        }
        (a = Uu("" + e)),
          l.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href", a);
        break;
      case "contentEditable":
      case "spellCheck":
      case "draggable":
      case "value":
      case "autoReverse":
      case "externalResourcesRequired":
      case "focusable":
      case "preserveAlpha":
        e != null && typeof e != "function" && typeof e != "symbol"
          ? l.setAttribute(a, "" + e)
          : l.removeAttribute(a);
        break;
      case "inert":
      case "allowFullScreen":
      case "async":
      case "autoPlay":
      case "controls":
      case "default":
      case "defer":
      case "disabled":
      case "disablePictureInPicture":
      case "disableRemotePlayback":
      case "formNoValidate":
      case "hidden":
      case "loop":
      case "noModule":
      case "noValidate":
      case "open":
      case "playsInline":
      case "readOnly":
      case "required":
      case "reversed":
      case "scoped":
      case "seamless":
      case "itemScope":
        e && typeof e != "function" && typeof e != "symbol"
          ? l.setAttribute(a, "")
          : l.removeAttribute(a);
        break;
      case "capture":
      case "download":
        e === !0
          ? l.setAttribute(a, "")
          : e !== !1 &&
              e != null &&
              typeof e != "function" &&
              typeof e != "symbol"
            ? l.setAttribute(a, e)
            : l.removeAttribute(a);
        break;
      case "cols":
      case "rows":
      case "size":
      case "span":
        e != null &&
        typeof e != "function" &&
        typeof e != "symbol" &&
        !isNaN(e) &&
        1 <= e
          ? l.setAttribute(a, e)
          : l.removeAttribute(a);
        break;
      case "rowSpan":
      case "start":
        e == null || typeof e == "function" || typeof e == "symbol" || isNaN(e)
          ? l.removeAttribute(a)
          : l.setAttribute(a, e);
        break;
      case "popover":
        F("beforetoggle", l), F("toggle", l), Ou(l, "popover", e);
        break;
      case "xlinkActuate":
        Nt(l, "http://www.w3.org/1999/xlink", "xlink:actuate", e);
        break;
      case "xlinkArcrole":
        Nt(l, "http://www.w3.org/1999/xlink", "xlink:arcrole", e);
        break;
      case "xlinkRole":
        Nt(l, "http://www.w3.org/1999/xlink", "xlink:role", e);
        break;
      case "xlinkShow":
        Nt(l, "http://www.w3.org/1999/xlink", "xlink:show", e);
        break;
      case "xlinkTitle":
        Nt(l, "http://www.w3.org/1999/xlink", "xlink:title", e);
        break;
      case "xlinkType":
        Nt(l, "http://www.w3.org/1999/xlink", "xlink:type", e);
        break;
      case "xmlBase":
        Nt(l, "http://www.w3.org/XML/1998/namespace", "xml:base", e);
        break;
      case "xmlLang":
        Nt(l, "http://www.w3.org/XML/1998/namespace", "xml:lang", e);
        break;
      case "xmlSpace":
        Nt(l, "http://www.w3.org/XML/1998/namespace", "xml:space", e);
        break;
      case "is":
        Ou(l, "is", e);
        break;
      case "innerText":
      case "textContent":
        break;
      default:
        (!(2 < a.length) ||
          (a[0] !== "o" && a[0] !== "O") ||
          (a[1] !== "n" && a[1] !== "N")) &&
          ((a = Jd.get(a) || a), Ou(l, a, e));
    }
  }
  function Yc(l, t, a, e, u, n) {
    switch (a) {
      case "style":
        Nf(l, e, n);
        break;
      case "dangerouslySetInnerHTML":
        if (e != null) {
          if (typeof e != "object" || !("__html" in e)) throw Error(s(61));
          if (((a = e.__html), a != null)) {
            if (u.children != null) throw Error(s(60));
            l.innerHTML = a;
          }
        }
        break;
      case "children":
        typeof e == "string"
          ? Za(l, e)
          : (typeof e == "number" || typeof e == "bigint") && Za(l, "" + e);
        break;
      case "onScroll":
        e != null && F("scroll", l);
        break;
      case "onScrollEnd":
        e != null && F("scrollend", l);
        break;
      case "onClick":
        e != null && (l.onclick = An);
        break;
      case "suppressContentEditableWarning":
      case "suppressHydrationWarning":
      case "innerHTML":
      case "ref":
        break;
      case "innerText":
      case "textContent":
        break;
      default:
        if (!Tf.hasOwnProperty(a))
          l: {
            if (
              a[0] === "o" &&
              a[1] === "n" &&
              ((u = a.endsWith("Capture")),
              (t = a.slice(2, u ? a.length - 7 : void 0)),
              (n = l[Il] || null),
              (n = n != null ? n[a] : null),
              typeof n == "function" && l.removeEventListener(t, n, u),
              typeof e == "function")
            ) {
              typeof n != "function" &&
                n !== null &&
                (a in l
                  ? (l[a] = null)
                  : l.hasAttribute(a) && l.removeAttribute(a)),
                l.addEventListener(t, e, u);
              break l;
            }
            a in l
              ? (l[a] = e)
              : e === !0
                ? l.setAttribute(a, "")
                : Ou(l, a, e);
          }
    }
  }
  function Kl(l, t, a) {
    switch (t) {
      case "div":
      case "span":
      case "svg":
      case "path":
      case "a":
      case "g":
      case "p":
      case "li":
        break;
      case "img":
        F("error", l), F("load", l);
        var e = !1,
          u = !1,
          n;
        for (n in a)
          if (a.hasOwnProperty(n)) {
            var i = a[n];
            if (i != null)
              switch (n) {
                case "src":
                  e = !0;
                  break;
                case "srcSet":
                  u = !0;
                  break;
                case "children":
                case "dangerouslySetInnerHTML":
                  throw Error(s(137, t));
                default:
                  hl(l, t, n, i, a, null);
              }
          }
        u && hl(l, t, "srcSet", a.srcSet, a, null),
          e && hl(l, t, "src", a.src, a, null);
        return;
      case "input":
        F("invalid", l);
        var c = (n = i = u = null),
          f = null,
          y = null;
        for (e in a)
          if (a.hasOwnProperty(e)) {
            var T = a[e];
            if (T != null)
              switch (e) {
                case "name":
                  u = T;
                  break;
                case "type":
                  i = T;
                  break;
                case "checked":
                  f = T;
                  break;
                case "defaultChecked":
                  y = T;
                  break;
                case "value":
                  n = T;
                  break;
                case "defaultValue":
                  c = T;
                  break;
                case "children":
                case "dangerouslySetInnerHTML":
                  if (T != null) throw Error(s(137, t));
                  break;
                default:
                  hl(l, t, e, T, a, null);
              }
          }
        Mf(l, n, c, f, y, i, u, !1), Du(l);
        return;
      case "select":
        F("invalid", l), (e = i = n = null);
        for (u in a)
          if (a.hasOwnProperty(u) && ((c = a[u]), c != null))
            switch (u) {
              case "value":
                n = c;
                break;
              case "defaultValue":
                i = c;
                break;
              case "multiple":
                e = c;
              default:
                hl(l, t, u, c, a, null);
            }
        (t = n),
          (a = i),
          (l.multiple = !!e),
          t != null ? Qa(l, !!e, t, !1) : a != null && Qa(l, !!e, a, !0);
        return;
      case "textarea":
        F("invalid", l), (n = u = e = null);
        for (i in a)
          if (a.hasOwnProperty(i) && ((c = a[i]), c != null))
            switch (i) {
              case "value":
                e = c;
                break;
              case "defaultValue":
                u = c;
                break;
              case "children":
                n = c;
                break;
              case "dangerouslySetInnerHTML":
                if (c != null) throw Error(s(91));
                break;
              default:
                hl(l, t, i, c, a, null);
            }
        Rf(l, e, u, n), Du(l);
        return;
      case "option":
        for (f in a)
          if (a.hasOwnProperty(f) && ((e = a[f]), e != null))
            switch (f) {
              case "selected":
                l.selected =
                  e && typeof e != "function" && typeof e != "symbol";
                break;
              default:
                hl(l, t, f, e, a, null);
            }
        return;
      case "dialog":
        F("beforetoggle", l), F("toggle", l), F("cancel", l), F("close", l);
        break;
      case "iframe":
      case "object":
        F("load", l);
        break;
      case "video":
      case "audio":
        for (e = 0; e < fu.length; e++) F(fu[e], l);
        break;
      case "image":
        F("error", l), F("load", l);
        break;
      case "details":
        F("toggle", l);
        break;
      case "embed":
      case "source":
      case "link":
        F("error", l), F("load", l);
      case "area":
      case "base":
      case "br":
      case "col":
      case "hr":
      case "keygen":
      case "meta":
      case "param":
      case "track":
      case "wbr":
      case "menuitem":
        for (y in a)
          if (a.hasOwnProperty(y) && ((e = a[y]), e != null))
            switch (y) {
              case "children":
              case "dangerouslySetInnerHTML":
                throw Error(s(137, t));
              default:
                hl(l, t, y, e, a, null);
            }
        return;
      default:
        if (In(t)) {
          for (T in a)
            a.hasOwnProperty(T) &&
              ((e = a[T]), e !== void 0 && Yc(l, t, T, e, a, void 0));
          return;
        }
    }
    for (c in a)
      a.hasOwnProperty(c) && ((e = a[c]), e != null && hl(l, t, c, e, a, null));
  }
  function gv(l, t, a, e) {
    switch (t) {
      case "div":
      case "span":
      case "svg":
      case "path":
      case "a":
      case "g":
      case "p":
      case "li":
        break;
      case "input":
        var u = null,
          n = null,
          i = null,
          c = null,
          f = null,
          y = null,
          T = null;
        for (g in a) {
          var E = a[g];
          if (a.hasOwnProperty(g) && E != null)
            switch (g) {
              case "checked":
                break;
              case "value":
                break;
              case "defaultValue":
                f = E;
              default:
                e.hasOwnProperty(g) || hl(l, t, g, null, e, E);
            }
        }
        for (var m in e) {
          var g = e[m];
          if (((E = a[m]), e.hasOwnProperty(m) && (g != null || E != null)))
            switch (m) {
              case "type":
                n = g;
                break;
              case "name":
                u = g;
                break;
              case "checked":
                y = g;
                break;
              case "defaultChecked":
                T = g;
                break;
              case "value":
                i = g;
                break;
              case "defaultValue":
                c = g;
                break;
              case "children":
              case "dangerouslySetInnerHTML":
                if (g != null) throw Error(s(137, t));
                break;
              default:
                g !== E && hl(l, t, m, g, e, E);
            }
        }
        Wn(l, i, c, f, y, T, n, u);
        return;
      case "select":
        g = i = c = m = null;
        for (n in a)
          if (((f = a[n]), a.hasOwnProperty(n) && f != null))
            switch (n) {
              case "value":
                break;
              case "multiple":
                g = f;
              default:
                e.hasOwnProperty(n) || hl(l, t, n, null, e, f);
            }
        for (u in e)
          if (
            ((n = e[u]),
            (f = a[u]),
            e.hasOwnProperty(u) && (n != null || f != null))
          )
            switch (u) {
              case "value":
                m = n;
                break;
              case "defaultValue":
                c = n;
                break;
              case "multiple":
                i = n;
              default:
                n !== f && hl(l, t, u, n, e, f);
            }
        (t = c),
          (a = i),
          (e = g),
          m != null
            ? Qa(l, !!a, m, !1)
            : !!e != !!a &&
              (t != null ? Qa(l, !!a, t, !0) : Qa(l, !!a, a ? [] : "", !1));
        return;
      case "textarea":
        g = m = null;
        for (c in a)
          if (
            ((u = a[c]),
            a.hasOwnProperty(c) && u != null && !e.hasOwnProperty(c))
          )
            switch (c) {
              case "value":
                break;
              case "children":
                break;
              default:
                hl(l, t, c, null, e, u);
            }
        for (i in e)
          if (
            ((u = e[i]),
            (n = a[i]),
            e.hasOwnProperty(i) && (u != null || n != null))
          )
            switch (i) {
              case "value":
                m = u;
                break;
              case "defaultValue":
                g = u;
                break;
              case "children":
                break;
              case "dangerouslySetInnerHTML":
                if (u != null) throw Error(s(91));
                break;
              default:
                u !== n && hl(l, t, i, u, e, n);
            }
        Df(l, m, g);
        return;
      case "option":
        for (var Z in a)
          if (
            ((m = a[Z]),
            a.hasOwnProperty(Z) && m != null && !e.hasOwnProperty(Z))
          )
            switch (Z) {
              case "selected":
                l.selected = !1;
                break;
              default:
                hl(l, t, Z, null, e, m);
            }
        for (f in e)
          if (
            ((m = e[f]),
            (g = a[f]),
            e.hasOwnProperty(f) && m !== g && (m != null || g != null))
          )
            switch (f) {
              case "selected":
                l.selected =
                  m && typeof m != "function" && typeof m != "symbol";
                break;
              default:
                hl(l, t, f, m, e, g);
            }
        return;
      case "img":
      case "link":
      case "area":
      case "base":
      case "br":
      case "col":
      case "embed":
      case "hr":
      case "keygen":
      case "meta":
      case "param":
      case "source":
      case "track":
      case "wbr":
      case "menuitem":
        for (var C in a)
          (m = a[C]),
            a.hasOwnProperty(C) &&
              m != null &&
              !e.hasOwnProperty(C) &&
              hl(l, t, C, null, e, m);
        for (y in e)
          if (
            ((m = e[y]),
            (g = a[y]),
            e.hasOwnProperty(y) && m !== g && (m != null || g != null))
          )
            switch (y) {
              case "children":
              case "dangerouslySetInnerHTML":
                if (m != null) throw Error(s(137, t));
                break;
              default:
                hl(l, t, y, m, e, g);
            }
        return;
      default:
        if (In(t)) {
          for (var ml in a)
            (m = a[ml]),
              a.hasOwnProperty(ml) &&
                m !== void 0 &&
                !e.hasOwnProperty(ml) &&
                Yc(l, t, ml, void 0, e, m);
          for (T in e)
            (m = e[T]),
              (g = a[T]),
              !e.hasOwnProperty(T) ||
                m === g ||
                (m === void 0 && g === void 0) ||
                Yc(l, t, T, m, e, g);
          return;
        }
    }
    for (var d in a)
      (m = a[d]),
        a.hasOwnProperty(d) &&
          m != null &&
          !e.hasOwnProperty(d) &&
          hl(l, t, d, null, e, m);
    for (E in e)
      (m = e[E]),
        (g = a[E]),
        !e.hasOwnProperty(E) ||
          m === g ||
          (m == null && g == null) ||
          hl(l, t, E, m, e, g);
  }
  var jc = null,
    Cc = null;
  function pn(l) {
    return l.nodeType === 9 ? l : l.ownerDocument;
  }
  function Nr(l) {
    switch (l) {
      case "http://www.w3.org/2000/svg":
        return 1;
      case "http://www.w3.org/1998/Math/MathML":
        return 2;
      default:
        return 0;
    }
  }
  function xr(l, t) {
    if (l === 0)
      switch (t) {
        case "svg":
          return 1;
        case "math":
          return 2;
        default:
          return 0;
      }
    return l === 1 && t === "foreignObject" ? 0 : l;
  }
  function Gc(l, t) {
    return (
      l === "textarea" ||
      l === "noscript" ||
      typeof t.children == "string" ||
      typeof t.children == "number" ||
      typeof t.children == "bigint" ||
      (typeof t.dangerouslySetInnerHTML == "object" &&
        t.dangerouslySetInnerHTML !== null &&
        t.dangerouslySetInnerHTML.__html != null)
    );
  }
  var Xc = null;
  function Sv() {
    var l = window.event;
    return l && l.type === "popstate"
      ? l === Xc
        ? !1
        : ((Xc = l), !0)
      : ((Xc = null), !1);
  }
  var Hr = typeof setTimeout == "function" ? setTimeout : void 0,
    bv = typeof clearTimeout == "function" ? clearTimeout : void 0,
    qr = typeof Promise == "function" ? Promise : void 0,
    Tv =
      typeof queueMicrotask == "function"
        ? queueMicrotask
        : typeof qr < "u"
          ? function (l) {
              return qr.resolve(null).then(l).catch(zv);
            }
          : Hr;
  function zv(l) {
    setTimeout(function () {
      throw l;
    });
  }
  function oa(l) {
    return l === "head";
  }
  function Br(l, t) {
    var a = t,
      e = 0,
      u = 0;
    do {
      var n = a.nextSibling;
      if ((l.removeChild(a), n && n.nodeType === 8))
        if (((a = n.data), a === "/$")) {
          if (0 < e && 8 > e) {
            a = e;
            var i = l.ownerDocument;
            if ((a & 1 && ou(i.documentElement), a & 2 && ou(i.body), a & 4))
              for (a = i.head, ou(a), i = a.firstChild; i; ) {
                var c = i.nextSibling,
                  f = i.nodeName;
                i[_e] ||
                  f === "SCRIPT" ||
                  f === "STYLE" ||
                  (f === "LINK" && i.rel.toLowerCase() === "stylesheet") ||
                  a.removeChild(i),
                  (i = c);
              }
          }
          if (u === 0) {
            l.removeChild(n), Su(t);
            return;
          }
          u--;
        } else
          a === "$" || a === "$?" || a === "$!"
            ? u++
            : (e = a.charCodeAt(0) - 48);
      else e = 0;
      a = n;
    } while (a);
    Su(t);
  }
  function Qc(l) {
    var t = l.firstChild;
    for (t && t.nodeType === 10 && (t = t.nextSibling); t; ) {
      var a = t;
      switch (((t = t.nextSibling), a.nodeName)) {
        case "HTML":
        case "HEAD":
        case "BODY":
          Qc(a), Jn(a);
          continue;
        case "SCRIPT":
        case "STYLE":
          continue;
        case "LINK":
          if (a.rel.toLowerCase() === "stylesheet") continue;
      }
      l.removeChild(a);
    }
  }
  function Ev(l, t, a, e) {
    for (; l.nodeType === 1; ) {
      var u = a;
      if (l.nodeName.toLowerCase() !== t.toLowerCase()) {
        if (!e && (l.nodeName !== "INPUT" || l.type !== "hidden")) break;
      } else if (e) {
        if (!l[_e])
          switch (t) {
            case "meta":
              if (!l.hasAttribute("itemprop")) break;
              return l;
            case "link":
              if (
                ((n = l.getAttribute("rel")),
                n === "stylesheet" && l.hasAttribute("data-precedence"))
              )
                break;
              if (
                n !== u.rel ||
                l.getAttribute("href") !==
                  (u.href == null || u.href === "" ? null : u.href) ||
                l.getAttribute("crossorigin") !==
                  (u.crossOrigin == null ? null : u.crossOrigin) ||
                l.getAttribute("title") !== (u.title == null ? null : u.title)
              )
                break;
              return l;
            case "style":
              if (l.hasAttribute("data-precedence")) break;
              return l;
            case "script":
              if (
                ((n = l.getAttribute("src")),
                (n !== (u.src == null ? null : u.src) ||
                  l.getAttribute("type") !== (u.type == null ? null : u.type) ||
                  l.getAttribute("crossorigin") !==
                    (u.crossOrigin == null ? null : u.crossOrigin)) &&
                  n &&
                  l.hasAttribute("async") &&
                  !l.hasAttribute("itemprop"))
              )
                break;
              return l;
            default:
              return l;
          }
      } else if (t === "input" && l.type === "hidden") {
        var n = u.name == null ? null : "" + u.name;
        if (u.type === "hidden" && l.getAttribute("name") === n) return l;
      } else return l;
      if (((l = pt(l.nextSibling)), l === null)) break;
    }
    return null;
  }
  function Av(l, t, a) {
    if (t === "") return null;
    for (; l.nodeType !== 3; )
      if (
        ((l.nodeType !== 1 || l.nodeName !== "INPUT" || l.type !== "hidden") &&
          !a) ||
        ((l = pt(l.nextSibling)), l === null)
      )
        return null;
    return l;
  }
  function Zc(l) {
    return (
      l.data === "$!" ||
      (l.data === "$?" && l.ownerDocument.readyState === "complete")
    );
  }
  function pv(l, t) {
    var a = l.ownerDocument;
    if (l.data !== "$?" || a.readyState === "complete") t();
    else {
      var e = function () {
        t(), a.removeEventListener("DOMContentLoaded", e);
      };
      a.addEventListener("DOMContentLoaded", e), (l._reactRetry = e);
    }
  }
  function pt(l) {
    for (; l != null; l = l.nextSibling) {
      var t = l.nodeType;
      if (t === 1 || t === 3) break;
      if (t === 8) {
        if (
          ((t = l.data),
          t === "$" || t === "$!" || t === "$?" || t === "F!" || t === "F")
        )
          break;
        if (t === "/$") return null;
      }
    }
    return l;
  }
  var Lc = null;
  function Yr(l) {
    l = l.previousSibling;
    for (var t = 0; l; ) {
      if (l.nodeType === 8) {
        var a = l.data;
        if (a === "$" || a === "$!" || a === "$?") {
          if (t === 0) return l;
          t--;
        } else a === "/$" && t++;
      }
      l = l.previousSibling;
    }
    return null;
  }
  function jr(l, t, a) {
    switch (((t = pn(a)), l)) {
      case "html":
        if (((l = t.documentElement), !l)) throw Error(s(452));
        return l;
      case "head":
        if (((l = t.head), !l)) throw Error(s(453));
        return l;
      case "body":
        if (((l = t.body), !l)) throw Error(s(454));
        return l;
      default:
        throw Error(s(451));
    }
  }
  function ou(l) {
    for (var t = l.attributes; t.length; ) l.removeAttributeNode(t[0]);
    Jn(l);
  }
  var zt = new Map(),
    Cr = new Set();
  function _n(l) {
    return typeof l.getRootNode == "function"
      ? l.getRootNode()
      : l.nodeType === 9
        ? l
        : l.ownerDocument;
  }
  var Kt = M.d;
  M.d = { f: _v, r: Ov, D: Mv, C: Dv, L: Rv, m: Uv, X: xv, S: Nv, M: Hv };
  function _v() {
    var l = Kt.f(),
      t = mn();
    return l || t;
  }
  function Ov(l) {
    var t = ja(l);
    t !== null && t.tag === 5 && t.type === "form" ? eo(t) : Kt.r(l);
  }
  var ge = typeof document > "u" ? null : document;
  function Gr(l, t, a) {
    var e = ge;
    if (e && typeof t == "string" && t) {
      var u = yt(t);
      (u = 'link[rel="' + l + '"][href="' + u + '"]'),
        typeof a == "string" && (u += '[crossorigin="' + a + '"]'),
        Cr.has(u) ||
          (Cr.add(u),
          (l = { rel: l, crossOrigin: a, href: t }),
          e.querySelector(u) === null &&
            ((t = e.createElement("link")),
            Kl(t, "link", l),
            Gl(t),
            e.head.appendChild(t)));
    }
  }
  function Mv(l) {
    Kt.D(l), Gr("dns-prefetch", l, null);
  }
  function Dv(l, t) {
    Kt.C(l, t), Gr("preconnect", l, t);
  }
  function Rv(l, t, a) {
    Kt.L(l, t, a);
    var e = ge;
    if (e && l && t) {
      var u = 'link[rel="preload"][as="' + yt(t) + '"]';
      t === "image" && a && a.imageSrcSet
        ? ((u += '[imagesrcset="' + yt(a.imageSrcSet) + '"]'),
          typeof a.imageSizes == "string" &&
            (u += '[imagesizes="' + yt(a.imageSizes) + '"]'))
        : (u += '[href="' + yt(l) + '"]');
      var n = u;
      switch (t) {
        case "style":
          n = Se(l);
          break;
        case "script":
          n = be(l);
      }
      zt.has(n) ||
        ((l = x(
          {
            rel: "preload",
            href: t === "image" && a && a.imageSrcSet ? void 0 : l,
            as: t,
          },
          a,
        )),
        zt.set(n, l),
        e.querySelector(u) !== null ||
          (t === "style" && e.querySelector(ru(n))) ||
          (t === "script" && e.querySelector(du(n))) ||
          ((t = e.createElement("link")),
          Kl(t, "link", l),
          Gl(t),
          e.head.appendChild(t)));
    }
  }
  function Uv(l, t) {
    Kt.m(l, t);
    var a = ge;
    if (a && l) {
      var e = t && typeof t.as == "string" ? t.as : "script",
        u =
          'link[rel="modulepreload"][as="' + yt(e) + '"][href="' + yt(l) + '"]',
        n = u;
      switch (e) {
        case "audioworklet":
        case "paintworklet":
        case "serviceworker":
        case "sharedworker":
        case "worker":
        case "script":
          n = be(l);
      }
      if (
        !zt.has(n) &&
        ((l = x({ rel: "modulepreload", href: l }, t)),
        zt.set(n, l),
        a.querySelector(u) === null)
      ) {
        switch (e) {
          case "audioworklet":
          case "paintworklet":
          case "serviceworker":
          case "sharedworker":
          case "worker":
          case "script":
            if (a.querySelector(du(n))) return;
        }
        (e = a.createElement("link")),
          Kl(e, "link", l),
          Gl(e),
          a.head.appendChild(e);
      }
    }
  }
  function Nv(l, t, a) {
    Kt.S(l, t, a);
    var e = ge;
    if (e && l) {
      var u = Ca(e).hoistableStyles,
        n = Se(l);
      t = t || "default";
      var i = u.get(n);
      if (!i) {
        var c = { loading: 0, preload: null };
        if ((i = e.querySelector(ru(n)))) c.loading = 5;
        else {
          (l = x({ rel: "stylesheet", href: l, "data-precedence": t }, a)),
            (a = zt.get(n)) && Vc(l, a);
          var f = (i = e.createElement("link"));
          Gl(f),
            Kl(f, "link", l),
            (f._p = new Promise(function (y, T) {
              (f.onload = y), (f.onerror = T);
            })),
            f.addEventListener("load", function () {
              c.loading |= 1;
            }),
            f.addEventListener("error", function () {
              c.loading |= 2;
            }),
            (c.loading |= 4),
            On(i, t, e);
        }
        (i = { type: "stylesheet", instance: i, count: 1, state: c }),
          u.set(n, i);
      }
    }
  }
  function xv(l, t) {
    Kt.X(l, t);
    var a = ge;
    if (a && l) {
      var e = Ca(a).hoistableScripts,
        u = be(l),
        n = e.get(u);
      n ||
        ((n = a.querySelector(du(u))),
        n ||
          ((l = x({ src: l, async: !0 }, t)),
          (t = zt.get(u)) && Kc(l, t),
          (n = a.createElement("script")),
          Gl(n),
          Kl(n, "link", l),
          a.head.appendChild(n)),
        (n = { type: "script", instance: n, count: 1, state: null }),
        e.set(u, n));
    }
  }
  function Hv(l, t) {
    Kt.M(l, t);
    var a = ge;
    if (a && l) {
      var e = Ca(a).hoistableScripts,
        u = be(l),
        n = e.get(u);
      n ||
        ((n = a.querySelector(du(u))),
        n ||
          ((l = x({ src: l, async: !0, type: "module" }, t)),
          (t = zt.get(u)) && Kc(l, t),
          (n = a.createElement("script")),
          Gl(n),
          Kl(n, "link", l),
          a.head.appendChild(n)),
        (n = { type: "script", instance: n, count: 1, state: null }),
        e.set(u, n));
    }
  }
  function Xr(l, t, a, e) {
    var u = (u = Q.current) ? _n(u) : null;
    if (!u) throw Error(s(446));
    switch (l) {
      case "meta":
      case "title":
        return null;
      case "style":
        return typeof a.precedence == "string" && typeof a.href == "string"
          ? ((t = Se(a.href)),
            (a = Ca(u).hoistableStyles),
            (e = a.get(t)),
            e ||
              ((e = { type: "style", instance: null, count: 0, state: null }),
              a.set(t, e)),
            e)
          : { type: "void", instance: null, count: 0, state: null };
      case "link":
        if (
          a.rel === "stylesheet" &&
          typeof a.href == "string" &&
          typeof a.precedence == "string"
        ) {
          l = Se(a.href);
          var n = Ca(u).hoistableStyles,
            i = n.get(l);
          if (
            (i ||
              ((u = u.ownerDocument || u),
              (i = {
                type: "stylesheet",
                instance: null,
                count: 0,
                state: { loading: 0, preload: null },
              }),
              n.set(l, i),
              (n = u.querySelector(ru(l))) &&
                !n._p &&
                ((i.instance = n), (i.state.loading = 5)),
              zt.has(l) ||
                ((a = {
                  rel: "preload",
                  as: "style",
                  href: a.href,
                  crossOrigin: a.crossOrigin,
                  integrity: a.integrity,
                  media: a.media,
                  hrefLang: a.hrefLang,
                  referrerPolicy: a.referrerPolicy,
                }),
                zt.set(l, a),
                n || qv(u, l, a, i.state))),
            t && e === null)
          )
            throw Error(s(528, ""));
          return i;
        }
        if (t && e !== null) throw Error(s(529, ""));
        return null;
      case "script":
        return (
          (t = a.async),
          (a = a.src),
          typeof a == "string" &&
          t &&
          typeof t != "function" &&
          typeof t != "symbol"
            ? ((t = be(a)),
              (a = Ca(u).hoistableScripts),
              (e = a.get(t)),
              e ||
                ((e = {
                  type: "script",
                  instance: null,
                  count: 0,
                  state: null,
                }),
                a.set(t, e)),
              e)
            : { type: "void", instance: null, count: 0, state: null }
        );
      default:
        throw Error(s(444, l));
    }
  }
  function Se(l) {
    return 'href="' + yt(l) + '"';
  }
  function ru(l) {
    return 'link[rel="stylesheet"][' + l + "]";
  }
  function Qr(l) {
    return x({}, l, { "data-precedence": l.precedence, precedence: null });
  }
  function qv(l, t, a, e) {
    l.querySelector('link[rel="preload"][as="style"][' + t + "]")
      ? (e.loading = 1)
      : ((t = l.createElement("link")),
        (e.preload = t),
        t.addEventListener("load", function () {
          return (e.loading |= 1);
        }),
        t.addEventListener("error", function () {
          return (e.loading |= 2);
        }),
        Kl(t, "link", a),
        Gl(t),
        l.head.appendChild(t));
  }
  function be(l) {
    return '[src="' + yt(l) + '"]';
  }
  function du(l) {
    return "script[async]" + l;
  }
  function Zr(l, t, a) {
    if ((t.count++, t.instance === null))
      switch (t.type) {
        case "style":
          var e = l.querySelector('style[data-href~="' + yt(a.href) + '"]');
          if (e) return (t.instance = e), Gl(e), e;
          var u = x({}, a, {
            "data-href": a.href,
            "data-precedence": a.precedence,
            href: null,
            precedence: null,
          });
          return (
            (e = (l.ownerDocument || l).createElement("style")),
            Gl(e),
            Kl(e, "style", u),
            On(e, a.precedence, l),
            (t.instance = e)
          );
        case "stylesheet":
          u = Se(a.href);
          var n = l.querySelector(ru(u));
          if (n) return (t.state.loading |= 4), (t.instance = n), Gl(n), n;
          (e = Qr(a)),
            (u = zt.get(u)) && Vc(e, u),
            (n = (l.ownerDocument || l).createElement("link")),
            Gl(n);
          var i = n;
          return (
            (i._p = new Promise(function (c, f) {
              (i.onload = c), (i.onerror = f);
            })),
            Kl(n, "link", e),
            (t.state.loading |= 4),
            On(n, a.precedence, l),
            (t.instance = n)
          );
        case "script":
          return (
            (n = be(a.src)),
            (u = l.querySelector(du(n)))
              ? ((t.instance = u), Gl(u), u)
              : ((e = a),
                (u = zt.get(n)) && ((e = x({}, a)), Kc(e, u)),
                (l = l.ownerDocument || l),
                (u = l.createElement("script")),
                Gl(u),
                Kl(u, "link", e),
                l.head.appendChild(u),
                (t.instance = u))
          );
        case "void":
          return null;
        default:
          throw Error(s(443, t.type));
      }
    else
      t.type === "stylesheet" &&
        (t.state.loading & 4) === 0 &&
        ((e = t.instance), (t.state.loading |= 4), On(e, a.precedence, l));
    return t.instance;
  }
  function On(l, t, a) {
    for (
      var e = a.querySelectorAll(
          'link[rel="stylesheet"][data-precedence],style[data-precedence]',
        ),
        u = e.length ? e[e.length - 1] : null,
        n = u,
        i = 0;
      i < e.length;
      i++
    ) {
      var c = e[i];
      if (c.dataset.precedence === t) n = c;
      else if (n !== u) break;
    }
    n
      ? n.parentNode.insertBefore(l, n.nextSibling)
      : ((t = a.nodeType === 9 ? a.head : a), t.insertBefore(l, t.firstChild));
  }
  function Vc(l, t) {
    l.crossOrigin == null && (l.crossOrigin = t.crossOrigin),
      l.referrerPolicy == null && (l.referrerPolicy = t.referrerPolicy),
      l.title == null && (l.title = t.title);
  }
  function Kc(l, t) {
    l.crossOrigin == null && (l.crossOrigin = t.crossOrigin),
      l.referrerPolicy == null && (l.referrerPolicy = t.referrerPolicy),
      l.integrity == null && (l.integrity = t.integrity);
  }
  var Mn = null;
  function Lr(l, t, a) {
    if (Mn === null) {
      var e = new Map(),
        u = (Mn = new Map());
      u.set(a, e);
    } else (u = Mn), (e = u.get(a)), e || ((e = new Map()), u.set(a, e));
    if (e.has(l)) return e;
    for (
      e.set(l, null), a = a.getElementsByTagName(l), u = 0;
      u < a.length;
      u++
    ) {
      var n = a[u];
      if (
        !(
          n[_e] ||
          n[Jl] ||
          (l === "link" && n.getAttribute("rel") === "stylesheet")
        ) &&
        n.namespaceURI !== "http://www.w3.org/2000/svg"
      ) {
        var i = n.getAttribute(t) || "";
        i = l + i;
        var c = e.get(i);
        c ? c.push(n) : e.set(i, [n]);
      }
    }
    return e;
  }
  function Vr(l, t, a) {
    (l = l.ownerDocument || l),
      l.head.insertBefore(
        a,
        t === "title" ? l.querySelector("head > title") : null,
      );
  }
  function Bv(l, t, a) {
    if (a === 1 || t.itemProp != null) return !1;
    switch (l) {
      case "meta":
      case "title":
        return !0;
      case "style":
        if (
          typeof t.precedence != "string" ||
          typeof t.href != "string" ||
          t.href === ""
        )
          break;
        return !0;
      case "link":
        if (
          typeof t.rel != "string" ||
          typeof t.href != "string" ||
          t.href === "" ||
          t.onLoad ||
          t.onError
        )
          break;
        switch (t.rel) {
          case "stylesheet":
            return (
              (l = t.disabled), typeof t.precedence == "string" && l == null
            );
          default:
            return !0;
        }
      case "script":
        if (
          t.async &&
          typeof t.async != "function" &&
          typeof t.async != "symbol" &&
          !t.onLoad &&
          !t.onError &&
          t.src &&
          typeof t.src == "string"
        )
          return !0;
    }
    return !1;
  }
  function Kr(l) {
    return !(l.type === "stylesheet" && (l.state.loading & 3) === 0);
  }
  var vu = null;
  function Yv() {}
  function jv(l, t, a) {
    if (vu === null) throw Error(s(475));
    var e = vu;
    if (
      t.type === "stylesheet" &&
      (typeof a.media != "string" || matchMedia(a.media).matches !== !1) &&
      (t.state.loading & 4) === 0
    ) {
      if (t.instance === null) {
        var u = Se(a.href),
          n = l.querySelector(ru(u));
        if (n) {
          (l = n._p),
            l !== null &&
              typeof l == "object" &&
              typeof l.then == "function" &&
              (e.count++, (e = Dn.bind(e)), l.then(e, e)),
            (t.state.loading |= 4),
            (t.instance = n),
            Gl(n);
          return;
        }
        (n = l.ownerDocument || l),
          (a = Qr(a)),
          (u = zt.get(u)) && Vc(a, u),
          (n = n.createElement("link")),
          Gl(n);
        var i = n;
        (i._p = new Promise(function (c, f) {
          (i.onload = c), (i.onerror = f);
        })),
          Kl(n, "link", a),
          (t.instance = n);
      }
      e.stylesheets === null && (e.stylesheets = new Map()),
        e.stylesheets.set(t, l),
        (l = t.state.preload) &&
          (t.state.loading & 3) === 0 &&
          (e.count++,
          (t = Dn.bind(e)),
          l.addEventListener("load", t),
          l.addEventListener("error", t));
    }
  }
  function Cv() {
    if (vu === null) throw Error(s(475));
    var l = vu;
    return (
      l.stylesheets && l.count === 0 && Jc(l, l.stylesheets),
      0 < l.count
        ? function (t) {
            var a = setTimeout(function () {
              if ((l.stylesheets && Jc(l, l.stylesheets), l.unsuspend)) {
                var e = l.unsuspend;
                (l.unsuspend = null), e();
              }
            }, 6e4);
            return (
              (l.unsuspend = t),
              function () {
                (l.unsuspend = null), clearTimeout(a);
              }
            );
          }
        : null
    );
  }
  function Dn() {
    if ((this.count--, this.count === 0)) {
      if (this.stylesheets) Jc(this, this.stylesheets);
      else if (this.unsuspend) {
        var l = this.unsuspend;
        (this.unsuspend = null), l();
      }
    }
  }
  var Rn = null;
  function Jc(l, t) {
    (l.stylesheets = null),
      l.unsuspend !== null &&
        (l.count++,
        (Rn = new Map()),
        t.forEach(Gv, l),
        (Rn = null),
        Dn.call(l));
  }
  function Gv(l, t) {
    if (!(t.state.loading & 4)) {
      var a = Rn.get(l);
      if (a) var e = a.get(null);
      else {
        (a = new Map()), Rn.set(l, a);
        for (
          var u = l.querySelectorAll(
              "link[data-precedence],style[data-precedence]",
            ),
            n = 0;
          n < u.length;
          n++
        ) {
          var i = u[n];
          (i.nodeName === "LINK" || i.getAttribute("media") !== "not all") &&
            (a.set(i.dataset.precedence, i), (e = i));
        }
        e && a.set(null, e);
      }
      (u = t.instance),
        (i = u.getAttribute("data-precedence")),
        (n = a.get(i) || e),
        n === e && a.set(null, u),
        a.set(i, u),
        this.count++,
        (e = Dn.bind(this)),
        u.addEventListener("load", e),
        u.addEventListener("error", e),
        n
          ? n.parentNode.insertBefore(u, n.nextSibling)
          : ((l = l.nodeType === 9 ? l.head : l),
            l.insertBefore(u, l.firstChild)),
        (t.state.loading |= 4);
    }
  }
  var yu = {
    $$typeof: vl,
    Provider: null,
    Consumer: null,
    _currentValue: Y,
    _currentValue2: Y,
    _threadCount: 0,
  };
  function Xv(l, t, a, e, u, n, i, c) {
    (this.tag = 1),
      (this.containerInfo = l),
      (this.pingCache = this.current = this.pendingChildren = null),
      (this.timeoutHandle = -1),
      (this.callbackNode =
        this.next =
        this.pendingContext =
        this.context =
        this.cancelPendingCommit =
          null),
      (this.callbackPriority = 0),
      (this.expirationTimes = Zn(-1)),
      (this.entangledLanes =
        this.shellSuspendCounter =
        this.errorRecoveryDisabledLanes =
        this.expiredLanes =
        this.warmLanes =
        this.pingedLanes =
        this.suspendedLanes =
        this.pendingLanes =
          0),
      (this.entanglements = Zn(0)),
      (this.hiddenUpdates = Zn(null)),
      (this.identifierPrefix = e),
      (this.onUncaughtError = u),
      (this.onCaughtError = n),
      (this.onRecoverableError = i),
      (this.pooledCache = null),
      (this.pooledCacheLanes = 0),
      (this.formState = c),
      (this.incompleteTransitions = new Map());
  }
  function Jr(l, t, a, e, u, n, i, c, f, y, T, E) {
    return (
      (l = new Xv(l, t, a, i, c, f, y, E)),
      (t = 1),
      n === !0 && (t |= 24),
      (n = ct(3, null, null, t)),
      (l.current = n),
      (n.stateNode = l),
      (t = Mi()),
      t.refCount++,
      (l.pooledCache = t),
      t.refCount++,
      (n.memoizedState = { element: e, isDehydrated: a, cache: t }),
      Ni(n),
      l
    );
  }
  function wr(l) {
    return l ? ((l = Wa), l) : Wa;
  }
  function kr(l, t, a, e, u, n) {
    (u = wr(u)),
      e.context === null ? (e.context = u) : (e.pendingContext = u),
      (e = Ft(t)),
      (e.payload = { element: a }),
      (n = n === void 0 ? null : n),
      n !== null && (e.callback = n),
      (a = It(l, e, t)),
      a !== null && (dt(a, l, t), Ve(a, l, t));
  }
  function $r(l, t) {
    if (((l = l.memoizedState), l !== null && l.dehydrated !== null)) {
      var a = l.retryLane;
      l.retryLane = a !== 0 && a < t ? a : t;
    }
  }
  function wc(l, t) {
    $r(l, t), (l = l.alternate) && $r(l, t);
  }
  function Wr(l) {
    if (l.tag === 13) {
      var t = $a(l, 67108864);
      t !== null && dt(t, l, 67108864), wc(l, 67108864);
    }
  }
  var Un = !0;
  function Qv(l, t, a, e) {
    var u = S.T;
    S.T = null;
    var n = M.p;
    try {
      (M.p = 2), kc(l, t, a, e);
    } finally {
      (M.p = n), (S.T = u);
    }
  }
  function Zv(l, t, a, e) {
    var u = S.T;
    S.T = null;
    var n = M.p;
    try {
      (M.p = 8), kc(l, t, a, e);
    } finally {
      (M.p = n), (S.T = u);
    }
  }
  function kc(l, t, a, e) {
    if (Un) {
      var u = $c(e);
      if (u === null) Bc(l, t, e, Nn, a), Ir(l, e);
      else if (Vv(u, l, t, a, e)) e.stopPropagation();
      else if ((Ir(l, e), t & 4 && -1 < Lv.indexOf(l))) {
        for (; u !== null; ) {
          var n = ja(u);
          if (n !== null)
            switch (n.tag) {
              case 3:
                if (((n = n.stateNode), n.current.memoizedState.isDehydrated)) {
                  var i = ma(n.pendingLanes);
                  if (i !== 0) {
                    var c = n;
                    for (c.pendingLanes |= 2, c.entangledLanes |= 2; i; ) {
                      var f = 1 << (31 - nt(i));
                      (c.entanglements[1] |= f), (i &= ~f);
                    }
                    Ut(n), (ol & 6) === 0 && ((yn = _t() + 500), cu(0));
                  }
                }
                break;
              case 13:
                (c = $a(n, 2)), c !== null && dt(c, n, 2), mn(), wc(n, 2);
            }
          if (((n = $c(e)), n === null && Bc(l, t, e, Nn, a), n === u)) break;
          u = n;
        }
        u !== null && e.stopPropagation();
      } else Bc(l, t, e, null, a);
    }
  }
  function $c(l) {
    return (l = li(l)), Wc(l);
  }
  var Nn = null;
  function Wc(l) {
    if (((Nn = null), (l = Ya(l)), l !== null)) {
      var t = q(l);
      if (t === null) l = null;
      else {
        var a = t.tag;
        if (a === 13) {
          if (((l = V(t)), l !== null)) return l;
          l = null;
        } else if (a === 3) {
          if (t.stateNode.current.memoizedState.isDehydrated)
            return t.tag === 3 ? t.stateNode.containerInfo : null;
          l = null;
        } else t !== l && (l = null);
      }
    }
    return (Nn = l), null;
  }
  function Fr(l) {
    switch (l) {
      case "beforetoggle":
      case "cancel":
      case "click":
      case "close":
      case "contextmenu":
      case "copy":
      case "cut":
      case "auxclick":
      case "dblclick":
      case "dragend":
      case "dragstart":
      case "drop":
      case "focusin":
      case "focusout":
      case "input":
      case "invalid":
      case "keydown":
      case "keypress":
      case "keyup":
      case "mousedown":
      case "mouseup":
      case "paste":
      case "pause":
      case "play":
      case "pointercancel":
      case "pointerdown":
      case "pointerup":
      case "ratechange":
      case "reset":
      case "resize":
      case "seeked":
      case "submit":
      case "toggle":
      case "touchcancel":
      case "touchend":
      case "touchstart":
      case "volumechange":
      case "change":
      case "selectionchange":
      case "textInput":
      case "compositionstart":
      case "compositionend":
      case "compositionupdate":
      case "beforeblur":
      case "afterblur":
      case "beforeinput":
      case "blur":
      case "fullscreenchange":
      case "focus":
      case "hashchange":
      case "popstate":
      case "select":
      case "selectstart":
        return 2;
      case "drag":
      case "dragenter":
      case "dragexit":
      case "dragleave":
      case "dragover":
      case "mousemove":
      case "mouseout":
      case "mouseover":
      case "pointermove":
      case "pointerout":
      case "pointerover":
      case "scroll":
      case "touchmove":
      case "wheel":
      case "mouseenter":
      case "mouseleave":
      case "pointerenter":
      case "pointerleave":
        return 8;
      case "message":
        switch (Dd()) {
          case of:
            return 2;
          case rf:
            return 8;
          case Eu:
          case Rd:
            return 32;
          case df:
            return 268435456;
          default:
            return 32;
        }
      default:
        return 32;
    }
  }
  var Fc = !1,
    ra = null,
    da = null,
    va = null,
    hu = new Map(),
    mu = new Map(),
    ya = [],
    Lv =
      "mousedown mouseup touchcancel touchend touchstart auxclick dblclick pointercancel pointerdown pointerup dragend dragstart drop compositionend compositionstart keydown keypress keyup input textInput copy cut paste click change contextmenu reset".split(
        " ",
      );
  function Ir(l, t) {
    switch (l) {
      case "focusin":
      case "focusout":
        ra = null;
        break;
      case "dragenter":
      case "dragleave":
        da = null;
        break;
      case "mouseover":
      case "mouseout":
        va = null;
        break;
      case "pointerover":
      case "pointerout":
        hu.delete(t.pointerId);
        break;
      case "gotpointercapture":
      case "lostpointercapture":
        mu.delete(t.pointerId);
    }
  }
  function gu(l, t, a, e, u, n) {
    return l === null || l.nativeEvent !== n
      ? ((l = {
          blockedOn: t,
          domEventName: a,
          eventSystemFlags: e,
          nativeEvent: n,
          targetContainers: [u],
        }),
        t !== null && ((t = ja(t)), t !== null && Wr(t)),
        l)
      : ((l.eventSystemFlags |= e),
        (t = l.targetContainers),
        u !== null && t.indexOf(u) === -1 && t.push(u),
        l);
  }
  function Vv(l, t, a, e, u) {
    switch (t) {
      case "focusin":
        return (ra = gu(ra, l, t, a, e, u)), !0;
      case "dragenter":
        return (da = gu(da, l, t, a, e, u)), !0;
      case "mouseover":
        return (va = gu(va, l, t, a, e, u)), !0;
      case "pointerover":
        var n = u.pointerId;
        return hu.set(n, gu(hu.get(n) || null, l, t, a, e, u)), !0;
      case "gotpointercapture":
        return (
          (n = u.pointerId), mu.set(n, gu(mu.get(n) || null, l, t, a, e, u)), !0
        );
    }
    return !1;
  }
  function Pr(l) {
    var t = Ya(l.target);
    if (t !== null) {
      var a = q(t);
      if (a !== null) {
        if (((t = a.tag), t === 13)) {
          if (((t = V(a)), t !== null)) {
            (l.blockedOn = t),
              jd(l.priority, function () {
                if (a.tag === 13) {
                  var e = rt();
                  e = Ln(e);
                  var u = $a(a, e);
                  u !== null && dt(u, a, e), wc(a, e);
                }
              });
            return;
          }
        } else if (t === 3 && a.stateNode.current.memoizedState.isDehydrated) {
          l.blockedOn = a.tag === 3 ? a.stateNode.containerInfo : null;
          return;
        }
      }
    }
    l.blockedOn = null;
  }
  function xn(l) {
    if (l.blockedOn !== null) return !1;
    for (var t = l.targetContainers; 0 < t.length; ) {
      var a = $c(l.nativeEvent);
      if (a === null) {
        a = l.nativeEvent;
        var e = new a.constructor(a.type, a);
        (Pn = e), a.target.dispatchEvent(e), (Pn = null);
      } else return (t = ja(a)), t !== null && Wr(t), (l.blockedOn = a), !1;
      t.shift();
    }
    return !0;
  }
  function ld(l, t, a) {
    xn(l) && a.delete(t);
  }
  function Kv() {
    (Fc = !1),
      ra !== null && xn(ra) && (ra = null),
      da !== null && xn(da) && (da = null),
      va !== null && xn(va) && (va = null),
      hu.forEach(ld),
      mu.forEach(ld);
  }
  function Hn(l, t) {
    l.blockedOn === t &&
      ((l.blockedOn = null),
      Fc ||
        ((Fc = !0),
        h.unstable_scheduleCallback(h.unstable_NormalPriority, Kv)));
  }
  var qn = null;
  function td(l) {
    qn !== l &&
      ((qn = l),
      h.unstable_scheduleCallback(h.unstable_NormalPriority, function () {
        qn === l && (qn = null);
        for (var t = 0; t < l.length; t += 3) {
          var a = l[t],
            e = l[t + 1],
            u = l[t + 2];
          if (typeof e != "function") {
            if (Wc(e || a) === null) continue;
            break;
          }
          var n = ja(a);
          n !== null &&
            (l.splice(t, 3),
            (t -= 3),
            Wi(n, { pending: !0, data: u, method: a.method, action: e }, e, u));
        }
      }));
  }
  function Su(l) {
    function t(f) {
      return Hn(f, l);
    }
    ra !== null && Hn(ra, l),
      da !== null && Hn(da, l),
      va !== null && Hn(va, l),
      hu.forEach(t),
      mu.forEach(t);
    for (var a = 0; a < ya.length; a++) {
      var e = ya[a];
      e.blockedOn === l && (e.blockedOn = null);
    }
    for (; 0 < ya.length && ((a = ya[0]), a.blockedOn === null); )
      Pr(a), a.blockedOn === null && ya.shift();
    if (((a = (l.ownerDocument || l).$$reactFormReplay), a != null))
      for (e = 0; e < a.length; e += 3) {
        var u = a[e],
          n = a[e + 1],
          i = u[Il] || null;
        if (typeof n == "function") i || td(a);
        else if (i) {
          var c = null;
          if (n && n.hasAttribute("formAction")) {
            if (((u = n), (i = n[Il] || null))) c = i.formAction;
            else if (Wc(u) !== null) continue;
          } else c = i.action;
          typeof c == "function" ? (a[e + 1] = c) : (a.splice(e, 3), (e -= 3)),
            td(a);
        }
      }
  }
  function Ic(l) {
    this._internalRoot = l;
  }
  (Bn.prototype.render = Ic.prototype.render =
    function (l) {
      var t = this._internalRoot;
      if (t === null) throw Error(s(409));
      var a = t.current,
        e = rt();
      kr(a, e, l, t, null, null);
    }),
    (Bn.prototype.unmount = Ic.prototype.unmount =
      function () {
        var l = this._internalRoot;
        if (l !== null) {
          this._internalRoot = null;
          var t = l.containerInfo;
          kr(l.current, 2, null, l, null, null), mn(), (t[Ba] = null);
        }
      });
  function Bn(l) {
    this._internalRoot = l;
  }
  Bn.prototype.unstable_scheduleHydration = function (l) {
    if (l) {
      var t = gf();
      l = { blockedOn: null, target: l, priority: t };
      for (var a = 0; a < ya.length && t !== 0 && t < ya[a].priority; a++);
      ya.splice(a, 0, l), a === 0 && Pr(l);
    }
  };
  var ad = p.version;
  if (ad !== "19.1.1") throw Error(s(527, ad, "19.1.1"));
  M.findDOMNode = function (l) {
    var t = l._reactInternals;
    if (t === void 0)
      throw typeof l.render == "function"
        ? Error(s(188))
        : ((l = Object.keys(l).join(",")), Error(s(268, l)));
    return (
      (l = O(t)),
      (l = l !== null ? b(l) : null),
      (l = l === null ? null : l.stateNode),
      l
    );
  };
  var Jv = {
    bundleType: 0,
    version: "19.1.1",
    rendererPackageName: "react-dom",
    currentDispatcherRef: S,
    reconcilerVersion: "19.1.1",
  };
  if (typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ < "u") {
    var Yn = __REACT_DEVTOOLS_GLOBAL_HOOK__;
    if (!Yn.isDisabled && Yn.supportsFiber)
      try {
        (Ee = Yn.inject(Jv)), (ut = Yn);
      } catch {}
  }
  return (
    (Tu.createRoot = function (l, t) {
      if (!N(l)) throw Error(s(299));
      var a = !1,
        e = "",
        u = So,
        n = bo,
        i = To,
        c = null;
      return (
        t != null &&
          (t.unstable_strictMode === !0 && (a = !0),
          t.identifierPrefix !== void 0 && (e = t.identifierPrefix),
          t.onUncaughtError !== void 0 && (u = t.onUncaughtError),
          t.onCaughtError !== void 0 && (n = t.onCaughtError),
          t.onRecoverableError !== void 0 && (i = t.onRecoverableError),
          t.unstable_transitionCallbacks !== void 0 &&
            (c = t.unstable_transitionCallbacks)),
        (t = Jr(l, 1, !1, null, null, a, e, u, n, i, c, null)),
        (l[Ba] = t.current),
        qc(l),
        new Ic(t)
      );
    }),
    (Tu.hydrateRoot = function (l, t, a) {
      if (!N(l)) throw Error(s(299));
      var e = !1,
        u = "",
        n = So,
        i = bo,
        c = To,
        f = null,
        y = null;
      return (
        a != null &&
          (a.unstable_strictMode === !0 && (e = !0),
          a.identifierPrefix !== void 0 && (u = a.identifierPrefix),
          a.onUncaughtError !== void 0 && (n = a.onUncaughtError),
          a.onCaughtError !== void 0 && (i = a.onCaughtError),
          a.onRecoverableError !== void 0 && (c = a.onRecoverableError),
          a.unstable_transitionCallbacks !== void 0 &&
            (f = a.unstable_transitionCallbacks),
          a.formState !== void 0 && (y = a.formState)),
        (t = Jr(l, 1, !0, t, a ?? null, e, u, n, i, c, f, y)),
        (t.context = wr(null)),
        (a = t.current),
        (e = rt()),
        (e = Ln(e)),
        (u = Ft(e)),
        (u.callback = null),
        It(a, u, e),
        (a = e),
        (t.current.lanes = a),
        pe(t, a),
        Ut(t),
        (l[Ba] = t.current),
        qc(l),
        new Bn(t)
      );
    }),
    (Tu.version = "19.1.1"),
    Tu
  );
}
var dd;
function uy() {
  if (dd) return tf.exports;
  dd = 1;
  function h() {
    if (
      !(
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ > "u" ||
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE != "function"
      )
    )
      try {
        __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE(h);
      } catch (p) {
        console.error(p);
      }
  }
  return h(), (tf.exports = ey()), tf.exports;
}
var ny = uy();
const iy = [
    {
      id: "nova-slice-lab",
      name: "Nova Slice Lab",
      coords: [-122.4098, 37.8001],
      description: "Award‑winning Neapolitan pies in North Beach.",
      city: "North Beach",
      rating: 4.8,
      price: "$$$",
      thumbnail: "https://persistent.oaistatic.com/pizzaz/pizzaz-1.png",
    },
    {
      id: "midnight-marinara",
      name: "Midnight Marinara",
      coords: [-122.4093, 37.799],
      description: "Focaccia‑style squares, late‑night favorite.",
      city: "North Beach",
      rating: 4.6,
      price: "$",
      thumbnail: "https://persistent.oaistatic.com/pizzaz/pizzaz-2.png",
    },
    {
      id: "cinder-oven-co",
      name: "Cinder Oven Co.",
      coords: [-122.4255, 37.7613],
      description: "Thin‑crust classics on 18th Street.",
      city: "Mission",
      rating: 4.5,
      price: "$$",
      thumbnail: "https://persistent.oaistatic.com/pizzaz/pizzaz-3.png",
    },
    {
      id: "neon-crust-works",
      name: "Neon Crust Works",
      coords: [-122.4388, 37.7775],
      description: "Deep‑dish and cornmeal crust favorites.",
      city: "Alamo Square",
      rating: 4.5,
      price: "$$",
      thumbnail: "https://persistent.oaistatic.com/pizzaz/pizzaz-6.png",
    },
    {
      id: "luna-pie-collective",
      name: "Luna Pie Collective",
      coords: [-122.4077, 37.799],
      description: "Wood‑fired pies and burrata in North Beach.",
      city: "North Beach",
      rating: 4.6,
      price: "$$",
      thumbnail: "https://persistent.oaistatic.com/pizzaz/pizzaz-4.png",
    },
    {
      id: "bricklight-deep-dish",
      name: "Bricklight Deep Dish",
      coords: [-122.4097, 37.7992],
      description: "Chicago‑style pies from Tony Gemignani.",
      city: "North Beach",
      rating: 4.4,
      price: "$$$",
      thumbnail: "https://persistent.oaistatic.com/pizzaz/pizzaz-5.png",
    },
    {
      id: "garden-ember-pies",
      name: "Garden Ember Pies",
      coords: [-122.438, 37.7722],
      description: "Neighborhood spot with seasonal toppings.",
      city: "Lower Haight",
      rating: 4.4,
      price: "$$",
      thumbnail: "https://persistent.oaistatic.com/pizzaz/pizzaz-1.png",
    },
    {
      id: "atlas-fire-pizzeria",
      name: "Atlas Fire Pizzeria",
      coords: [-122.4123, 37.7899],
      description: "Sourdough, wood‑fired pies near Nob Hill.",
      city: "Nob Hill",
      rating: 4.6,
      price: "$$$",
      thumbnail: "https://persistent.oaistatic.com/pizzaz/pizzaz-2.png",
    },
    {
      id: "circuit-slice-garage",
      name: "Circuit Slice Garage",
      coords: [-122.4135, 37.7805],
      description: "Crispy‑edged Detroit‑style in SoMa.",
      city: "SoMa",
      rating: 4.5,
      price: "$$",
      thumbnail: "https://persistent.oaistatic.com/pizzaz/pizzaz-3.png",
    },
    {
      id: "velvet-mozza-lounge",
      name: "Velvet Mozza Lounge",
      coords: [-122.4019, 37.7818],
      description: "Bianca pies and cocktails near Yerba Buena.",
      city: "Yerba Buena",
      rating: 4.3,
      price: "$$",
      thumbnail: "https://persistent.oaistatic.com/pizzaz/pizzaz-6.png",
    },
  ],
  cy = { places: iy }; /**
 * @license lucide-react v0.536.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const fy = (h) => h.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(),
  sy = (h) =>
    h.replace(/^([A-Z])|[\s-_]+(\w)/g, (p, _, s) =>
      s ? s.toUpperCase() : _.toLowerCase(),
    ),
  vd = (h) => {
    const p = sy(h);
    return p.charAt(0).toUpperCase() + p.slice(1);
  },
  Sd = (...h) =>
    h
      .filter((p, _, s) => !!p && p.trim() !== "" && s.indexOf(p) === _)
      .join(" ")
      .trim(),
  oy = (h) => {
    for (const p in h)
      if (p.startsWith("aria-") || p === "role" || p === "title") return !0;
  }; /**
 * @license lucide-react v0.536.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
var ry = {
  xmlns: "http://www.w3.org/2000/svg",
  width: 24,
  height: 24,
  viewBox: "0 0 24 24",
  fill: "none",
  stroke: "currentColor",
  strokeWidth: 2,
  strokeLinecap: "round",
  strokeLinejoin: "round",
}; /**
 * @license lucide-react v0.536.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const dy = I.forwardRef(
  (
    {
      color: h = "currentColor",
      size: p = 24,
      strokeWidth: _ = 2,
      absoluteStrokeWidth: s,
      className: N = "",
      children: q,
      iconNode: V,
      ...el
    },
    O,
  ) =>
    I.createElement(
      "svg",
      {
        ref: O,
        ...ry,
        width: p,
        height: p,
        stroke: h,
        strokeWidth: s ? (Number(_) * 24) / Number(p) : _,
        className: Sd("lucide", N),
        ...(!q && !oy(el) && { "aria-hidden": "true" }),
        ...el,
      },
      [
        ...V.map(([b, x]) => I.createElement(b, x)),
        ...(Array.isArray(q) ? q : [q]),
      ],
    ),
); /**
 * @license lucide-react v0.536.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const bd = (h, p) => {
  const _ = I.forwardRef(({ className: s, ...N }, q) =>
    I.createElement(dy, {
      ref: q,
      iconNode: p,
      className: Sd(`lucide-${fy(vd(h))}`, `lucide-${h}`, s),
      ...N,
    }),
  );
  return (_.displayName = vd(h)), _;
}; /**
 * @license lucide-react v0.536.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const vy = [
    ["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }],
    ["path", { d: "M8 12h8", key: "1wcyev" }],
    ["path", { d: "M12 8v8", key: "napkw2" }],
  ],
  yy = bd("circle-plus", vy); /**
 * @license lucide-react v0.536.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const hy = [
    [
      "path",
      {
        d: "M11.525 2.295a.53.53 0 0 1 .95 0l2.31 4.679a2.123 2.123 0 0 0 1.595 1.16l5.166.756a.53.53 0 0 1 .294.904l-3.736 3.638a2.123 2.123 0 0 0-.611 1.878l.882 5.14a.53.53 0 0 1-.771.56l-4.618-2.428a2.122 2.122 0 0 0-1.973 0L6.396 21.01a.53.53 0 0 1-.77-.56l.881-5.139a2.122 2.122 0 0 0-.611-1.879L2.16 9.795a.53.53 0 0 1 .294-.906l5.165-.755a2.122 2.122 0 0 0 1.597-1.16z",
        key: "r04s7s",
      },
    ],
  ],
  my = bd("star", hy);
function Td(h) {
  var p,
    _,
    s = "";
  if (typeof h == "string" || typeof h == "number") s += h;
  else if (typeof h == "object")
    if (Array.isArray(h)) {
      var N = h.length;
      for (p = 0; p < N; p++)
        h[p] && (_ = Td(h[p])) && (s && (s += " "), (s += _));
    } else for (_ in h) h[_] && (s && (s += " "), (s += _));
  return s;
}
function Cn() {
  for (var h, p, _ = 0, s = "", N = arguments.length; _ < N; _++)
    (h = arguments[_]) && (p = Td(h)) && (s && (s += " "), (s += p));
  return s;
}
const gy = { DEV: !1, MODE: "production" },
  zd = typeof import.meta < "u" ? gy : void 0,
  Sy = !!zd?.DEV,
  by =
    (typeof navigator < "u" &&
      /(jsdom|happy-dom)/i.test(navigator.userAgent)) ||
    typeof globalThis.happyDOM == "object",
  Ed = zd?.MODE === "test" || by,
  Ty = typeof window < "u",
  Ad = typeof document < "u",
  zy = Ty && Ad,
  Ey = (h) => {
    const p = h.currentTarget;
    if (!(p instanceof HTMLElement)) return;
    const _ = p.offsetWidth;
    let s = 0.985;
    _ <= 80
      ? (s = 0.96)
      : _ <= 150
        ? (s = 0.97)
        : _ <= 220
          ? (s = 0.98)
          : _ > 600 && (s = 0.995),
      p.style.setProperty("--scale", s.toString());
  },
  yd = (h, p) => {
    const _ = () => {
      const V = setTimeout(h);
      return () => {
        clearTimeout(V);
      };
    };
    if (
      !zy ||
      typeof window.requestAnimationFrame != "function" ||
      (Ad && document.visibilityState === "hidden")
    )
      return _();
    let N = 2,
      q = window.requestAnimationFrame(function V() {
        (N -= 1), N === 0 ? h() : (q = window.requestAnimationFrame(V));
      });
    return () => {
      typeof window.cancelAnimationFrame == "function" &&
        window.cancelAnimationFrame(q);
    };
  },
  Ay = (h) =>
    Object.keys(h).reduce((_, s) => {
      const N = h[s];
      if (N || N === 0) {
        const q = s.startsWith("--") ? "" : "--",
          V = typeof N == "number" ? `${N}px` : N;
        _[`${q}${s}`] = V;
      }
      return _;
    }, {}),
  py = (h) => {
    const p = I.Children.toArray(h),
      _ = [];
    let s = "";
    const N = () => {
      s !== "" && (_.push(s), (s = ""));
    };
    for (const q of p)
      if (!(q == null || typeof q == "boolean")) {
        if (typeof q == "string" || typeof q == "number") {
          s += String(q);
          continue;
        }
        N(), _.push(q);
      }
    return N(), _;
  },
  pd = (h) => {
    const p = py(h),
      _ = I.Children.count(p);
    return I.Children.map(p, (s) => {
      if (typeof s == "string" && s.trim())
        return _ <= 1 ? s : L.jsx("span", { children: s });
      if (I.isValidElement(s)) {
        const N = s,
          { children: q, ...V } = N.props;
        return q != null ? I.cloneElement(N, V, pd(q)) : N;
      }
      return s;
    });
  };
I.createContext(null);
var nf, hd;
function _y() {
  if (hd) return nf;
  hd = 1;
  var h = "Expected a function",
    p = NaN,
    _ = "[object Symbol]",
    s = /^\s+|\s+$/g,
    N = /^[-+]0x[0-9a-f]+$/i,
    q = /^0b[01]+$/i,
    V = /^0o[0-7]+$/i,
    el = parseInt,
    O = typeof jn == "object" && jn && jn.Object === Object && jn,
    b = typeof self == "object" && self && self.Object === Object && self,
    x = O || b || Function("return this")(),
    $ = Object.prototype,
    J = $.toString,
    bl = Math.max,
    Tl = Math.min,
    zl = function () {
      return x.Date.now();
    };
  function dl(R, X, ul) {
    var cl,
      El,
      Ul,
      Hl,
      fl,
      $l,
      et = 0,
      Yl = !1,
      S = !1,
      M = !0;
    if (typeof R != "function") throw new TypeError(h);
    (X = pl(X) || 0),
      Ol(ul) &&
        ((Yl = !!ul.leading),
        (S = "maxWait" in ul),
        (Ul = S ? bl(pl(ul.maxWait) || 0, X) : Ul),
        (M = "trailing" in ul ? !!ul.trailing : M));
    function Y(tl) {
      var al = cl,
        Fl = El;
      return (cl = El = void 0), (et = tl), (Hl = R.apply(Fl, al)), Hl;
    }
    function sl(tl) {
      return (et = tl), (fl = setTimeout(U, X)), Yl ? Y(tl) : Hl;
    }
    function o(tl) {
      var al = tl - $l,
        Fl = tl - et,
        qa = X - al;
      return S ? Tl(qa, Ul - Fl) : qa;
    }
    function A(tl) {
      var al = tl - $l,
        Fl = tl - et;
      return $l === void 0 || al >= X || al < 0 || (S && Fl >= Ul);
    }
    function U() {
      var tl = zl();
      if (A(tl)) return D(tl);
      fl = setTimeout(U, o(tl));
    }
    function D(tl) {
      return (fl = void 0), M && cl ? Y(tl) : ((cl = El = void 0), Hl);
    }
    function j() {
      fl !== void 0 && clearTimeout(fl), (et = 0), (cl = $l = El = fl = void 0);
    }
    function P() {
      return fl === void 0 ? Hl : D(zl());
    }
    function Q() {
      var tl = zl(),
        al = A(tl);
      if (((cl = arguments), (El = this), ($l = tl), al)) {
        if (fl === void 0) return sl($l);
        if (S) return (fl = setTimeout(U, X)), Y($l);
      }
      return fl === void 0 && (fl = setTimeout(U, X)), Hl;
    }
    return (Q.cancel = j), (Q.flush = P), Q;
  }
  function Ol(R) {
    var X = typeof R;
    return !!R && (X == "object" || X == "function");
  }
  function Ml(R) {
    return !!R && typeof R == "object";
  }
  function vl(R) {
    return typeof R == "symbol" || (Ml(R) && J.call(R) == _);
  }
  function pl(R) {
    if (typeof R == "number") return R;
    if (vl(R)) return p;
    if (Ol(R)) {
      var X = typeof R.valueOf == "function" ? R.valueOf() : R;
      R = Ol(X) ? X + "" : X;
    }
    if (typeof R != "string") return R === 0 ? R : +R;
    R = R.replace(s, "");
    var ul = q.test(R);
    return ul || V.test(R) ? el(R.slice(2), ul ? 2 : 8) : N.test(R) ? p : +R;
  }
  return (nf = dl), nf;
}
_y();
var Oy = typeof window < "u" ? I.useLayoutEffect : I.useEffect;
function My(h, p) {
  const _ = I.useRef(h);
  Oy(() => {
    _.current = h;
  }, [h]),
    I.useEffect(() => {
      if (!p && p !== 0) return;
      const s = setTimeout(() => {
        _.current();
      }, p);
      return () => {
        clearTimeout(s);
      };
    }, [p]);
}
const Dy = "_LoadingIndicator_7yl6f_1",
  Ry = { LoadingIndicator: Dy },
  Uy = ({ className: h, size: p, strokeWidth: _, style: s, ...N }) =>
    L.jsx("div", {
      ...N,
      className: Cn(Ry.LoadingIndicator, h),
      style: s || Ay({ "indicator-size": p, "indicator-stroke": _ }),
    });
function Ny(h) {
  return (p) => {
    h.forEach((_) => {
      typeof _ == "function" ? _(p) : _ != null && (_.current = p);
    });
  };
}
const xy = () => Ed,
  md = (h, p = !1, _ = "TransitionGroup") => {
    const s = [];
    return (
      I.Children.forEach(h, (N) => {
        if (N && typeof N == "object" && "key" in N && N.key) s.push(N);
        else if (p)
          throw new Error(`Child elements of <${_} /> must include a \`key\``);
      }),
      s
    );
  },
  Te = () => {},
  ze = (h) => {
    const p = I.useRef(h);
    return (p.current = h), I.useCallback((_) => p.current(_), []);
  };
function Hy(h, p, _, s) {
  const N = h.reduce((O, b) => ({ ...O, [b.key]: 1 }), {}),
    q = p.reduce((O, b) => ({ ...O, [b.component.key]: 1 }), {}),
    V = h.filter((O) => !q[O.key]).map(_),
    el = p.map((O) => ({
      ...O,
      component: h.find(({ key: b }) => b === O.component.key) || O.component,
      shouldRender: !!N[O.component.key],
    }));
  return s === "append" ? el.concat(V) : V.concat(el);
}
function qy(h, p, _) {
  if ((Ed || Sy) && p && _ > 1)
    throw new Error(`Cannot use forwardRef with multiple children in <${h} />`);
}
const By = "_TransitionGroupChild_1hv1z_1",
  Yy = { TransitionGroupChild: By },
  _d = {
    enter: !1,
    enterActive: !1,
    exit: !1,
    exitActive: !1,
    interrupted: !1,
  },
  jy = (h) => ({ ..._d, enter: !h }),
  Cy = (h, p) => {
    switch (p.type) {
      case "enter-before":
        return {
          enter: !0,
          enterActive: !1,
          exit: !1,
          exitActive: !1,
          interrupted: h.interrupted || h.exit,
        };
      case "enter-active":
        return {
          enter: !0,
          enterActive: !0,
          exit: !1,
          exitActive: !1,
          interrupted: !1,
        };
      case "exit-before":
        return {
          enter: !1,
          enterActive: !1,
          exit: !0,
          exitActive: !1,
          interrupted: h.interrupted || h.enter,
        };
      case "exit-active":
        return {
          enter: !1,
          enterActive: !1,
          exit: !0,
          exitActive: !0,
          interrupted: !1,
        };
      case "done":
      default:
        return _d;
    }
  },
  Gy = ({
    ref: h,
    as: p,
    children: _,
    className: s,
    transitionId: N,
    style: q,
    preventMountTransition: V,
    shouldRender: el,
    enterDuration: O,
    exitDuration: b,
    removeChild: x,
    onEnter: $,
    onEnterActive: J,
    onEnterComplete: bl,
    onExit: Tl,
    onExitActive: zl,
    onExitComplete: dl,
  }) => {
    const [Ol, Ml] = I.useReducer(Cy, jy(V || !1)),
      vl = I.useRef(!1),
      pl = I.useRef(null),
      R = I.useRef(O);
    R.current = O;
    const X = I.useRef(b);
    X.current = b;
    const ul = I.useRef(null),
      cl = I.useCallback(
        (El) => {
          const Ul = pl.current;
          if (!(!Ul || El === ul.current))
            switch (((ul.current = El), El)) {
              case "enter":
                $(Ul);
                break;
              case "enter-active":
                J(Ul);
                break;
              case "enter-complete":
                bl(Ul);
                break;
              case "exit":
                Tl(Ul);
                break;
              case "exit-active":
                zl(Ul);
                break;
              case "exit-complete":
                dl(Ul);
                break;
            }
        },
        [$, J, bl, Tl, zl, dl],
      );
    return (
      Iv.useLayoutEffect(() => {
        if (!el) {
          let Hl;
          Ml({ type: "exit-before" }), cl("exit");
          const fl = yd(() => {
            Ml({ type: "exit-active" }),
              cl("exit-active"),
              (Hl = window.setTimeout(() => {
                cl("exit-complete"), x();
              }, X.current));
          });
          return () => {
            fl(), Hl !== void 0 && clearTimeout(Hl);
          };
        }
        if (V && !vl.current) {
          vl.current = !0;
          return;
        }
        let El;
        Ml({ type: "enter-before" }), cl("enter");
        const Ul = yd(() => {
          Ml({ type: "enter-active" }),
            cl("enter-active"),
            (El = window.setTimeout(() => {
              Ml({ type: "done" }), cl("enter-complete");
            }, R.current));
        });
        return () => {
          Ul(), El !== void 0 && clearTimeout(El);
        };
      }, [el, V, x, cl]),
      I.useEffect(
        () => () => {
          vl.current = !1;
        },
        [],
      ),
      L.jsx(p, {
        ref: Ny([pl, h]),
        className: Cn(s, Yy.TransitionGroupChild),
        "data-transition-id": N,
        style: q,
        "data-entering": Ol.enter ? "" : void 0,
        "data-entering-active": Ol.enterActive ? "" : void 0,
        "data-exiting": Ol.exit ? "" : void 0,
        "data-exiting-active": Ol.exitActive ? "" : void 0,
        "data-interrupted": Ol.interrupted ? "" : void 0,
        children: _,
      })
    );
  },
  Xy = (h) => {
    const { enterMountDelay: p, preventMountTransition: _ } = h,
      s = !_ && p != null ? p : null,
      [N, q] = I.useState(s == null);
    return My(() => q(!0), N ? null : s), N ? L.jsx(Gy, { ...h }) : null;
  },
  Qy = (h) => {
    const {
        ref: p,
        as: _ = "span",
        children: s,
        className: N,
        transitionId: q,
        style: V,
        enterDuration: el = 0,
        exitDuration: O = 0,
        preventInitialTransition: b = !0,
        enterMountDelay: x,
        insertMethod: $ = "append",
        disableAnimations: J = xy(),
      } = h,
      bl = ze(h.onEnter ?? Te),
      Tl = ze(h.onEnterActive ?? Te),
      zl = ze(h.onEnterComplete ?? Te),
      dl = ze(h.onExit ?? Te),
      Ol = ze(h.onExitActive ?? Te),
      Ml = ze(h.onExitComplete ?? Te);
    I.Children.forEach(s, (X) => {
      if (X && !X.key)
        throw new Error(
          "Child elements of <TransitionGroup /> must include a `key`",
        );
    });
    const vl = I.useCallback(
        (X) => ({
          component: X,
          shouldRender: !0,
          removeChild: () => {
            R((ul) => ul.filter((cl) => X.key !== cl.component.key));
          },
          onEnter: bl,
          onEnterActive: Tl,
          onEnterComplete: zl,
          onExit: dl,
          onExitActive: Ol,
          onExitComplete: Ml,
        }),
        [bl, Tl, zl, dl, Ol, Ml],
      ),
      [pl, R] = I.useState(() =>
        md(s).map((X) => ({ ...vl(X), preventMountTransition: b })),
      );
    return (
      I.useLayoutEffect(() => {
        R((X) => {
          const ul = md(s);
          return Hy(ul, X, vl, $);
        });
      }, [s, $, vl]),
      qy("TransitionGroup", p, I.Children.count(s)),
      J
        ? L.jsx(L.Fragment, {
            children: I.Children.map(s, (X) =>
              L.jsx(_, {
                ref: p,
                className: N,
                style: V,
                "data-transition-id": q,
                children: X,
              }),
            ),
          })
        : L.jsx(L.Fragment, {
            children: pl.map(({ component: X, ...ul }) =>
              L.jsx(
                Xy,
                {
                  ...ul,
                  as: _,
                  className: N,
                  transitionId: q,
                  enterDuration: el,
                  exitDuration: O,
                  enterMountDelay: x,
                  style: V,
                  ref: p,
                  children: X,
                },
                X.key,
              ),
            ),
          })
    );
  },
  Zy = "_Button_1864l_1",
  Ly = "_ButtonInner_1864l_4",
  Vy = "_ButtonLoader_1864l_749",
  cf = { Button: Zy, ButtonInner: Ly, ButtonLoader: Vy },
  ff = (h) => {
    const {
        type: p = "button",
        color: _ = "primary",
        variant: s = "solid",
        pill: N = !0,
        uniform: q = !1,
        size: V = "md",
        iconSize: el,
        gutterSize: O,
        loading: b,
        selected: x,
        block: $,
        opticallyAlign: J,
        children: bl,
        className: Tl,
        onClick: zl,
        disabled: dl,
        disabledTone: Ol,
        inert: Ml = b,
        ...vl
      } = h,
      pl = dl || Ml,
      R = I.useCallback(
        (X) => {
          dl || zl?.(X);
        },
        [zl, dl],
      );
    return L.jsxs("button", {
      type: p,
      className: Cn(cf.Button, Tl),
      "data-color": _,
      "data-variant": s,
      "data-pill": N ? "" : void 0,
      "data-uniform": q ? "" : void 0,
      "data-size": V,
      "data-gutter-size": O,
      "data-icon-size": el,
      "data-loading": b ? "" : void 0,
      "data-selected": x ? "" : void 0,
      "data-block": $ ? "" : void 0,
      "data-optically-align": J,
      onPointerEnter: Ey,
      disabled: pl,
      "aria-disabled": pl,
      tabIndex: pl ? -1 : void 0,
      "data-disabled": dl ? "" : void 0,
      "data-disabled-tone": dl ? Ol : void 0,
      onClick: R,
      ...vl,
      children: [
        L.jsx(Qy, {
          className: cf.ButtonLoader,
          enterDuration: 250,
          exitDuration: 150,
          children: b && L.jsx(Uy, {}, "loader"),
        }),
        L.jsx("span", { className: cf.ButtonInner, children: pd(bl) }),
      ],
    });
  },
  Ky = "_Image_4p440_1",
  Jy = { Image: Ky },
  gd = new Set(),
  wy = ({
    src: h,
    className: p,
    draggable: _ = !1,
    onLoad: s,
    onError: N,
    forceRenderAfterLoadFail: q = !1,
    ...V
  }) => {
    const [el, O] = I.useState(() => h && gd.has(h)),
      [b, x] = I.useState(!1),
      $ = el || (b && q);
    return !h || (!q && b)
      ? null
      : L.jsx("img", {
          ...V,
          src: h,
          className: Cn(Jy.Image, p),
          onLoad: (J) => {
            O(!0), gd.add(h), s?.(J);
          },
          onError: (J) => {
            x(!0), N?.(J);
          },
          "data-loaded": $ ? "" : void 0,
          draggable: _,
        });
  };
function ky() {
  const h = cy?.places || [];
  return L.jsx("div", {
    className:
      "antialiased w-full text-black px-4 pb-2 border border-black/10 rounded-2xl sm:rounded-3xl overflow-hidden bg-white",
    children: L.jsxs("div", {
      className: "max-w-full",
      children: [
        L.jsxs("div", {
          className:
            "flex flex-row items-center gap-4 sm:gap-4 border-b border-black/5 py-4",
          children: [
            L.jsx("div", {
              className:
                "sm:w-18 w-16 aspect-square rounded-xl bg-cover bg-center",
              style: {
                backgroundImage:
                  "url(https://persistent.oaistatic.com/pizzaz/title.png)",
              },
            }),
            L.jsxs("div", {
              children: [
                L.jsx("div", {
                  className: "text-base sm:text-xl font-medium",
                  children: "National Best Pizza List",
                }),
                L.jsx("div", {
                  className: "text-sm text-black/60",
                  children: "A ranking of the best pizzerias in the world",
                }),
              ],
            }),
            L.jsx("div", {
              className: "flex-auto hidden sm:flex justify-end pr-2",
              children: L.jsx(ff, {
                color: "primary",
                variant: "solid",
                size: "md",
                children: "Save List",
              }),
            }),
          ],
        }),
        L.jsxs("div", {
          className: "min-w-full text-sm flex flex-col",
          children: [
            h
              .slice(0, 7)
              .map((p, _) =>
                L.jsx(
                  "div",
                  {
                    className: "px-3 -mx-2 rounded-2xl hover:bg-black/5",
                    children: L.jsxs("div", {
                      style: {
                        borderBottom:
                          _ === 6 ? "none" : "1px solid rgba(0, 0, 0, 0.05)",
                      },
                      className:
                        "flex w-full items-center hover:border-black/0! gap-2",
                      children: [
                        L.jsx("div", {
                          className: "py-3 pr-3 min-w-0 w-full sm:w-3/5",
                          children: L.jsxs("div", {
                            className: "flex items-center gap-3",
                            children: [
                              L.jsx(wy, {
                                src: p.thumbnail,
                                alt: p.name,
                                className:
                                  "h-10 w-10 sm:h-11 sm:w-11 rounded-lg object-cover ring ring-black/5",
                              }),
                              L.jsx("div", {
                                className:
                                  "w-3 text-end sm:block hidden text-sm text-black/40",
                                children: _ + 1,
                              }),
                              L.jsxs("div", {
                                className:
                                  "min-w-0 sm:pl-1 flex flex-col items-start h-full",
                                children: [
                                  L.jsx("div", {
                                    className:
                                      "font-medium text-sm sm:text-md truncate max-w-[40ch]",
                                    children: p.name,
                                  }),
                                  L.jsxs("div", {
                                    className:
                                      "mt-1 sm:mt-0.25 flex items-center gap-3 text-black/70 text-sm",
                                    children: [
                                      L.jsxs("div", {
                                        className: "flex items-center gap-1",
                                        children: [
                                          L.jsx(my, {
                                            strokeWidth: 1.5,
                                            className: "h-3 w-3 text-black",
                                          }),
                                          L.jsx("span", {
                                            children: p.rating?.toFixed
                                              ? p.rating.toFixed(1)
                                              : p.rating,
                                          }),
                                        ],
                                      }),
                                      L.jsx("div", {
                                        className:
                                          "whitespace-nowrap sm:hidden",
                                        children: p.city || "–",
                                      }),
                                    ],
                                  }),
                                ],
                              }),
                            ],
                          }),
                        }),
                        L.jsx("div", {
                          className:
                            "hidden sm:block text-end py-2 px-3 text-sm text-black/60 whitespace-nowrap flex-auto",
                          children: p.city || "–",
                        }),
                        L.jsx("div", {
                          className: "py-2 whitespace-nowrap flex justify-end",
                          children: L.jsx(ff, {
                            "aria-label": `Add ${p.name}`,
                            color: "secondary",
                            variant: "ghost",
                            size: "sm",
                            uniform: !0,
                            children: L.jsx(yy, {
                              strokeWidth: 1.5,
                              className: "h-5 w-5",
                            }),
                          }),
                        }),
                      ],
                    }),
                  },
                  p.id,
                ),
              ),
            h.length === 0 &&
              L.jsx("div", {
                className: "py-6 text-center text-black/60",
                children: "No pizzerias found.",
              }),
          ],
        }),
        L.jsx("div", {
          className: "sm:hidden px-0 pt-2 pb-2",
          children: L.jsx(ff, {
            color: "primary",
            variant: "solid",
            size: "md",
            block: !0,
            children: "Save List",
          }),
        }),
      ],
    }),
  });
}
ny.createRoot(document.getElementById("pizzaz-list-root")).render(
  L.jsx(ky, {}),
);
const $y = void 0;
export { $y as default };
//# sourceMappingURL=pizzaz-list.js.map
