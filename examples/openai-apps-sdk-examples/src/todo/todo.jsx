...
const _MotionGrip = motion.create(GripVertical);
...
function CircleCheckbox({ checked, onToggle, label }) {
  return (
    <div
      ...
      className="flex h-4 w-4 cursor-pointer select-none items-center justify-center rounded-full border border-gray-400 outline-none focus-visible:ring-2 focus-visible:ring-black/20"
      aria-label={label}
    >
      <AnimatePresence initial={false}>
        {checked && (
          <motion.div
            ...
            className="h-[11.5px] w-[11.5px] rounded-full bg-black"
          />
        )}
      </AnimatePresence>
    </div>
...
function BaseCard({ children }) {
  return (
    <MotionCard className="absolute top-0 left-0 h-full w-full overflow-hidden rounded-3xl border border-black/10 bg-gray-50/100 shadow-[0px_8px_14px_rgba(0,0,0,0.05)]">
      {children}
    </MotionCard>
  );
}
  ...
  return (
    <AnimatePresence initial={false}>
      {isOpen && (
        ...
        >
          <div
            className="flex flex-col gap-2 px-8 pt-0 pb-3.5 text-sm"
            onClick={(e) => {
              e.stopPropagation();
              onClickInside?.();
            }}
          >
            <input
              ...
              className="-ml-1 w-full border-0 bg-transparent text-black/55 text-sm placeholder-black/30 outline-none focus:ring-0 focus-visible:ring-0"
            />
          </div>
        </motion.div>
      )}
  ...
  const HiddenAnchorInput = forwardRef(function HiddenAnchorInput(props, ref) {
    return (
      <input
        {...props}
        ref={ref}
        className="pointer-events-none absolute top-full left-0 h-[1px] w-[1px] opacity-0"
        aria-hidden
        tabIndex={-1}
      />
    );
    ...
    return (
      <div className="relative flex items-center justify-center px-2 py-2">
        <button
          ...
          className="absolute left-2 font-medium text-[12px] text-black/70 hover:text-black disabled:pointer-events-none disabled:opacity-30"
          aria-label="Previous month"
        >
          Prev
        </button>
        <div className="font-semibold text-[12px] text-black/75">
          {monthYearLabel(date)}
        </div>
        <button
          type="button"
          ...
          className="absolute right-2 font-medium text-[12px] text-black/70 hover:text-black disabled:pointer-events-none disabled:opacity-30"
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="border-black/5 border-b bg-white"
      >
        {/* Top row */}
        <div
        ...
        >
          ...
          <input
            ...
            className="!text-base flex-auto border-0 border-transparent bg-transparent p-0 leading-tight outline-none focus-visible:ring-0"
            value={item.title}
          />
          ...
          >
            ...
            {dateLabel && (
              <button
                ...
                className="rounded-md text-black/40 text-sm hover:text-black/70"
              >
                {dateLabel}
              </button>
            )}
            ...
            <div
              id={portalId}
              className="absolute top-full left-0 z-[70] mt-1"
            />
          ...
          <div
            className="relative flex w-2 shrink-0 items-center justify-center"
            onClick={(e) => e.stopPropagation()}
          >
            <motion.button
              type="button"
              ...
              className="cursor-pointer select-none rounded-md p-1 text-black/30 hover:text-black"
            ...
            <AnimatePresence>
              {menuOpen && (
                <motion.div
                  ...
                  className="absolute top-full right-0 z-20 mt-1 min-w-[120px] rounded-md border border-black/10 bg-white shadow-md"
                  role="menu"
                  onClick={(e) => e.stopPropagation()}
                >
                  <button
  ...
  return (
    ...
    >
      <motion.div
        ...
        exit={{ opacity: 0 }}
        transition={{ type: "spring", bounce: 0.16, duration: 0.56 }}
        className="absolute top-0 left-0 z-10 flex h-14 w-full bg-white"
      />
      <motion.div
        ...
        transition={{ type: "spring", bounce: 0.16, duration: 0.56 }}
        className="m-5 origin-top-left text-md tracking-tight"
      >
        <input
          ref={titleInputRef}
          type="text"
          ...
          className="m-0 w-full border-0 bg-transparent p-0 text-inherit outline-none focus:ring-0 focus-visible:ring-0"
        />
      </motion.div>
  ...
  return (
    <motion.div animate={{ height: isExpanded ? "100%" : height }}>
      <div
        ...
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
        className="flex cursor-pointer items-center gap-3 border-black/10 border-b p-5 text-md hover:bg-white/40"
        onClick={() => setCurrentTodoList(ref, index)}
      >
        <div className="flex-1">{title}</div>
        <MotionTrash
          animate={{ opacity: isHovered ? 1 : 0 }}
          className="h-4 w-4 cursor-pointer text-black/30 hover:text-black"
          onClick={(e) => {
            e.stopPropagation();
            onDelete(index);
          }}
...
export function App() {
  ...
  return (
    <div className="my-5 antialiased">
      ...
      >
        <BaseCard>
          <div ref={ref} className="h-full w-full pt-9">
            <div className="absolute top-0 left-0 z-20 flex w-full p-5">
              <MotionList
                initial={{ opacity: 0 }}
