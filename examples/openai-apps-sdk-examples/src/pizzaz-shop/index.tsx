...
function SelectedCartItemPanel({
  ...
  return (
    <div className="space-y-4">
      <div className="overflow-hidden rounded-none border-black/5 border-b bg-white">
      ...
      <div className="flex flex-col gap-3 px-5 pb-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center rounded-full bg-black/[0.04] px-1 py-1 text-black">
            ...
            <span className="mx-2 min-w-[10px] text-center font-medium text-base">
              {item.quantity}
            </span>
            <Button
              type="button"
        ...
        {item.detailSummary ? (
          <p className="font-medium text-black text-sm">{item.detailSummary}</p>
        ) : null}
          ...
          <div className="grid grid-cols-3 gap-3 rounded-3xl border border-black/[0.05] px-4 py-2 text-center sm:grid-cols-4">
            {nutritionFacts.map((fact) => (
              <div key={`${item.id}-${fact.label}`} className="space-y-0.5">
                <p className="font-medium text-base text-black">{fact.value}</p>
                <p className="text-black/60 text-xs">{fact.label}</p>
              </div>
            ))}
          </div>
        ) : null}
    ...
    <>
      {!shouldShowCheckoutOnly && (
        <header className="hidden space-y-4 sm:block">
          <h2 className="text-black text-xl">Checkout details</h2>
        </header>
      )}

      <section className="space-y-4 border-black/5 border-t pt-3">
        <div className="space-y-0">
          <h3 className="font-medium text-sm">Delivery address</h3>
        </div>
        <div className="space-y-0">
          <p className="font-medium text-base text-slate-900 text-sm">
        ...
        <div className="mt-1 flex flex-row items-center gap-3">
          <div className="flex flex-1 items-center justify-between rounded-xl border border-black/35 bg-white px-4 py-2.5 shadow-sm">
            <div>
              <p className="font-medium text-slate-900 text-sm">Fast</p>
              <p className="line-clamp-1 text-black/50 text-xs">
                50 min - 2 hr 10 min
              </p>
            </div>
            <span className="font-semibold text-[#047857] text-sm">Free</span>
          </div>
          <div className="flex flex-1 items-center justify-between rounded-xl border border-black/10 px-4 py-2.5">
            <div>
              <p className="font-medium text-slate-900 text-sm">Priority</p>
              <p className="line-clamp-1 text-black/50 text-xs">35 min</p>
            </div>
            <span className="font-semibold text-[#047857] text-sm">Free</span>
          </div>
        </div>
      ...
      <section className="space-y-4 border-black/5 border-t pt-3">
        <div>
          <h3 className="font-medium text-black text-sm">Delivery tip</h3>
          <p className="text-black/50 text-xs">100% goes to the shopper</p>
        </div>
      ...
      <section className="space-y-1 border-black/5 border-t pt-3 text-center">
        <div className="flex items-center justify-between">
          <span className="text-black/70 text-sm">Subtotal</span>
          <span className="text-black text-md">${subtotal.toFixed(2)}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-black/70 text-sm">Total</span>
          <span className="font-medium text-black text-md">
            ${total.toFixed(2)}
          </span>
        </div>
        <p className="mt-3 mb-4 border-black/5 border-b text-black/50 text-xs"></p>
        <Button
          type="button"
          color="primary"
          variant="solid"
...
function App() {
  ...
  useEffect(() => {
    setCartItems((previous) =>
      cartItemsEqual(previous, resolvedCartItems)
        ? previous
        : resolvedCartItems,
    );
  }, [resolvedCartItems]);
  ...
  const cartPanel = (
    <section>
      {!shouldShowCheckoutOnly && (
        <header className="mb-4 flex flex-col gap-3 border-black/5 border-b px-0 pb-3 sm:flex-row sm:items-center sm:justify-between">
          {!isFullscreen ? (
            ...
            <div className="text-black/70 text-lg">Results</div>
          )}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col gap-3 pe-6 pt-3 pb-4 text-left">
                    ...
                    {shortDescription ? (
                      <p
                        className="text-black/50 text-sm leading-snug"
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center rounded-full bg-black/[0.04] px-1.5 py-1 text-black">
                        ...
                        <span className="min-w-[20px] px-1 text-center font-medium text-sm">
                          {item.quantity}
                        </span>
                        <button
                          type="button"
      ...
      <div className="flex w-full flex-col gap-6 px-4">
        <div className="divide-y divide-black/5">
          {cartSummaryItems.length ? (
            ...
            <p className="rounded-2xl border border-black/20 border-dashed bg-white/90 p-6 text-center text-black/50 text-sm">
              Your cart is empty.
            </p>
          )}
        </div>
        ...
        <div className="space-y-0.5">
          <div className="flex items-center justify-between font-medium text-black text-sm">
            <span>Subtotal</span>
        ...
        <button
          ...
          className="mx-auto mb-4 w-full rounded-full bg-[#FF5100] px-6 py-2.5 font-semibold text-sm text-white transition-colors hover:bg-[#ff6a26] disabled:cursor-not-allowed disabled:bg-black/20"
        ...
        {!isFullscreen && !shouldShowCheckoutOnly && (
          <div className="flex justify-center">
            <button
              ...
              className="rounded-full border border-black/10 px-4 py-2 font-medium text-black/70 text-sm transition-colors hover:border-black/40 hover:text-black"
            >
              See all items
            </button>
          </div>
