...
export const evaluateCandidateTool = {
  description:
    "Submit a candidate screening evaluation with score and recommendation",
  parameters: z.object({
    candidateName: z.string().describe("The candidate's full name"),
    score: z.number().min(1).max(10).describe("Screening score from 1-10"),
    recommendation: z
      .enum(["proceed_to_interview", "reject", "needs_more_info"])
      .describe(
        "Recommendation: proceed to interview, reject, or request more information",
      ),
