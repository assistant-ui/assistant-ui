...
Every pull request that changes packages must include a changeset, otherwise your changes won't be published to npm. CI enforces this: the Changeset Semver Check fails a pull request that edits a published package's shipped files without a changeset naming that package. Tests and top-level Markdown files do not count, comment-only source edits do, and a `package.json` edit counts when it changes what consumers install. Bumping `version`, editing `scripts` or `devDependencies`, and moving a dependency range that names another workspace package are all handled by the release itself, so they need nothing from you; an `exports`, `files`, `bin`, `sideEffects` or third-party range edit needs a changeset naming that package.
