...
- Keep `@react-native/metro-config` in the Expo example aligned with the `react-native` version. It is a development dependency required by the Metro configuration and must not drift independently during dependency updates. The fallback regex protects this pair when Expo repinning is skipped; a successful `expo install --fix` still needs the pair reviewed because Expo does not manage the `@react-native/*` scope.
