...
- Keep `@react-native/metro-config` in the Expo example aligned with the `react-native` version. It is a development dependency required by the Metro configuration and must not drift independently during dependency updates.
