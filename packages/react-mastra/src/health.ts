...
export const checkHealthThresholds = (health: MastraHealthCheck): {
  ...
  if (health.metrics.memoryUsage > 200) {
    warnings.push(`High memory usage: ${health.metrics.memoryUsage.toFixed(2)}MB`);
  }
  if (health.metrics.memoryUsage > 500) {
    errors.push(`Critical memory usage: ${health.metrics.memoryUsage.toFixed(2)}MB`);
  }

  // Response time checks
  if (health.metrics.averageResponseTime > 500) {
    warnings.push(`Slow response time: ${health.metrics.averageResponseTime.toFixed(2)}ms`);
  }
  if (health.metrics.averageResponseTime > 1000) {
    errors.push(`Critical response time: ${health.metrics.averageResponseTime.toFixed(2)}ms`);
  }

  // Error rate checks
  if (health.metrics.errorRate > 0.05) {
    warnings.push(`High error rate: ${(health.metrics.errorRate * 100).toFixed(2)}%`);
  }
  if (health.metrics.errorRate > 0.1) {
    errors.push(`Critical error rate: ${(health.metrics.errorRate * 100).toFixed(2)}%`);
  }
