...
describe("DataStreamDecoder interleaved tool-call args (#5290)", () => {
  ...
  it("does not crash when text arrives between tool-call begin and its args", async () => {
    // Wire order from #5290: begin tool call, then an interleaved text delta
    // (which closes the tool call's args stream), then an args delta for the
    // same tool call id. Before the fix the final `c:` frame enqueued onto a
    // closed controller and threw mid-decode.
    ...
    // The interleaved text must survive, and the tool call part must exist.
    expect(
      chunks.some(
