...
`.`, `./utils`, `./ai-sdk`, `./resumable`, `./resumable/redis`, `./resumable/ioredis`. `./ai-sdk` exports `frontendTools`, which converts the tool schemas an assistant-ui client uploads into AI SDK tools; it needs the optional `ai` peer. The two Redis adapters are optional peer dependencies; install whichever client your stack already uses.
