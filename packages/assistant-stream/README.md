...
`.`, `./utils`, `./ai-sdk`, `./internal`, `./resumable`, `./resumable/redis`, `./resumable/ioredis`. `./ai-sdk` exports `frontendTools`, which converts the tool schemas an assistant-ui client uploads into AI SDK tools; it requires `ai@^7`, install it alongside. `./internal` is not part of the public API; it carries the helpers `@assistant-ui/ai-sdk` shares with `./ai-sdk`. The two Redis adapters are optional peer dependencies; install whichever client your stack already uses.
