...
describe("ThreadMessages", () => {
  ...
  describe("MessagesFlatList auto-scroll", () => {
    ...
    it("lands the initialize scroll on the first measured content-size event", async () => {
      h.state.thread.messages = [{ id: "1", role: "user" }];
      await mountFlatList({ components: messageComponents });
      const props = getFlatListProps();
      ...
      await act(async () => {
        ...
        props.onContentSizeChange?.(0, 1541);
      });

      expect(h.scrollToOffset).toHaveBeenNthCalledWith(2, {
        offset: 741,
        animated: false,
      });
