...
- A markdown message re-parses its whole text once per token; the parser memo skips the second same-input message-part render while only the paragraph that changed re-renders.
