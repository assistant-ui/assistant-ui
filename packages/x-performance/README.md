...
| Wall-time benches | `bench/*.bench.ts` head against base, paired and interleaved on one runner | The Performance workflow, once when a PR opens or leaves draft and on every push while the PR carries the `type/perf` label, as one sticky PR comment shared with the trace lane | Never. Only an infrastructure error reddens the job |
