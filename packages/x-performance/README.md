...
- A Streamdown message renders its primitive twice per token with `defer` on or off. Deferred work stays below that boundary, so it does not add a third render.
