...
import { createAssistantStream } from "./createAssistantStream";
import {
  AssistantStreamEncoder,
  AssistantStreamDecoder,
} from "./AssistantStreamResponse";
import { ReadonlyJSONValue } from "../utils/json-types";
import { AssistantStreamChunk } from "./types";
...
async function encodeAndDecode(
  ...
  const encodedStream = stream.pipeThrough(new AssistantStreamEncoder());
  ...
  // Decode the stream back to AssistantStreamChunk
  return reconstructedStream.pipeThrough(new AssistantStreamDecoder());
}

describe("AssistantStream serialization and deserialization", () => {
