...
async function scanDirectory(dir: string, baseDir: string): Promise<FileContent[]> {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        const skipDirs = ['node_modules', 'dist', 'build', '.next', '.git', '.turbo'];
        if (!skipDirs.includes(entry.name)) {
          const subFiles = await scanDirectory(fullPath, baseDir);
          files.push(...subFiles);
        }
      } else if (entry.isFile()) {
        const includeExts = ['.ts', '.tsx', '.js', '.jsx', '.json', '.css', '.md', '.mdx'];
