...
const docsInputSchema = z.object({
  paths: z.array(z.string()).min(1).describe('Documentation paths to retrieve (e.g., ["getting-started", "api-reference/primitives/Thread"])')
});
  ...
  if (await pathExists(fullPath)) {
    ...
    if (stats.isDirectory()) {
      ...
      return {
        ...
        directories,
        files: files.map(f => f.replace(MDX_EXTENSION, '')),
        content: Object.keys(contents).length > 0 ? 
          JSON.stringify(contents, null, 2) : undefined
      };
    }
  }
