...
export async function listDirContents(dirPath: string): Promise<{
  ...
  try {
    ...
    const files = items
      .filter(item => item.isFile() && (extname(item.name) === MDX_EXTENSION || extname(item.name) === MD_EXTENSION))
      .map(item => item.name)
      .sort();
...
export function findNearestPaths(requestedPath: string, availablePaths: string[]): string[] {
