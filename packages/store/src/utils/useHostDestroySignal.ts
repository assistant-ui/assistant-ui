 ...
 * aborts on a real unmount and nothing else. The abort itself runs in a
 * microtask because React forbids scheduling updates from inside an
 * insertion effect and abort listeners may update surviving components.
 * React-land only: under tap's dispatcher the insertion effect is a plain
 * effect and would abort on every soft unmount.
 */
