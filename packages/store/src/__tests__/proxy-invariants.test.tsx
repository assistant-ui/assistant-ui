...
describe("proxy invariants", () => {
  ...
  it("allows cleanup cancellation while denying other stale actions", async () => {
    ...
    const Consumer: FC = () => {
      const aui = useAui();
      const client = aui.thread as any;
      cancelThreadRun = client.cancelRun;
      cancelComposer = client.cancel;
      sendMessage = client.send;
      useEffect(() => () => client.cancelRun(), [client]);
      return null;
    };
    ...
    await new Promise<void>((resolve) => queueMicrotask(resolve));
    expect(() => cancelThreadRun()).not.toThrow();
    expect(cancelRun).toHaveBeenCalledTimes(1);
    expect(warning).toHaveBeenCalledWith(
  ...
  it("reconnects before descendant effects run", async () => {
    ...
    const Consumer: FC = () => {
      const aui = useAui();
      const [client] = useState<any>(() => aui.thread().item({ index: 0 }));
      useEffect(() => {
    ...
    const DerivedApp: FC<{ renderId: number }> = ({ renderId }) => {
      const aui = useAui({
        ...
        item: Derived({
          source: "thread",
          query: { index: 0 },
          get: (aui: any) => aui.thread.item({ index: 0 }),
        } as never),
      } as unknown as useAui.Props);
      probe.aui = aui;
