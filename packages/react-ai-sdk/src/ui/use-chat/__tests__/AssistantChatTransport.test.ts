...
describe("AssistantChatTransport — catalog integration (Phase 4)", () => {
  function makeRuntime(
    tools: Record<string, Tool<any, any>> | undefined,
    deferredTools: Record<string, Tool<any, any>> | undefined,
  ) {
  ...
  function makeAuiWithCatalogs(catalogIds: string[]) {
    return {
      toolCatalogs: Object.assign(
        () => ({
          list: () =>
            catalogIds.map((id) => ({
              catalogId: id,
  ...
  it("with Anthropic adapter and one catalog, injects __aui_catalog_search__ entry", async () => {
    let capturedOptions: any;
    const transport = new AssistantChatTransport({
      toolWireFormat: "anthropic",
      prepareSendMessagesRequest: (async (opts: any) => {
    ...
    transport.setRuntime(makeRuntime(undefined, undefined));
    transport.setAui(makeAuiWithCatalogs(["product-catalog"]));
    ...
    expect(
      capturedOptions.body.tools["__aui_catalog_search__product-catalog"],
    ).toBeDefined();
  });

  it("with OpenAI adapter and one catalog, injects tool_search__<id> entry", async () => {
    let capturedOptions: any;
    const transport = new AssistantChatTransport({
      toolWireFormat: "openai",
      prepareSendMessagesRequest: (async (opts: any) => {
    ...
    transport.setRuntime(makeRuntime(undefined, undefined));
    transport.setAui(makeAuiWithCatalogs(["shop"]));
    ...
    const entry = capturedOptions.body.tools["tool_search__shop"] as any;
    expect(entry).toBeDefined();
    expect(entry.type).toBe("tool_search");
    expect(entry.execution).toBe("client");
  });

  it("without setAui called, no catalog entries are injected", async () => {
    let capturedOptions: any;
    const transport = new AssistantChatTransport({
      toolWireFormat: "anthropic",
      prepareSendMessagesRequest: (async (opts: any) => {
    ...
    transport.setRuntime(makeRuntime(undefined, { deferredTool: stubTool() }));
    // No setAui call
    ...
    const catalogKeys = Object.keys(capturedOptions.body.tools).filter((k) =>
