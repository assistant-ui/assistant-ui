...
describe("useLangGraphRuntime", () => {
  ...
  describe("run serialization", () => {
    ...
    it("keeps repeated explicit tool call ids distinct in a resume batch", async () => {
      const runConfig = {
        custom: { configurable: { model_name: "gpt-5.4-nano" } },
      };
      const streamMock = vi.fn(async function* (
            ...
            data: [
              {
                ...
                content: "",
                tool_calls: [
                  { id: "tc-duplicate", name: "first_tool", args: {} },
                  { id: "tc-duplicate", name: "second_tool", args: {} },
                ],
              },
            ],
          };
      ...
      await waitForToolCallPart(auiResult.current, "tc-duplicate");
      await waitForToolCallPart(auiResult.current, "tc-duplicate-1");
      await waitFor(() =>
        expect(auiResult.current.thread.getState().isRunning).toBe(false),
      );
      ...
      await waitFor(() => expect(streamMock).toHaveBeenCalledTimes(2));
      expect(streamMock.mock.calls[1]?.[0]).toMatchObject([
        { type: "tool", tool_call_id: "tc-duplicate" },
        { type: "tool", tool_call_id: "tc-duplicate-1" },
      ]);
      expect(streamMock.mock.calls[1]?.[1].runConfig).toEqual(runConfig);
    });
    ...
    it("does not enqueue a duplicate result while its resume is running", async () => {
