...
class IncrementalToolCallArgsParser {
  private text = "";
  ...
  static from(text: string, fallback: ReadonlyJSONObject) {
    const parser = new IncrementalToolCallArgsParser(fallback);
  ...
  private clone() {
    const parser = new IncrementalToolCallArgsParser(this.args);
