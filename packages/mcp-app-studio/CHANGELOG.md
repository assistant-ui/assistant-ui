# mcp-app-studio

## 0.3.3

### Patch Changes

- 605d825: chore: update dependencies

## 0.3.2

### Patch Changes

- 3719567: chore: update deps

## 0.3.1

### Patch Changes

- 218fb69: refactor(mcp-app-studio): download template from GitHub instead of bundling in package

## 0.3.0

### Minor Changes

- a5c7e86: feat: Add mcp-app-studio package

  A CLI and development workbench for building ChatGPT Apps. Includes:
  - Interactive CLI for scaffolding new projects (`npx mcp-app-studio my-app`)
  - Local development workbench with live preview
  - OpenAI SDK simulation for testing widgets
  - MCP server generation for backend tools
  - Export bundler for production deployment
