...
# MCP Apps Patterns[](#mcp-apps-patterns)

This document covers common patterns and recipes for building MCP Apps.

## Tools that are private to Apps[](#tools-that-are-private-to-apps)

Set [`Tool._meta.ui.visibility`](../interfaces/app.McpUiToolMeta.html#visibility) to `["app"]` to make tools only callable by Apps (hidden from the model). This is useful for UI-driven actions like updating quantities, toggling settings, or other interactions that shouldn't appear in the model's tool list.
...
*See [`examples/system-monitor-server/`](https://github.com/modelcontextprotocol/ext-apps/tree/main/examples/system-monitor-server) for a full implementation of this pattern.*

## Reading large amounts of data via chunked tool calls[](#reading-large-amounts-of-data-via-chunked-tool-calls)
...
*See [`examples/pdf-server/`](https://github.com/modelcontextprotocol/ext-apps/tree/main/examples/pdf-server) for a full implementation of this pattern.*

## Giving errors back to model[](#giving-errors-back-to-model)
...
**Client-side**: If a runtime error occurs (e.g., API failure, permission denied, resource unavailable), use [`updateModelContext`](../classes/app.App.html#updatemodelcontext) to inform the model:
...
## Matching host styling (CSS variables, theme, and fonts)[](#matching-host-styling-css-variables-theme-and-fonts)
...
*See [`examples/basic-server-vanillajs/`](https://github.com/modelcontextprotocol/ext-apps/tree/main/examples/basic-server-vanillajs) and [`examples/basic-server-react/`](https://github.com/modelcontextprotocol/ext-apps/tree/main/examples/basic-server-react) for full implementations of this pattern.*

## Entering / Exiting fullscreen[](#entering-exiting-fullscreen)

Toggle fullscreen mode by calling [`requestDisplayMode`](../classes/app.App.html#requestdisplaymode):
...
Listen for display mode changes via [`onhostcontextchanged`](../classes/app.App.html#onhostcontextchanged) to update your UI:
...
*See [`examples/shadertoy-server/`](https://github.com/modelcontextprotocol/ext-apps/tree/main/examples/shadertoy-server) for a full implementation of this pattern.*

## Passing contextual information from the App to the Model[](#passing-contextual-information-from-the-app-to-the-model)

Use [`updateModelContext`](../classes/app.App.html#updatemodelcontext) to keep the model informed about what the user is viewing or interacting with. Structure the content with YAML frontmatter for easy parsing:
...
*See [`examples/map-server/`](https://github.com/modelcontextprotocol/ext-apps/tree/main/examples/map-server) for a full implementation of this pattern.*

## Sending large follow-up messages[](#sending-large-follow-up-messages)

When you need to send more data than fits in a message, use [`updateModelContext`](../classes/app.App.html#updatemodelcontext) to set the context first, then [`sendMessage`](../classes/app.App.html#sendmessage) with a brief prompt to trigger a response:
...
*See [`examples/transcript-server/`](https://github.com/modelcontextprotocol/ext-apps/tree/main/examples/transcript-server) for a full implementation of this pattern.*

## Persisting view state[](#persisting-view-state)
...
**Client-side**: Receive the UUID in [`ontoolresult`](../classes/app.App.html#ontoolresult) and use it as the storage key:
...
## Pausing computation-heavy views when out of view[](#pausing-computation-heavy-views-when-out-of-view)
...
## Lowering perceived latency[](#lowering-perceived-latency)

Use [`ontoolinputpartial`](../classes/app.App.html#ontoolinputpartial) to receive streaming tool arguments as they arrive, allowing you to show a loading preview before the complete input is available.
...
*See [`examples/threejs-server/`](https://github.com/modelcontextprotocol/ext-apps/tree/main/examples/threejs-server) for a full implementation of this pattern.*
