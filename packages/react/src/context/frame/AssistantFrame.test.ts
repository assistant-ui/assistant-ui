...
describe("AssistantFrame Integration", () => {
  ...
  it("should execute tools through the frame boundary", async () => {
    ...
    const toolExecute = vi.fn().mockResolvedValue({ results: ["result1", "result2"] });
    registry.addTool({
      toolName: "search",
      description: "Search the web",
      parameters: z.object({ query: z.string() }),
    ...
    const resultPromise = searchTool!.execute!({ query: "test query" }, {} as any);
  ...
  it("should handle tool execution errors", async () => {
    ...
    const toolExecute = vi.fn().mockRejectedValue(new Error("Tool execution failed"));
    registry.addTool({
    ...
    await expect(failingTool!.execute!({ input: "test" }, {} as any)).rejects.toThrow("Tool execution failed");
