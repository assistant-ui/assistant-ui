...
`scopeId` is the storage's stable identity: two storages with the same `scopeId` must read and write the same persisted data. Servers with `bearer` or `oauth` auth key their connection on it, so swapping to a differently-scoped storage reconnects (and rebinds the OAuth provider) instead of leaving a live connection on the replaced store. `McpManagerResource` also clears and rehydrates custom servers when the declared scope changes, serializing writes independently per scope; a same-scope replacement preserves the current state. Replacing one undeclared storage with another retains the legacy behavior and does not key a reconnect or rehydration. `McpLocalStorage` derives `local-storage:<keyPrefix>` when backed by the shared `globalThis.localStorage` and declares no scope for a custom `storage` backing unless `scopeId` is passed; `McpMemoryStorage` scopes each instance uniquely (`memory:<id>`), since each holds private data.
