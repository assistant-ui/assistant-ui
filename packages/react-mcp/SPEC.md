...
Persisted authentication is accepted only when `serverUrl` identifies the same server URL. Storage implementations must preserve this field. OAuth records written before endpoint binding was introduced are treated as unbound and require one manual reconnect after upgrading. Bearer records are host-authored, so a host that persists `token` must write the MCP server URL as `serverUrl` in the same record.
