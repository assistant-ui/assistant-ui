...
type ExternalStoreAdapterBase<T> = {
  ...
  /**
   * Called with each message a voice session adds to the thread: every finalized transcript, and every text message typed into a session that takes typed text (it carries no `metadata.modality`). The host appends it to its own messages under the same id, which is how the runtime knows the host carries it. A message that finalizes while `isLoading` is true is delivered once loading ends, to the callback on the adapter current at that moment. Switching conversations before the load ends drops it: a host that builds a runtime per conversation leaves it on the runtime it switched away from, and a host that reuses one runtime receives it on the conversation it switched to. A host that does not implement this callback keeps these messages for the session only.
   */
