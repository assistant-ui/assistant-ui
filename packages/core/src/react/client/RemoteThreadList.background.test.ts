...
describe("RemoteThreadList backgroundThreads", () => {
  it("preserves the switched body when initialization collapses a duplicate", async () => {
    const list = deferred<{
    ...
    const remoteMountsBeforeCollapse = tracker.mounts.filter(
      (id) => id === "remote-1",
    ).length;
