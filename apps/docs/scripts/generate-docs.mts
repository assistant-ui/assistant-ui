...
import {
  ...
  SECTION_ORDER,
  analyzeApiSurface,
  collectExportTypeDocs,
  discoverIntegrationExports,
  primitivePartTypeDocName,
  readPrimitiveParts,
  type ApiSection,
  type ExportInfo,
  type TypeDoc,
  type TypeDocBindings,
  typeDocBindingForIntegration,
  writeTypeDocsFile,
  writeTypeDocs,
} from "./api-surface.mts";
...
function readFrontmatter(
  ...
  const frontmatter: AuthoredFrontmatter = {};
  for (const line of source.slice(4, end).split("\n")) {
    ...
    if (key === "title" || key === "description") {
      frontmatter[key] = unquoteFrontmatterScalar(value ?? "");
    }
  }
  return frontmatter;
}
...
function writeSectionIndex(section: ApiSection, pages: PageSummary[]): void {
  ...
  const frontmatter = fs.existsSync(indexPath)
...
function authoredTitleOrSeed(
  filePath: string,
  frontmatter: AuthoredFrontmatter | undefined,
  seed: string,
): string {
...
function validatePageDescription(
  ...
  const desc = authoredDescription(frontmatter);
...
function writeIntegrationPages(): void {
  ...
  const pageSummaries = INTEGRATION_PACKAGES.map((integration) => {
    const filePath = path.join(sectionDir, `${integration.slug}.mdx`);
    const frontmatter = fs.existsSync(filePath)
      ? readFrontmatter(filePath, fs.readFileSync(filePath, "utf8"))
      : {};
    return {
      slug: integration.slug,
      title: frontmatter.title ?? integration.packageName,
      description: frontmatter.description ?? "",
    };
  });
  const indexAuthored = readAuthoredPageParts(section, "index", []);
...
const { exports } = analyzeApiSurface();
