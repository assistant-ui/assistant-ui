...
- The chat route, with that page's POST handler: a resource route in React Router (app/routes/api.chat.ts exporting it as \`action\`, registered in app/routes.ts), a server route in TanStack Start (src/routes/api/chat.ts, under \`server.handlers.POST\`), or a separate server for Vite, which serves no API, with /api proxied to it through \`server.proxy\` in vite.config.ts.
- assistant.tsx (in src/ on Vite and TanStack Start, app/ on React Router), exporting <Assistant />: it creates the runtime with \`useChatRuntime({ transport: new AssistantChatTransport({ api: "/api/chat" }) })\` from @assistant-ui/ai-sdk and renders <Thread /> in a full height container (\`<div className="h-dvh">\`, as init's scaffold does) inside \`<AssistantRuntimeProvider runtime={runtime}>\` from @assistant-ui/react, as that page's "Use it in your app" step shows.
...
In the app: install @assistant-ui/react, @assistant-ui/react-langgraph and @langchain/langgraph-sdk; add the assistant-ui registry to components.json and run \`npx shadcn@latest add @assistant-ui/thread\`; create lib/chatApi.ts and components/MyAssistant.tsx from the quickstart's manual setup; set NEXT_PUBLIC_LANGGRAPH_API_URL and NEXT_PUBLIC_LANGGRAPH_ASSISTANT_ID in .env.local. Vite, React Router and TanStack Start expose only VITE_ variables to the browser: name them VITE_LANGGRAPH_API_URL and VITE_LANGGRAPH_ASSISTANT_ID and read them through import.meta.env. On Expo, follow /docs/runtimes/langgraph/quickstart.md?platform=rn instead: it installs @assistant-ui/react-native, takes the thread from /docs/react-native.md, and reads EXPO_PUBLIC_LANGGRAPH_API_URL and EXPO_PUBLIC_LANGGRAPH_ASSISTANT_ID. Configure the graph's model from the provider answer.

### Model provider

Map the model answer (or the provider already in the project) onto the route or graph: install the provider package (@ai-sdk/openai, @ai-sdk/anthropic, @ai-sdk/google, @openrouter/ai-sdk-provider, @ai-sdk/xai, @ai-sdk/mistral, @ai-sdk/deepseek, @ai-sdk/groq or @ai-sdk/fireworks for the AI SDK and Mastra; the matching langchain-* package for LangGraph), use the answer's model in the chat route or graph, run "env <input-id> --file .env.local" in the project that hosts the chat route to write the key (a standalone server loads it itself, for example with \`node --env-file=.env.local\`), and map reasoningEffort onto the provider's reasoning setting when it is set. Never invent a key or a model name. Restart the dev server after writing the key.
