...
Nothing inside the element opens or closes the panel: `open` and `onOpenChange` do. The trigger's count always reads `sources.length`, whether the panel is open or not; an empty `sources` array leaves the trigger reading "0" and the panel opens onto an empty grid or list, depending on `layout`.
...
| `layout` | `"grid" \| "list"` | `"grid"` | `"grid"` renders the two-column card grid; `"list"` renders one compact line per source instead. |
