...
`ReadonlyThreadProvider` wraps children in a read-only thread. Thread mutations are ignored, and composer text, sending, adding attachments, and setting a quote have no effect. The normal thread primitives render it exactly like any other thread.
...
| Part | Renders | Notes |
|------|---------|-------|
| `ReadonlyThreadProvider` | children | Wraps `children` in a thread scope built from `messages`. Thread mutations and composer text, send, addAttachment, and setQuote calls have no effect. |
