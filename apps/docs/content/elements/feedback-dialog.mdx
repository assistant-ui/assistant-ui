...
export function MessageFeedback() {
  ...
  return (
    <FeedbackDialog
      ...
      onSubmit={() => {
        const comment = [...selected, note.trim()].filter(Boolean).join(". ");
        aui.message.submitFeedback({ type: "negative", ...(comment ? { comment } : {}) });
        setSent(true);
      }}
    />
  );
...
`submitFeedback` reaches a `FeedbackAdapter` you register on the runtime. Its `submit` receives the message, type, and optional `comment`, so reasons and a note travel in `comment`. Structured reasons still need their own transport when they must remain separate.
