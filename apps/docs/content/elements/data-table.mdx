...
<div data-slot="data-table">
  <table>
    ...
    <thead>
      <tr>{/* column headers: Model / Context / Cost */}</tr>
    </thead>
    <tbody key={cycle}>
...
The native table exposes a `Model usage` accessible name, column headers, rows, and cells. The row list is keyed on `cycle`, so changing it remounts every row and replays the 80ms-per-row stagger from the top. An empty `rows` array renders only the header, with no placeholder row and no empty-state message.
...
`ModelUsage` is `{ name: string; context: string; cost: string }`. All other `div` props are forwarded to the root. The rendered table has a default `Model usage` caption and native column, row, and cell semantics.
