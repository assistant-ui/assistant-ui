...
| `useModelSelectorContext()` | `() => ModelSelectorContextValue` | The raw context sub-components read from; throws outside `ModelSelectorRoot`. Not re-exported from the `.kit` path. |
...
```tsx title="components/assistant-ui/elements/model-picker.kit.tsx"
"use client";
