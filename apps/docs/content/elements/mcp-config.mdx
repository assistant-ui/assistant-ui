...
`AuthFields` renders a bearer-token input or an OAuth scopes input depending on the selected `AuthSelect` value by default. Pass `children` to render your own inputs; the render props include controlled input props for both values, including the active field's validation metadata.
...
`bearerToken` and `scopes` are controlled input props. Spread the active value onto an input or pass the props through a custom field so changes reach the form submission. The bearer props include `aria-invalid` and `aria-describedby` when the bearer-token validation error is shown; render `McpAddFormPrimitive.Error` to provide the associated error element.
...
| `AuthFields` | inputs | Bearer token or OAuth scopes input, matching the current `AuthSelect` value. Its render props expose controlled `bearerToken` and `scopes` input props. |
