...
Standalone, you own the answer: `onConfirm` receives the pick and `choice` shows it back.
...
Return a promise from `onConfirm` when the answer has to reach a server first. The list locks while it is pending, and a rejection reopens it with the error message, because the answer never landed:
...
The list has three shapes. Without `onConfirm` it only displays its options. With `onConfirm` a single pick commits on the spot, and a multiple selection toggles checkboxes and commits on the confirm button. With `choice` it is a receipt: only the chosen options remain, each marked selected, and nothing takes input. An empty `choice` reads "Nothing selected".
...
| `onConfirm` | `(ids: string[]) => void \| Promise<void>` | | Commits the answer. Without it the list only displays its options. |
| `confirmLabel` | `string` | `"Confirm"` | The confirm button's label in multiple mode. |
| `choice` | `string[]` | | The committed answer. When set, the list renders as a receipt. |
