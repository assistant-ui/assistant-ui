...
`VoiceConversation` reads the session, volume, controls, and voice transcripts itself. The capability condition keeps the call screen out of runtimes that do not configure a voice adapter.
...
Build from the same state when you need a different call layout. Voice turns are ordinary messages, so select `s.thread.messages` and retain only `metadata.modality === "voice"` for the transcript:
...
Select the array and derive in `useMemo`; a selector that maps to a fresh array re-renders on every store update. The adapter only reports `"listening"` and `"speaking"` for `mode`; infer a separate `"thinking"` phase if your screen needs one.
...
| Transcript | Voice modality messages | Reads `s.thread.messages` and renders only messages with `metadata.modality === "voice"`. |
