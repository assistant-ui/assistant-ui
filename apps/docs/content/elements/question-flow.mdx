...
Standalone, hold the completed answers in your own state and pass them to `choice` to replace the flow with its receipt.
...
Return a promise when the answers need to reach a server first. The final option locks while it is pending, and a rejection keeps the question open with the error message:
...
When `choice` is set, the card is a receipt. It has one row for each answered step, with the question above its selected labels, and no controls.
...
| `onComplete` | `(answers) => void \| Promise<void>` | | Receives every answer at the final step. Without it, the flow is display only. |
| `completeLabel` | `string` | `"Submit"` | Label for the final multiple-selection action. |
| `choice` | `Record<string, string[]>` | | Completed answers. When set, the root becomes a receipt. |
