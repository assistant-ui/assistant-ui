...
<Callout>
This demo uses [`AssistantShell`](/docs/ui/assistant-shell), which includes `thread-list` as a dependency and provides a complete app shell layout with a collapsible sidebar. For custom implementations, you can use `thread-list` directly.
</Callout>
...
Use `assistant-shell` for a complete app shell layout or `thread-list` for custom layouts.
