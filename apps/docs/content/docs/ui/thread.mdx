...
The default thread renders [Welcome Suggestions](/docs/ui/welcome-suggestions) next to the composer of a new chat, fed by the static `Suggestions(...)` scope. To build a custom display instead, use the Suggestions API directly. See the [Suggestions guide](/docs/guides/suggestions) for detailed configuration.
