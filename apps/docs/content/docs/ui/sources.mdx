...
import { SourcesLongTitleSample } from "@/components/pages/docs/samples/sources/long-title-truncation";
import { SourcesVariantsSample } from "@/components/pages/docs/samples/sources/variant-showcase";
...
`SourceIcon` falls back to the `E` domain initial when the favicon fails to load.

<PreviewCode
  file="components/pages/docs/samples/sources/favicon-fallback"
  name="SourcesFaviconFallback"
>
...
The source keeps its full title and uses a smaller maximum width for the visible text.

<PreviewCode
  file="components/pages/docs/samples/sources/long-title-truncation"
  name="SourcesLongTitle"
>
  <SourcesLongTitleSample />
</PreviewCode>

### Visual Variants

This sample shows each style.
...
The default export is a `SourceMessagePartComponent`. It shows URL sources as external links and document sources as badges without links. It also includes compound sub-components for custom layouts.
