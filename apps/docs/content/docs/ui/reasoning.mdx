...
import { ReasoningCompletedSample } from "@/components/docs/samples/reasoning/completed-with-duration";
...
Set `duration` (in seconds) on `ReasoningTrigger` to show the completed reasoning duration in the trigger label. The disclosure starts closed because `defaultOpen` is `false`. Select the trigger to open or close the reasoning text.
