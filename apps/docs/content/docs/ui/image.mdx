...
```tsx
<Image.Root variant="default" />  // Border with subtle background
<Image.Root variant="bordered" /> // Thicker border
<Image.Root variant="ghost" />    // No border or background
```
...
<ImageRoot variant="bordered" size="lg">
  <ImageDialog src="https://example.com/photo.jpg" alt="Photo">
    <div>
...
| `Image.Preview` | `<img>` element with loading/error states |
| `Image.Filename` | Optional filename display below image |
| `Image.Dialog` | Fullscreen preview dialog on click |
