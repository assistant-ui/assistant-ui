...
```tsx
<File.Root variant="default" /> // Border with subtle background
<File.Root variant="outline" /> // Thicker border only
<File.Root variant="ghost" />   // No border or background
```
...
<FileRoot variant="outline" size="lg">
...
| `File.Icon` | MIME type-aware icon |
