...
<Callout type="info">
  This component requires server-side setup to [forward token usage metadata](#forward-token-usage-from-your-route-handler). Without it, ContextDisplay will show nothing.
</Callout>
...
|-----------|-------------|
| `Root` | Fetches token usage, provides shared context, wraps children in a tooltip |
...
**Ring** uses semantic stroke colors:
- `< 80%`: `stroke-primary`
- `80% – 95%`: `stroke-warning`
- `> 95%`: `stroke-destructive`

**Bar** uses explicit colors:
- `< 50%`: `bg-emerald-500`
- `50% – 80%`: `bg-amber-500`
- `> 80%`: `bg-red-500`
