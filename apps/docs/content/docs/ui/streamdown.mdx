...
| `smooth` | `boolean \| SmoothOptions` | `false` | Typewriter-style reveal via `useSmooth`. While the source part is running, the caret and controls stay active until the reveal catches up; once the source settles, remaining text commits immediately. Prefer the native `animated` prop for entrance animations |
