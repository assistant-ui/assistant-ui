...
When building custom code components, you can use the context-based hooks `useIsStreamdownCodeBlock` and `useStreamdownPreProps` to detect whether you're inside a code block or inline code:
...
<Callout type="info">
  Internally, `PreOverride` injects a `data-block="true"` attribute on code elements within block contexts using `cloneElement`, which the code adapter checks for reliable detection. The context hooks remain available for custom components that need to differentiate between inline and block code.
</Callout>
