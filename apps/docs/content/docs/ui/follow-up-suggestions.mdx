...
This adds a `/components/assistant-ui/elements/follow-up-suggestions.tsx` file to your project. The default `thread` component already renders `ThreadFollowupSuggestions`, so installing it standalone is only needed when you build your own thread layout.
...
```tsx title="/components/assistant-ui/elements/thread.tsx" {1,10}
