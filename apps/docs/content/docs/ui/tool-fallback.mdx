...
import { ToolFallbackApprovalSample } from "@/components/docs/samples/tool-fallback/approval";
...
Select Allow or Deny. The `resume` callback records the choice, removes the buttons, and shows the result. A tool call whose approval gate is decided shows no buttons. Edit your project's copy of `tool-fallback.tsx` to add trust escalation, edit-args, custom labels, or analytics hooks.

<PreviewCode
  file="components/docs/samples/tool-fallback/approval"
