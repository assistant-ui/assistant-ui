...
Open the picker and select a model. The trigger shows the new model after the picker closes.

<PreviewCode file="components/pages/docs/samples/model-selector/selected-model" name="SelectableModel" base={<ModelSelectorSelectedSample />}>
...
Open the picker and select an effort level. The trigger shows the selected level next to the model name, and the picker stays open so you can compare levels.
...
`ModelSelector.Item` accepts custom `children` in place of the default name and description row. This demo uses that to render capability badges with the optional `badge` registry item:
...
Open the picker to compare the provider groups. Select an available model. The unavailable model stays disabled.

<PreviewCode file="components/pages/docs/samples/model-selector/provider-groups-and-availability" name="GroupedModelSelector" base={<ModelSelectorGroupedSample />}>
