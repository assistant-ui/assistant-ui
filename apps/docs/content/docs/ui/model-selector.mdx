...
Icons are opt-in per model — omit `icon` and the entry renders text-only. To keep the whole selector text-only even when models carry icons, pass `showIcons={false}`:

```tsx
<ModelSelector models={models} showIcons={false} />
```
...
<ParametersTable
  ...
  parameters={[
    ...
    },
    {
      ...
      type: "boolean",
      default: "true",
      description: "Render each model's icon in the trigger and the dropdown items. Pass false to keep the selector text-only.",
    },
    {
      name: "variant",
