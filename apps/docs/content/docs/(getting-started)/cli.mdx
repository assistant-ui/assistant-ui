...
The command fetches the [assistant-ui skills](/docs/llm#skills) plugin pinned to this CLI release into `~/.cache/assistant-ui/skills` (or `$XDG_CACHE_HOME`), then runs `claude` with `--plugin-dir` for that session only. Nothing is written into your project; run `npx skills add assistant-ui/skills` to install the skills permanently. The `claude` CLI must be on your `PATH`, and a `GITHUB_TOKEN` avoids GitHub's unauthenticated rate limit on the first fetch.
