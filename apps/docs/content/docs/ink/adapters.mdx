...
`SimpleTextAttachmentAdapter` and `SimpleImageAttachmentAdapter` work the same in the terminal as on web and React Native. Text contents are sent to the model wrapped in `<attachment name="...">` tags, with XML entities escaped in the quoted filename, and images are sent as base64 data URLs.
