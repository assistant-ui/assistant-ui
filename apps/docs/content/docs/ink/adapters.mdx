...
`SimpleTextAttachmentAdapter` and `SimpleImageAttachmentAdapter` have the same API and behavior as their web and React Native counterparts, but are implemented terminal-safe: they read files with `file.text()` / `file.arrayBuffer()` instead of the browser-only `FileReader`, which does not exist in Node. Text contents are sent to the model wrapped in `<attachment name=...>` tags; images are sent as base64 data URLs.
