...
Import the toolkit hook and mount it with `<AuiProvider aui={useAui()} config={config}>` where `const config = AuiConfig({ tools: Tools({ toolkit }) })`, as shown in the [Tools](#tools) section above.
