...
ToolApproval is automatically rendered inside `ToolFallback` when a tool call has `requires-action` status and both `addResult` and `resume` are available. You can also use it standalone.
...
### With ToolFallback (recommended)

Pass approval configuration via `approvalProps`:

```tsx
<MessageContent
  renderToolCall={({ part, ...rest }) => (
    <ToolFallback
      {...part}
      {...rest}
      approvalProps={{
...
| `allowEdit` | `boolean` | `!!interrupt` | Show the args editor. Defaults to `true` when interrupt is present |
