...
Container `Box` for the status bar. Defaults to `gap={1}` so its children are space-separated; forwards all other `Box` props.

| Prop | Type | Description |
|------|------|-------------|
| `children` | `ReactNode` | Status bar sub-components |
| `gap` | `number` | Spacing between children (default: `1`) |
| `...rest` | `ComponentProps<typeof Box>` | Forwarded to the underlying Ink `Box` |
...
`Text` component reporting the thread's run state. Resolves to one of `"running"` (while `s.thread.isRunning`), `"error"` (when the last assistant message is `incomplete` with reason `error`), or `"idle"`. The label color follows the resolved state — `green` for idle, `yellow` for running, `red` for error — regardless of the `format` output.

| Prop | Type | Description |
|------|------|-------------|
| `format` | `(status: "idle" \| "running" \| "error") => string` | Custom label formatter (default: returns the state string verbatim) |
