...
Primitives for rendering unified diffs and file comparisons in the terminal. `DiffView` is a convenience component that handles parsing; the `DiffPrimitive.*` components can be composed individually for custom layouts.

### DiffView

All-in-one component that parses a patch string or computes a diff from two files and renders headers + content for each file.
...
Renders a single diff line with indicator (`+`/`-`/` `), coloring, and optional line number.
