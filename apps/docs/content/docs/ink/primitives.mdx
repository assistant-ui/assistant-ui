...
The inline editor opened by `E` supports character append and `Backspace` only — there is no cursor movement, selection, or arrow-key navigation. `Enter` submits the edited JSON; `Esc` cancels without resolving.
