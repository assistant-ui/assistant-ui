...
<Callout type="warn">
During server rendering, React never commits or runs effects, so `clientRef.current` stays `null` for the entire render. Do not access it from render-time paths such as Suspense reads. If a resource needs its parent client while rendering on the server, use `useAui()` instead, as `Derived` resources do.
</Callout>
