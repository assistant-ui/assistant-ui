...
<Callout type="warn">
`clientRef.current` is only assigned during a commit, so it stays `null` throughout server rendering because React never commits or runs effects there. It is also `null` during the first client render. Never read `clientRef.current` during the resource body; read it in `useEffect` or in a method called later.
</Callout>
