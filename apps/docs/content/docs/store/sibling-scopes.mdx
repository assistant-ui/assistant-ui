...
<Callout type="warn">
`clientRef.current` is only assigned during a commit, so it stays `null` throughout server rendering because React never commits or runs effects there. It is also `null` during the first client render. Never read it during the resource body. `clientRef.parent` is available from the first render and is safe to read during rendering, including on the server.
</Callout>
