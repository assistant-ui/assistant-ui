...
Data renderers receive one `data` prop containing the part payload (`part.data`), typed as `DataUIProps`. The part's `type` and `name` are intentionally not passed as component attributes. Register a named renderer with `aui.dataRenderers.setDataUI` or a fallback with `setFallbackDataUI`:

```ts
import { defineComponent, h } from "vue";
import type { DataUIProps } from "@assistant-ui/vue";

const Chart = defineComponent({
  props: ["data"],
  setup: (props: DataUIProps<{ points: number[] }>) => () =>
    h("pre", null, JSON.stringify(props.data.points)),
});

aui.dataRenderers.setDataUI("chart", Chart);
```
