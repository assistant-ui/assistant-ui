...
Data renderers receive one `data` prop containing the part payload (`part.data`). The part's `type` and `name` are intentionally not passed as component attributes. Register a named renderer with `aui.dataRenderers.setDataUI` or a fallback with `setFallbackDataUI`:
...
const Chart = defineComponent({
  props: ["data"],
  setup: (props: { data: { points: number[] } }) => () =>
