...
- `McpLocalStorage()` — default. Stores under the `aui-mcp:` prefix in `window.localStorage`. Changing `keyPrefix` reloads the matching storage scope.
- `McpMemoryStorage()` — in-process Map. Use for SSR/tests where localStorage is absent.
- `McpCustomStorage({...})` — bring your own load/save. Use for app-controlled backends (e.g. POST to your API). If its account, tenant, or workspace can change, `storageScopeKey` is required because the manager cannot infer scope identity from the implementation.
...
export function createMcpConfig(
  ...
  const apiBase = `/api/workspaces/${encodeURIComponent(workspaceId)}/mcp`;
    ...
    mcp: McpManagerResource({
      ...
      storage: McpCustomStorage({
        loadCustomServers: async () =>
          fetch(`${apiBase}/servers`).then((r) => r.json()),
        ...
        loadAuthState: async (id) =>
          fetch(`${apiBase}/auth/${id}`).then((r) =>
            r.ok ? r.json() : null,
          ),
        saveAuthState: async (id, state) =>
          fetch(`${apiBase}/auth/${id}`, {
            method: "PUT",
            body: JSON.stringify(state),
          }),
        clearAuthState: async (id) =>
          fetch(`${apiBase}/auth/${id}`, { method: "DELETE" }),
      }),
    }),
  });
}
...
Read `workspaceId` from your authenticated session or router and use it for both the API partition and `storageScopeKey`. Changing the key closes the previous scope's connections, clears its servers, and loads the new scope before persistence resumes. `McpLocalStorage` derives this identity from `keyPrefix` automatically.
