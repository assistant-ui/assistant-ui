...
Authentication records include the MCP server URL and are used only for that endpoint. Custom storage implementations must round-trip the `serverUrl` field passed to `saveAuthState`. OAuth records created by older versions do not contain this binding and require the user to connect again once after upgrading. Bearer records are written by the host application, so any implementation that persists `token` must also write the MCP server URL as `serverUrl` in the same record.
