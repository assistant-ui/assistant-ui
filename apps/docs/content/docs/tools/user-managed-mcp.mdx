...
Set `storageScopeKey` when a custom storage backend changes with the current account, tenant, or workspace. Changing the key clears the previous scope's servers and loads the new scope before persistence resumes. `McpLocalStorage` derives this identity from `keyPrefix` automatically.
