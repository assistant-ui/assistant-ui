...
The form owns its own draft state and submits via `aui.mcp.addCustomServer(...)`. Pass a render function to `AuthFields` to fully customize it while keeping the auth values bound to the form:
...
The `bearerToken` and `scopes` render props are controlled input props. The bearer props also carry the validation `aria-invalid` and `aria-describedby` values when `<McpAddFormPrimitive.Error />` renders the bearer-token error.
