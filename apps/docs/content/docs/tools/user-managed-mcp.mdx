...
Authentication records include the MCP server URL and are used only for that exact endpoint. Custom storage implementations must round-trip the `serverUrl` field passed to `saveAuthState`. Records created by older versions do not contain this binding and require the user to connect again once after upgrading.
