...
<Callout type="info">
  `sendAutomaticallyWhen` is a single slot, and OpenUI's predicate alone never resumes after other tools' results. If your app also has tools that rely on `lastAssistantMessageIsCompleteWithToolCalls` (the predicate the other guides in this section use), compose the two: `(options) => lastAssistantMessageIsCompleteWithToolCalls(options) || shouldContinueAfterOpenUIPrompt(options)`.
</Callout>
