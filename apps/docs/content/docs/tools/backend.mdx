...
`frontendTools` directly when you're not on the generative build. A backend
that only converts tool schemas can import it from `assistant-stream/ai-sdk`
instead, which needs just `assistant-stream` and `ai` installed.
