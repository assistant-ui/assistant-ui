...
`frontendTools`, `AISDKToolkit`, and `generativeTools` run on the server and do
no React work, so they also ship from `@assistant-ui/react-ai-sdk/server`:

```ts title="server.ts"
import { frontendTools, AISDKToolkit } from "@assistant-ui/react-ai-sdk/server";
```

Prefer that entry in an API route or a standalone backend; it keeps the client
runtime out of the module graph. The package root exports the same symbols and
keeps working, which is what the Next.js examples above use, since there the
route and the client ship from one install.

`@assistant-ui/react-ai-sdk` remains the package to install either way, and it
declares `react` as a peer dependency, so a backend that never renders still
resolves React. Reach for `frontendTools` rather than converting the uploaded
schemas by hand: besides `inputSchema`, it attaches the `toModelOutput` that
unwraps the envelope described under [Multi-modal results](#multi-modal-results),
and a hand-rolled conversion that omits it sends the raw envelope to the model
