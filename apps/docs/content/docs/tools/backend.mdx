...
resolves React. If that matters for your deployment, convert the uploaded tools
yourself: each entry is a JSON Schema, and `jsonSchema()` from `ai` is all that
is needed to turn one into an AI SDK tool.
