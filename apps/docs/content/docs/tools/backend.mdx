...
The adapter is still the package to install, including for a backend that never
renders: these helpers convert between the AI SDK's shapes and assistant-ui's,
which is adapter work, so they live with the adapter rather than in a
framework-neutral package. Installing it in an Express or Hono service leaves
`react` as an unmet peer dependency of `@ai-sdk/react`; nothing installs React
itself, and nothing at runtime needs it.
