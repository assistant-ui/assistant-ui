...
Unlike Slack, the payload is not serialized to a string and carries no size cap of its own, so it always survives the round trip.
...
| Payload size | Advisory, against Teams' 100 KB bot message limit | Warned, never truncated, so you decide whether to split |
