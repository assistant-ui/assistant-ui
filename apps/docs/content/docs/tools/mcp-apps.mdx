...
Callback `headers` and custom `fetch` references may change between renders without reloading the widget. When a callback begins targeting a different account or workspace, key the host resource with that identity:
