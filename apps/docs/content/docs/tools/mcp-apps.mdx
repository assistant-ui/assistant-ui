...
When `headers` is a function, set `hostKey` to the current account or workspace ID and update it when that identity changes. Callback and custom `fetch` references may change between renders without reloading the widget; `hostKey` explicitly controls when dynamically authenticated resources reload.
...
- Keep custom `host` and `handlers` references stable across renders (e.g. module-scope constants or `useMemo`); an unstable custom-host identity will tear down and refetch the widget on every parent re-render.
