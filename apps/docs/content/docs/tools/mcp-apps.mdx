...
`requestDisplayMode` should update host chrome and return the mode that was actually honored, then push that mode back through `hostContext.displayMode`. Echoing `{ mode }` without applying it tells the widget the change succeeded while the host stays `inline`.
