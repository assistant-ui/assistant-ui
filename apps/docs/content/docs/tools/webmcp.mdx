...
- A registration the host rejects is dropped with a `console.warn`, and its name is left alone — the bridge never unregisters a tool it does not own. That name is then not retried while the tool stays in the model context, so a permanent collision warns once rather than on every sync. The trade-off is that a *transient* collision is not retried either: if the page later frees the name, the bridge does not notice. Removing the tool from the model context and adding it back retries it, as does remounting.
