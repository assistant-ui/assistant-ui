...
With `@assistant-ui/react-langgraph`, use `unstable_createLangGraphStream` with `streamSubgraphs: true` and set `unstable_allowCancellation: true` to wire up the stop button. When your graph runs a subgraph, the subgraph's messages stream through the namespaced event handlers.
