...
The array keeps its identity while no task changes, so a component that subscribes to it does not rerender for unrelated streamed tokens.
...
For imperative lookup, `aui.thread.task({ id })` resolves one task by its toolCallId (or use an index selector).
