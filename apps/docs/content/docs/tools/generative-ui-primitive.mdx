...
The producer is your backend, not the model's tool call: you decide what goes into the part and prompt your agent to emit it. That is the dividing line against the [`present` tool](/docs/tools/generative-ui), where the model calls a tool whose schema assistant-ui generated.
