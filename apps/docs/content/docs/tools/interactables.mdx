...
Auto-generated tools use a partial schema in which every top-level field is optional. The AI only sends the fields it wants to change; omitted fields keep their current values. Inside a nested object the schema keeps its required fields, because the merge below replaces the object whole: the AI sends a complete `settings`, never a `settings` with one key.
