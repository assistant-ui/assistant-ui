...
Selection is not a special concept — it's just state. If you want the AI to know which interactable the user is focused on, add a field to your state and set it like any other:
...
function NoteCard({ noteId }: { noteId: string }) {
  ...
  return (
    <div onClick={() => setState((prev) => ({ ...prev, selected: true }))}>
      {state.title}
    </div>
  );
}
