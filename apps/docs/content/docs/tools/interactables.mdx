...
Each snapshot defaults to `[Current state of "note": {...}]`. Pass a second `format` argument to control how it reads to the model.

<Callout type="warn">
  This step is required, not optional. Without `injectInteractableContext`, the
  model can still _call_ `update_*` tools but is **blind to current state** — it
  can't see what's on the task board or which note is selected, so it edits
...
| `config.scope` | `"app" \| "thread"?` | Where state persists and re-seeds from — see [Scope](#scope) (default `"app"`) |
...
3. **State snapshot** — the interactable's current state is stamped onto the user message you send as a frozen snapshot in `metadata.custom`, re-stamped only when it changes (no system-prompt mutation). Your route turns that snapshot into model-visible text by calling `injectInteractableContext` — see [Surface state to the model](#4-surface-state-to-the-model-in-your-route).
...
Each interactable has a `scope` that decides where its state lives and how it is re-seeded after a reload. It defaults to `"app"`.
...
```tsx
useAssistantInteractable("draft", {
...
<Callout type="warn">
  ...
  re-seeds from the last sent snapshot, or `initialState` if it was never sent.
  If an edit must outlive a reload before the next message, send it or use
  `scope: "app"` with a persistence adapter.
</Callout>

## Persistence

By default, `scope: "app"` interactable state is in-memory and lost on page refresh. You can add persistence by providing a save callback:
