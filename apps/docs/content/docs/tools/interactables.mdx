...
State changes are automatically debounced (500ms) before saving. When the owning component unmounts, any pending save is flushed immediately. If the initial load fails, queued changes are kept, the error is exposed through the sync metadata, and the load is retried on the next edit or explicit `flush()` before saving.
