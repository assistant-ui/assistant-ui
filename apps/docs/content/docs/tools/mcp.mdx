...
MCP tools execute on the server, so approval is a server-side tool gate, not a
`humanTool()` result. Add `needsApproval` to the MCP tool object before passing
it to `streamText`. The tool name stays the same, so your custom renderer or
the default `ToolFallback` receives `approval` and `respondToApproval` like any
other backend tool:

```ts title="app/api/chat/route.ts"
const mcpTools = await mcpClient.tools();
const deleteRepository = mcpTools.github_delete_repository;

if (!deleteRepository) {
  throw new Error('MCP tool "github_delete_repository" was not found.');
}
...
With `AISDKToolkit`, apply the same wrapping to the object returned by
`await aiToolkit.tools(...)` before handing it to `streamText`.
...
Use this pattern for backend-owned actions such as deleting, writing, deploying,
or calling privileged MCP tools. Use `humanTool()` only when the user supplies
the tool result itself. For custom approval UIs, see
[Server-side approval gates](/docs/tools/tool-ui#server-side-approval-gates).
