...
<Callout type="info">
  Zod Mini exposes JSON Schema conversion as a module-level function. Wrap a
  Mini schema with `z.toJSONSchema(schema)` before passing it as `parameters`:
  ...
  const tool = {
    parameters: z.toJSONSchema(weatherParameters),
  };
  ```
</Callout>
