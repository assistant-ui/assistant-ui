...
| `Carousel` | `carousel` block of `card` elements | A non-card child that would have rendered is dropped with a warning; a card that cannot map cleanly is reshaped to title and body, and reports its images and actions separately if it had any |
...
Inside a `Carousel` that fallback is unavailable, so an over-full card is reshaped to title and body text instead, with a `fallback` warning. Every child's text survives the reshape, including a `Caption`'s and a `Button`'s label, but a hero image and any action do not, and those are reported separately as `dropped` so the two facts stay distinguishable.
