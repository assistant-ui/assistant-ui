...
An `Input` with a non-empty `$action.type` enables `block_actions` on its containing input block and configures `on_enter_pressed`, so Slack sends its typed text when the user presses Enter. This applies to single-line and multiline inputs; Slack's Enter trigger differs from the browser vocabulary's Ctrl/Cmd+Enter shortcut for multiline inputs. Inputs without an actionable type remain passive and can still be collected by a modal's `view_submission` handler.
