...
<Callout type="warn">
The table budgets above are the converter's, and Slack has since raised the platform ceiling for `data_table` to 200 data rows and 20,000 characters. Trees larger than the converter's limit are clamped even though Slack would now accept them.
</Callout>
