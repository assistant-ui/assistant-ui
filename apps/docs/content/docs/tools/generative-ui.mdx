...
<Callout type="info">
  **Looking for the new API?** `@assistant-ui/react-generative-ui` provides a
  newer, experimental approach using `JSONGenerativeUI` + a `present` tool call
  + a closed vocabulary (`defaultGenerativeUILibrary`) with `ActionRegistry`
  dispatch. See [Generative UI — Experimental](/docs/tools/generative-ui-experimental).
  This page documents the original `MessagePrimitive.GenerativeUI` primitive,
