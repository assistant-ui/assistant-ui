...
Any value inside `$action` can be a `{ "$field": name }` reference instead, which resolves to a control's current value when the action fires. It reads the control with that `name` in the nearest enclosing `Form`, or else anywhere in the same `JSONGenerativeUI` render, collected the way a `Form` collects it, and a reference that finds no value is dropped. A tree rendered with `renderGenerativeUI` directly resolves references only inside a form or an element marked `data-aui="root"`. `Input` and `Select` take a `defaultValue`, so a button can send an edited value without a form:
...
On Teams, `decodeSubmitData` resolves these references from the card's inputs; on Slack, a button click carries no other control's value, so the converter drops them with a warning.
