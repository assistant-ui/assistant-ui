...
Each dispatched action, including its `$input`, is recorded on the tool call so stored conversations retain the interaction. When the UI is registered through `promptUser()`, a handler's non-`undefined` return value becomes the `prompt_user` tool result and resumes the model run. `present` actions never create a tool result.
