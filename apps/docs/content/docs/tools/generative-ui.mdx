...
- Every `Table` in one conversion draws from a single 10,000-character
  `data_table` budget, in document order. An early wide table can clamp a later
  table's rows, or leave too little room for its header row — in which case that
  table is dropped entirely, with a `dropped` warning.
...
One Teams limit is advisory rather than enforced. Every other cap clamps its input, but the payload size check only measures: a card over the 80,000-byte soft threshold produces a `clamped` warning naming Teams' 100 KB bot message limit and is then returned unchanged, so trimming the tree to fit is up to you.

For a component-by-component view of how this vocabulary lands on each target, see [Render targets](/docs/tools/render-targets).
