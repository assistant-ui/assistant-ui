...
The first two are tool-driven, so the model decides when UI appears. The last two are backend-driven, so your agent does. `present` and the primitive both take a JSON component tree, but in [different, non-interchangeable shapes](/docs/tools/generative-ui-primitive#spec-shape).
