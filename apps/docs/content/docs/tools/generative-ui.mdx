...
Outside a `Form` or `Card` with `asForm`, each `RadioGroup` uses a generated native group name, so cards can reuse a logical `name` without clearing each other's selection. That logical name remains the key for form values and `$field` references. Inside one form, groups with the same name share one selection. If you mount multiple independent React apps on the same page, configure distinct [`identifierPrefix` values](https://react.dev/reference/react/useId#using-the-same-id-prefix-on-the-client-and-the-server) and use each app's same prefix for server rendering and hydration.
