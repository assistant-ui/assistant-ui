      ...
      <SizePicker
        ...
        onPick={(size) => {
          void unstable_recordInteraction?.({
            type: "action",
            payload: { type: "pick_size", $input: { size } },
          });
          addResult({ size });
        }}
      />
...
Recording is best effort. The payload must be plain JSON of at most 16,384 characters, a tool call keeps its latest 32 entries within 65,536 characters (earlier ones are counted in `omitted`), and the promise rejects when a runtime cannot record, which never affects the user's action. The local runtime and the AI SDK, AG-UI and A2A runtimes record interactions; a readonly thread ignores them. Interactions are never sent to the model or the agent.
