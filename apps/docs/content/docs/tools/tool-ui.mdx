  ...
  deploy: {
    ...
    render: ({ args, approval, respondToApproval, result }) => {
      ...
      if (approval?.approved === undefined) {
        const answer = (response: ToolApprovalResponse) => {
          setError(null);
          respondToApproval(response).catch((failure: unknown) =>
        ...
        return (
          <div>
            ...
            <button onClick={() => answer({ approved: true })}>Approve</button>
            <button onClick={() => answer({ approved: false, reason: "user denied" })}>
              Deny
            </button>
...
The runtime records that as `{ approvalId, approved: true, text }`, because answering a question is not refusing it. A `text` response to a request that declares neither `display: "text"` nor `allowFreeform` throws, so a host never receives an answer it has nowhere to record. A question has no meaningful refusal, so the default `ToolFallback` renders a text field for one and omits the Deny control it adds to a decision.
