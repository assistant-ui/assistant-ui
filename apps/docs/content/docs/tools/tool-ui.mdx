...
The hook uses parser metadata when available. The AI SDK marks settled tool
arguments complete even while tool execution continues, so received fields in
`propStatus` become `"complete"` at that point too.
Without parser metadata, a running part remains conservatively `"streaming"`:
some runtimes build `argsText` from an incomplete argument snapshot. Any
non-running lifecycle settles argument streaming. `"complete"` does not
guarantee valid arguments or a successful tool result.
