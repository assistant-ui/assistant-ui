...
The runtime and its adapters retain state for the persistence scope under which they mounted. If that scope can change without a page reload, key `MyProvider` by a stable value containing every identifier your persistence endpoints use. The routes in this guide scope data by the signed-in account, so the account id is sufficient. If your routes also filter by workspace, combine the account and workspace ids.

```tsx title="runtime/ScopedProvider.tsx"
...
export function ScopedProvider({
  ...
  return <MyProvider key={scopeKey}>{children}</MyProvider>;
}
```

This wrapper works the same way with the React, React Native, and React Ink versions of `MyProvider` above. The key remounts `useRemoteThreadListRuntime` and all per-thread runtimes, so the new scope starts with a fresh thread list, history, and composer. Any in-flight run from the old scope is stopped during unmount. Keying only `AssistantRuntimeProvider` does not remount the hook that owns the runtime.
