...
### Remount when the persistence scope changes

A history adapter is bound to its mounted runtime and loads once. When the signed-in account or workspace changes, remount the component that creates the runtime instead of replacing only `adapters.history`. A full remount keeps the thread list, history, composer, queue, and in-flight runs in the same persistence scope.

```tsx title="app/assistant.tsx"
export function Assistant({ workspaceId }: { workspaceId: string }) {
  return <WorkspaceAssistant key={workspaceId} workspaceId={workspaceId} />;
}

function WorkspaceAssistant({ workspaceId }: { workspaceId: string }) {
  const history = useMemo(
    () => createHistoryAdapter(workspaceId),
    [workspaceId],
  );
  const runtime = useChatRuntime({ adapters: { history } });
...
Put the `key` on the component that calls `useChatRuntime`. Keying only `AssistantRuntimeProvider` does not remount the hook that owns the runtime.
