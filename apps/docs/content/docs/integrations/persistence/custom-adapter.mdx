...
The runtime and its adapters retain state for the signed-in account under which they mounted. If your application can change accounts without a page reload, key `MyProvider` by the account identifier:

```tsx title="app/assistant.tsx"
"use client";

import { MyProvider } from "./runtime/MyProvider";
...
export function Assistant({
  ...
  return <MyProvider key={accountId}>{children}</MyProvider>;
}
```

The key remounts `useRemoteThreadListRuntime` and all per-thread runtimes, so the new account starts with a fresh thread list, history, composer, and queue. Any in-flight run from the old account is stopped during unmount. Keying only `AssistantRuntimeProvider` does not remount the hook that owns the runtime.
