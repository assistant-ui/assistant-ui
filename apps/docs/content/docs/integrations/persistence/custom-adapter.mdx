...
The adapter calls your endpoints. `unstable_useAdapters` injects the per-thread history adapter on the `RemoteThreadList` store entry. `unstable_Provider` is the same adapters for `useRemoteThreadListRuntime`.
