...
```mermaid
flowchart LR
    browser["browser"] --> auth["/api/auth/[...all]"] --> handlers["better-auth handlers"]
    browser --> routes["/api/chat<br/>/api/threads/*"] --> session["auth.api.getSession<br/>({ headers })"] --> uid["session.user.id"] --> scope["scope queries by userId"]
```
