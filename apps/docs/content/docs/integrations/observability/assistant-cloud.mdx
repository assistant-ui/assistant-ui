...
- `createAssistantCloudTraceExporter({ apiKey, baseUrl? })` sends OTLP/HTTP JSON or protobuf to `POST /v1/traces` on `https://backend.assistant-api.com` using API-key authentication only. Pass `baseUrl` when your backend host differs.
