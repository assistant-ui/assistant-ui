...
- **Pin compatible versions.** `@mastra/ai-sdk` supports AI SDK v5, v6, and v7. Its `toAISdkStream` default is v5, so set `version` to match the major version of your installed `ai` package.
