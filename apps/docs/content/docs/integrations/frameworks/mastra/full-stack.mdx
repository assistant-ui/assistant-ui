...
Persisted thread lists and interactive workflow suspension are different: the browser must call Mastra's memory and workflow APIs. Expose those APIs from the same origin or run the [separate Mastra server](/docs/integrations/frameworks/mastra/separate-server), then configure `useMastraRuntime` with a `MastraClient`, an `agentId`, and a stable `resourceId`. See [Threads and workflows](/docs/integrations/frameworks/mastra/threads-and-workflows) for the complete client wiring.

Use the authenticated account or tenant ID for `resourceId`, not a random value per render. The active assistant-ui thread ID is sent separately as Mastra's memory thread ID. Your server must authorize both values before reading history or resuming a workflow.
...
- **Server-only features** such as tools, RAG, model routing, and evals need no extra frontend integration. Memory thread navigation and workflow suspension do require the client APIs described above.
