...
Shared adapters (attachments, speech, feedback, history) work the same way as described in [adapters](/docs/runtimes/concepts/adapters). Multi-thread support uses [AssistantCloud](/docs/cloud) or a [custom thread list](/docs/runtimes/concepts/threads).
...
<InstallCommand npm={["agents@latest", "@cloudflare/ai-chat@latest", "ai@latest", "@ai-sdk/openai@latest"]} />
...
`this.messages` is the persisted history for this Durable Object instance. Each unique agent `name` you connect with from the client (step 6) gets its own instance and its own message log.
...
`routeAgentRequest` handles WebSocket upgrades, agent lookup by URL path, and the `/get-messages` HTTP endpoint that the frontend uses for history rehydration.

Wire the Durable Object binding and CORS in `wrangler.jsonc`:
...
<InstallCommand npm={["agents@latest", "@cloudflare/ai-chat@latest"]} />
...
`useAgentChat`'s return type is `Omit<ReturnType<typeof useChat>, "addToolOutput"> & { ... }`. The `addToolOutput` option shape differs slightly between the two: `useChat` accepts `{ state, tool, toolCallId, ... }`; `useAgentChat` accepts `{ state, toolCallId, toolName?, ... }`. At runtime the call paths converge through `useAISDKRuntime` without issue. If the TypeScript compiler flags the call, narrow with `chat satisfies Parameters<typeof useAISDKRuntime>[0]` or cast at the call site.
