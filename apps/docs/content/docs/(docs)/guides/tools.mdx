...
For enterprise apps where tool access depends on user roles or permissions, wrap `useAssistantTool` with a permission check. Return a rejected payload when the user lacks access instead of throwing:
...
function usePermissionCheckedTool<TArgs, TResult>(options: {
  toolName: string;
  description: string;
  ...
  useAssistantTool({
    ...
    execute: async (args): Promise<PermissionResult<TResult>> => {
      if (!options.hasPermission()) {
        return { status: "rejected", reason: "Insufficient permissions" };
      }
      const data = await options.execute(args);
...
function AdminTools() {
  ...
  usePermissionCheckedTool({
    toolName: "deleteRecord",
    description: "Delete a record from the database",
    parameters: z.object({ id: z.string() }),
    hasPermission: () => user.role === "admin",
    execute: async ({ id }) => deleteRecord(id),
  });
...
<Callout type="tip">
  This pattern is used in production by [Nussknacker](https://github.com/TouK/nussknacker) (713 stars), a low-code stream processing platform. All frontend tools go through a `useFrontendAiTool` wrapper that enforces permission checking before execution.
</Callout>

### Registration-via-Render Pattern

Another way to organize tools is as null-rendering React components. Each tool is a self-contained component placed inside the runtime provider — no `useAssistantTool` calls needed at the provider level:

```tsx
function ListScenariosAiTool() {
  useAssistantTool({
    toolName: "listScenarios",
    description: "List all available scenarios",
    parameters: z.object({ filter: z.string().optional() }),
    execute: async ({ filter }) => fetchScenarios(filter),
  });
  return null; // This component exists only for side-effects
}

function OpenScenarioAiTool() {
  useAssistantTool({
    toolName: "openScenario",
    description: "Open a specific scenario by name",
    parameters: z.object({ name: z.string() }),
    execute: async ({ name }) => openScenario(name),
  });
  return null;
}

// Register all tools by rendering the components
function App() {
  return (
    <AssistantRuntimeProvider runtime={runtime}>
      <ListScenariosAiTool />
...
This pattern scales well when tools have React context dependencies (auth, router, etc.) that need to be available during execution.
