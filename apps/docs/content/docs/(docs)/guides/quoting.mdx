...
When a user selects text in an assistant message, a floating toolbar appears with a Quote button. Clicking it it calls `composer.setQuote()` to store the selection on the composer. The [Quote component](/docs/ui/quote) does this out of the box.
