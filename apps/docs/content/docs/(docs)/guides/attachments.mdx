...
### CSV / Spreadsheet Adapter

Parse CSV files and inject their content as structured text so the LLM can reason about the data:
...
class CSVAttachmentAdapter implements AttachmentAdapter {
  ...
  async add({ file }: { file: File }): Promise<PendingAttachment> {
    ...
    if (file.size > maxSize) {
      throw new Error("CSV file size exceeds 5MB limit");
    }
      ...
      content: [
        {
          ...
          text: [
            ...
            preview,
            "```",
            rowCount > 5 ? `... and ${rowCount - 5} more rows` : "",
          ]
            .filter(Boolean)
            .join("\n"),
        },
...
Use it with `CompositeAttachmentAdapter` to support CSV alongside other file types:
...
<Callout type="tip">
  **Seen in production:** [Wealthfolio](https://github.com/afadil/wealthfolio) (7k stars) uses a CSV attachment adapter to let users upload brokerage export files directly into the AI assistant chat, which then parses and imports the transactions.
</Callout>
...
Attachment components render in three contexts, identified by the `source` property on `AttachmentRuntime`:

| Source | Where it renders | Component |
|--------|-----------------|-----------|
| `"thread-composer"` | The main message input | `ComposerPrimitive.Attachments` |
| `"edit-composer"` | Editing an existing message | `ComposerPrimitive.Attachments` (inside edit mode) |
| `"message"` | A sent message's attachments | `MessagePrimitive.Attachments` |

When building custom attachment UIs, check the source to render appropriate controls:
...
function MyAttachment() {
  ...
  const source = attachmentRuntime.source;

  // Composer attachments can be removed; message attachments cannot
  if (source === "thread-composer" || source === "edit-composer") {
    return <EditableAttachment onRemove={() => attachmentRuntime.remove()} />;
  }
  return <ReadOnlyAttachment />;
}
```
