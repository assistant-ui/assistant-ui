...
### Suggestion Buttons (append vs. setText)

`aui.thread().append()` sends immediately. `aui.composer().setText()` only stages text without sending. When the thread is already running, appending would queue a second run — instead, stage the text so the user can send it after the current run completes.
...
function SuggestionButton({ prompt }: { prompt: string }) {
  ...
  const handleClick = () => {
    if (!isRunning) {
      // Thread is idle — send immediately
      aui.thread().append({ role: "user", content: [{ type: "text", text: prompt }] });
      aui.composer().setText("");
    } else {
      // Thread is running — stage the text for after the run
...
<Callout type="tip">
  This pattern is used in production by [Unsloth](https://github.com/unslothai/unsloth) (53k stars) for their AI Studio suggestion chips.
</Callout>

### Edit While Running (Cancel + Resend)

When a user edits a message while the assistant is running, you typically want to cancel the current run and automatically resend after cancellation completes. Use `useAuiEvent("thread.runEnd")` with a ref flag:
...
function EditableMessage() {
  ...
  // After a cancel triggered by edit, auto-resend
  useAuiEvent("thread.runEnd", () => {
    if (!resendAfterCancelRef.current) return;
  ...
  const handleEdit = (newText: string) => {
    ...
    if (isRunning) {
      // Cancel the current run; runEnd handler will resend
...
Use `aui.threadListItem().rename()` to programmatically rename the current thread — useful for auto-naming threads based on the first message:

```tsx
function AutoRenameOnFirstMessage() {
  const aui = useAui();

  useAuiEvent("composer.send", () => {
    const { messages } = aui.thread().getState();
    if (messages.length > 0) return; // Only rename on first message
...
### Auto-Generate Thread Title After Run

An alternative approach: generate the title from the first AI response rather than the user's question. Use `useThreadRuntime` to subscribe to the `runEnd` event and derive the title from message content:
...
function AutoTitleGenerator() {
  ...
  useEffect(() => {
    return threadRuntime.unstable_on("runEnd", () => {
      const { messages, metadata } = threadRuntime.getState();

      // Only auto-title once (on first run completion)
      if (metadata?.custom?.titled) return;
...
<Callout type="tip">
  This pattern is used in production by [Exograph](https://github.com/exograph/exograph) (344 stars), which auto-generates thread titles from the first user message using the runtime event system.
</Callout>
...
### Triggering Messages from External Systems

To send messages to the assistant from outside the React tree (Redux actions, global event bus, etc.), expose the runtime via a ref stored outside React:

```tsx
import { useAssistantRuntime } from "@assistant-ui/react";
import { useEffect, useRef } from "react";

// Global ref — accessible from anywhere
let globalRuntime: ReturnType<typeof useAssistantRuntime> | null = null;

// Bridge component — render once inside AssistantRuntimeProvider
...
// Call from Redux middleware, event handlers, or anywhere else
export function askAssistant(prompt: string) {
  globalRuntime?.thread.append({
    role: "user",
...
Then in a Redux thunk or saga:

```ts
import { askAssistant } from "@/lib/assistant-bridge";

// Redux action
export const analyzeScenario = (scenarioId: string) => async (dispatch) => {
  dispatch(openAssistantPanel());
  askAssistant(`Analyze scenario ${scenarioId} and suggest optimizations`);
};
```

<Callout type="tip">
  This pattern is used in production by [Nussknacker](https://github.com/TouK/nussknacker) (713 stars), a stream processing platform. They dispatch a Redux action that calls `runtime.thread.append()`, so any part of the app can trigger the AI assistant without knowing about assistant-ui internals.
</Callout>
