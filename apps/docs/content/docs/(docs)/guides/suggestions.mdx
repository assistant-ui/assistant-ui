...
Suggestions can change based on which page the user is viewing. Combine `Suggestions` with route-based logic to show relevant options:

```tsx
function DashboardProvider({ children, page }: { children: ReactNode; page: string }) {
  ...
  const suggestions = useMemo(() => {
    switch (page) {
      case "compliance":
        return [
          { title: "Risk Assessment", label: "for EU AI Act", prompt: "Run a risk assessment for the current project" },
          { title: "Generate Report", label: "compliance summary", prompt: "Generate a compliance summary report" },
        ];
      case "analytics":
        return [
          { title: "Explain Trend", label: "last 30 days", prompt: "What explains the trend in the last 30 days?" },
          { title: "Compare Periods", label: "month over month", prompt: "Compare this month to last month" },
        ];
      default:
        return ["What can you help me with?", "Show me recent activity"];
    }
  }, [page]);
...
<Callout type="tip">
  **Seen in production:** [VerifyWise](https://github.com/verifywise-ai/verifywise) (237 stars) uses page-aware suggestions to show different prompts on compliance, risk, and dashboard pages — each tailored to the data the user is currently viewing.
</Callout>
