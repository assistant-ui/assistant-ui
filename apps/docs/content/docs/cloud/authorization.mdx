...
Thread state is scoped to the `AssistantCloud` instance. Recreate that instance when the active account or workspace changes so threads from the previous scope are cleared.
