...
Both sign RS256 tokens whose keys are published at a JWKS endpoint; create the rule with the project's issuer and that endpoint, then return the session's access token or the Firebase id token from `authToken`.
...
The same client writes as well as reads, so an app with no browser, a Slack or Teams bot, a backend agent, a batch job, is persisted and measured the same way: create the thread, store each message, report the run. Every route the browser SDK uses accepts the key; only traces, the project read API and MCP are key only.
