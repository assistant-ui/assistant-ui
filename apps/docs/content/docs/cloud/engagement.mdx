...
description: The sixteen user interactions the SDK records without message text, the message feedback and custom scores stored next to a conversation, and the evaluators the cloud runs for you.
---
...
| Kind | assistant-ui runtime | `useCloudChat` (retired) |
|---|---|---|
...
Events are buffered in the browser and posted to `POST /v1/events` in batches of up to 50: after 2 seconds of quiet, when 20 events are waiting, and when the page is hidden or unloaded, using `keepalive` so the last batch survives navigation. A failed batch is dropped silently; events never block the UI and never surface errors.
