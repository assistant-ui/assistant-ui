...
| Evaluator verdicts are empty | The project has no evaluator rules, or a configured rule has no verdicts. | Select **Configure evaluators** to add a rule. A configured rule without results reads **No verdicts yet**. |
