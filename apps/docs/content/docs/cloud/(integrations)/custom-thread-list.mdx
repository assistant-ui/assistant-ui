...
| `upsert` | `false` | Set to `true` when each stable external id identifies one conversation; retries reuse the workspace's existing thread. Requires Assistant Cloud API v0.2.0 or later. |
