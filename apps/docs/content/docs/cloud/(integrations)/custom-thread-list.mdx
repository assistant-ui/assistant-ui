...
The hook gives the runtime Cloud threads and the default history, attachment and feedback adapters. A direct adapter is available when you need the list without the wrapper runtime.
...
| `delete` | `undefined` | Runs before the Cloud thread is deleted. It receives the Cloud thread id. |
