...
- **Copied messages.** LangGraph, LangChain, ADK and any runtime built on `useExternalStoreRuntime` keep the transcript on their backend. When a run settles, the runtime stores a copy of the messages that are new or changed as `aui/v0`, keyed by the backend's message ids, with any tool interaction recorded in the conversation. The dashboard shows these threads like any other and feedback can rate their messages; the runtime never reads the copy back. `telemetry: { messages: false }` on the cloud client keeps the copy out, and `telemetry: false` turns it off together with run reports and events.
