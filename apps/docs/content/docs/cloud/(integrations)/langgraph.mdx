...
| A conversation stops being copied partway | The cloud refused the thread as a whole: it was deleted, or its end user is past the plan's monthly active user cap. The console says `[assistant-ui] The cloud refused copies to thread <id>` once, and nothing more is sent for that thread until the page reloads. | Restore the thread or raise the cap, then reload. |
