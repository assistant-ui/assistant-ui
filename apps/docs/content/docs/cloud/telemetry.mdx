...
Every assistant response is a **run**. When a run finishes, the SDK sends one run report to the project: what happened, how long it took, which model and tools were involved, and how it ended. Reports carry no message content beyond the assistant's own output text, and they are the source of the Runs, Models, Users and Overview pages in the dashboard.
...
| `tool_calls` with `tool_source` | yes | yes | yes |
