...
The project read API answers project wide observability and conversation reads for a backend service. Send an API key to `https://backend.assistant-api.com`; these routes do not need a workspace header, and the cloud never returns system runs such as thread title runs. See [Conventions](/docs/cloud/api) for the shared hosts, credentials and error shapes.
...
| `group_by` | `model` | | With `model`, group rows by day, provider and model. |
...
```json title="Response"
{
  "messages": [
    {
      "id": "message_000000000000000000000000",
      "parent_id": null,
