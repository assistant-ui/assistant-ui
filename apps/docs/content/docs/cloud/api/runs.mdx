...
| `tool_args` | any JSON value | no | At most 50,000 characters when serialized. |
| `tool_result` | any JSON value | no | At most 50,000 characters when serialized. |
...
| `input` | any JSON value | At most 50,000 characters when serialized. |
...
| `tools` | value | no | Passed with the run request. |
| `system` | string | no | Passed with the run request. |
