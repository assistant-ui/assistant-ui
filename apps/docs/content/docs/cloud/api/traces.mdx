...
The receiver recognizes the six operation names `chat`, `text_completion`, `embeddings`, `execute_tool`, `invoke_agent` and `create_agent`. A recognized GenAI span with no GenAI ancestor starts a run, and its recognized GenAI descendants belong to that run. A span linked through non GenAI spans is reparented to its nearest GenAI ancestor. Spans without a recognized operation are rejected and counted in `partialSuccess`.
