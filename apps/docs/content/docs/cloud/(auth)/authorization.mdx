...
The API records the `Aui-Sdk` header in the background. The SDK sends its own name and version, then registered identities, on every request. It records up to 8 valid `name/version` entries. A matched Name claim also records the user's display name. See [auth providers](/docs/cloud/auth-providers) for both settings.
