...
Each submitted entry is normalized and de duplicated before it is stored. An invalid entry refuses the save with the same message.
...
After de duplication, the list may contain at most 32 entries. A larger list is refused with:
...
|---|---|
| Allowed origins | 32 after de duplication |
