...
Both token requests have a 30 second deadline. The client shares one request among calls using the same `localStorage` and base URL. When Web Locks are available, tabs also share one request through the `assistant-cloud:anonymous-auth:${baseUrl}` lock.
