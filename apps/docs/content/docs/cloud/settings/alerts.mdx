...
|---|---|---|---|
| `daily_cost_usd` | The sum of today's run cost. | USD | It is evaluated even when the total is zero. |
...
| `daily_runs` | The sum of today's runs. | runs | It is evaluated even when the total is zero. |
| `period_active_users` | Active users in the current UTC calendar month. | users | It is evaluated even when the total is zero. |
