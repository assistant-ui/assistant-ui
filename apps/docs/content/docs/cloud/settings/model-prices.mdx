...
const withPrice = (report: AssistantCloudRunReport) => {
  ...
  const cost_usd =
    report.input_tokens * inputUsdPerToken +
...
A report without token counts is sent unchanged, so the catalog still prices it. A caller supplied `cost_usd` takes precedence over catalog and override prices. The SDK's report type does not name the cost fields yet, which is why the cast is needed; the API accepts them.
