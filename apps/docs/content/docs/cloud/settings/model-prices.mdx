...
const priceFor = (report: AssistantCloudRunReport) =>
  (report.input_tokens ?? 0) * inputUsdPerToken +
  (report.output_tokens ?? 0) * outputUsdPerToken;
...
const cloud = new AssistantCloud({
  ...
  telemetry: {
    beforeReport: (report) =>
      ({ ...report, cost_usd: priceFor(report) }) as AssistantCloudRunReport,
  },
});
```

A caller supplied `cost_usd` takes precedence over catalog and override prices. The SDK's report type does not name the cost fields yet, which is why the cast is needed; the API accepts them.
