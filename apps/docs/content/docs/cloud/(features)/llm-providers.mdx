...
Every model field is a free text id with the provider's own list as suggestions: the **Models** list on the provider form, the Model row on **Settings › Thread titles** and **Settings › Intelligence**, and the Model field of an assistant. The dashboard asks the provider for its models over a public HTTPS URL with the provider's credentials and gives up after ten seconds. A saved provider's answer is kept in server memory, keyed by the provider and its last update, so an edited provider is asked again: a list is reused for up to an hour, a failure for at most a minute. A provider that is not saved yet is asked with the credentials entered in its form.
...
|---|---|
| `no_list` | This provider type does not list its models: an OpenAI compatible provider with neither a base URL nor a preset URL. |
...
|---|---|---|
| The model field offers no list | The provider did not answer and the catalog has no models for the provider key. The note under the list says why: `The provider refused the key (HTTP 401)`, `The provider took too long to list its models`, and so on. | Fix what the note names and load the models again, or type the id: any id the provider serves is accepted. |
