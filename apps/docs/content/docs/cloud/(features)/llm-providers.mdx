...
| Amazon Bedrock | `https://bedrock.{AWS_REGION}/foundation-models`, signed with AWS credentials. The list keeps only models with `TEXT` output and `ON_DEMAND` inference. When no region is supplied, it uses `us-east-1`. |
