...
The **Connect** tab offers an assistant-ui snippet and an Other snippet, both for the `@assistant-ui/react-harness-sdk` package that ships with the preview; copy them from the tab, which fills in the origin and the ids. In both, `url` is the path of the first allowed endpoint, or `/api/chat` when the allowlist is empty. `origin` is the harness origin. `credential` is a project API key or a token minted with `POST /v1/auth/tokens`. `workspaceId` identifies the workspace whose conversation is opened, and `threadId` identifies that conversation.
...
const threadList = HarnessCloudThreadList({
  url: "/api/chat",
