...
| Model | The first model on the provider's Models list | Any model id, typed or picked from the provider's list, which is ranked for structured output; see [how model discovery works](/docs/cloud/llm-providers#how-model-discovery-works). Saving with analysis on is refused with `Choose a model for this provider` only when the field is empty and the provider's list is too. | Selects the model used for classification. |
