...
In a React app, run an assistant in the active Cloud thread with `useCloudRuntime` from `@assistant-ui/react-data-stream`:
...
const runtime = useCloudRuntime({
  cloud,
  assistantId: "assistant_0…",
});
```

The hook sends the active thread's messages in the Cloud request shape and consumes the returned UI message stream.
