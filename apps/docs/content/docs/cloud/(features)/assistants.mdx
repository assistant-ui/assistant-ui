...
Run an assistant from your server. For a browser runtime, use `useCloudRuntime` from `@assistant-ui/react-data-stream`; it sends the thread-message shape expected by the runs endpoint, including completed frontend tool calls.
