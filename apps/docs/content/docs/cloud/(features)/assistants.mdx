...
| **Temperature** | 0.7 | The dashboard slider accepts 0 to 2 in steps of 0.1. Create accepts 0 to 1; update accepts 0 to 2. | Stores the model temperature. |
...
From an assistant-ui app, `useCloudRuntime` runs the assistant. The runtime keeps the thread list and message history in the cloud like every other runtime, and the response comes from the assistant's provider instead of a route of your own. The hook is part of the preview and is exported from `@assistant-ui/react-data-stream`.

```tsx title="app/assistant/page.tsx"
"use client";
...
export default function AssistantPage() {
  const runtime = useCloudRuntime({ cloud, assistantId: "assistant_0…" });
