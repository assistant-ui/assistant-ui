...
| **Model** | None | Any model id of 1 to 255 characters, typed or picked from the provider's list, which suggests its tool capable models first; see [how model discovery works](/docs/cloud/llm-providers#how-model-discovery-works). | Stores the provider model name. |
