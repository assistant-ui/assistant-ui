...
- **Engagement events.** Sixteen interactions, from a send to a stop, a copy, a regenerate or a thread switch, recorded without message text.
...
| Object | What it is |
|---|---|
| **Project** | The unit of everything: its own API hosts, API keys, auth rules, allowed origins, retention, features and plan. The id, `proj_` and twelve characters, names the frontend host. |
...
| LangGraph, LangChain or Google ADK runtimes | thread list, titles, feedback; transcripts stay with the backend | [LangGraph, LangChain and Google ADK](/docs/cloud/langgraph) |
...
| The AI SDK's `useChat` with your own UI | the retired `useCloudChat` hook, and what to use instead | [AI SDK](/docs/cloud/ai-sdk) |
