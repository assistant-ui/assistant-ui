...
This is also the way to a runtime that has no `cloud` option of its own. `useAgUiRuntime` from `@assistant-ui/react-ag-ui` and `useA2ARuntime` from `@assistant-ui/react-a2a` read the history, attachments and feedback adapters from the context this hook provides, so passing either as the `runtimeHook` gives an AG-UI or A2A app the cloud thread list and titles, persistence of every message, feedback, attachments, and run reports read from the stored assistant messages:
...
A runtime that wants to store messages in another shape implements `ThreadHistoryAdapter.withFormat(adapter)` with a `MessageFormatAdapter`, the same seam the AI SDK runtime uses for `ai-sdk/v6`. `createFormattedPersistence` in `assistant-cloud` handles the id mapping and parent chaining, and `reportTelemetry(items, { durationMs, stepTimestamps, message })` sends the run report; the `message` is the thread message the items came from, whose status and timing complete the report. See the [package reference](/docs/api-reference/integrations/assistant-cloud).
