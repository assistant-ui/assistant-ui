...
```
3. Add the assistant-cloud API key as environment variable `ASSISTANT_API_KEY`

```bash
ASSISTANT_API_KEY=[YOUR_API_KEY]
```
4. Create a client side AssistantCloud instance. Note: this will create an anonymous user id that is tied to the user's browser session. For connecting to an auth provider to persist threads for a user based on a workspace and/or user id, look at [Cloud Authorization Docs](/docs/cloud/authorization).
