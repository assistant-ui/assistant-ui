...
The thread component installed with `npx assistant-ui add thread` shows thumbs up and thumbs down on assistant messages whenever its runtime has a feedback adapter. Assistant Cloud runtimes provide that adapter. After a thumbs down, it offers an optional **What went wrong?** comment with **Send** and **Skip**.
...
The default thread already offers this flow. Use this recipe when a custom thread needs to collect more detail after a thumbs down. It opens a small form and submits the current message's negative rating with the comment.
...
Assistant Cloud stores the comment with the rating and shows it on the thread in the dashboard. A comment requires an Assistant Cloud server that accepts the `comment` field. A self-hosted installation needs the release that carries this change. Rating again without a comment keeps the stored one; switching between thumbs up and thumbs down clears it.
