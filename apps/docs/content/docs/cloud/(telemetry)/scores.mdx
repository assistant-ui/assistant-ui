  ...
  return (
    <form
      onSubmit={(event) => {
        ...
        if (trimmedComment) {
          aui.message.submitFeedback({ type: "negative", comment: trimmedComment });
        } else {
