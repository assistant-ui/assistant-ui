...
export default function Chat() {
  ...
  return (
    <div>
      {/* Thread list */}
      <ul>
        {threadStore.threads.map((t) => (
          <li key={t.id} onClick={() => threadStore.selectThread(t.id)}>
            {t.title || "New conversation"}
          </li>
        ))}
        <li onClick={() => threadStore.selectThread(null)}>New chat</li>
      </ul>
...
| `threadStore` | `UseThreadsResult` | Thread management (see below) |
...
| Value | Type | Description |
|-------|------|-------------|
| `threadStore.threads` | `CloudThread[]` | Active threads sorted by recency |
| `threadStore.threadId` | `string \| null` | Current thread ID (`null` for a new unsaved chat) |
| `threadStore.selectThread` | `(id: string \| null) => void` | Switch threads or pass `null` for a new chat |
| `threadStore.isLoading` | `boolean` | `true` during initial load or refresh |
| `threadStore.error` | `Error \| null` | Last error, if any |
| `threadStore.refresh` | `() => Promise<boolean>` | Re-fetch the thread list |
| `threadStore.delete` | `(id: string) => Promise<boolean>` | Delete a thread |
| `threadStore.rename` | `(id: string, title: string) => Promise<boolean>` | Rename a thread |
| `threadStore.archive` | `(id: string) => Promise<boolean>` | Archive a thread |
| `threadStore.unarchive` | `(id: string) => Promise<boolean>` | Unarchive a thread |
| `threadStore.generateTitle` | `(threadId: string) => Promise<string \| null>` | Generate a title using AI |
...
**Returns:** `UseThreadsResult` — same shape as `threadStore` from `useCloudChat()`.
