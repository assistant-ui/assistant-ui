...
| `threads.generateTitle` | `(threadId: string, options?: { automatic?: boolean }) => Promise<string \| null>` | Generate a title using AI; `automatic` preserves a concurrent manual rename |
