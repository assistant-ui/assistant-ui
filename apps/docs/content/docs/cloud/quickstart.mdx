...
With `NEXT_PUBLIC_ASSISTANT_BASE_URL` set, the React runtimes create an anonymous cloud client on their own when you pass no `cloud`. The other platforms construct the client explicitly, as the next step shows.
