...
Everything in Assistant Cloud hangs off a project. A project has two hosts, a plan and a set of settings; inside it, workspaces own threads, threads hold messages, and every assistant response leaves a run with its spans, a few events and, when someone rates it, a score. This page names those objects once, so the other pages can use the words without explaining them again.
...
| Your provider's token | The token's `sub` | The claim the auth rule names as the workspace, `sub` when the rule uses the user id, so a workspace claim lets users share threads |
