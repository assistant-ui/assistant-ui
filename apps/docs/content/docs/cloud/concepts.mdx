...
| Your provider's token | The token's `sub` | The token's `sub` as well: rules created in the dashboard take the workspace from it, so each user sees their own threads |
