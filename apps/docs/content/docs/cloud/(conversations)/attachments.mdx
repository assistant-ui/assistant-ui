...
`generatePresignedUploadUrl` returns `{ success, signedUrl, expiresAt, publicUrl, key }`. `generatePresignedDownloadUrl` accepts either `{ key }` or the legacy `{ url }` form and returns `{ signedUrl, expiresAt, key }`.
