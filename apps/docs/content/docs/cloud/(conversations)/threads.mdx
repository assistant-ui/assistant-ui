...
`useCloudThreadListAdapter` accepts the cloud client plus the callbacks that connect a cloud thread to your application. The callback options are committed when the adapter is created, so changing either callback later does not rebuild it.
...
const threadListAdapter = useCloudThreadListAdapter({
  ...
  delete: async (threadId) => {
    await removeConversationFromApplication(threadId);
  },
});
```
