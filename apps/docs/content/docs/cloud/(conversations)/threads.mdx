...
| `upsert` | `false` | A boolean | Set to `true` when each stable `externalId` identifies one conversation; retries reuse the workspace's existing thread. Requires a Cloud API version that supports `upsert`; older self-hosted servers may reject the field. |
