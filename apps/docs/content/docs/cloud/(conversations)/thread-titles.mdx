...
| Model | The first model on the provider's Models list | Any model id, typed or picked from the provider's list, which is ranked for tool calling; see [how model discovery works](/docs/cloud/llm-providers#how-model-discovery-works). Saving with titles on is refused with `Choose a model for this provider` only when the field is empty and the provider's list is too. | The model that writes the title. |
