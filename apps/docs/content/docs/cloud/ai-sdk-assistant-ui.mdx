...
- `outcome_type` (`"length"`, `"content_filter"`) is read from `finishReason` in the message metadata (`finishReason: part.finishReason` below); a failed AI SDK run also reports its `error` and `error_code` from the terminal message; `"aborted"` and `"disconnected"` are reported by `useCloudChat` only
- `input_tokens` / `output_tokens`: token usage statistics

`useChatRuntime` reports `first_token_ms` and failed-run `error` with `error_code` from the terminal runtime message. `useCloudChat` reports the same fields from its client-side run state.
