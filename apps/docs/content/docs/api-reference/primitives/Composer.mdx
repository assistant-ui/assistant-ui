...
Renders a button to start speech recognition (dictation) to convert voice to text.
...
Renders a button to stop the current speech recognition session.
...
<ComposerPrimitive.If listening>
  <div className="listening-preview">
    <ComposerPrimitive.ListeningTranscript className="italic text-muted" />
  </div>
</ComposerPrimitive.If>
```
...
<ParametersTable
  ...
  parameters={[
    ...
    },
    {
      ...
      required: false,
      type: "boolean | undefined",
      description: "Render children if speech recognition (dictation) is active.",
    },
  ]}
/>
