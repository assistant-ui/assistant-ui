...
A generic export button that calls a custom export handler with the message content.

This primitive renders a `<button>` element unless `asChild` is set.

<ParametersTable
  type="ActionBarPrimitiveExportProps"
  parameters={[
    {
      name: "asChild",
    },
    {
      ...
      required: true,
      description:
        "A callback function that receives the message content. Use this to implement custom export logic (PDF, JSON, TXT, etc.).",
    },
  ]}
/>
...
Exports the message content as a Markdown file download. This is a convenience wrapper around `Export` with built-in Markdown download logic.
...
<ParametersTable
  ...
  parameters={[
    ...
    },
    {
      ...
      type: "string",
      description:
        "The filename for the downloaded Markdown file. Defaults to 'message-{timestamp}.md'.",
    },
  ]}
/>
