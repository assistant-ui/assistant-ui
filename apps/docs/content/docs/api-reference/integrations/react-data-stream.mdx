...
Create a runtime that connects to Assistant Cloud using the data stream protocol. This hook provides integration with Assistant Cloud's thread management, authentication, and persistence features.
...
- **Thread Management**: Automatic thread creation and persistence
- **Authentication**: Support for anonymous, JWT, and API key authentication
- **Error Handling**: Built-in error handling with customizable callbacks
- **Performance**: Optimized for production use with memoization support
- **Security**: Secure communication with Assistant Cloud infrastructure

**When to Use:**

- Building applications that use Assistant Cloud
- Need persistent thread management across sessions
- Require user authentication and authorization
- Want to leverage Assistant Cloud's infrastructure
...
**Note:** This hook inherits all parameters from `useDataStreamRuntime` in addition to the cloud-specific parameters listed below.
  ...
  parameters={[
    {
      ...
      type: "AssistantCloud",
      description:
        "The Assistant Cloud instance. Must be configured with proper authentication (anonymous, JWT, or API key).",
    },
    {
      name: "assistantId",
      type: "string",
      description:
        "The ID of the assistant to connect to. Found in your Assistant Cloud project settings under the assistant configuration.",
    },
    {
      name: "onResponse",
      type: "(response: Response) => void | Promise<void>",
