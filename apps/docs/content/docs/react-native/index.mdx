...
The fastest way to get started with assistant-ui for React Native. The scaffold is an Expo Router app with the assistant-ui elements installed from the registry (thread, composer, attachments, thread list) and the AI SDK runtime wired up.
...
Wrap your Metro config. `withAui` from `@assistant-ui/metro` is only needed when you ship `"use generative"` toolkits; leave it out otherwise.
...
module.exports = withUniwindConfig(withAui(getDefaultConfig(__dirname)), {
