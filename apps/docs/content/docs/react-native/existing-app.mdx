...
Keep your header, your navigation and your runtime provider. Render `Thread` where the message list used to be, and style its input through the `ComposerInput` slot: `ComposerPrimitive.Input` takes your `StyleSheet` styles and keeps the composer binding, the IME guard and the growing textarea on web, and Enter to send there; on a device the thread's send button submits. The slot mounts in the new message composer and in the edit composer. A different editor altogether (rich text, mentions) is the case the [elements guide](/docs/react-native/elements#thread-slots) shows. `useThemeTokens` below stands for whatever hook returns your `StyleSheet` theme:
