...
The elements also read `thread.capabilities`: when your runtime has no reload, edit or branch switching, those controls render disabled, so a runtime that only implements `onNew` gets a working thread.

<Callout title="One copy of every assistant-ui package">
`@assistant-ui/react-native` links `@assistant-ui/core`, `@assistant-ui/store` and `@assistant-ui/tap` at matching versions, and the elements and your runtime must resolve the same copy of each. Upgrade them together, and check that `npm ls @assistant-ui/tap` (or `pnpm why @assistant-ui/tap`) shows one version.
</Callout>
...
The elements are styled through the sixteen `--color-*` tokens that the [scaffold's `global.css`](/docs/react-native#set-up-uniwind) declares. Declare the same set under `@layer theme` with the light and dark variants, using the values your `StyleSheet` theme returns, so every element, now or later, finds its colors:
...
The tokens follow the OS color scheme by default, the same source `useColorScheme` reads, so a `StyleSheet` screen and an element next to it switch together. An app with its own theme toggle calls `Uniwind.setTheme("dark")` from `uniwind` alongside its existing switch.
...
Keep your header, your navigation and your runtime provider. Render `Thread` where the message list used to be, and hand it your own input through the `ComposerInput` slot when the built in one is not enough (a rich text editor, mentions, your own keyboard accessory). The slot mounts in the new message composer and in the edit composer, and the composer state is one `useAui()` away. `useThemeTokens` below stands for whatever hook returns your `StyleSheet` theme:
...
function ComposerInput() {
  ...
  return (
    <TextInput
      ...
      onSubmitEditing={() => aui.composer.send()}
...
const styles = StyleSheet.create({
  input: { minHeight: 40, paddingHorizontal: 10, paddingVertical: 4, fontSize: 16 },
});
```
