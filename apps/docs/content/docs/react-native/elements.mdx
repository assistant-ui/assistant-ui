...
| File, image | shipped; files show payload size and open http(s) URLs only, and images add a loading placeholder plus tap to zoom |
| Typing indicator, error state, stopped run, approval card, agent status, tool timeline, message queue | shipped, same props |
| Reasoning | shipped, with the same disclosure state machine and live preview; native uses no animation, swaps chevrons instead of rotating them, and omits the web fades |
