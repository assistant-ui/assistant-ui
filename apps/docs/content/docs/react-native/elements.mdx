...
`Rail` overlays the message list with a component that positions itself, and the thread hands it what the list shows through `useThreadViewport`: the ids of the messages on screen, how far the last screenful has been scrolled, the list's height, and `scrollToMessage`. The conversation map's runtime binding is built for that slot: install `conversation-map` and pass it through.
