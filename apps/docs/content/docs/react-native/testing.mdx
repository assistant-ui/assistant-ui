...
Run the suite with your app's Vitest command. The kit itself uses a named `react-native` project in `packages/ui/vitest.config.ts`, which keeps these tests apart from its web and Vue projects.
...
vi.mock("react-native", async (importOriginal) => {
  ...
  const React = await import("react");
  const Image = ({ source, testID }: { source?: { uri?: string }; testID?: string }) =>
...
vi.mock("react-native", async (importOriginal) => {
  ...
  const View = ({ children, className, testID, accessibilityLabel }: {
...
Keep that mock local to the test that needs DOM inspection. A real `View` remains the better choice when you only need text or a press handler.
