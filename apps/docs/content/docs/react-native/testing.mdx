...
The mock forwards every prop it does not name and fires `onLayout` once, because an element that measures itself waits for that event before it renders its final state. Keep the mock local to the test that needs DOM inspection. A real `View` remains the better choice when you only need text or a press handler.
