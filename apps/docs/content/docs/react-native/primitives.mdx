...
`Pressable` for adding attachments. It is disabled while the composer cannot accept attachments. React Native has no file input, so `onPress` receives the platform picker (for example `expo-document-picker` or `expo-image-picker`) and hands the selection to `aui.composer.addAttachment`.
