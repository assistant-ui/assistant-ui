...
`FlatList`-based message list with automatic runtime integration. This uses `ThreadPrimitive.MessagesFlatList` internally and keeps the same backwards-compatible name as earlier React Native releases.

It now auto-scrolls by default. If your app already manages message-list scrolling, pass `autoScroll={false}` or disable the individual `scrollToBottomOn*` options.
...
| `...rest` | `ThreadPrimitive.MessagesFlatList` props | Same props as `ThreadPrimitive.MessagesFlatList` |

### ThreadPrimitive.MessagesFlatList

React Native message viewport backed by `FlatList`. It scopes each row to its message, forwards refs to the underlying `FlatList`, and handles scroll-to-bottom behavior for common chat flows.
