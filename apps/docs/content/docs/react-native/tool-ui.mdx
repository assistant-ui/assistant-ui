...
An approval renderer receives `approval` and `respondToApproval`. Render the controls only while `approval.approved` is undefined and no `resolution` is recorded, since a cancelled or expired request leaves `approved` unset but can no longer be answered. When the host lists options, answer with the matching `optionId` so the decision keeps the requested scope, and offer "always allow" only when an `allow-always` option exists: a declared option list is a host constraint, so the card never adds an approval path beyond it. Without options, a boolean response records a one-time decision. `respondToApproval` rejects when the runtime could not record the decision; the card stays mounted, so catching the rejection leaves the user free to answer again.
...
export const RunCommandApproval: ToolCallMessagePartComponent<{
  ...
  const optionOf = (kind: string) =>
    approval.options?.find((candidate) => candidate.kind === kind);
  const respond = async (kind: string, approved: boolean) => {
    const option = optionOf(kind);
    try {
      await respondToApproval(option ? { optionId: option.id } : { approved });
    } catch {
      return;
    }
  };
  ...
  return (
    <ApprovalCard
      ...
      onAllowOnce={() => respond("allow-once", true)}
      {...(allowAlways && {
        onAlwaysAllow: () => respond("allow-always", true),
      })}
      onDeny={() => respond("reject-once", false)}
    />
  );
};
```
