...
Use this adapter in the `attachments` slot instead of the image adapter when this is the only accepted file type. Combine it with `SimpleImageAttachmentAdapter` in `CompositeAttachmentAdapter` when one composer accepts both.
