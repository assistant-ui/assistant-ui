...
## Enable the runtime capability

Attachments are enabled by the `attachments` adapter slot. A core local runtime takes that slot through `useLocalRuntime`.
...
`CompositeAttachmentAdapter` chooses the first adapter whose `accept` value matches the file. `SimpleImageAttachmentAdapter` accepts `image/*`, and `SimpleTextAttachmentAdapter` accepts the text formats in its exported accept string.

Use the same `attachments` slot when a remote thread host provides adapters through `RuntimeAdapterProvider`. The adapter makes the attachment capability available to the composer, while the installed image picker adds its already encoded image descriptors directly.
...
import { ComposerPrimitive, useAui } from "@assistant-ui/react-native";
