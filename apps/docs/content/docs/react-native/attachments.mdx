...
The built in `SimpleImageAttachmentAdapter` and `SimpleTextAttachmentAdapter` read a pending file through `FileReader`, which on React Native needs a real `Blob`. An Expo picker result is a plain object with a `uri`, so hand those adapters a Blob backed file if you want to use them for a picker of your own: `new File([await fetch(asset.uri).then((r) => r.blob())], asset.name, { type: asset.mimeType })`. The document adapter on this page reads the URI itself instead.
