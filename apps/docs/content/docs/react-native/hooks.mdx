...
Import the toolkit hook, hoist `const config = AuiConfig({ tools: Tools({ toolkit }) })` in the component body, and mount it with `<AuiProvider aui={useAui()} config={config}>`, as shown in the [Tools](#tools) section above.
