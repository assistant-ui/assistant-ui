...
This is what the shadcn Thread component uses by default. When `turnAnchor="top"` is set, `MessagePrimitive.Root` automatically registers the latest assistant message as the top-anchor target and the preceding user message as the anchor — no additional component is required. The viewport itself manages a stable reserve element that provides the missing scroll range while the assistant response grows.

Use `fillClampThreshold` and `fillClampOffset` to control how much of a long user message remains visible when `turnAnchor="top"`. Messages up to `fillClampThreshold` stay fully visible. For messages taller than `fillClampThreshold`, `fillClampOffset` controls how much of the message's bottom edge remains visible above the assistant response:
...
<Callout type="warn">
  `ThreadPrimitive.ViewportSlack` is deprecated and is now a no-op pass-through. Top-anchor target registration is handled automatically by `MessagePrimitive.Root` when `turnAnchor="top"`. Remove `ViewportSlack` from your tree; if you need to tune the tall-user clamp, pass `fillClampThreshold` and `fillClampOffset` directly to `ThreadPrimitive.Viewport`.
</Callout>
