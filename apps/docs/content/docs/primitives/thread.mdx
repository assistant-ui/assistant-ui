...
<PrimitivePartsTable primitive={ThreadPrimitiveDocs} />
...
Adds min-height for scroll anchoring with `turnAnchor="top"`. Renders a `<div>` element unless `asChild` is set.

```tsx
<MessagePrimitive.Root>
  <MessagePrimitive.Parts />
  <ThreadPrimitive.ViewportSlack className="min-h-[40vh]" />
</MessagePrimitive.Root>
```
