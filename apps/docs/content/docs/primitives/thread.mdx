...
<Callout type="warn">
  `ThreadPrimitive.ViewportSlack` has been removed from the public API. Top-anchor target registration is handled automatically by `MessagePrimitive.Root` when `turnAnchor="top"`. Remove `ViewportSlack` from your tree; if you need to tune the tall-message clamp, pass `topAnchorMessageClamp` directly to `ThreadPrimitive.Viewport`.
</Callout>
