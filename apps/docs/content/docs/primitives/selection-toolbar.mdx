...
`Root` renders via a portal to `document.body`. In the current implementation, it positions itself with `position: fixed`, `top: rect.top - 8px`, centered horizontally from the selection rect, and `z-index: 50`.
