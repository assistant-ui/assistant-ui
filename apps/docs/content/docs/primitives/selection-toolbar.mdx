...
The SelectionToolbar primitive is a floating toolbar that appears when the user selects text within a message. It lets users quote selected text into the composer. The toolbar positions itself automatically above the selection — you provide the styling and actions.
...
`Root` renders via a portal to `document.body` and positions itself automatically — 8px above the selection, horizontally centered. No manual positioning is needed. The toolbar uses `position: fixed` with `z-index: 50`.
...
|------|---------|---------|
| `Root` | `<div>` (portal) | Floating container, auto-positioned above selection |
| `Quote` | `<button>` | Quotes the selected text into the composer |
...
For full prop details on every part, see the [SelectionToolbarPrimitive API Reference](/docs/reference/primitives/selection-toolbar).
