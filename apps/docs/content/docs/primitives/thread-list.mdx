...
A working thread list in its simplest form:
...
`Archive`, `Unarchive`, and `Delete` buttons auto-disable when the capability isn't available from the runtime. No manual checking required.
...
The shadcn [ThreadList](/docs/ui/thread-list) component wraps these primitives with Tailwind styling in a sidebar layout, including a skeleton loading state and a pre-built more menu with archive and delete actions. Start there for a working thread list out of the box. Reach for the primitives directly when you need a custom layout, different actions, or integration with your own sidebar or navigation system.
