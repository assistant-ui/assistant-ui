...
For scroll-driven loading, wrap the `useThreadListLoadMore` hook in your own `IntersectionObserver` at the application layer; assistant-ui ships the explicit button to keep the primitive surface predictable.
