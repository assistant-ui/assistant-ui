...
| `Content` | — | Legacy alias for `Parts` |
...
| `Unstable_PartsGrouped` | — | Groups consecutive parts by a custom grouping function *(unstable)* |
| `Unstable_PartsGroupedByParentId` | — | Groups parts by parent ID *(unstable, deprecated — use `Unstable_PartsGrouped`)* |
...
- `MessagePrimitive.If` is deprecated. Prefer `AuiIf`.
