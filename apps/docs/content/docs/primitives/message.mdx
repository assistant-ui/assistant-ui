...
4. **`tools.mcpApp`**: the generic MCP App renderer registered via `Tools({ mcpApp })`, which claims only a call whose `mcp.app.resourceUri` is a `ui://` app
5. **`tools.Fallback`**: catch-all for unmatched tool calls from the deprecated `components` prop
6. **`part.toolUI`**: the resolved tool UI exposed directly in the children render function
