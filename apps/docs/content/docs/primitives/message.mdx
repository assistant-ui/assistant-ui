...
The older `components` prop is still supported, but it is deprecated. Prefer the children API for new code.
...
The deprecated `components` prop is still useful when you want grouped reasoning/tool rendering or inline by-name registries:
...
For new code, prefer the children API first, then drop down to `components` only when you need these grouping features.
...
<MessagePrimitive.Attachments>
  {({ attachment }) => {
    if (attachment.type === "image")
      return <img alt={attachment.name} className="max-w-xs rounded-lg" />;
