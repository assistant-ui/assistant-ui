...
<PrimitivePartsTable primitive={AssistantModalPrimitiveDocs} />
...
Button that toggles the modal open and closed. Renders a `<button>` element.
...
Positions the trigger and content relative to a shared anchor. Renders a `<div>` element.
