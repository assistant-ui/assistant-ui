...
const schemas = toToolsJSONSchema(tools, { filter: () => true });
```

`toToolsJSONSchema` filters out `disabled` and `backend` tools by default, so most callers can drop the `filter` option entirely.
