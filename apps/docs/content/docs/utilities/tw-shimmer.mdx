...
Uses modern CSS features: `@property`, relative `oklch()`, `color-mix()`, `tan()`, and CSS Container Queries.

**Supported:** Chrome 119+, Firefox 128+, Safari 16.4+

Older browsers degrade gracefully. The text shimmer is gated behind `@supports (color: oklch(from red l c h))`, so below the floor the element keeps its normal fill and renders as plain, static text instead of animating. `shimmer-bg` degrades to a static block.
