...
<Callout type="info">
  The clone must be a direct child. The host must be a one-line text box with no padding, border, or other in-flow content. Use `aria-hidden="true"` and `inert` on the clone so assistive technology reads the text one time.
</Callout>
...
Speed controls how fast the shimmer moves in pixels per second. Track width tells the animation how far the highlight travels for timing calculations.

```html
<span class="shimmer shimmer-speed-200 text-foreground/40" style="--shimmer-track-width: 400px">
  Fast, wide shimmer
</span>
```
