...
Older browsers degrade gracefully. The text shimmer is gated behind `@supports (background-clip: text) and (color: oklch(from red l c h))`, so below the floor the element keeps its normal fill and renders as plain, static text instead of animating. `shimmer-bg` degrades to a static block.
