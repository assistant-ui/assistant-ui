...
The inert clone makes the compositor path the default in Chrome and Safari. Firefox keeps the clone hidden and uses the host's `background-position` fallback. One-node markup from earlier releases remains supported through the same fallback.
...
Base utility for text shimmer. Uses the compositor when it contains a direct `shimmer-clone`.

#### `shimmer-clone`

Hidden inert text copy that supplies the glyph mask for `shimmer`.
