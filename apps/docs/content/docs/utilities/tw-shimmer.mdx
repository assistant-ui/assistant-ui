...
The clone must include `aria-hidden="true"` and `inert` because it supplies glyphs for the mask, not accessible content. Reduced motion stops either animation and leaves the host text visible.
...
The default text path uses independent `translate` animation and text masking.

**Supported:** Chrome 111+, Firefox 113+, Safari 16.4+

Chrome and Safari run the optimized compositor animation. Firefox uses the existing `background-position` paint fallback. Older browsers degrade gracefully: unsupported shimmer effects do not appear.
