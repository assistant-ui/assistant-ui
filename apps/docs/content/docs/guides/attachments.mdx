...
Nothing reaches the runtime until every attachment is complete. An attachment still uploading inside `add()` is waited for, so `send()` is never called on an attachment the adapter cannot finalize yet. A send that fails takes its content back into the composer: the text returns ahead of anything typed since, the attachments return with it, and the one that failed carries `status: { type: "incomplete", reason: "error" }` so it can be retried or removed.

The message the composer is sending is available as `composer.submission`, and the thread row that renders it carries the same value as `message.submission`:
