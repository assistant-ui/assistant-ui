...
`acquireLease(streamId, options?)` is optional. It elects exactly like `acquire`, but a `"producer"` result also carries a lease (`{ role: "producer", lease: { token } }`; consumers get `{ role: "consumer" }`). The context prefers it when present and passes the lease to every `append` and `finalize` call from that producer task; `resumableContext.delete(streamId)` is administrative and never carries a lease, so the `delete` lease parameter is only exercised by direct store callers. Store a per-acquisition token with the stream and, when a lease is given, reject `append` with `ResumableStreamError("missing")` and make `finalize` and `delete` no-ops when the token no longer matches. This is what stops a producer whose stream expired from writing into the stream a later `acquire` created under the same `streamId`. A store without `acquireLease` still works through `acquire`, but forfeits that fence: an expired producer that keeps running can append to or finalize its successor's stream.

The target for that check is the same atomicity `acquire` has: compare the token in the same round trip that writes, with a Lua script or `MULTI`/`WATCH` block in Redis, `UPDATE ... WHERE stream_id = ? AND lease_token = ?` in SQL, or the single-threaded object body in Durable Objects. A read-then-write check leaves a window where a reacquisition lands between the compare and the write. The bundled Redis store finalizes through a compare-and-set script today and validates the lease before `append` and `delete` writes; [#7198](https://github.com/assistant-ui/assistant-ui/pull/7198) moves those two behind conditional scripts as well. Its `append` window is contained because data keys are scoped per generation, so a stale write lands in the old producer's orphaned key; a lease-holding `delete` is the call to treat as unfenced until then.
