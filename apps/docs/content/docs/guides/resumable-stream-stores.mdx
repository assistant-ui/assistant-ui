...
`acquireLease(streamId, options?)` is optional. It elects exactly like `acquire`, but a `"producer"` result also carries a lease (`{ role: "producer", lease: { token } }`; consumers get `{ role: "consumer" }`). The context prefers it when present and passes the lease to every `append` and `finalize` call from that producer task; `resumableContext.delete(streamId)` is administrative and never carries a lease, so the `delete` lease parameter is only exercised by direct store callers. Store a per-acquisition token with the stream and, when a lease is given, compare it atomically with the mutation itself, in the same round trip that writes: a Lua script or `MULTI`/`WATCH` block in Redis, `UPDATE ... WHERE stream_id = ? AND lease_token = ?` in SQL, the single-threaded object body in Durable Objects. Reject `append` with `ResumableStreamError("missing")` and make `finalize` and `delete` no-ops when the token no longer matches. A read-then-write check is not enough: a reacquisition can land between the compare and the write, and the stale producer writes into the successor's stream anyway. A backend that cannot compare and write in one step should not implement `acquireLease`, so callers do not mistake it for a fenced store. This is what stops a producer whose stream expired from writing into the stream a later `acquire` created under the same `streamId`. A store without `acquireLease` still works through `acquire`, but forfeits that fence: an expired producer that keeps running can append to or finalize its successor's stream.
