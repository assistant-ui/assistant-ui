...
The target for that check is the same atomicity `acquire` has: compare the token in the same round trip that writes, with a Lua script or `MULTI`/`WATCH` block in Redis, `UPDATE ... WHERE stream_id = ? AND lease_token = ?` in SQL, or the single-threaded object body in Durable Objects. A read-then-write check leaves a window where a reacquisition lands between the compare and the write. The bundled Redis store compares in one step for `finalize` (a compare-and-set script) and validates the lease before the write for `append`, whose window is contained because data keys are scoped per generation, so a stale write lands in the old producer's orphaned key.
