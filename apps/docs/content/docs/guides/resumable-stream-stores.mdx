...
`finalize(streamId, status, error?)` flips the stream into a terminal state. Pending and future `read` iterables drain buffered entries and then either complete (`"done"`) or throw with `error` (`"error"`). Implementations must make `finalize` idempotent: a duplicate call with the same status is a no-op, and the producer task may retry on transient errors. Resolve `false` from a call that finalized nothing, whether because the stream was already terminal or because the lease no longer owns it; the context then skips its `onFinalize` hook for that producer, and reports a superseded `"done"` through `onError` instead. A store that resolves without a value is taken to have finalized, so a store written before this return value keeps working.
