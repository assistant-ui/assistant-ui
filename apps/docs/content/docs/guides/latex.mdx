...
<Callout type="tip">
Inside `MarkdownTextPrimitive`, the raw streamed text first passes through `useSmooth` (character by character accumulation), and the currently revealed text then passes through `preprocess` (delimiter normalization) before reaching the markdown parser. This keeps the smoother's input prefix-stable while allowing delimiter replacement as soon as a complete pair is revealed; a partial delimiter remains in the smoothing buffer rather than being parsed mid-fragment.
</Callout>
