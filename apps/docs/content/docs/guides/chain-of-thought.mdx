...
When a model like OpenAI's `gpt-5.4-mini` responds, it may emit a sequence of reasoning tokens and tool calls before producing its final text answer. Use `MessagePrimitive.GroupedParts` to group those adjacent reasoning and tool-call parts into a single collapsible "thinking" section.
...
Chain of Thought is most useful with models that produce reasoning tokens (e.g. OpenAI `gpt-5.4-mini`). Here's an example backend route using the AI SDK:
