...
Suggestions can be provided as either strings or objects with a required prompt and optional title and label. When title is omitted it falls back to the prompt; when label is omitted it defaults to empty:
...
The static `Suggestions(...)` API covers welcome screens. For follow up prompts that depend on the conversation, a tool result, or your backend, push suggestions through the runtime itself. They land on `thread.suggestions` rather than the static `suggestions` scope, so they render through a different component. Runtime suggestions use the `ThreadSuggestion` shape: a required `prompt` plus optional `title` and `label`, matching the static suggestion format.
