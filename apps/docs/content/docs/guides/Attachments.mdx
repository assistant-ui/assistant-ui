...
Handles audio files and converts them to data URLs for playback in the chat UI:

```tsx
const audioAdapter = new SimpleAudioAttachmentAdapter();
// Accepts: audio/* (MP3, WAV, OGG, etc.)
// Output: CompleteAttachment with content: [{ type: "audio", audio: { data, format } }]
// For unsupported formats, falls back to: [{ type: "file", data, mimeType, filename }]
```

The adapter supports common audio formats including:
- MP3 (audio/mpeg)
- WAV (audio/wav)
- OGG (audio/ogg)
...
Handles video files and converts them to data URLs for playback in the chat UI:

```tsx
const videoAdapter = new SimpleVideoAttachmentAdapter();
// Accepts: video/* (MP4, WebM, OGG, etc.)
// Output: CompleteAttachment with content: [{ type: "file", data, mimeType, filename }]
```

The adapter supports common video formats including:
- MP4 (video/mp4)
- WebM (video/webm)
- OGG Video (video/ogg)
...
Combines multiple adapters to support various file types. Here's an example supporting all media types:
