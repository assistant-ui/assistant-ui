...
`MessagePrimitive.GenerativeUI` is a legacy compatibility primitive for
rendering UI described by the agent at runtime as a JSON spec. New assistant-ui
apps should prefer model-driven Tool UI with `defineToolkit` and
`Tools({ toolkit })`, where the model calls a named tool and that tool's
renderer displays generated UI from its arguments.
...
> **Deprecated:** `MessagePrimitive.GenerativeUI`, `GenerativeUIRender`, and the
> `generative-ui` message part schema are kept for backwards compatibility.
> Prefer `defineToolkit` tool renderers for new generated UI work.
...
|---------|-----|----------|-----------|
| **Tool UI** | `defineToolkit` / `Tools({ toolkit })` | Model-driven generated UI, dashboards, cards, comparisons, forms, pickers, charts | Tool **args** stream while the model fills them in |
| **Legacy Generative UI primitive** | `MessagePrimitive.GenerativeUI` + allowlist | Existing integrations that already emit native `generative-ui` message parts | Native `generative-ui` parts update progressively when the part spec changes incrementally |
| **Standalone Tool UI renderer** | `makeAssistantToolUI` / `useAssistantToolUI` | Rendering tools defined elsewhere (backend, MCP, LangGraph) | Tool **args** stream while the model fills them in |
...
- **New assistant-ui apps** → `defineToolkit` / `Tools({ toolkit })`
