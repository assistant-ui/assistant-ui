...
While a tool is running, its arguments arrive as partial JSON. Use `useToolArgsStatus` (whole-args status) and `useToolArgsFieldStatus` (per-field status) inside a tool UI render function to react to each field as it streams in. Both are exported from `@assistant-ui/react`.

```tsx
import { useToolArgsStatus, useToolArgsFieldStatus } from "@assistant-ui/react";
...
const SearchToolUI = makeAssistantToolUI<{ query: string; limit: number }, unknown>({
  ...
  render: ({ args }) => {
    ...
    const limitStatus = useToolArgsFieldStatus(["limit"]);
    ...
    return (
      <div>
        ...
        {limitStatus.type !== "running" && <span> (limit: {args.limit})</span>}
      </div>
    );
  },
});
...
`propStatus` maps each key to `"streaming"` | `"complete"` once that key appears in the partial JSON stream. `useToolArgsFieldStatus` returns the full status object for a nested path and resolves to `"complete"` once the field is fully parsed.
