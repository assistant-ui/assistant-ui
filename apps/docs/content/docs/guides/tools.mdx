...
The `Tools()` API is the recommended way to register tools in assistant-ui. It provides centralized tool registration, keeps each tool's model schema and renderer together, and works across all runtimes.

<Callout type="warn">
  `makeAssistantTool`, `makeAssistantToolUI`, `useAssistantTool`, and
  `useAssistantToolUI` are deprecated. See the
  [toolkit migration guide](/docs/migrations/toolkit-tools) for the mechanical
  migration.
</Callout>
...
Each key in a toolkit is the tool name the model sees. Each value is a `ToolDefinition` with the tool's model metadata, optional executor, and optional UI renderer:
...
Toolkits are also where you express client-side knowledge about server-defined tools. If your backend already defines a tool's description, parameters, and executor, register a minimal backend entry and only add the client renderer:
...
For frontend tools, put the schema and browser executor in the same entry:
...
Human tools use `type: "human"` and resume when the UI calls `addResult`:
...
Tools are enabled by default. Set `disabled: true` when a tool should remain known to the client but not be exposed to the model for the current scope.
  ...
  search: {
    ...
    render: ({ args }) => {
      ...
      return (
        <div>
          <span
            className={propStatus.query === "streaming" ? "animate-pulse" : ""}
          >
            {args.query ?? "..."}
          </span>
          {propStatus.limit === "complete" && (
            <span> (limit: {args.limit})</span>
          )}
        </div>
      );
    },
...
The component and hook registration APIs are still exported for compatibility, but new code should use toolkits:

- `makeAssistantTool` -> toolkit entry with `execute`/`render`
- `useAssistantTool` -> toolkit entry registered through `useAui({ tools: Tools({ toolkit }) })`
- `makeAssistantToolUI` -> toolkit entry with `render`
- `useAssistantToolUI` -> toolkit entry with `render`, or an inline `MessagePrimitive.Parts` override for one message tree

See [Migrating Tools to Toolkits](/docs/migrations/toolkit-tools) for before/after examples, including dynamic tools that depend on React state.
...
1. **Use toolkits for new code**: keep a tool's schema, executor, disabled state, and renderer in the same `Tools({ toolkit })` registration
...
## Migrating from Component-Based APIs
