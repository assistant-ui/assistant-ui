...
While the AI is generating tool arguments, `useInteractableState` reflects the partial state in real time. You can use this to show a loading or skeleton UI while the AI is mid-stream:

```tsx
function TaskBoard() {
  const id = useAssistantInteractable("taskBoard", config);
  const [state, { isPending }] = useInteractableState(id, taskBoardInitialState);

  return (
    <div>
      <h2>Tasks {isPending && <span className="animate-spin">⋯</span>}</h2>
...
`isPending` is true while a persistence save is in-flight. For streaming-driven skeleton state (before any tasks exist), check `state.tasks.length === 0` alongside the running status from your runtime.
