...
Of the shipped runtimes, the AI SDK runtime implements `onVoiceTranscript`: it appends each transcript to the `useChat` messages, where the history adapter stores it like a typed turn. `react-a2a`, `react-ag-ui`, `react-google-adk`, `react-langchain`, `react-langgraph`, and `react-opencode` accept a voice adapter but do not implement it, so on those runtimes transcripts render for the session only, never reach the provider thread, a history adapter, or the model, and are dropped on disconnect.
