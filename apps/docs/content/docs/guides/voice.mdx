...
While a session is connected the composer cannot send: `canSend` is false and `thread.append` rejects with "Cannot send a text message while a voice session is connected". Runs, including tool result continuations, edits, and reloads are rejected while connected as well. Hanging up commits the sentence the assistant was speaking. A queue on the local runtime holds its items while a session is connected and resumes after disconnect; a queue owned by an external store host is the host's to pause. Disconnect first; the session is the only input for its duration.
