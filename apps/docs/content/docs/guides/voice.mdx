...
A finalized transcript is an ordinary message from then on. The local runtime writes it to the repository and to the history adapter as it finalizes, so it persists, reloads, and reaches the model on the next run like a typed turn. An external store runtime hands each finalized transcript to `onVoiceTranscript`; a host that does not implement it keeps transcripts for the session only, and `disconnectVoice` drops them.

While a session is connected the composer cannot send: `canSend` is false and `thread.append` rejects with "Cannot send a text message while a voice session is connected". Disconnect first; the session is the only input for its duration.
