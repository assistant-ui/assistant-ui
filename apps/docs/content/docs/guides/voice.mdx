...
While a session is connected the composer cannot send: `canSend` is false and `thread.append` rejects with "Cannot send a text message while a voice session is connected". Edits and reloads are rejected while connected as well. On the local runtime so are runs and tool result continuations; an external store forwards tool results and approvals to the host, which decides. Hanging up commits the sentence the assistant was speaking. A queue on the local runtime holds its items while a session is connected and resumes after disconnect; a queue owned by an external store host is the host's to pause. Disconnect first; the session is the only input for its duration.
