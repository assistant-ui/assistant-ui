...
- Both carry `metadata.modality: "voice"`, so the action bar hides edit and reload on spoken turns and your own components can tell a spoken message from a typed one.
