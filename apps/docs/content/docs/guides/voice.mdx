...
Where a finalized transcript goes depends on the runtime. Session only means the transcripts render while the session is connected, never reach the provider thread, a history adapter, or the model, and are dropped on disconnect.
