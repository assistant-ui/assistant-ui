...
A producer fenced out of its stream, because the stream expired and a later `run` reacquired the same `streamId`, reports once through `onError` with a `ResumableStreamError` whose `code` is `"missing"`, and never through `onFinalize`; the `onFinalize` for that stream comes from the producer that finalized it.
