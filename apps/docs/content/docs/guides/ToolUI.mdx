...
With assistant-ui, you can show a custom UI when a tool is called to let the user know what is happening. In this example, we'll be experimenting with a tool call for web search.

<ToolUISample />

### Frontend & Backend Example With Vercel AI SDK

In this example, we'll illustrate a tool for web search that shows a weather UI when the user asks about the weather in Tokyo. The tool UI displays a loading state while fetching data, then shows the current temperature with appropriate day/night icons. This demonstrates how you can create interactive, visually appealing tool UIs that enhance the user experience during tool execution.
