...
<Callout>
Using Mastra? Check out the [Mastra Generative UI guide](/docs/runtimes/mastra/tool-ui-example) for specific examples and patterns.
</Callout>
...
Use `makeAssistantToolUI` to create a reusable tool UI component:
...
const WeatherToolUI = makeAssistantToolUI<WeatherArgs, WeatherResult>({
  ...
  render: ({ args, status, result }) => {
    ...
    return (
      <div className="weather-card p-4 rounded-lg bg-blue-50">
        <h3 className="font-bold text-lg">{args.location}</h3>
        <div className="grid grid-cols-2 gap-4 mt-2">
          <div>
            <p className="text-2xl">{result.temperature}°{args.unit === "celsius" ? "C" : "F"}</p>
...
Create standalone tool UI components that auto-register when mounted:
    ...
    render: useInlineRender(({ args, result }) => {
      ...
      return (
        <div className="inventory-status">
          ...
          <p>Stock for {productId}: {result.quantity} units</p>
...
>({
  ...
  render: ({ args, result, addResult }) => {
    ...
    return (
      <div className="p-4 border-2 border-yellow-400 rounded">
...
<Thread
  components={{
    ToolFallback: ({ toolName, args, result }) => (
      <div className="tool-fallback p-3 bg-gray-100 rounded">
