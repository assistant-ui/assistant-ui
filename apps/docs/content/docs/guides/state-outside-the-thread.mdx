...
Render from state and use events for side effects such as a toast, a sound, or an analytics call. `useAuiEvent` resolves the scope in the subscriber's own context and throws when it is absent there, so a component that is not under a `thread` scope listens at the root with `{ scope: "*", event }`.
