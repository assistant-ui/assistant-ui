...
import { AuiProvider, Tools, type Toolkit, useAui, useInlineRender } from "@assistant-ui/react";
...
function HeavyComputationToolProvider({
  children,
}: {
  children: ReactNode;
}) {
  ...
  weather: {
    ...
    render: ({ args }) => {
      ...
      return (
        <div>
          <span
            className={propStatus.location === "streaming" ? "animate-pulse" : ""}
          >
            {args.location ?? "..."}
          </span>
