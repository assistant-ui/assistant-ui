...
Use `useToolArgsFieldStatus` to show validation states for individual fields during streaming. It is exported from `@assistant-ui/react` and must be called inside a tool-call message part context:
