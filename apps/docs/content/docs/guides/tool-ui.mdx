...
For the canonical `useToolArgsStatus` pattern, see [Field-Level Streaming State](#field-level-streaming-state). That section covers how `propStatus` tracks each streamed argument field and how to call the hook inside a tool-call message part context.
