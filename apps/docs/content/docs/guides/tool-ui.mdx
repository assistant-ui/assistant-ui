...
```tsx
import { Tools, type Toolkit, useAui } from "@assistant-ui/react";
import { z } from "zod";
...
function MyRuntimeProvider({ children }: { children: React.ReactNode }) {
  ...
  return <AssistantRuntimeProvider aui={aui}>{children}</AssistantRuntimeProvider>;
}
```
...
Use a local toolkit when the renderer needs component state:

```tsx
import { AuiProvider, Tools, useAui, useInlineRender } from "@assistant-ui/react";
...
The same `render` shape works inside the [`Tools()`](/docs/guides/tools) toolkit's `render` field and with `MessagePrimitive.Parts`'s inline `tools.by_name` overrides.
...
- **Tool UI**: the **model** decides what to render by calling a tool whose args become the component's data. Register the renderer via the [`Tools()`](/docs/guides/tools) toolkit's `render` field. Args stream incrementally, so you observe partial state via `status` / `useToolArgsStatus` and may need [Deferred Rendering](#deferred-rendering) for components that should only mount with final data.
