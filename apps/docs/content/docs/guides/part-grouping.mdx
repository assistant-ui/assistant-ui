...
When using assistant-stream libraries, you can give related parts a shared parent ID:
...
const stream = createAssistantStream(async (controller) => {
  ...
  researchController.addToolCallPart({
    toolName: "search",
    args: { query: "climate data" },
    response: { result: "climate data results" },
  });
