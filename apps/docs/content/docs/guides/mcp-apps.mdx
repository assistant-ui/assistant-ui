...
let mcpClientPromise: ReturnType<typeof experimental_createMCPClient> | undefined;
const getMcpClient = () => {
  mcpClientPromise ??= experimental_createMCPClient({
...
const mcpAppOptions: McpAppRendererOptions = {
  loadResource: async (app) => {
    ...
    const { contents } = await client.readResource({ uri: app.resourceUri });
