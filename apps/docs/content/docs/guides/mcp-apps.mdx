...
import { Tools, McpAppRenderer } from "@assistant-ui/react";
...
`callTool`, `readResource`, `listResources`, and the resource fetch are auto-wired to `host.url`. `openLink` defaults to `window.open(url, "_blank", "noopener,noreferrer")`. `sendMessage` defaults to appending a user message to the current thread (accepts `string`, `{ prompt }`, `{ text }`, or `{ message }`). Override or extend via `handlers` for `requestDisplayMode`, `updateModelContext`, or lifecycle events.
...
export async function POST(req: Request) {
  ...
  switch (method) {
    ...
    case "tools/call":
      return Response.json(
        await client.callTool({ name: params.name, arguments: params.arguments }),
      );
    case "resources/read":
