...
`callTool`, `readResource`, `listResources`, and the resource fetch are auto-wired to `host.url`. `handlers` is reserved for capabilities that must run in the browser (`openLink`, `sendMessage`, `requestDisplayMode`, lifecycle events).
