...
`createResumableSessionStorage` returns a `ResumableClientStorage` backed by `window.sessionStorage`. Pass `{ key }` to namespace per route or per chat surface; `key` also accepts a getter that is read lazily on every access, so you can derive it from the active thread's id (see [Multiple threads](#multiple-threads)). Or supply your own implementation of the three methods (`getStreamId`, `setStreamId`, `clear`). If you are running on a transport that already wraps `fetch` or `prepareReconnectToStreamRequest`, the `resumable` option composes with your existing handlers.
...
Scope the key to the thread, and construct the transport and storage per thread inside the per-thread runtime hook rather than once at module level. Derive the key from the thread list item's id with a getter so it is read lazily on every access:
...
const threadListAdapter: RemoteThreadListAdapter = {
  /* list, initialize, rename, archive, delete, fetch, … */
};
  ...
  const transport = useMemo(
    () =>
      new AssistantChatTransport({
        api: "/api/chat",
        resumable: {
          storage: createResumableSessionStorage({ key: () => threadId }),
          resumeApi: (streamId) => `/api/chat/resume/${streamId}`,
        },
      }),
    [threadId],
...
Each thread's runtime hook constructs its own transport and storage, keyed by that thread's id, so one conversation's stream id can never be read or cleared by another. `useChatRuntime` is a no-op thread list when nested as a `runtimeHook`, so it only runs the per-thread chat runtime against the per-thread transport; the real thread list is the outer `useRemoteThreadListRuntime`. Key by the local `id` (`s.threadListItem.id`), which is stable from mount (so a static `key` works too) — avoid `remoteId`, which is `undefined` until the thread's first send initializes it and would fall back to the shared default key, reintroducing the cross-wire for new threads. The getter reads the id lazily on every access.
