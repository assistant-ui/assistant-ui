...
`createResumableSessionStorage` returns a `ResumableClientStorage` backed by `window.sessionStorage`. Pass `{ key }` to namespace per route or chat surface; the built-in storage automatically keeps each thread's pending stream id separate. An idle follower that learns a stream id from its backend can call `storage.setStreamId(streamId, threadId)` after the corresponding message exists in local history to attach without remounting. Custom storage can support the same behavior by implementing the optional `subscribe(listener, threadId)` method. A custom storage shared by multiple threads must honor the optional `threadId` arguments on its methods; alternatively, use one storage instance per thread. If you are running on a transport that already wraps `fetch` or `prepareReconnectToStreamRequest`, the `resumable` option composes with your existing handlers.
