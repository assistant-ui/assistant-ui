...
`onResumeError` runs when the client finds a stored stream id but the reconnect attempt fails. Use it to show a toast, report telemetry, or mark the thread as needing retry. A transient error keeps the stream id so manual Resume can retry; automatic reconnect attempts that stream only once per mount. A 204 or 404 from the resume endpoint clears the matching stream id.
...
The default finish detector scans the SSE body for the AI SDK `"type":"finish"` marker. Override `isFinishEvent` on the `resumable` option when you ship a custom encoder. Without a matching finish event, storage retains the stream ID after an otherwise completed response, so the composer can continue offering a stale Resume action.

### Stop and resume in the composer

With the resumable transport above, pressing **Stop generating** retains the
stream ID. Automatic reconnect after a reload still uses `chat.resumeStream()`.
For a manual **Resume generating** control, pass `canResume: true` only when
the server guarantees the resumed stream's `start.messageId` matches the
interrupted assistant message. Some AI SDK responses omit that ID; replaying
one of those streams after Stop can append a second answer instead of
continuing the partial one. In that case, leave manual Resume disabled. When
the server emits `finish`, storage is cleared.

Typing a new draft keeps Send available. Resumable storage alone does not
expose manual Resume. Custom adapters can opt in with
