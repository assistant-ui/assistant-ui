...
`createResumableSessionStorage` returns a `ResumableClientStorage` backed by `window.sessionStorage`. Pass `{ key }` to namespace per route or per chat surface, or supply your own implementation of the three required methods (`getStreamId`, `setStreamId`, `clear`). The built-in storage notifies `useChatRuntime` when `storage.setStreamId(id)` is called after mount. An idle follower that learns a stream id from its backend can call this method after the corresponding message exists in local history to attach without remounting. Custom storage can support the same behavior by implementing the optional `subscribe(listener)` method. If you are running on a transport that already wraps `fetch` or `prepareReconnectToStreamRequest`, the `resumable` option composes with your existing handlers.
