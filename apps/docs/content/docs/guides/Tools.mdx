...
Create a complete tool with execution logic and optional UI. Returns a React component that registers the tool when rendered:
...
- **AI SDK**: Tools defined in API routes with `streamText({ tools: {...} })`
...
- **Custom Runtime**: Tools passed via adapter context

All runtimes support the same frontend tool UI components. See the [runtime documentation](/docs/runtimes) for specific examples.
