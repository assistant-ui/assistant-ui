...
**Reading a whole scope:** pass a scope name instead of a selector to read that scope's state from its React context. The component re-renders whenever the scope's state changes.

```tsx
const { isRunning, messageIds } = useAuiState("thread");
const { role, content } = useAuiState("message");
```

**Linked child state:** a scope's state carries only its own fields. Fields that mirror a child scope — `thread.messages`, `thread.composer`, `message.parts`, `message.composer`, `threads.main`, `threads.threadItems` — are *linked state*: they resolve lazily from the child scopes and are only readable inside a `useAuiState(selector)` callback. They are not present on `useAuiState("scope")` or `aui.scope.getState()`, which throw a descriptive error if you read them. Use the ids field (`thread.messageIds`, `threads.threadIds`) or the child scope's own accessor (`aui.thread.message({ id }).getState()`) there.
...
const last = aui.thread.message({ index: count - 1 }).getState();

// ❌ throws: linked state outside a selector
...
| -------------- | ------------------------------------------------------------------------------------------------ | -------------------------------------------------- |
| ThreadList     | `mainThreadId`, `newThreadId`, `threadIds`, `archivedThreadIds`, `isLoading`; `threadItems`, `main` (linked, selector-only) | Manages all available conversation threads |
...
| Thread         | `isRunning` (may be explicitly set by the runtime rather than derived from last-message status), `isLoading`, `isDisabled`, `isEmpty`, `messageIds`, `capabilities`, `suggestions`; `messages`, `composer` (linked, selector-only) | Active conversation state and message history |
| Message        | `role`, `content`, `status`, `attachments`, `parentId`, `branchNumber`, `branchCount`, `isLast`, `index`; `parts`, `composer` (linked, selector-only) | Individual message content and metadata |
