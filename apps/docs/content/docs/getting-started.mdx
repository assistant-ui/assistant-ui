...
2. Ensure the file is named `.env` (not `.env.txt`)
3. Restart your development server after adding the key
4. Check that the key has sufficient credits/permissions
...
**Thread Component Not Rendering**

If the Thread component appears blank:

1. Check browser console for React errors
2. Verify `AssistantRuntimeProvider` wraps your app
3. Ensure runtime is properly configured
4. Test with a simple message

```tsx
// Debug by adding a test message
...
2. For custom styling, check the [UI customization docs](/docs/ui/Thread)

3. Verify Tailwind is configured if using styled components
</Callout>
...
When testing with external APIs, you may encounter CORS errors:
...
// ✅ Use a Next.js API route instead
const runtime = useLocalRuntime({
  async run({ messages }) {
...
Create a server-side API route to avoid CORS issues:
...
1. **Enable verbose logging**:
   ...
   const runtime = useLocalRuntime({
     async run(options) {
       console.log("Runtime called with:", options);
       // your implementation
     }
   });
   ```
