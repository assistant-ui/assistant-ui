...
`flushTapSync` runs a callback and synchronously flushes the tap updates that
callback schedules, so the new state is readable immediately after. An update
scheduled before the call commits in that flush only if the callback also
updates the same root; otherwise it commits on the next scheduled flush.
