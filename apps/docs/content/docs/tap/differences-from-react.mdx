...
## `useLayoutEffect` and `useInsertionEffect` run with `useEffect`

tap has no layout phase. `useLayoutEffect` and `useInsertionEffect` are accepted (so React code ports cleanly) and run in the commit, in call order, like `useEffect`. `useLayoutEffect` behaves like `useEffect` in every other way.

`useInsertionEffect` keeps React's lifetime: its cleanup runs when its dependencies change or the resource unmounts for good, and a soft unmount (a hidden `<Activity>`, a Strict Mode replay, a `mountOnSubscribe` root losing its last subscriber) leaves it in place. That makes it the place for teardown that must survive a hide, such as closing a connection only when the owner is gone.
