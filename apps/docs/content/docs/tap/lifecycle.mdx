...
A `createTapRoot` root with `mountOnSubscribe: true` follows the same model when its subscriber count drops to zero: effect cleanups run, state is preserved, and the next subscriber remounts the tree, like an Activity going hidden and visible again.

Both keep `useInsertionEffect` mounted, as React does for a hidden Activity; its cleanup runs only when the resource is unmounted for good.
