...
All providers in a frame share one origin policy. Registering an explicit origin tightens an existing wildcard policy, while registering two different explicit origins throws an error. The last unsubscribe resets the policy so a later registration can choose a different origin.
