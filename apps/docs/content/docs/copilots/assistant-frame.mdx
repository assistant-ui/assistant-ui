...
toolHandle.update({
  toolName: "calculator",
  description: "Perform safe calculations", // Updated description
  parameters: z.object({ expression: z.string() }),
  execute: async ({ expression }) => {
    return { result: safeEval(expression) }; // Updated implementation
  },
});
...
Execute user code safely in an isolated iframe:

```tsx
// In sandboxed iframe
registry.addTool({
  toolName: "executeCode",
  description: "Run JavaScript code safely",
