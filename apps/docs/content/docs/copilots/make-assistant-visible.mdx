...
const EditableInput = makeAssistantVisible(Input, {
  ...
  settleDelayMs: 100, // Wait after click or edit actions (default: 2000)
});
```
...
- `config`: Optional configuration object
  ...
  - `settleDelayMs`: Time to wait after a successful click or edit before the tool completes. Defaults to `2000`. Use `0` to disable the wait. Values below `0`, above `2147483647`, or non-finite values throw a `RangeError` when `makeAssistantVisible` is called
...
- Click and edit tools wait for `settleDelayMs` after a successful action so the interface can update before the assistant continues; a configured value is rendered as `data-action-settle-delay` on the action element
