...
4. Register the toolkit once with `config={AuiConfig({ tools: Tools({ toolkit }) })}` on your runtime provider.
...
Register it like any toolkit: `config={AuiConfig({ tools: Tools({ toolkit }) })}`.
