...
const Scoped = ({ children }) => (
  <AuiProvider aui={useAui()} config={AuiConfig(scopes)}>
    {children}
  </AuiProvider>
);

<AuiProvider aui={parent} config={AuiConfig({})}>
  <Scoped />
</AuiProvider>;
```

Where `{ parent: null }` was used to detach from context, `<AuiProvider aui={null} config={AuiConfig({})}>` now provides an isolated empty root.

## `AuiProvider` Grammar

`AuiProvider` takes a `config` built with `AuiConfig(...)` — raw object literals are a type error. At the top level, `config` alone creates the subtree's client. Nested under a parent provider, `aui` is mandatory: `aui={useAui()}` extends the parent, `aui={null}` isolates (dev-enforced). An empty config provides the `aui` client as-is. `ref` receives the resulting client after mount.

```tsx
// Top-level root
<AuiProvider config={AuiConfig({ tools: Tools({ toolkit }) })}>

// Nested: extend the parent
<AuiProvider aui={useAui()} config={AuiConfig({ tools: Tools({ toolkit }) })}>

// Nested: isolate from the parent
<AuiProvider aui={null} config={AuiConfig({ tools: Tools({ toolkit }) })}>
```
...
// After
<AuiProvider aui={client} config={AuiConfig({})}>
