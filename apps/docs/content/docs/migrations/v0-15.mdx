...
const Scoped = ({ children }) => {
  ...
  return (
    <AuiProvider aui={useAui()} config={config}>
      {children}
    </AuiProvider>
  );
};
...
<AuiProvider aui={parent} config={rootConfig}>
  <Scoped />
</AuiProvider>;
```

Where `{ parent: null }` was used to detach from context, `<AuiProvider aui={null} config={config}>` where `const config = AuiConfig({})` now provides an isolated empty root.
...
<AuiProvider aui={null} config={config}>
```
...
<AuiProvider aui={client} config={config}>
<AuiProvider aui={null} config={config}>
```
