...
An experimental export with no annotation, or one whose window has closed without an extension, fails CI, so the dates in the source are current by construction.
