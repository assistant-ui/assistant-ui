...
The model selector attaches a ref to `PopoverContent` so its side stays stable while filtering. Current assistant-ui copies already forward this ref. Check `/components/ui/popover.tsx` first. If `PopoverContent` already uses `forwardRef` and passes `ref={ref}` to `PopoverPrimitive.Content`, no change is needed.

For an older copy that still declares `PopoverContent` as a plain function, wrap it with `forwardRef<HTMLDivElement, React.ComponentProps<typeof PopoverPrimitive.Content>>`, accept the ref as the second argument, and pass it to the existing `<PopoverPrimitive.Content ref={ref}>`. Preserve the portal, props, defaults, `data-slot`, and class list, then set `PopoverContent.displayName = "PopoverContent"`.
