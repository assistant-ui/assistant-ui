...
Messages queued for the previous thread are discarded only if the switch proceeds,
before calling your adapter. An abandoned switch preserves the queue.

The callback ownership check assumes frontend-tool cancellation settles immediately,
as on the normal message-append path. If your application resumes a run while a
previous frontend tool is still settling, coordinate that work before switching;
the callback precedence guarantee does not cover that overlap.
