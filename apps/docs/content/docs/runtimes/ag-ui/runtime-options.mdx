...
Data parts stay in the assistant-ui message (and in anything a history adapter persists) but are not sent back to the agent, since the AG-UI assistant record has no field for them. Framework integrations emit their own plumbing over this channel (`on_interrupt`, `PredictState`, `Exit`, `hook_error`, `state_update_error`, `system:*`, `MultiAgentHandoff`), and those names surface as data parts like any other, so only register renderers for names your backend owns.
