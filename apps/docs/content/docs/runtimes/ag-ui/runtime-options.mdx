...
| `foldReasoning` | `boolean` | Fold reasoning records from `MESSAGES_SNAPSHOT` events into the surrounding assistant-side messages. Snapshot messages have no run identifier, so folding is bounded by user and system messages. Defaults to `false`. |
...
time, the same way a live run never stores them. Pass `foldReasoning: true` when
the persisted snapshot represents one turn as reasoning, tool, and assistant
records and the thread should render them as one assistant message. Because the
AG-UI snapshot schema has no run identifier, folding stops at user and system
messages; leave it disabled when adjacent assistant records must remain
separate.
