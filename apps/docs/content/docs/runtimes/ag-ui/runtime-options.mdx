...
Two limits apply to `encryptedValue`. An imported `ReasoningMessage` that carries it keeps it at `providerMetadata.agui.encryptedValue` on the part and is re-emitted with the field intact, so a reloaded thread replays the signature it was given. Reasoning produced by a live run does not carry one, because the runtime does not yet consume the `REASONING_ENCRYPTED_VALUE` event, and a record whose readable `content` is empty is dropped at import, so a signature is not preserved on its own. A provider that rejects unsigned reasoning therefore needs its history supplied from a snapshot rather than rebuilt from a live run.
