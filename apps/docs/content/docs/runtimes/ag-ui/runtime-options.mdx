...
| `ACTIVITY_SNAPSHOT` | Appended to the in-flight assistant message as an `agui-activity/<activityType>` `data` part keyed by `messageId` (or by `activityType` when absent); a later snapshot for the same `messageId` replaces it unless `replace` is `false`. |
