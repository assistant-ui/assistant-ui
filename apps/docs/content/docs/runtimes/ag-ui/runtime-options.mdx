...
| `ACTIVITY_SNAPSHOT` | For activity types other than `a2ui-surface` and `mcp-apps` (see above), appended to the in-flight assistant message as an `agui-activity/<activityType>` `data` part keyed by `messageId` (or by `activityType` when absent); a later snapshot for the same `messageId` replaces it unless `replace` is `false`. |
| `ACTIVITY_DELTA` | Applies a JSON Patch to the activity part its `ACTIVITY_SNAPSHOT` created. A delta with no matching snapshot is ignored. |
| `STEP_STARTED` / `STEP_FINISHED` | Received and ignored; the runtime has no step boundary to map them onto. |
