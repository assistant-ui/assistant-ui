...
| `onCancel` | `() => void` | Cancellation callback fired when a run is cancelled, including user cancel, thread switching, and runtime teardown. |
...
Starting a thread switch cancels the active agent run. Cancelling an active run
invokes `onCancel`; a newer thread selection, replacement run, or queued send
started synchronously by that callback takes precedence over the original switch.
When the switch is accepted, messages queued for the previous thread are discarded
before calling your adapter. A switch abandoned because of the cancellation callback
does not clear that queue. If the adapter subsequently rejects, discarded messages
are not restored.

If a frontend tool from an earlier run is still executing, a replacement run started
from `onCancel` may stream into the thread being switched to. Let earlier frontend
tools finish before resuming a run and switching threads; the callback precedence
guarantee does not cover that overlap.
...
A thread-list load or creation completion is ignored if messages or state change, or a new run starts, while the adapter is pending. New work takes precedence over the older history response; the runtime does not merge that response into the new run or resume it automatically.
