...
Sending reasoning back is what the protocol asks for, and it is worth knowing what your server does with it. A `reasoning` record in the run input has to be handled on the inbound side, and not every implementation did: `ag-ui-langgraph` below 0.0.41 raises `ValueError: Unsupported message role: reasoning` on the second turn of any thread that produced reasoning, because its converter only recognised the user, assistant, system, and tool roles. 0.0.41 and later skip inbound `reasoning` and `developer` records instead, so the turn succeeds. If you run that integration, keep it at 0.0.41 or newer.

Skipping is also the reference behaviour rather than an outlier, so treat reasoning replay as best effort: a server is free to discard what it receives, and `ag-ui-langgraph` does. Reasoning continuity across turns therefore depends on your server choosing to use it, and an `encryptedValue` reaches the provider only if the server forwards it.
