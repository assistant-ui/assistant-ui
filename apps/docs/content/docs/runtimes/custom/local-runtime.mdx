...
If the user sends another message instead of answering, the pause ends, because a run only resumes from the last message. Each open approval records `approval.resolution: "cancelled"`, the paused message settles as `incomplete` with reason `cancelled`, and a result added to it later is stored without invoking your adapter again. The next run still receives those calls without results, so answer each one before the history reaches a provider; `toGenericMessages` from `assistant-stream` reports them as not completed.
