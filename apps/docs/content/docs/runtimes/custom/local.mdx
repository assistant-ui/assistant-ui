...
<Callout>
LocalRuntime manages chat state internally and provides built-in support for message editing, reloading, and branch switching. When paired with [Assistant Cloud](/docs/cloud/overview), it also supports multiple conversation threads with full thread management capabilities.
...
- **Multi-thread support** - Full thread management when used with Assistant Cloud
- **Thread persistence** - Via history adapter or Assistant Cloud integration
...
<Cards>
  ...
  <Card
    ...
    description="Full thread management capabilities when integrated with Assistant Cloud"
  />
  <Card
...
LocalRuntime supports multiple conversation threads when integrated with Assistant Cloud:
...
With Assistant Cloud enabled, you get:
...
<Callout>
Without Assistant Cloud, LocalRuntime manages a single conversation thread. For custom multi-thread implementations without cloud, use [ExternalStoreRuntime](/docs/runtimes/custom/external-store) with a thread list adapter.
</Callout>
...
<Callout type="info">
The history adapter handles persistence for the current thread. For multi-thread support with custom storage (without Assistant Cloud), use ExternalStoreRuntime.
</Callout>
...
| Multi-Thread Support  | Yes (with Assistant Cloud)                       | Yes (with thread list adapter)                        |
| Custom Thread Storage | No (use ExternalStoreRuntime)                   | Yes                                                   |
