...
<Callout type="tip">
  **Real-world reference:** [Wealthfolio](https://github.com/afadil/wealthfolio) is an open-source investment portfolio desktop app (7k+ stars) that uses `ExternalStoreRuntime` with database-backed thread persistence, streaming state updates, CSV file attachments, and 11 custom tool UIs — an excellent reference for production-grade implementations.
</Callout>
...
Display existing conversation history without allowing new messages — useful for public sharing, observability dashboards, and session replay:

```tsx
import { useExternalStoreRuntime, AssistantRuntimeProvider } from "@assistant-ui/react";
import { ThreadPrimitive } from "@assistant-ui/react";

function ReadOnlyConversation({ messages }: { messages: MyMessage[] }) {
  const runtime = useExternalStoreRuntime({
    isRunning: false,                           // Never show a loading spinner
    messages,
    convertMessage,
    onNew: async () => {},                      // No-op: ignore any new message attempts
  });

  return (
    <AssistantRuntimeProvider runtime={runtime}>
      {/* Render messages only — no Composer */}
...
The key is **omitting `ComposerPrimitive`** from your thread component tree. The chat input simply won't exist. Even if `onNew` is called somehow, the no-op handler ensures nothing happens.

<Callout type="tip">
  **Seen in production:** [SurfSense](https://github.com/MODSetter/SurfSense) (13k stars) uses this exact pattern to render publicly-shared conversation links — the full chat history is displayed beautifully using assistant-ui components, but visitors can't send new messages.
</Callout>

If you need full read-only rendering with the `Thread` component, use `ReadonlyThreadProvider` from `@assistant-ui/react`:

```tsx
import { ReadonlyThreadProvider, ThreadPrimitive, type ThreadMessage } from "@assistant-ui/react";

function ConversationReplay({ messages }: { messages: readonly ThreadMessage[] }) {
  return (
...
### Streaming with RAF Throttling

When streaming tokens from your backend, each delta triggers a state update. At high token rates this can cause hundreds of re-renders per second. Use `requestAnimationFrame` to batch updates to ~60fps:
...
const handleNew = useCallback(async (message: AppendMessage) => {
  ...
  // Mutable buffer — only flushed to React state on animation frames
  let buffer = "";
  ...
  try {
    for await (const token of streamFromBackend(message)) {
      buffer += token;
      scheduleFlush();
    }
  } finally {
    if (rafId !== null) cancelAnimationFrame(rafId);
    if (pending) flush(); // Final flush
    setIsRunning(false);
  }
}, []);
```

This pattern prevents React from re-rendering on every single token while still keeping the UI responsive.

<Callout type="tip">
  **Seen in production:** [Wealthfolio](https://github.com/afadil/wealthfolio) (7k+ stars) uses this exact RAF throttling approach for streaming text, reasoning, and tool call deltas simultaneously — keeping their desktop app smooth even during long-running financial analysis responses.
</Callout>

### Tool UI Registry Pattern

When you have many tool UIs (5+), organize them with a registry map instead of registering each one individually:

```tsx title="tool-uis/index.ts"
...
// Map tool names to their UI components
...
```tsx title="thread.tsx"
import { makeAssistantToolUI } from "@assistant-ui/react";
import { toolUIs } from "./tool-uis";

// Register all tool UIs from the registry
const ToolUIComponents = Object.entries(toolUIs).map(
  ([toolName, Component]) => {
    const ToolUI = makeAssistantToolUI({
      toolName,
      render: (props) => <Component {...props} />,
    });
    return <ToolUI key={toolName} />;
  },
);
```

This keeps tool UI registration centralized and makes it easy to add new tools without modifying the thread component.

### Optimistic Thread Creation

When the user sends their first message in a new thread, add the thread to the sidebar immediately rather than waiting for the server response:

```tsx
const handleNew = useCallback(async (message: AppendMessage) => {
  // ... send message to backend

  // When backend returns the new thread ID:
  const newThreadId = response.threadId;

  // Optimistically add to thread list cache immediately
  queryClient.setQueryData(["threads"], (old) => {
    if (!old) return old;
    const exists = old.some((t) => t.id === newThreadId);
  ...
  // Invalidate later to sync with server
  setTimeout(() => {
...
This eliminates the flash where the sidebar is empty before the server confirms the thread was created.
...
- [Wealthfolio — Production ExternalStoreRuntime Reference](https://github.com/afadil/wealthfolio) — open-source desktop investment tracker with streaming, thread persistence, 11 tool UIs, and CSV attachments
