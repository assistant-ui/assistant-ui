...
<Callout type="info">
**Real-world integration:** Anthropic's official [Claude Managed Agents quickstart](https://github.com/anthropics/claude-quickstarts/tree/main/managed-agents/assistant-ui) uses `useExternalStoreRuntime` to fold each session's event log into assistant-ui messages, then wraps it with `useRemoteThreadListRuntime` so the session list becomes the thread list. It includes live event streaming, persistent history, tool approvals, attachments, and custom tool UIs.
</Callout>
