...
When shared metadata changes frequently but each message reads only a small part
of it, use `getMetadataKey` to keep unrelated messages cached. The returned key
must change whenever the metadata read by `callback` changes for that message.
Return a primitive or a value whose identity stays stable while that metadata is
unchanged:

```tsx
type MyMetadata = useExternalMessageConverter.Metadata & {
  labelById: Record<string, string>;
};

const convertWithMetadata: useExternalMessageConverter.Callback<MyMessage> = (
  message,
  metadata,
) => {
  const { labelById } = metadata as MyMetadata;
  return {
    role: message.role,
    content: [
      {
        type: "text",
        text: `${message.content} ${labelById[message.id] ?? ""}`,
      },
    ],
  };
};

const getMetadataKey: useExternalMessageConverter.GetMetadataKey<MyMessage> = (
  message,
  metadata,
) => (metadata as MyMetadata).labelById[message.id];

const convertedMessages = useExternalMessageConverter({
  callback: convertWithMetadata,
  messages,
  isRunning,
  metadata: { labelById } as MyMetadata,
  getMetadataKey,
});
```
