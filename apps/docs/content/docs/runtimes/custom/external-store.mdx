...
| `onVoiceTranscript` | Persist finalized voice transcripts. Convert the `ThreadMessage` to the host message type when one is used and retain its id. A transcript that finalizes while `isLoading` is true arrives once loading ends, on whichever adapter is current then. |
