...
<Callout type="info">
  If you don't need custom storage, consider using [Assistant Cloud](/docs/cloud/overview) for managed multi-thread support with zero database setup.
</Callout>
...
2. **New conversation**: User sends first message → calls `initialize()` to create thread record → saves message via history adapter
...
Without a properly implemented `ThreadHistoryAdapter`, messages will not be saved. The thread list adapter only manages thread metadata - you **must** provide a history adapter via `unstable_Provider` for message persistence.
</Callout>

## Complete Database Integration Example

This example shows a full implementation using browser localStorage. You can adapt this pattern for any database (PostgreSQL, MongoDB, Firebase, etc.) by replacing the `dbConnection` implementation.
...
<Steps>
  <Step>
    ...
    // Database interface for threads and messages
    // Adapt this to your actual database (PostgreSQL, MongoDB, etc.)
...
<Steps>
  <Step>
    **App loads** → `list()` called → Threads appear in ThreadList
  </Step>
  <Step>
    **User selects thread** → Provider mounts → `load()` called → Messages populate
  </Step>
  <Step>
    **User sends first message on new thread** → `initialize()` called → `remoteId` becomes available → `append()` called
  </Step>
  <Step>
    **Subsequent messages** → `append()` called immediately (remoteId already exists)
  </Step>
</Steps>

<Callout type="warning" title="First Message Timing">
On a brand new thread, the first user message triggers `initialize()`. Until `initialize()` completes, `remoteId` is `null`. Your history adapter's `append()` should handle this gracefully by checking if `remoteId` exists before saving.

The example above shows the recommended pattern: check for `remoteId` and log a warning if it's missing. assistant-ui will automatically retry the append after initialization completes.
</Callout>

### Message Format Conversion

Your database likely stores messages differently than assistant-ui's internal format. The history adapter is responsible for converting between formats:
...
<Callout type="info">
The `content` field can include text, tool calls, images, and more. Store it as JSON/JSONB in your database to preserve the full structure.
</Callout>
...
CREATE TABLE threads (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL, -- For multi-tenant
...
CREATE TABLE messages (
  ...
  parent_id TEXT, -- For message branches
...
interface MessageDocument {
  ...
  parentId: string | null;
  role: "user" | "assistant" | "system";
  content: any; // Full assistant-ui content format
  createdAt: Date;
}
...
```typescript
// Thread list: SET
// Key: `user:{userId}:threads`
// Value: Set of thread IDs

// Thread metadata: HASH
// Key: `thread:{threadId}`
// Fields: { title, archived, createdAt }

// Thread messages: LIST
// Key: `thread:{threadId}:messages`
// Value: JSON-serialized messages in order
```
...
unstable_Provider: ({ children }) => {
  const threadListItem = useThreadListItem();
  const history: ThreadHistoryAdapter = { /* implementation */ };
...
**Cause:** Trying to save before `initialize()` completes and `remoteId` is available.

**Solution:**

The history adapter's `append()` will be called even before `remoteId` exists. assistant-ui handles this by:
1. Calling `append()` when user sends message
2. Calling `initialize()` to create thread
3. Re-calling `append()` after initialization completes

Your `append()` should gracefully handle the missing `remoteId`:

```tsx
async append(item) {
  if (!remoteId) {
    // Thread not initialized yet - this is normal for the first message
    // assistant-ui will retry after initialize() completes
    console.log("Waiting for thread initialization...");
    return;
  }
...
<ParametersTable
  ...
  parameters={[
    ...
    },
    {
      ...
      description: "Update thread title. Called when user renames a thread in the UI.",
      required: true,
    },
    {
      name: "archive",
      type: "(remoteId: string) => Promise<void>",
      description: "Mark thread as archived. Throw error to revert optimistic UI update.",
      required: true,
    },
    {
      name: "unarchive",
      type: "(remoteId: string) => Promise<void>",
      description: "Restore archived thread to regular status.",
      required: true,
    },
    {
      name: "delete",
      type: "(remoteId: string) => Promise<void>",
      description: "Permanently delete thread and all its messages. Throw error to prevent deletion.",
      required: true,
    },
    {
      name: "generateTitle",
...
<ParametersTable
  ...
  parameters={[
    {
      name: "load",
      type: "() => Promise<ExportedMessageRepository>",
      description:
    ...
    {
      name: "append",
      type: "(item: ExportedMessageRepositoryItem) => Promise<void>",
      description:
    ...
    {
      name: "resume",
      type: "(options: ChatModelRunOptions) => AsyncGenerator<ChatModelRunResult>",
      description:
...
interface ExportedMessageRepositoryItem {
  parentId: string | null;
...
|---------|-------------------|-----------------|
| Setup Complexity | Moderate - requires database setup | Simple - zero configuration |
| Database Required | Yes - you provide | No - managed for you |
| Multi-Tenant | Your implementation | Built-in user isolation |
| Cost | Your infrastructure costs | Usage-based pricing |
| Data Control | Full - you own all data | Managed - data stored by Anthropic |
| Customization | Unlimited - any database, any schema | Limited to provided features |
| Maintenance | You manage backups, scaling | Fully managed |
| Best For | Custom requirements, existing DB, enterprise | Quick setup, prototypes, standard needs |
