  ...
  <Step>
    ...
    const threadListAdapter: RemoteThreadListAdapter = {
      async list(params) {
        ...
        const { threads, cursor } = await response.json();
        return {
  ...
  parameters={[
    {
      ...
      description:
        "Return threads for the current page. If your backend supports pagination, pass the cursor from the previous response. Return a cursor to indicate more pages are available.",
      required: true,
    },
    {
      name: "initialize",
...
- `list()` hydrates threads on mount and during refreshes. If it returns a `cursor`, the runtime enables pagination.
- `loadMore()` fetches the next page by calling `list({ cursor })` and appending the results. Duplicate threads are automatically skipped.
...
If your thread list can grow large, implement cursor-based pagination in your `list()` method. Return a `cursor` string alongside threads to indicate more pages are available.

```tsx
const threadListAdapter: RemoteThreadListAdapter = {
  async list(params) {
    const response = await fetch(
      `/api/threads?cursor=${params?.cursor ?? ""}`,
    );
    ...
    return {
      ...
      cursor: nextCursor, // undefined when no more pages
    };
  },
  // ...other methods
};
...
On the UI side, use `canLoadMore` and `loadMore()` to build infinite scroll or a "load more" button:
...
function LoadMoreButton() {
  ...
  return (
    <button onClick={() => aui.threads().loadMore()}>Load more</button>
  );
}
```
