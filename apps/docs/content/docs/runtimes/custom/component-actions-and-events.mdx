...
There are two ways a component can talk back to your app:

- `invoke`: "I need a decision/result."  
  Example: user clicks **Refresh quote** and waits for a success/error result.
- `emit`: "FYI, something happened."  
  Example: user switched tabs; log analytics or update local UI, no response
  required.

If you remember one thing: use `invoke` for request/response, use `emit` for
