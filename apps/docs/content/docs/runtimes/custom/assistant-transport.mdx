...
| Assistant Cloud | `cloud`: the cloud backs the thread list, every request's `threadId` is the cloud thread id, feedback is stored, and settled messages are copied for the dashboard |

Speech, dictation and suggestions are not currently exposed by `AssistantTransport`. Drop down to `ExternalStoreRuntime` if you need them.
