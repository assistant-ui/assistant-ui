...
User `add-message` commands include a client-generated `message.id`. Preserve it when creating the backend message so the optimistic message is reconciled with the settled message and Assistant Cloud can copy it.
...
{
  type: "add-message",
  message: { role: "user", id: "client-message-id", parts: [...] },
