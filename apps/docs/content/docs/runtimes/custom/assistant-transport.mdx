...
supported by the Assistant Transport runtime. Messages are produced by the
state converter, and the runtime has no local message writer for
assistant-ui-only interaction data. Calling the method rejects with
`Runtime does not support recording tool interactions.`; no record is added to
the agent state or sent as a transport command.
