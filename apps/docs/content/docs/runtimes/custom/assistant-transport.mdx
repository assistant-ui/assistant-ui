...
To offer `ComposerPrimitive.Resume`, also pass `canResume` from your backend's
checkpoint state for the current thread. Configuring a resume endpoint does not
prove that it currently retains a run, so the control stays disabled without this
explicit state. Clear the flag when the checkpoint finishes, expires, or the thread
changes. The runtime withholds availability while a request is in flight.

```tsx
const runtime = useAssistantTransportRuntime({
  // ...
  resumeApi: "/api/resume",
  canResume: hasCheckpointForCurrentThread,
});
```
