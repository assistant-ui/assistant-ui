...
| `useLangGraphSetMessages` | _(none)_ | Replace the message list while the thread is idle (history pagination, optimistic edits). Throws while a run is in progress; the next run re-seeds from the current list. |
