...
| `delete` | `(threadId: string) => Promise<void>` | Optional. Called before the thread list deletes a thread, with the cloud thread id when `cloud` is set; read that thread's `external_id` to delete the backend thread, as [LangGraph](/docs/cloud/langgraph) shows. |
