...
Every `task` subagent transcript is attached to its spawning tool call as `ToolCallMessagePart.messages`, keyed by the `task` tool call id. It streams while the subagent runs and nests recursively when subagents spawn subagents. `MessagePartPrimitive.Messages` renders the transcript and `s.thread.tasks` lists it. See [Multi-agent tools](/docs/tools/multi-agent). Threads loaded from history carry these transcripts because the SDK seeds `stream.subagents` from checkpoint messages and seeds the scoped message projections from history.
