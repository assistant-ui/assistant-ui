...
supported by `useStreamRuntime`. The runtime renders messages from graph state
and submits changes through `stream.submit`; it has no local writer for
assistant-ui-only interaction data. Calling the method rejects with
`Runtime does not support recording tool interactions.`, and the interaction
is not stored in graph history.
