...
The package automatically normalizes snake_case event fields and artifact media fields from ADK Python backends to camelCase. No configuration is needed; connect to either ADK JS or ADK Python servers. Event fields and nested media fields (`function_call` → `functionCall`, `inline_data` → `inlineData`, `mime_type` → `mimeType`, `file_data` → `fileData`, `file_uri` → `fileUri`, `requested_tool_confirmations` → `requestedToolConfirmations`) are handled.
