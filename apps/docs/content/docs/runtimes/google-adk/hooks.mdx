...
Pass `req.toolCallId` unchanged. It identifies the pending `adk_request_credential` call, and the helper combines that request's auth configuration with the credential before replying.
