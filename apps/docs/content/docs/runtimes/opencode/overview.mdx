...
supported by `useOpenCodeRuntime`. The runtime projects messages from OpenCode
history and events, and its adapter does not write assistant-ui-only interaction
data into that history. Calling the method rejects with
`Runtime does not support recording tool interactions.`, and the record is not
stored in the OpenCode session.
