...
- Instead of calling the AI SDK's `streamText` directly, we call `agent.stream(messages, { format: "aisdk" })` to process the chat messages using the agent's configuration and model.
