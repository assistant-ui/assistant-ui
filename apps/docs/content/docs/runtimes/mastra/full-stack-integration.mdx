...
Add the Mastra integration package and dependencies to your project:

```bash npm2yarn
npm install @assistant-ui/react-mastra @mastra/core@latest @mastra/memory@latest @ai-sdk/openai
```
...
Now update your frontend to use the dedicated Mastra runtime. Replace the existing runtime configuration in your page component:
...
Key changes:
- Import `useMastraRuntime` from `@assistant-ui/react-mastra`
- Configure the runtime with your agent ID and optional memory
- The runtime will automatically handle streaming and state management
