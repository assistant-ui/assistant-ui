...
const runtime = useMastraRuntime({
  agentId: "chef-agent",
  api: "/api/chat",
  memory: { storage: "libsql" },  // Enable conversation memory
  workflow: { workflowId: "cooking-workflow" },  // Enable specific workflow
  eventHandlers: {
...
function ChatComponent() {
  ...
  const { workflows, startWorkflow } = useMastraWorkflows({
    workflowId: "chef-workflow",
  });
...
**Before:**
```tsx
// Manual workflow handling required
...
const { workflows, startWorkflow, resumeWorkflow } = useMastraWorkflows({
  agentId: "chef-agent",
});

// Workflows are automatically managed by the runtime
```
