...
graph TD
    ...
    B -->|AG-UI Agent| J[Use AG-UI Runtime]
    B -->|A2A Agent| K[Use A2A Runtime]
...
<Cards>
  ...
  <Card
    ...
    description="For AG-UI compatible agents with streaming, tools, and reasoning"
    href="/docs/runtimes/ag-ui"
  />
  <Card
    title="A2A Protocol"
    description="For Google's Agent-to-Agent protocol with tasks and artifacts"
...
- **AG-UI Runtime** → Built on `ExternalStoreRuntime` with AG-UI protocol adapter
- **A2A Runtime** → Built on `ExternalStoreRuntime` with Google's A2A protocol adapter
...
- **[AG-UI Example](https://github.com/assistant-ui/assistant-ui/tree/main/examples/with-ag-ui)** - AG-UI protocol integration
