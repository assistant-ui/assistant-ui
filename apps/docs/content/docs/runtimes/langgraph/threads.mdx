...
Keep the adapter reference stable across renders, for example by defining it at module scope or memoizing it. Replacing the adapter resets the thread list and stops active runs before loading the replacement adapter's state.
