...
supported by `useLangGraphRuntime`. Tool messages come from LangGraph state and
the runtime has no client-side writer for assistant-ui-only interaction data;
calling the method rejects with `Runtime does not support recording tool
interactions.`, and no interaction is stored in the graph checkpoint.
