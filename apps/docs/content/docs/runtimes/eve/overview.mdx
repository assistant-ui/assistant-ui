...
`clientContext`. For example, `runConfig: { custom: { model: "fast" } }` arrives
as `clientContext: { model: "fast" }` for that turn only. The assistant-ui
`custom` envelope is unwrapped because Eve turns `clientContext` into a context
message the model reads. An empty or absent `custom` bag is omitted.
