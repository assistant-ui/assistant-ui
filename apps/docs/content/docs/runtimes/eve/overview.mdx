...
Every value in the bag must be JSON-serializable, because it is serialized onto
the wire and Eve rejects a non-serializable object with a 400. A `Date` or a
`BigInt` that other runtimes would ignore fails the send here.
