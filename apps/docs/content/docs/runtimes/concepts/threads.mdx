...
A run that is still streaming is cancelled first, and a thread that has not been sent yet is left alone because it holds no remote state.

How the refetch happens depends on the runtime. When it declares the capability, the thread runtime is reused: composer drafts survive, existing messages stay rendered while the fresh state loads, and the returned promise settles with the refetch, rejecting if it fails. `useLangGraphRuntime` declares it whenever you pass `load`. Otherwise the runtime hook is remounted, which re-runs `load()` at the cost of discarding unsent composer input, and the promise resolves once the new runtime attaches. If that difference matters to your UI, gate the affordance you build on `useAuiState((s) => s.thread.capabilities.refetchThread)`.
