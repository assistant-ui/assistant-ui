...
<ParametersTable
  ...
  parameters={[
    ...
    },
    {
      ...
      description:
        "Optional hook called by the RemoteThreadList store entry. Return the same adapters the Provider injects. Per-thread history requires the thread factory to be keyed with withKey.",
    },
  ]}
/>
