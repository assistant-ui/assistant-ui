...
- `unstable_useAdapters` is a hook. The `RemoteThreadList` store entry calls it inside the client tree, so Vue and Svelte hosts that mount through `createAssistantClient` get the same adapters as a React hook host.
- `unstable_Provider` is a React component. `useRemoteThreadListRuntime` renders it. The store entry ignores it.
...
const adapterWithHistory: RemoteThreadListAdapter = {
  ...
  unstable_Provider({ children }) {
    return (
      <RuntimeAdapterProvider adapters={useThreadListAdapters()}>
        {children}
      </RuntimeAdapterProvider>
    );
  },
...
<ParametersTable
  ...
  parameters={[
    ...
    },
    {
      ...
      description:
        "Optional React wrapper rendered around each active thread by useRemoteThreadListRuntime. Inject thread-scoped adapters (history, attachments) here.",
    },
    {
      name: "unstable_useAdapters",
      type: "() => RemoteThreadListAdapters | null | undefined",
      description:
        "Optional hook called by the RemoteThreadList store entry. Return the same adapters the Provider injects.",
    },
  ]}
/>
