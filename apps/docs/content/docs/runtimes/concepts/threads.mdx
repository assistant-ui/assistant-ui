...
| --- | --- | --- | --- |
| AssistantCloud | LocalRuntime and runtime adapters that accept `cloud` | assistant-cloud | You want it managed; auth, sync, persistence handled |
...
`AssistantCloud` is the managed multi-thread service. Pass an instance to `useLocalRuntime` or a runtime adapter that accepts `cloud`, and threads, persistence, sync, and titles are handled for you.
