...
What happens to a run in progress depends on the path. On the in-place path a running thread is cancelled first, and only when the runtime reports it can cancel; otherwise the refetch races that run. The remount path tears the runtime down, which ends the run either way. Both are disruptive enough that this belongs on an event rather than a short timer: drive it from a state change like the one above, or skip the call while `useAuiState((s) => s.thread.isRunning)` is true.
