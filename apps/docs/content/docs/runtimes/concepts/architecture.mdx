...
Use `useRuntimeStatus()` when app chrome needs one status value for badges, loading indicators, error toasts, or retry controls.
...
The hook derives `idle`, `loading`, `running`, `requires-action`, `error`, `cancelled`, and `incomplete` from the active thread state and terminal assistant message status.
