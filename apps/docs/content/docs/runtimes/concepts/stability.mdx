...
| `unstable_Provider` | Various adapters | React-component face on `RemoteThreadListAdapter`. Must render children synchronously. `useRemoteThreadListRuntime` renders it; the `RemoteThreadList` store entry ignores it. |
| `unstable_useAdapters` | Various adapters | Tap-hook face on `RemoteThreadListAdapter`. The `RemoteThreadList` store entry calls it and provides the returned adapters to the thread factory. |
