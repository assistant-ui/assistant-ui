...
When a runtime can switch accounts, workspaces, or another backing store, set
`scopeId` to a stable identifier for that storage scope. Keep it unchanged when
recreating the adapter for the same scope, and change it before loading another
scope. A scope change clears the previous history, discards any pending result
from that scope, and loads the current adapter. Adapters that omit `scopeId` are
