...
AI SDK v6 introduced major API changes: async `convertToModelMessages`, a new tool schema, and `toUIMessageStreamResponse()`. v5 still works but no longer receives new features in `@assistant-ui/ai-sdk`. Pinning to `1.1.21` keeps you on the v5-compatible surface.
...
import { useVercelUseChatRuntime } from "@assistant-ui/ai-sdk";
...
| `@assistant-ui/ai-sdk` | `@1.1.21` | `@1.3.40` |
| `@ai-sdk/openai` | `^1` | `^3` |
...
1. Update `ai` from `@^5` to `@^6`, and `@assistant-ui/ai-sdk` to the latest line.
