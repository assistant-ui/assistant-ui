...
Voice transcripts from a connected voice session are appended to the `useChat` messages as they finalize, carrying `metadata.modality: "voice"`, so they persist and reload like typed turns.
