...
Finalized voice transcripts are appended to the AI SDK message list with their original id and `metadata.modality: "voice"`, so they remain in the thread after `disconnectVoice()`. This is handled by the AI SDK runtime alongside its `onImport` message path; no separate transcript callback is required.
