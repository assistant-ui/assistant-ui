...
The answer is recorded on the message part before the handler runs, without starting a chat request, so a run the handler continues on the same chat keeps it and a second response to the same request is rejected. If the handler throws, the request reopens and `respondToApproval` rejects with the handler's error.
