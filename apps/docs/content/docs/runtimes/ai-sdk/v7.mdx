...
The answer applies to the approval as soon as the handler starts, and a second response to the same request is rejected. It is held by the runtime and never written into the `useChat` messages, so `sendAutomaticallyWhen` cannot send it to your chat route, and a reloaded thread shows the request as your backend's messages record it. If the handler throws, the request reopens and `respondToApproval` rejects with the handler's error.
