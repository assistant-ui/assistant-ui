...
For a runtime that stays mounted while its account or workspace changes, set `historyAdapter.key` to a stable scope identifier. Changing the key reloads history for the new scope; omitting it preserves load-once behavior.
