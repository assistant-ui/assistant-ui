...
The answer applies to the approval as soon as the handler starts, and a second response to the same request is rejected while the approval remains unresolved. It is held with the Chat and never written into the `useChat` messages, so `sendAutomaticallyWhen` cannot send it to your chat route. A runtime mounted again over the same Chat (for example switching away from an in-memory thread and back) keeps the answer applied until the Chat records the approval's resolution. A full page reload or a newly constructed Chat still has no in-memory host answer, so bring the resumed run back into the chat (for example with `resumeStream`) and have your endpoint refuse a second resume. If the handler throws, the request reopens and `respondToApproval` rejects with the handler's error.
