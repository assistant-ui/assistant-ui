...
Voice transcripts from a connected voice session are appended to the `useChat` messages as they finalize, carrying `metadata.modality: "voice"`, and the history adapter stores each one as it lands, so a voice only session persists and reloads like typed turns.
