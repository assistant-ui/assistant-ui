...
Creates a runtime integrated with AI SDK's `useChat` hook. By default, it uses `AssistantChatTransport` which sends requests to `/api/chat` and automatically forwards system messages and frontend tools to your backend.
