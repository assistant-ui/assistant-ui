...
<Steps>
  <Step>
Use `messageMetadata` in your Next.js route to attach `usage` and `modelId` from the `finish-step` event.
```tsx
...
```
> Note: assistant-ui automatically aggregates token usage from `finish-step`. Alternatively, you can emit total usage from the `finish` event: `if (part.type === "finish") return { usage: part.totalUsage };`
  </Step>
  <Step>
