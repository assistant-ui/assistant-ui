...
Like `tapReducer`, but with a `getDerivedState` function that transforms the state after every reducer update. If `getDerivedState` returns a new value (by `Object.is`), the derived value becomes the current state and triggers a re-render.

```ts
type Todo = { id: string; text: string; done: boolean };
type State = { todos: Todo[]; doneCount: number };
...
const [state, dispatch] = tapReducerWithDerivedState(
  ...
  { todos: initialTodos, doneCount: 0 },
);
```
