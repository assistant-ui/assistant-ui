...
it is a React render, so a React error boundary catches it. Under
[`useTapRoot`](/tap/docs/tap/trees-and-rerenders#separate-scheduling-with-usetaproot)
or `createTapRoot` it is a scheduler flush, so the error escapes as an uncaught
error instead — the same route every other resource render error takes on those
hosts.
