...
## A throwing snapshot behaves as it does in React

`useSyncExternalStore` matches React here: when a store notifies and
`getSnapshot` throws, the snapshot counts as changed, the resource re-renders,
and the error surfaces during that render rather than in the store's notify
loop. A throw that repairs itself before the re-render reads the snapshot
