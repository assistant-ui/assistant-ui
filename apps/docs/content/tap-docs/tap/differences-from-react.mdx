...
What differs is *which* re-render, and that is decided by the host at the root
of the resource tree, not by the hook that reads the resource. When the tree is
hosted by a React component — [`useResource`](/tap/docs/tap/api-reference#useresource)
in a component, or
[`useTapRoot`](/tap/docs/tap/trees-and-rerenders#separate-scheduling-with-usetaproot),
which escalates a throwing update to a host render — the error surfaces in a
React render and an error boundary catches it. Under a bare
[`createTapRoot`](/tap/docs/tap/api-reference#createtaproot) — no React host —
the re-render runs in the scheduler flush, so the error escapes as an uncaught
