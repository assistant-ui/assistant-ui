...
const useFiltered = (props: { items: string[]; query: string }) => {
...
const Filtered = resource(useFiltered);
```
...
[`useResourceRoot`](/tap/docs/tap/composition#useresourceroot) breaks this chain.
...
[`createResourceRoot`](/tap/docs/tap/outside-react) enable this automatically in
development.
