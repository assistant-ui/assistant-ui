...
```
createTapRoot(() => useResource(App()))  <- tree root
  ├─ useResource(Sidebar)
  │    └─ useResource(NavItem)
  └─ useResource(Main)
       ├─ useResource(Header)
...
`useTapRoot` breaks this chain: it creates a **subtree boundary**. Everything below it becomes a separate tree that re-renders independently. The parent doesn't re-render when the subtree updates.
...
This only applies to tap-scheduled trees (`createTapRoot` / `useTapRoot`). For `useResource` trees, use `flushSync` from `react-dom` instead.
