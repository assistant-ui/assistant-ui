...
dependency arrays, refs, and effect cleanup all work as you expect. This page
covers the few places tap deliberately differs.
...
When you genuinely need stored state that resets on prop changes (React's
`getDerivedStateFromProps` pattern), use the experimental
[`useReducerWithDerivedState`](/tap/docs/api-reference#usereducerwithderivedstate).
...
[`useResourceRoot`](/tap/docs/resources#subscribable-boundary) breaks this chain.
...
## `useMemo` never evicts its cache

React may discard a `useMemo` result and recompute it. tap never does, so
`useMemo(() => new EventEmitter(), [])` is a reliable compute-once: the value is
created on first render and kept for the resource's lifetime.
