...
Gorp keeps a client-side replica of server state in sync over any duplex
transport (typically a WebSocket). Commands flow up, deep-patch ops flow
down, and the client renders an *optimistic* view that layers pending
commands on top of the last server-confirmed state.
