...
Accessing `aui.counter` never throws, and the accessor is always truthy — `if (aui.counter)` does not tell you anything. When the scope hasn't been provided by any `AuiProvider` above, the accessor still answers `source` (`null`), `query`, and `name`; calling it or reading any other property throws. Check availability through `aui.optional`, the client-side mirror of `s.optional`:

```tsx
const aui = useAui();

aui.optional.counter?.increment(); // no-op when the scope is unavailable

if (aui.optional.counter) {
  // safe to use
  aui.optional.counter.increment();
}
```

`aui.optional.counter` is `undefined` when the scope isn't available, and the same bound accessor as `aui.counter` when it is.
