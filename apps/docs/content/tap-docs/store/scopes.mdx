...
const App = () => {
  return (
    <AuiProvider config={AuiConfig({ threadList: ThreadListResource() })}>
      <ThreadView />
    </AuiProvider>
  );
};
...
You don't have to fill every scope at once. Each level of your tree can extend the store with its own scopes — a nested provider passes `aui={useAui()}` to extend the parent (or `aui={null}` to isolate):

```tsx
const ThreadView = () => {
  // extends the parent store with a thread scope
  return (
    <AuiProvider aui={useAui()} config={AuiConfig({ thread: ThreadResource() })}>
      <MessageList />
    </AuiProvider>
  );
};
