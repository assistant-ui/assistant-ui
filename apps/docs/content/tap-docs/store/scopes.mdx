...
You declare what scopes exist by augmenting `ClientRegistry`:
...
Because `ClientRegistry` uses TypeScript module augmentation, different packages can each register their own scopes. A `@my-org/chat` package can register `thread` and `message`, while `@my-org/analytics` registers `analytics` — and your app can access all of them through the same `useAuiState` and `useAui` hooks.
...
Each key matches a name from `ClientRegistry`. Each value is a Tap resource element that implements the scope's methods.
