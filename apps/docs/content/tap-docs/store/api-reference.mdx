...
The `useAui(scopes)` extension overload is deprecated; use `<AuiProvider aui={useAui()} config={AuiConfig(scopes)}>` instead.
...
```tsx
// Top-level root
<AuiProvider config={AuiConfig({ counter: CounterResource() })}>{children}</AuiProvider>

// Nested: extend the parent (or aui={null} to isolate)
<AuiProvider aui={useAui()} config={AuiConfig({ counter: CounterResource() })}>{children}</AuiProvider>
```

Provides an `AssistantClient` to the React tree, created from a `config` built with `AuiConfig(...)`. Nested under a parent provider, `aui` is mandatory: `aui={useAui()}` extends the parent, `aui={null}` isolates. An empty config provides the `aui` client as-is. `ref` receives the resulting client after mount. Child components access the client via `useAui()`.
