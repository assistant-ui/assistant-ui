...
The store object returned by `useAui()`. Each scope is an accessor. `optional` exposes the same scopes, but an unavailable scope resolves to `undefined` instead of a throwing accessor: `aui.optional.counter?.increment()`. `subscribe` fires on any state change. `on` subscribes to typed events.
...
A scope accessor exposing the scope's methods as properties (`aui.counter.increment()`). Read `.source`, `.query`, and `.name` for metadata. An unavailable scope's accessor is truthy but throws on use; check availability via `aui.optional.<scope>`. The call signature (`aui.counter()`) is deprecated and returns the same accessor.
