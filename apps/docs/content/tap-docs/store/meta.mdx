...
The accessor itself is always truthy, so `if (aui.message)` does not tell you anything. Check availability through `aui.optional`, which resolves an unavailable scope to `undefined` instead of a throwing accessor:

```tsx
aui.optional.message; // undefined — no message scope in context
aui.optional.message?.getState(); // safe: undefined when unavailable
