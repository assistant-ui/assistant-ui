...
Unlike React, calling `set` or `dispatch` during the render phase throws an error. To adjust state in response to changing props, use `useReducerWithDerivedState` instead (tap's equivalent of React's `getDerivedStateFromProps`).
...
- **`createResourceRoot`** enables it automatically in development.
