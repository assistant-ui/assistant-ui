...
The runtime uses `useChatRuntime` from `@assistant-ui/react-ai-sdk` with an
`AssistantChatTransport`. When the Expo app targets the hosted assistant-ui
endpoint, it first obtains a signed anonymous session and sends that token in a
header on every chat request:
