          ...
          return (
            <div className="bg-muted/50 flex min-h-[68px] items-center gap-3 rounded-md border-2 border-red-400 p-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-muted-foreground text-sm">{result?.error || "Unknown error"}</span>
              </div>
            </div>
          );
        }
        if(!result?.result) {
          return (
            <div className="bg-muted/50 flex min-h-[68px] items-center gap-3 rounded-md border-2 border-blue-400 p-3">
              ...
              <div className="flex flex-col">
                <span className="text-sm font-semibold">Geocoding location...</span>
                <span className="text-muted-foreground text-sm">Please wait while we find your location</span>
              </div>
            </div>
          );
        }
    ...
    useAssistantTool({
      ...
      execute: async (args: { query: string, longitude: number, latitude: number }) => {
        try {
          ...
          if (data.hourly && data.hourly.time && data.hourly.temperature_2m) {
            ...
            currentHourIndex = currentHourIndex > 0
              ? currentHourIndex - 1
              : currentHourIndex === -1
      ...
      render: ({ args, result }) => {
        ...
        const isDay = result?.success 
          ? new Date(result.timestamp).getHours() >= 6 && new Date(result.timestamp).getHours() < 18
          : true;
  
        return (
          <div className="mt-4 bg-muted/50 hover:bg-muted/70 flex min-h-[68px] items-center gap-3 rounded-md border-2 border-blue-400 p-3 transition-all duration-300 hover:border-blue-500 hover:shadow-md">
            {isLoading ? (
