...
- The plan review is a grid of cards, two across once the body is `@lg` wide and a column below: one card per section the agent wrote (What I found, What I will install, Steps), a card for anything under another heading so nothing is lost, and Open questions last with the note box; a note sends a change request, an empty one installs. A card is `bg-muted rounded-xl p-4` with its title left and a muted eyebrow right. The plan page never scrolls in its collapsed state: a card shows at most three rows, each on one truncated line, and hides the section's full markdown behind a details disclosure whose label counts the rows it holds (`Project details · 2 more`); an open disclosure is the one thing allowed to overflow. The note box is two rows with its label read out only; the row above the cards keeps the earlier-revisions toggle left and the revision label right.
