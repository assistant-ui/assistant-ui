...
export function AskAiBall() {
  ...
  return (
    <div className="group fixed right-5 bottom-5 z-50 hidden md:block">
      <button
        type="button"
        onClick={toggle}
        aria-label="Ask AI"
      ...
      <button
        ...
        className="border-border/60 bg-background text-muted-foreground hover:text-foreground absolute -top-1.5 -right-1.5 grid size-5 place-items-center rounded-(--radius-capsule) border opacity-0 transition-opacity group-hover:opacity-100 focus-visible:opacity-100"
      >
