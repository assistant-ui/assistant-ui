...
import type { SubmitWorkspaceEnvInput, WorkspaceEnvFile, WorkspaceEnvValueInput } from '@/components/agent-playground/augment/types';
...
function WorkspaceEnvRequestForm({
  ...
  const canSubmit = useMemo(() => rows.some((row) => row.name.trim() && row.value.trim()), [rows]);
  const atRowLimit = rows.length >= MAX_ENV_ROWS;

  const updateRow = (index: number, patch: Partial<EnvRow>) => {
    setRows((current) => current.map((row, candidate) => candidate === index ? { ...row, ...patch } : row));
  };
  ...
  return (
    <div className="absolute bottom-full left-0 right-0 z-30 mb-2 rounded-lg border border-sky-500/50 bg-background/95 p-2 text-xs shadow-xl backdrop-blur">
      <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
        <span className="font-medium text-sky-100">Add env keys</span>
        <span className="min-w-0 text-[11px] leading-snug text-muted-foreground">{SCOPE_HELP}</span>
        {queuedCount > 0 && (
        ...
        {rows.map((row, index) => (
          ...
          >
            <input
              ...
              className="h-7 min-w-0 rounded border border-border bg-background px-1.5 text-[11px] font-medium outline-none focus:border-sky-400"
            ...
            <input
              ...
              title={row.description ? `${row.name}: ${row.description}` : undefined}
            ...
            <select
              value={row.envFile}
              onChange={(event) => updateRow(index, { envFile: event.target.value as WorkspaceEnvFile })}
      ...
      <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
        <button
          type="button"
          className="inline-flex items-center gap-1 rounded border border-border px-2 py-1 text-[11px] font-medium text-foreground hover:bg-muted disabled:cursor-not-allowed disabled:opacity-40"
          onClick={addRow}
        ...
        <div className="flex flex-wrap items-center justify-end gap-1.5">
          ...
          <button
            type="button"
            className="rounded border border-border px-2.5 py-1 text-[11px] font-medium text-foreground hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <button
            type="button"
            className="rounded bg-sky-400 px-2.5 py-1 text-[11px] font-medium text-black disabled:cursor-not-allowed disabled:opacity-50"
            onClick={submit}
...
function initialRows(request: PendingWorkspaceEnvRequest): EnvRow[] {
  const fromRequest = request.requested.slice(0, MAX_ENV_ROWS).map((item): EnvRow => ({
    name: item.name,
    value: '',
