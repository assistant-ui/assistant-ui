...
class DebugCapture {
  ...
  captureStoreUpdate(trigger: string, store: unknown) {
    ...
    const toolPartCount = messages.reduce(
      (sum: number, m: any) =>
        sum + m.parts.filter((p: any) => p.type === "tool-call").length,
      0,
    );
  ...
  summary() {
    ...
    const eventTypes = sseEvents.reduce(
      (acc, e) => {
        acc[e.eventType ?? "unknown"] =
          (acc[e.eventType ?? "unknown"] ?? 0) + 1;
        return acc;
      },
    ...
    if (lastSnapshot) {
      console.log(
        `Final state: ${lastSnapshot.messageCount} messages, ${lastSnapshot.toolPartCount} tool parts`,
      );
  ...
  async compareWithServer(
    ...
    if (lastSnapshot) {
      lastSnapshot.messages.forEach((m, i) => {
        ...
        m.parts
          ...
          .forEach((p) => {
            console.log(
              `      └─ ${p.toolName}(${p.toolCallId}) hasResult=${p.hasResult}`,
            );
          });
      });
    }
    ...
    const serverToolParts = serverMessages.reduce((sum: number, m: any) => {
      ...
      return (
        sum +
        content.filter(
          (p: any) => p.type === "tool_call" || p.type === "tool-call",
        ).length
      );
    }, 0);
    ...
    serverMessages.forEach((m: any, i: number) => {
      const content = m.content ?? [];
      console.log(
        `  [${i}] ${m.role} (${m.id?.slice(0, 12)}): ${content.length} parts`,
      );
      content
        .filter((p: any) => p.type === "tool_call" || p.type === "tool-call")
        .forEach((p: any) => {
          const id = p.id ?? p.toolCallId;
          const name = p.name ?? p.toolName;
          console.log(
            `      └─ ${name}(${id?.slice(0, 12)}) hasResult=${p.result !== undefined}`,
          );
        });
    });
