...
export type PlaygroundRuntimeState = ReturnType<typeof useAugmentAssistantRuntime>;

export function createPlaygroundHeaderState(runtimeState: PlaygroundRuntimeState): PlaygroundHeaderState {
  ...
  return {
    ...
    hasWorkspace: Boolean(runtimeState.session?.workspace) || hasProvisionedWorkspace(runtimeState.messages),
...
function hasProvisionedWorkspace(messages: AssistantThreadMessageLike[]): boolean {
