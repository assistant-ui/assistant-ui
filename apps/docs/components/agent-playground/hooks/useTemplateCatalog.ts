...
import type { Category, Template } from '@/components/agent-playground/lib/templates';
...
const CATEGORY_DESCRIPTION: Record<string, string> = {
  ...
  'UI Patterns': 'Common patterns like generative UI, artifacts, and suggestions.',
...
export function toTemplate(example: FrontendExampleSummary, index: number): Template {
  const sourcePath = example.sourcePath?.trim();
  const env = example.env ?? [];
  const hasHostedPreview = example.preview.status !== 'missing' && Boolean(example.preview.url);
...
export function deriveCategories(examples: FrontendExampleSummary[]): Category[] {
  ...
  useEffect(() => {
    ...
    void augmentClient
      ...
      .catch((error: unknown) => {
        ...
        setState({ categories: [], templates: [], isLoading: false, error: message });
      });
