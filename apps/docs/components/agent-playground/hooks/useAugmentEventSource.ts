...
import {
  shouldReloadStateAfterReconnect,
  type ConnectionState,
} from "@/components/agent-playground/augment/events";
  ...
  useEffect(() => {
    ...
    const handleMessage = (
      ...
      try {
        ...
        onDebugRef.current?.("sse:message", {
          id: nextEventId,
          eventName,
          type: event.type,
        });
        if (nextEventId && seenEventIdSetRef.current.has(nextEventId)) {
          onDebugRef.current?.("sse:message:duplicate-skipped", {
            id: nextEventId,
            type: event.type,
          });
          return;
        ...
        onDebugRef.current?.("sse:message:parse-error", {
          eventName,
          data: message.data,
        });
    ...
    const connect = () => {
      if (closed) return;
      setConnectionState(
        lastEventIdRef.current || retryCountRef.current > 0
          ? "reconnecting"
          : "connecting",
      );
      onDebugRef.current?.("sse:connect:start", {
        sessionId,
        url: client.getEventsUrl(sessionId),
      });
      ...
      for (const eventName of SERVER_EVENT_TYPES) {
        eventSource.addEventListener(eventName, (message) =>
          handleMessage(message as MessageEvent<string>, eventName),
        );
      }

      eventSource.onerror = () => {
        onDebugRef.current?.("sse:error", {
          sessionId,
        ...
        if (retryCountRef.current >= MAX_RETRIES_BEFORE_FORCE_END) {
          onDebugRef.current?.("sse:force-agent-end", {
            sessionId,
