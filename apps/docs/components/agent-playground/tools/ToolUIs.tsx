...
export const ShellToolUI = makeAssistantToolUI<{ command?: string }, unknown>({
  ...
  render: ({ args, result, status }) => (
    ...
    >
      <ToolPayloadRenderer
        context={{ toolName: "shell", channel: "result", value: result }}
      />
    </ToolShell>
  ),
});
...
>({
  ...
  render: ({ args, result, status }) => (
    ...
    >
      <ToolPayloadRenderer
        context={{ toolName: "run_command", channel: "result", value: result }}
      />
    </ToolShell>
  ),
});
...
>({
  ...
  render: ({ args, result, status }) => (
    ...
    >
      ...
      />
      <ToolPayloadRenderer
        context={{ toolName: "write_file", channel: "result", value: result }}
        emptyLabel=""
      />
    </ToolShell>
  ),
...
>({
  ...
  render: ({ args, result, status }) => (
    ...
    >
      ...
      />
      <ToolPayloadRenderer
        context={{ toolName: "edit_file", channel: "result", value: result }}
        emptyLabel=""
      />
    </ToolShell>
  ),
...
>({
  ...
  render: ({ args, result, status }) => (
    ...
    >
      <ToolPayloadRenderer
        context={{ toolName: "read_file", channel: "result", value: result }}
      />
    </ToolShell>
  ),
});
...
export const SearchToolUI = makeAssistantToolUI<
  { pattern?: string; query?: string; path?: string },
  unknown
>({
  toolName: "grep",
  render: ({ args, result, status }) => (
    ...
    >
      {args.path ? (
        <div className="mb-2 text-muted-foreground text-xs">{args.path}</div>
      ) : null}
      <ToolPayloadRenderer
        context={{ toolName: "grep", channel: "result", value: result }}
      />
    </ToolShell>
  ),
});
...
>({
  ...
  render: ({ args, result, status }) => (
    ...
    >
      ...
      <ToolPayloadRenderer
        context={{ toolName: "glob", channel: "result", value: result }}
      />
    </ToolShell>
  ),
});
...
type AskUserArgs = {
  ...
  options?: Array<
    { label?: string; description?: string; value?: string } | string
  >;
  ...
  render: ({ args, result, status, toolCallId }) => {
    ...
    const resultAnswer =
      typeof result === "string"
        ? result
        : (result?.answer ?? result?.response ?? result?.selected);
    const waiting =
      status.type === "requires-action" ||
      (status.type === "running" && !resultAnswer);
    ...
    return (
      <ToolShell
        ...
        status={getToolVisualStatus(
          waiting ? "requires-action" : status.type,
          false,
      ...
      >
        <div className="space-y-3">
          <div className="font-medium text-foreground text-sm">
            {args.question ?? "The agent needs your input."}
          </div>
          {waiting ? (
            <div className="space-y-3">
              {options.length > 0 ? (
                <div className="grid gap-2">
                  {options.map((option) => (
                    <button
                      ...
                      className="rounded-lg border bg-background/70 px-3 py-2 text-left text-foreground text-sm transition-colors hover:border-ring disabled:cursor-not-allowed disabled:opacity-60"
                    >
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onKeyDown={(event) => {
                    if (event.key === "Enter")
                      void submitAnswer(freeformAnswer);
                  }}
                  placeholder={
                    options.length > 0
                  ...
                  className="min-w-0 flex-1 rounded-md border bg-background px-3 py-2 text-foreground text-sm outline-none placeholder:text-muted-foreground/70 focus:border-ring"
                />
                <button
                  type="button"
...
function normalizeQuestionOptions(options: AskUserArgs["options"]) {
  ...
  return options.map((option, index) => {
    if (typeof option === "string")
      return { label: option, value: option, description: undefined };
...
export const WorkspaceEnvToolUI = makeAssistantToolUI<
  WorkspaceEnvArgs,
  WorkspaceEnvResult
>({
  ...
  render: ({ args, result, status, artifact }) => {
    ...
    const [rows, setRows] = useState<EnvFormRow[]>(() =>
      initialEnvRows(effectiveArgs),
    );
    const [submitted, setSubmitted] = useState(false);
    const waiting =
      status.type === "requires-action" ||
    ...
    const hasAnyValue = rows.some(
      (row) => row.name.trim() && row.value.length > 0,
    );
    const hasMissingRequired = rows.some(
      (row) => row.required && (!row.name.trim() || row.value.length === 0),
    );
        ...
        {waiting ? (
          <div className="space-y-3">
            <div className="space-y-1">
              <div className="font-medium text-foreground text-sm">
                {effectiveArgs.reason ?? "The app needs environment values."}
              </div>
              {effectiveArgs.appPath ? (
            ...
            <div className="space-y-2">
              {rows.map((row, index) => (
                <div
                  key={`${row.name}-${index}`}
                  className="grid gap-2 rounded-md border bg-background/60 p-2 md:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)_8rem_2rem]"
                >
                  <div className="min-w-0">
                    <input
                      ...
                      className="w-full rounded-md border bg-background px-2 py-1.5 font-medium text-foreground text-xs outline-none placeholder:text-muted-foreground/70 focus:border-ring disabled:opacity-80"
                    />
                    {row.description ? (
                  ...
                  <input
                    ...
                    className="min-w-0 rounded-md border bg-background px-2 py-1.5 text-foreground text-xs outline-none placeholder:text-muted-foreground/70 focus:border-ring disabled:opacity-60"
                  />
                  <select
                    value={row.envFile}
                    onChange={(event) =>
                    ...
                    disabled={submitted}
                    className="rounded-md border bg-background px-2 py-1.5 text-foreground text-xs outline-none focus:border-ring disabled:opacity-60"
                  >
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <button
                ...
                onClick={addRow}
                disabled={submitted}
                className="inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1.5 font-medium text-foreground text-xs hover:bg-muted/60 disabled:cursor-not-allowed disabled:opacity-50"
              >
              ...
              <button
                ...
                className="rounded-md bg-primary px-3 py-1.5 font-medium text-primary-foreground text-xs disabled:cursor-not-allowed disabled:opacity-50"
              >
                Save keys
              </button>
            </div>
...
function workspaceEnvArgsFromArtifact(
  artifact: unknown,
  fallback: WorkspaceEnvArgs,
): WorkspaceEnvArgs {
  const record =
  ...
  const suspendPayload =
    record.suspendPayload && typeof record.suspendPayload === "object"
      ? (record.suspendPayload as WorkspaceEnvArgs)
      : null;
  return suspendPayload ? { ...fallback, ...suspendPayload } : fallback;
}

function normalizeEnvItems(
  items: WorkspaceEnvRequestItem[] | undefined,
...
function WorkspaceEnvResultView({
  result,
}: {
  result?: WorkspaceEnvResult | undefined;
}) {
  ...
  if (result?.status === "already_configured") {
    return (
      <div className="text-muted-foreground text-sm">
        Environment keys are already present.
      </div>
    );
  }
  return (
    <div className="space-y-2 text-sm">
      <div className="text-muted-foreground">
        Environment files updated. Values are hidden.
      </div>
      {keysAdded.length > 0 ? (
        <div className="text-muted-foreground text-xs">
          Added: {keysAdded.join(", ")}
        </div>
      ) : null}
      {keysUpdated.length > 0 ? (
        <div className="text-muted-foreground text-xs">
          Updated: {keysUpdated.join(", ")}
        </div>
      ) : null}
...
function NestedToolStatusBadge({
  status,
  isError,
}: {
  status?: string | undefined;
      ...
      <div className="space-y-4">
        ...
        <PayloadSection title="Subagent Stream">
          <ToolPayloadRenderer
            context={{
              toolName: "subagent",
              channel: "subagentText",
              value: args.subagentText,
            }}
            emptyLabel={
              status.type === "running"
...
const ListWorkspaceEnvStatusToolUI = makeGenericToolUI(
  "list_workspace_env_status",
);
