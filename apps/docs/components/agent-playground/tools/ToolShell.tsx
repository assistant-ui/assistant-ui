import { AlertCircle, CheckCircle2, ChevronDownIcon, Loader2, PauseCircle } from 'lucide-react';
...
export type ToolVisualStatus = 'running' | 'waiting' | 'done' | 'error' | undefined;
...
export function getToolVisualStatus(statusType?: string, isError?: boolean, hasResult?: boolean): ToolVisualStatus {
...
export function ToolShell({
  ...
  return (
    <Collapsible
      ...
      className={cn(
        'overflow-hidden rounded-md border border-border/60 bg-muted/20 border-l-2',
        toneClasses[tone] ?? toneClasses.default,
      )}
      style={{ '--animation-duration': `${ANIMATION_DURATION}ms` } as React.CSSProperties}
    >
      <CollapsibleTrigger
        className="group/trigger flex w-full items-center gap-2 px-3 py-2.5 text-left text-sm transition-colors hover:bg-muted/60"
      >
        <ChevronDownIcon
          className={cn(
