...
import {
  AGENT_PLAYGROUND_DEFAULT_MODEL_ID,
  AGENT_PLAYGROUND_DEFAULT_THINKING_LEVEL,
} from "./config";
import {
...
export function useAgentPlaygroundRuntime() {
  ...
  const [pendingWorkspaceEnvRequests, setPendingWorkspaceEnvRequests] =
    useState<PendingWorkspaceEnvRequest[]>([]);
  ...
  const loadExistingSession = useCallback(async (id: string) => {
    ...
    setIsRunning(
      state.session.status === "running" ||
        state.displayState?.isRunning === true,
    );
  }, []);
  ...
  useEffect(() => {
    ...
    const source = new EventSource(
      agentPlaygroundClient.getEventsUrl(sessionId),
    );
    ...
    const handleEvent = (message: MessageEvent) => {
      try {
        ...
        if (envRequest)
          setPendingWorkspaceEnvRequests((current) => [
            ...current.filter((item) => item.id !== envRequest.id),
            envRequest,
          ]);
        const clearEnvId = workspaceEnvClearIdFromEvent(event);
        if (clearEnvId)
          setPendingWorkspaceEnvRequests((current) =>
            current.filter((item) => item.id !== clearEnvId),
          );
  ...
  useEffect(() => {
    const target = selectedExample?.preview?.url
      ? previewFromEvent(
          {
            id: "selected-example",
            sessionId: sessionId ?? "",
  ...
  const ensureSession = useCallback(async () => {
    ...
    if (AGENT_PLAYGROUND_DEFAULT_THINKING_LEVEL !== "medium") {
      await agentPlaygroundClient.sendCommand(
        created.id,
        setThinkingLevelCommand(AGENT_PLAYGROUND_DEFAULT_THINKING_LEVEL as any),
      );
    }
    return created;
  }, [session]);
  ...
  const sendText = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        const prefixed = selectedExample
          ? `[Selected template: ${selectedExample.id}] ${trimmed}`
          : trimmed;
        const result = await agentPlaygroundClient.sendCommand(
          activeSession.id,
          sendMessageCommand(prefixed),
        );
        if (!result.accepted) {
  ...
  const runtime = useExternalStoreRuntime({
    ...
    onCancel: async () => {
      if (!sessionId) return;
      await agentPlaygroundClient.sendCommand(sessionId, {
        type: "abort",
        payload: {},
      });
      setIsRunning(false);
  ...
  const submitWorkspaceEnv = useCallback(
    async (input: SubmitWorkspaceEnvInput) => {
      ...
      await agentPlaygroundClient.sendCommand(
        sessionId,
        submitWorkspaceEnvCommand(input),
      );
      setPendingWorkspaceEnvRequests((current) =>
        current.filter((request) => request.id !== input.requestId),
      );
    },
    [sessionId],
  );

  const skipWorkspaceEnv = useCallback(
    async (requestId: string) => {
      if (!sessionId) return;
      await agentPlaygroundClient.sendCommand(
        sessionId,
        skipWorkspaceEnvCommand(requestId),
      );
      setPendingWorkspaceEnvRequests((current) =>
  ...
  const respondToApproval = useCallback(
    async (decision: "approve" | "decline" | "always_allow_category") => {
      ...
      await agentPlaygroundClient.sendCommand(
        sessionId,
        approveToolCallCommand(decision),
      );
