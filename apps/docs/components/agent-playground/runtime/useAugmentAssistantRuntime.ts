...
import { AugmentClient, augmentClient } from '@/components/agent-playground/augment/client';
import {
...
import type { AgentSession, ServerEvent, SessionCommand, SubmitWorkspaceEnvInput } from '@/components/agent-playground/augment/types';
...
export function useAugmentAssistantRuntime(client: AugmentClient = augmentClient) {
  const [store, setStore] = useState<AugmentAssistantStore>(initialAugmentAssistantStore);
  const [sessionReady, setSessionReady] = useState(false);
  const [eventLog, setEventLog] = useState<ServerEvent[]>([]);
  const [debugLog, setDebugLog] = useState<Array<{ id: number; at: string; message: string; data?: unknown }>>([]);
  ...
  const [pendingSend, setPendingSend] = useState<{ sessionId: string; text: string } | null>(null);
  ...
  const addDebugLog = useCallback((message: string, data?: unknown) => {
    ...
    const entry = { id: debugIdRef.current, at: new Date().toISOString(), message, data };
  ...
  const reconcileRunningFromServer = useCallback(async (id: string) => {
    try {
      const state = await client.getSessionState(id);
  ...
  const ensureThreadReady = useCallback(async (id: string) => {
    addDebugLog('ensureThreadReady:start', { sessionId: id });
    const state = await client.getSessionState(id);
    if (state.session.threadId) {
      addDebugLog('ensureThreadReady:already-ready', { sessionId: id, threadId: state.session.threadId });
      ...
      if (storeRef.current.session?.id === id && storeRef.current.threadId === state.session.threadId) {
        addDebugLog('ensureThreadReady:skip-rehydration', { sessionId: id, threadId: state.session.threadId });
    ...
    addDebugLog('ensureThreadReady:complete', { sessionId: id, threadId: refreshed.session.threadId });
    const hydrated = hydrateStoreFromSessionState(refreshed);
    debugCapture.captureStoreUpdate('hydrate:ensureThreadReady:refresh', hydrated);
    setStore(hydrated);
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      if (existingSessionId) {
        try {
          ...
          addDebugLog('bootstrap:stale-session-url-cleared', { sessionId: existingSessionId });
    ...
    bootstrap().catch((error) => {
      if (!cancelled) {
        addDebugLog('bootstrap:error', { error: error instanceof Error ? error.message : String(error) });
        setStore((current) => ({ ...current, lastError: error instanceof Error ? error.message : String(error) }));
        setSessionReady(true);
      }
    });
  ...
  }, [addDebugLog, client, ensureThreadReady, reloadState]);

  const onEvent = useCallback((event: ServerEvent) => {
    addDebugLog('sse:event', { id: event.id, type: event.type, threadId: event.threadId ?? null });
  ...
  const dispatchSessionCommand = useCallback(async (command: SessionCommand) => {
  ...
  const ensureSessionReady = async (): Promise<string> => {
    ...
    if (AUGMENT_DEFAULT_THINKING_LEVEL !== 'medium') {
      try {
        await client.sendCommand(session.id, setThinkingLevelCommand(AUGMENT_DEFAULT_THINKING_LEVEL));
  ...
  useEffect(() => {
    ...
    if (connectionState !== 'open') {
      addDebugLog('pendingSend:waiting-for-sse-open', { sessionId, connectionState });
      return;
    }
    ...
    setLastCommand(command);
    addDebugLog('pendingSend:send-command:start', { sessionId: pendingSend.sessionId, textLength: pendingSend.text.length });

    client.sendCommand(pendingSend.sessionId, command)
      .then((result) => {
        ...
        if (!result.accepted) {
          setStore((current) => ({
            ...current,
            isRunning: false,
            lastError: result.error ?? 'Message command was not accepted by the server.',
          }));
        }
      })
      .catch((error) => {
        ...
        setStore((current) => ({ ...current, isRunning: false, lastError: message }));
      })
      .finally(() => {
  ...
  const onNew = useCallback(async (message: AppendMessage) => {
    ...
    addDebugLog('onNew:start', { textLength: userText.length, currentSessionId: sessionId, connectionState });
    ...
    setStore((current) => ({
      ...current,
      messages: [...current.messages, createUserMessage(userText, [...(message.attachments ?? [])])],
      isRunning: true,
    }));
    setPendingSend({ sessionId: activeSessionId, text: sendText });
  }, [addDebugLog, connectionState, sessionId]);
  ...
  const approveTool = useCallback(async (approvalId: string, decision: ApprovalDecision) => {
    if (!sessionId) return;
    await dispatchSessionCommand(approveToolCallCommand(decision));
    setStore((current) => ({
      ...current,
      pendingApprovals: current.pendingApprovals[0]?.id === approvalId
  ...
  const respondToQuestion = useCallback(async (questionId: string, answer: string) => {
    if (!sessionId) return;
    await dispatchSessionCommand(respondToQuestionCommand(questionId, answer));
    setStore((current) => ({
      ...current,
      pendingQuestions: current.pendingQuestions.filter((question) => question.id !== questionId),
  ...
  const respondToToolSuspension = useCallback(async (resumeData: unknown) => {
    if (!sessionId) return;
  ...
  const submitWorkspaceEnv = useCallback(async (input: SubmitWorkspaceEnvInput) => {
    ...
    setStore((current) => ({
      ...current,
      pendingWorkspaceEnvRequests: current.pendingWorkspaceEnvRequests.filter((request) => request.id !== input.requestId),
    }));
  }, [dispatchSessionCommand, sessionId]);

  const continueWithoutWorkspaceEnv = useCallback(async (requestId: string) => {
    if (!sessionId) return;
    await dispatchSessionCommand(skipWorkspaceEnvCommand(requestId));
    setStore((current) => ({
      ...current,
      pendingWorkspaceEnvRequests: current.pendingWorkspaceEnvRequests.filter((request) => request.id !== requestId),
    }));
  }, [dispatchSessionCommand, sessionId]);
  ...
  const switchMode = useCallback(async (modeId: string) => {
    ...
    setStore((current) => current.session ? { ...current, session: { ...current.session, modeId } } : current);
  }, [dispatchSessionCommand, sessionId]);
  ...
  const setThinkingLevel = useCallback(async (level: ThinkingLevel) => {
    ...
    setStore((current) => (
      current.session ? { ...current, session: { ...current.session, thinkingLevel: level } } : current
    ));
  }, [dispatchSessionCommand, sessionId]);
    ...
    adapters: {
      ...
      threadList: {
        ...
        onSwitchToThread: async (threadId: string) => {
          if (!sessionId) return;
          await dispatchSessionCommand({ type: 'switchThread', payload: { threadId } });
  ...
  return useMemo(() => ({
    ...
    setThinkingLevel,
    setPendingPrefix,
  }), [approveTool, connectionState, continueWithoutWorkspaceEnv, debugLog, eventLog, hasLivePreview, lastCommand, lastCommandResult, respondToQuestion, respondToToolSuspension, runtime, sessionReady, setPendingPrefix, setThinkingLevel, store, submitWorkspaceEnv, switchMode]);
}
