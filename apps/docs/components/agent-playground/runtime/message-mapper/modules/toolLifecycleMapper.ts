...
export function appendToolArgsDelta(
  ...
  const toolCallId = String(
    payload.toolCallId ?? activeToolId(store) ?? `tool-${event.id}`,
  );
...
export function toolPartFromEvent(
  event: ServerEvent,
  ...
  return {
    ...
    result:
      options.status === "complete"
        ? payload.result
        : (payload.artifact ?? payload.result),
...
export function toolStatusFromActiveTool(
  activeTool: Record<string, any>,
): AssistantToolPart["status"] {
  const status = String(activeTool.status ?? "").toLowerCase();
  if (status === "completed" || status === "complete" || status === "done") {
    return {
      type: activeTool.isError ? "incomplete" : "complete",
      reason: activeTool.isError ? "error" : undefined,
    };
  }
  if (
    status === "waiting" ||
...
function activeToolId(store: AugmentAssistantStore): string | null {
  ...
  const lastTool = lastAssistant?.content
