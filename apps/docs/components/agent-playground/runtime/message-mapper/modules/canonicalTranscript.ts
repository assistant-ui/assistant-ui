...
export function partsFromPayloadMessage(
  payload: unknown,
): AssistantMessagePart[] {
  ...
  for (const rawPart of rawParts) {
    ...
    if (record.type === "tool_result" || record.type === "tool-result") {
      ...
      if (existingIndex !== -1) {
        ...
        if (DEBUG_MAPPING) {
          debugLog(
            `normalizeParts: merge tool_result`,
            `id=${toolCallId.slice(0, 12)} into existing tool_call at index ${existingIndex}`,
          );
        }
        normalized[existingIndex] = {
          ...
          status: {
            type: record.isError ? "incomplete" : "complete",
            reason: record.isError ? "error" : undefined,
          },
        };
      } else {
        if (DEBUG_MAPPING) {
          debugLog(
            `normalizeParts: orphan tool_result`,
            `id=${toolCallId.slice(0, 12)} name=${toolName} (no matching tool_call found)`,
          );
        }
        normalized.push({
          type: "tool-call",
  ...
  if (DEBUG_MAPPING) {
    ...
    if (tools.length > 0) {
      debugLog(
        ...
        `${tools.length} tool parts`,
        tools.map(
          (t: any) =>
            `${t.toolName}(${t.toolCallId.slice(0, 8)}, hasResult=${t.result !== undefined})`,
        ),
      );
    }
  }
...
export function normalizePart(part: unknown): AssistantMessagePart | null {
  ...
  if (record.type === "thinking")
    return { type: "reasoning", text: String(record.thinking ?? "") };
  ...
  if (record.type === "tool-call" || record.type === "tool_call") {
    return {
      ...
      argsText:
        typeof record.argsText === "string" ? record.argsText : undefined,
...
export function normalizeToolStatus(
  status: unknown,
): AssistantToolPart["status"] | undefined {
