...
export function mergeMessageParts(
  ...
  const incomingHasReasoning = incoming.some(
    (part) => part.type === "reasoning",
  );
  const incomingHasToolCalls = incoming.some(
...
export function mergeToolPart(
  ...
  return {
    ...
    ...incoming,
    toolName:
      incoming.toolName === "tool" ? existing.toolName : incoming.toolName,
    args,
    argsText:
      incoming.argsText && incoming.argsText !== "{}"
        ? incoming.argsText
        : existing.argsText,
