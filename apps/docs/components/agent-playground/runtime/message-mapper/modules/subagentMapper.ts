...
export function appendSubagentTextDelta(store: AugmentAssistantStore, event: ServerEvent): AugmentAssistantStore {
  const payload = asRecord(event.payload);
  const subagentId = subagentIdFromPayload(payload, event);
  const delta = String(payload.textDelta ?? payload.delta ?? payload.text ?? '');
  if (!delta) return store;

  const existing = findToolPart(store, subagentId);
  const currentText = typeof existing?.args.subagentText === 'string' ? existing.args.subagentText : '';
  const tool: AssistantToolPart = {
    type: 'tool-call',
    toolCallId: subagentId,
    toolName: 'subagent',
    args: mergeToolArgs(existing?.args ?? {}, { subagentText: `${currentText}${delta}` }),
...
export function subagentPartFromEvent(event: ServerEvent, options: { status: 'running' | 'complete' }): AssistantToolPart {
  const payload = asRecord(event.payload);
  const args: Record<string, unknown> = {};
  if (typeof payload.subagentName === 'string') args.subagentName = payload.subagentName;
  ...
  const subagentId = subagentIdFromPayload(payload, event);
  const result = options.status === 'complete'
...
export function appendNestedToolToSubagent(
  ...
  const messageIndex = store.messages.findIndex(
    (message) =>
      ...
      message.content.some((part) => part.type === 'tool-call' && part.toolCallId === parentId),
  );
  if (messageIndex === -1) return store;

  const message = store.messages[messageIndex]!;
  const parentIndex = message.content.findIndex((part) => part.type === 'tool-call' && part.toolCallId === parentId);
  if (parentIndex === -1) {
    return store;
  }
  ...
  const explicitSubId = typeof payload.subToolCallId === 'string' ? payload.subToolCallId : '';
  const legacySubId =
    typeof payload.toolCallId === 'string' && payload.toolCallId !== parentId
      ? payload.toolCallId
      : '';
  const runningMatch = status === 'complete'
    ? [...existingNestedTools].reverse().find((tool) => tool.toolName === subToolName && tool.status === 'running')
    : undefined;
  const subToolCallId =
    explicitSubId ||
    legacySubId ||
  ...
  const nestedTool: NestedToolInfo = {
    ...
    args: subToolArgs,
    result: status === 'complete' ? (payload.subToolResult ?? payload.result) : undefined,
    isError: status === 'complete' ? Boolean(payload.isError) : undefined,
    status,
  };

  const existingNestedIndex = existingNestedTools.findIndex((tool) => tool.toolCallId === subToolCallId);
  ...
  if (existingNestedIndex === -1) {
    ...
    updatedNestedTools[existingNestedIndex] = {
      ...
      result: nestedTool.result !== undefined ? nestedTool.result : existingNestedTool.result,
