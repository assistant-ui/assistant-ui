...
import {
  normalizeParts,
  partsFromPayloadMessage,
} from "./modules/canonicalTranscript";
...
export function hydrateStoreFromSessionState(
  ...
  const store: AugmentAssistantStore = {
    ...
    messages: (state.messages ?? [])
      ...
      .filter(Boolean) as AssistantThreadMessageLike[],
...
export function applyServerEvent(
  ...
  if (
    ...
    if (["message_update", "message_end"].includes(event.type)) {
      ...
      debugLog(
        `EVENT ${event.type}`,
        `msgId=${String(asRecord(p.message).id ?? "?").slice(0, 12)} raw=[${toolCalls.length} call, ${toolResults.length} result]`,
      );
      toolCalls.forEach((tc: any) => {
        debugLog(
          `  ├─ tool_call:`,
          `name=${tc.name ?? tc.toolName} id=${(tc.id ?? tc.toolCallId ?? "?").slice(0, 16)}`,
        );
      });
      toolResults.forEach((tr: any) => {
        debugLog(
          `  ├─ tool_result:`,
          `name=${tr.name ?? tr.toolName} id=${(tr.id ?? tr.toolCallId ?? "?").slice(0, 16)}`,
        );
      });
    } else {
      debugLog(
        `EVENT ${event.type}`,
        `toolCallId=${String(p.toolCallId ?? p.subagentId ?? "?").slice(0, 16)} toolName=${p.toolName ?? p.subToolName ?? "?"}`,
      );
    }
  }
  ...
  switch (event.type) {
    ...
    case "message_update":
      return upsertAssistantMessage(
        store,
        messageIdFromEvent(event),
        partsFromPayloadMessage(event.payload),
        true,
        false,
      );
    case "message_end":
    ...
    case "tool_input_start":
      return updateToolPartIfPresent(
        store,
        toolPartFromEvent(event, { status: "running" }),
      ).store;
    ...
    case "tool_end":
      return clearPendingQuestionForToolCall(
        clearPendingApprovalForToolCall(
          updateToolPartIfPresent(
            store,
            toolPartFromEvent(event, { status: "complete" }),
          ).store,
    ...
    case "subagent_start":
      return upsertToolPart(
        store,
        subagentPartFromEvent(event, { status: "running" }),
      );
    ...
    case "subagent_end":
      return upsertToolPart(
        store,
        subagentPartFromEvent(event, { status: "complete" }),
      );
    case "thread_created":
    ...
    case "thread_changed":
      return {
        ...store,
        threadId: stringAt(event.payload, ["threadId"]) ?? store.threadId,
      };
    case "mode_changed":
      return store.session
        ? {
            ...
            session: {
              ...store.session,
              modeId:
                stringAt(event.payload, ["modeId"]) ?? store.session.modeId,
            },
          }
        : store;
    case "usage_update":
    ...
    case "error":
      return {
        ...
        messages: appendErrorMessage(
          store.messages,
          errorMessageFromPayload(event.payload),
        ),
      };
    default:
      return store;
...
function normalizeStoredMessage(
  message: unknown,
): AssistantThreadMessageLike | null {
...
function applyAgentStart(store: AugmentAssistantStore): AugmentAssistantStore {
  const pendingFollowUps =
    !store.isRunning && store.pendingFollowUps.length > 0
...
function upsertAssistantMessage(
    ...
    if (existingMessageIndex === -1) {
      ...
      for (const toolResult of orphanToolResults) {
        ...
        if (existingTool) {
          ...
          const updateResult = updateToolPartIfPresent(
            updatedStore,
            mergedTool,
          );
          if (updateResult.found) {
            ...
            if (DEBUG_MAPPING) {
              debugLog(
                `upsertAssistantMessage: merged orphan result`,
                `toolCallId=${toolResult.toolCallId.slice(0, 12)} into existing tool (skipping new msg ${id.slice(0, 12)})`,
              );
            }
          } else {
      ...
      if (allResultsMerged) {
        if (DEBUG_MAPPING) {
          const allTools = updatedStore.messages.flatMap((m) =>
            m.content.filter((p) => p.type === "tool-call"),
          );
          debugLog(
            `STORE after merge (no new msg)`,
            `total=${allTools.length} tool parts:`,
            allTools.map(
              (t: any) => `${t.toolName}(${t.toolCallId.slice(0, 8)})`,
            ),
          );
        }
  ...
  const nextStore = removeToolPartsFromOtherMessages(
    store,
    id,
    incomingToolCallIds,
  );
  ...
  if (DEBUG_MAPPING) {
    ...
    debugLog(
      ...
      `msgId=${id.slice(0, 12)} incoming=${incomingTools.length} tools, migrated=${migratedTools.length} tools, authoritative=${authoritative}`,
    );
  }
  ...
  if (index === -1) {
    const result = {
      ...nextStore,
      messages: [...nextStore.messages, nextMessage],
      isRunning,
    };
    if (DEBUG_MAPPING) {
      const allTools = result.messages.flatMap((m) =>
        m.content.filter((p) => p.type === "tool-call"),
      );
      debugLog(
        `STORE after upsert (new msg)`,
        `total=${allTools.length} tool parts:`,
        allTools.map((t: any) => `${t.toolName}(${t.toolCallId.slice(0, 8)})`),
      );
    }
    return result;
  }
  ...
  const nextContent =
    migratedParts.length > 0
      ? mergeMessageParts(existingMessage.content, migratedParts, authoritative)
...
function addPendingFollowUp(
  ...
  const followUp: PendingFollowUp = {
    id,
    title:
      typeof payload.title === "string" ? payload.title : "Queued follow-up",
...
function addPendingWorkspaceEnvRequest(
  store: AugmentAssistantStore,
  event: ServerEvent,
): AugmentAssistantStore {
  const payload = asRecord(event.payload);
  const id =
    typeof payload.requestId === "string" ? payload.requestId : event.id;
  if (store.pendingWorkspaceEnvRequests.some((request) => request.id === id))
    return store;
  const requested = Array.isArray(payload.requested)
    ? (payload.requested
        .map((item): PendingWorkspaceEnvRequest["requested"][number] | null => {
          ...
          const envFile =
            record.envFile === ".env" || record.envFile === ".env.local"
              ? record.envFile
              : ".env.local";
          if (!name) return null;
          return {
  ...
  const request: PendingWorkspaceEnvRequest = {
    ...
    reason:
      typeof payload.reason === "string"
        ? payload.reason
        : "Add workspace environment values.",
    requested,
  };
  return {
    ...store,
...
function clearPendingWorkspaceEnvRequest(
  store: AugmentAssistantStore,
  event: ServerEvent,
): AugmentAssistantStore {
  const payload = asRecord(event.payload);
  ...
  const pendingWorkspaceEnvRequests = store.pendingWorkspaceEnvRequests.filter(
    (request) => request.id !== requestId,
  );
  return pendingWorkspaceEnvRequests.length ===
    store.pendingWorkspaceEnvRequests.length
    ? store
    : { ...store, pendingWorkspaceEnvRequests };
}
...
function applyPendingApprovalPayload(
  ...
  if (payload.toolName === "ask_user") {
    ...
    const tool = toolPartFromPayload(payload, fallbackId, {
      type: "requires-action",
      reason: "human-input",
    });
    const updated = updateToolPartIfPresent(store, tool);
    const withTool = updated.found
      ? updated.store
      : upsertToolPart(store, tool);
    return pendingQuestion
      ? applyPendingQuestion(withTool, pendingQuestion)
      : withTool;
  }
  ...
  if (withTool.pendingApprovals.some((item) => item.id === approval.id))
    return withTool;
  return {
    ...withTool,
    pendingApprovals: [...withTool.pendingApprovals, approval],
  };
}

function clearPendingApprovalForToolCall(
  store: AugmentAssistantStore,
  toolCallId: string,
): AugmentAssistantStore {
  if (!toolCallId) return store;
  const pendingApprovals = store.pendingApprovals.filter(
    (approval) => approval.toolCallId !== toolCallId,
  );
  return pendingApprovals.length === store.pendingApprovals.length
    ? store
    : { ...store, pendingApprovals };
}

function clearPendingQuestionForToolCall(
  store: AugmentAssistantStore,
  toolCallId: string,
): AugmentAssistantStore {
  if (!toolCallId) return store;
  const pendingQuestions = store.pendingQuestions.filter(
    (question) => question.toolCallId !== toolCallId,
  );
  return pendingQuestions.length === store.pendingQuestions.length
    ? store
...
function clearPendingInteractions(
  ...
  if (
    ...
    store.pendingQuestions.length === 0
  )
    return store;
...
function finalizeRunningAssistantMessages(
  store: AugmentAssistantStore,
): AugmentAssistantStore {
  let changed = false;
...
function addPendingQuestion(
  ...
  if (isCompletedTool(store, pendingQuestion.toolCallId ?? pendingQuestion.id))
    return store;
...
function applyDisplayStateRecord(
  store: AugmentAssistantStore,
  displayState: Record<string, any>,
): AugmentAssistantStore {
  let nextStore =
    typeof displayState.isRunning === "boolean"
      ? { ...store, isRunning: displayState.isRunning }
      : store;
  ...
  if (
    Object.keys(pendingSuspension).length > 0 &&
    !isCompletedTool(nextStore, pendingSuspensionToolCallId)
  ) {
    nextStore = applyToolSuspensionPayload(
      nextStore,
      pendingSuspension,
      pendingSuspensionToolCallId ?? crypto.randomUUID(),
    );
  }

  if (!nextStore.isRunning)
    return finalizeRunningAssistantMessages(
      clearPendingInteractions(nextStore),
    );
  ...
  if (
    ...
    !isCompletedTool(nextStore, pendingApprovalToolCallId)
  ) {
    nextStore = applyPendingApprovalPayload(nextStore, pendingApproval);
  }

  const pendingQuestion = pendingQuestionFromPayload(
    asRecord(displayState.pendingQuestion),
  );
  if (
    !pendingQuestion ||
    isCompletedTool(nextStore, pendingQuestion.toolCallId ?? pendingQuestion.id)
  )
    return nextStore;
...
function applyPendingQuestion(
  ...
  const toolCallId =
    ...
    latestToolCallIdByName(store, "ask_user") ??
    pendingQuestion.id;
  const withTool = upsertToolPart(store, {
    type: "tool-call",
    toolCallId,
  ...
  if (withTool.pendingQuestions.some((item) => item.id === pendingQuestion.id))
    return withTool;
  return {
    ...withTool,
    pendingQuestions: [...withTool.pendingQuestions, nextPendingQuestion],
  };
}

function applyToolSuspension(
  store: AugmentAssistantStore,
  event: ServerEvent,
): AugmentAssistantStore {
  const payload = asRecord(event.payload);
...
function latestToolCallIdByName(
  store: AugmentAssistantStore,
  toolName: string,
): string | null {
...
function addThread(
  ...
  const id =
    typeof thread.id === "string"
      ? thread.id
      : stringAt(event.payload, ["threadId"]);
  if (!id || store.threads.some((item) => item.id === id)) return store;
  return {
    ...store,
    threadId: id,
    threads: [
      { id, title: String(thread.title ?? id.slice(0, 8)), status: "regular" },
      ...store.threads,
    ],
  };
}

function isCompletedTool(
  store: AugmentAssistantStore,
  ...
  return Boolean(
    tool &&
      (tool.result !== undefined ||
        tool.status?.type === "complete" ||
        tool.status?.type === "incomplete"),
  );
}

function isSuccessfulWorkspaceTool(tool: AssistantToolPart): boolean {
  return (
    tool.toolName === "request_workspace" &&
    !tool.isError &&
    tool.status?.type === "complete" &&
    tool.result !== undefined
  );
