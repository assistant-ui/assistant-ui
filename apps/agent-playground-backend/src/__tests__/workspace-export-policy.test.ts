...
describe("workspace export policy", () => {
  it("excludes dependency, build, git, and env paths", () => {
    assert.equal(shouldExcludeRelativePath("node_modules/react/index.js"), true);
    ...
    assert.equal(shouldExcludeRelativePath("apps/web/.env.production.local"), true);
  ...
  it("builds tar excludes and safe filenames deterministically", () => {
    ...
    assert.equal(
      safeExportFilename({ productId: "Assistant UI!", sessionId: "ABCDEF1234567890" }),
