...
export function resolveModel(modelId: string): LanguageModel {
  ...
  if (slashIndex === -1) {
    ...
    throw new Error(
      `Invalid model ID "${modelId}". Expected "provider/model-name".`,
    );
  }
  ...
  if (!factory)
    throw new Error(
      `Unknown model provider "${provider}". Available: ${Object.keys(PROVIDERS).join(", ")}`,
    );

  const envVar = PROVIDER_ENV_VARS[provider];
  if (envVar && !process.env[envVar]) {
    throw new Error(
      `Missing API key for provider "${provider}". Set ${envVar}.`,
    );
  }
...
export function getDynamicModel({
  ...
  const ctx = requestContext?.get?.("harness") as
    | HarnessRequestContext<any>
    | undefined;
  const modelId =
    ctx?.getState?.()?.currentModelId ??
