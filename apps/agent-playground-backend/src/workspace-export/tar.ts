...
import { shouldExcludeRelativePath, toRelativeArchivePath } from "./policy.js";
...
function createTarHeader(options: {
  ...
  const name =
    options.type === "directory" && !options.name.endsWith("/")
      ? `${options.name}/`
      : options.name;
  let headerName = name;
  let headerPrefix = "";
  ...
  if (Buffer.byteLength(name) > 100) {
    ...
    while (
      (Buffer.byteLength(headerName) > 100 ||
        Buffer.byteLength(headerPrefix) > 155) &&
...
function paddingFor(size: number): Buffer {
  ...
  return remainder === 0
    ? Buffer.alloc(0)
    : Buffer.alloc(BLOCK_SIZE - remainder, 0);
}
