...
async function withRetry<T>(fn: () => Promise<T>): Promise<T> {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      const retryable = message.includes('fetch failed') ||
...
export async function exportBlaxelWorkspace(
  ...
  if (!sandbox?.process?.exec || !sandbox?.fs?.download) {
    throw new WorkspaceExportError('Sandbox provider does not expose Blaxel archive export APIs.', 501);
  }
  ...
  try {
    const result: any = await withRetry(() => sandbox.process.exec({
      command,
    ...
    await withRetry(() => sandbox.fs.download(remoteArchivePath, localArchivePath));
