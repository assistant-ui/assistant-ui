...
const BASE_RECIPES: Array<Omit<Recipe, 'productId'>> = [
...

  {
    ...
    capabilities: ['basic-chat', 'thread-list', 'persistent-threads', 'cloud-auth'],
    preview: { status: 'missing' },
    env: [
      {
        name: 'OPENAI_API_KEY',
...

  {
    ...
    capabilities: ['basic-chat', 'thread-list', 'persistent-threads', 'custom-backend'],
    preview: { status: 'missing' },
    env: [
      {
...

  {
    ...
    agent: {
      ...
      avoidWhen: [
        'user does not need reasoning display',
      ],
      keyFiles: {
        customizeUi: ['components/assistant-ui/thread.tsx'],
        changeModel: ['app/api/chat/route.ts'],
    ...
    env: [
      {
        ...
        scope: 'server',
        secret: true,
        description: 'OpenAI API key (used in demo, replaceable with any backend).',
      },
    ],
    agent: {
      useWhen: [
...

  {
    ...
    agent: {
      ...
      avoidWhen: [
        'user is fine with the default sidebar (use next-default)',
      ],
      keyFiles: {
        customizeUi: [
          'components/assistant-ui/thread.tsx',
...

  {
    ...
    agent: {
      ...
      avoidWhen: [
        'user wants simple linear chat (use next-minimal)',
      ],
      keyFiles: {
        customizeUi: ['components/assistant-ui/thread.tsx'],
        changeModel: ['app/api/chat/route.ts'],
...

  {
    ...
    agent: {
      ...
      limitations: [
        'Form schema must be defined by the developer',
      ],
    },
    verifyProfile: 'next',
  },

  {
    ...
    agent: {
      ...
      limitations: [
        'Requires running LangGraph server',
      ],
    },
    verifyProfile: 'next',
  },

  {
    ...
    agent: {
      ...
      limitations: [
        'Requires a running AG-UI compatible agent',
      ],
    },
    verifyProfile: 'next',
  },

  {
    ...
    agent: {
      ...
      avoidWhen: [
        'user wants a simpler cloud setup (use next-cloud template)',
      ],
      keyFiles: {
        customizeUi: ['components/assistant-ui/thread.tsx'],
        changeModel: ['app/api/chat/route.ts'],
...

  {
    ...
    agent: {
      ...
      avoidWhen: [
        'user does not need voice features',
      ],
      keyFiles: {
        customizeUi: ['components/assistant-ui/thread.tsx'],
        changeModel: ['app/api/chat/route.ts'],
      ...
      limitations: [
        'Requires ElevenLabs API access for speech features',
      ],
    },
    verifyProfile: 'next',
  },
  ...
  },
  {
    ...
    agent: {
      ...
      avoidWhen: [
        'user does not need media processing',
      ],
      keyFiles: {
        customizeUi: ['components/assistant-ui/thread.tsx'],
        changeModel: ['app/api/chat/route.ts'],
