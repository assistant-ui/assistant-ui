...
export function resolveWorkspaceAppPath(
  ...
  const workspaceRoot =
    ...
    (provisioned.providerKind === "sandbox" ? "/workspace" : undefined);
  if (!workspaceRoot)
    throw new Error(
      "Cannot determine workspace root for environment file operations.",
    );
  ...
  if (usesWindowsPaths(workspaceRoot)) {
    ...
    if (
      ...
      throw new Error(
        `Environment app path must stay inside the workspace root: ${appPath}`,
      );
    }
    return resolved;
  }
...
export async function getWorkspaceEnvStatus(options: {
  ...
  for (const envFile of ENV_FILES) {
    detectedByFile.set(
      envFile,
      await readEnvFilePresence(
        filesystem,
        joinWorkspacePath(options.appPath, envFile),
      ),
    );
  }
  ...
  for (const expected of options.expected ?? []) {
    ...
    byKey.set(expected.name, {
      ...
      hasValue:
        detectedByFile.get(envFile)?.get(expected.name) ??
        hasValueInAnyFile(detectedByFile, expected.name),
      envFile,
      source: "recipe",
...
function getFilesystem(
  workspace: Workspace,
): NonNullable<Workspace["filesystem"]> {
  const filesystem = workspace.filesystem;
  if (!filesystem)
    throw new Error("Current workspace has no filesystem provider.");
  return filesystem;
}
...
async function readEnvFilePresence(
...
function hasValueInAnyFile(
  ...
  return ENV_FILES.some(
    (envFile) => detectedByFile.get(envFile)?.get(name) === true,
  );
}

function compareEnvStatusVars(
  a: WorkspaceEnvStatusVar,
  b: WorkspaceEnvStatusVar,
): number {
