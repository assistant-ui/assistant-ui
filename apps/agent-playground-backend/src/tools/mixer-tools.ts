...
import {
  listRecipes,
  getRecipe,
} from '../mixer/catalog/index.js';
import type { PreviewMetadata } from '../mixer/catalog/types.js';
import type { ExamplePreviewStatus } from '../preview/types.js';
import { DEFAULT_SANDBOX_WORKSPACE_ROOT, getReservedWorkspaceEntries } from '../sandbox-templates/index.js';
...
function getWorkspaceRoot(provisioned: { workspacePath?: string; providerKind?: 'local' | 'sandbox' }) {
  return provisioned.workspacePath ?? (provisioned.providerKind === 'sandbox' ? SANDBOX_WORKSPACE_ROOT : undefined);
}
...
async function validateWorkspaceRootEmpty(workspaceFs: any, workspaceRoot: string): Promise<string[]> {
...
async function ensureWorkspaceDirs(workspaceFs: any, relativePath: string, workspaceRoot: string): Promise<void> {
...
async function copyScaffoldIntoWorkspace(options: {
  ...
  for (const relativePath of files) {
    await ensureWorkspaceDirs(options.workspaceFs, relativePath, options.workspaceRoot);
    ...
    const targetPath = options.workspaceRoot.endsWith('/')
      ...
      : `${options.workspaceRoot}/${relativePath}`;
    await options.workspaceFs.writeFile(targetPath, content, { recursive: true });
  }
...
async function seedPreinstalledNodeModules(options: {
  ...
  if (options.provisioned.providerKind !== 'sandbox' || !options.provisioned.sandboxInstance) {
    return {
      attempted: false,
      status: 'not_applicable',
      message: 'Preinstalled dependency seeding is only available for sandbox workspaces.',
    };
  }
  ...
  try {
    ...
    if (exitCode === 0) {
      return {
        attempted: true,
        status: 'seeded',
        message: output || 'Seeded node_modules from the preinstalled sandbox image directory.',
        durationMs,
      };
    }
    ...
    if (exitCode === 2 || exitCode === 3) {
      return {
        attempted: true,
        status: 'skipped',
        message: output || `Dependency seed skipped with exit code ${exitCode}.`,
        durationMs,
      };
    }
...
function summarizeScaffoldedFiles(files: string[]) {
  const topLevelEntries = [...new Set(files.map((file) => file.split('/')[0]).filter(Boolean))].sort();
  const markdownFiles = files
    .filter((file) => file.toLowerCase().endsWith('.md'))
    .map((file) => file.replaceAll('\\', '/'));
  const preferredGuide = markdownFiles.find((file) => file.toLowerCase().endsWith('/readme.md'))
    ...
    try {
      ...
      try {
        ...
        return {
          ...
          markdownFiles: summary.markdownFiles.map((file) => toWorkspacePath(workspaceRoot, file)),
          guideFile: summary.preferredGuide ? toWorkspacePath(workspaceRoot, summary.preferredGuide) : undefined,
...
export const showUiPreview = createTool({
  ...
  inputSchema: z.object({
    ...
    status: z
      ...
      .describe('Preview availability from the catalog preview registry or normalized runtime state'),
    previewUrl: z
      .string()
      .optional()
      .describe(
