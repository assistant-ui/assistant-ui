...
- The task is self-contained and can be delegated`,
    inputSchema: z.object({
      agentType: z
        .enum(subagentIds as [string, ...string[]])
      ...
      modelId: z
        .string()
        .optional()
        .describe("Optional model ID override for this task."),
    }),
    execute: async ({ agentType, task, modelId }, context) => {
      ...
      const harnessModelId =
        harnessCtx?.getSubagentModelId?.({ agentType }) ?? undefined;
      ...
      if (!resolvedModelId) {
        return {
          content:
            "No model ID available for subagent. Configure defaultModelId.",
          isError: true,
        };
      }
      ...
      const allowedWs = definition.allowedWorkspaceTools
      ...
      if (context?.requestContext) {
        subagentRequestContext = new RequestContext(
      ...
      try {
        const response = await subagent.stream(task, {
          maxSteps:
            definition.maxSteps ?? (definition.stopWhen ? undefined : 50),
          ...
          switch (chunk.type) {
            ...
            case "tool-result": {
              ...
              for (let i = toolCallLog.length - 1; i >= 0; i--) {
                if (
                  toolCallLog[i]?.toolCallId === chunk.payload.toolCallId &&
        ...
        if (abortSignal?.aborted) {
          ...
          return {
            content:
              abortResult +
              buildSubagentMeta(resolvedModelId, durationMs, toolCallLog),
            isError: false,
          };
        }
