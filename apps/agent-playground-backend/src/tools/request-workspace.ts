...
export const requestWorkspaceTool = createTool({
  ...
  inputSchema: z.object({
    reason: z.string().describe("Brief reason why workspace access is needed"),
  }),
  execute: async (_input, context) => {
    ...
    if (!harnessCtx) {
      return {
        ...
        message:
          "Cannot access harness context. Workspace cannot be provisioned.",
      };
    }
    ...
    const workspacePath =
      provisioned.workspacePath ?? state?.projectPath ?? process.cwd();
