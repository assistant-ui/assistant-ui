...
pkg.pr.new publishes installable package previews for a PR only while it carries the `preview` label. Add the label to start publishing, and every later push updates the preview until the label is removed.
