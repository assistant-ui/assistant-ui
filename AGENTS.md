...
Run `pnpm check:resource-memo` when bumping `@babel/core`, `babel-plugin-react-compiler`, or `react-compiler`; a green build does not prove the compiler toolchain is intact. A package that published code or declarations import belongs in `dependencies`, not `devDependencies`: `aui-build` keeps every package import external and fails the build when anything from `node_modules` would be bundled, so a devDependency import ships as a specifier consumers cannot resolve. A registry item must be self-contained: enumerate every `@/components/*` import and CSS `@import` as `registryDependencies`, so `shadcn add` never lands a file with an unresolvable import.
