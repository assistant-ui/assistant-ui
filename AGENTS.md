...
- **Size budgets gate.** `pnpm size:check` bundles every published entry with rolldown (all bare imports external, minified, gzip) and compares it with `size-budgets.json`. A move past max(2%, 256 B) in either direction fails. The file describes `main`: autofix.ci builds every package on each PR and runs `pnpm size:update`, which records the measured size of every entry of the packages the branch changed against `origin/main` (a change to `x-buildutils` counts for every package), so the diff rides in the PR as exactly what the change cost. A move on a package the branch never touched is reported and kept, because a dist the branch never rebuilt is not its claim to make; `pnpm size:update:all` re-records every entry, which is what a re-baseline on `main` needs.
