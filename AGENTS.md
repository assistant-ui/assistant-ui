...
A peerDependency on a package this workspace releases is load-bearing, not decorative: `scripts/check-changeset-semver.mjs` reads `dependencies` and `peerDependencies` into one graph to compute the release cascade. Because `.changeset/config.json` sets `onlyUpdatePeerDependentsWhenOutOfRange`, `changeset version` never rewrites a peer range that is still satisfied, so a hand-written floor there is the one published range nothing keeps current. Declare it as `workspace:^`, which publishes as `^<the version released alongside it>`: `@assistant-ui/core`, `@assistant-ui/store`, and `@assistant-ui/tap` reach a consumer through a distribution package rather than a direct install, so lock-step is the truth and `scripts/check-workspace-ranges.mjs` enforces it. A peer the consumer installs themselves (`@assistant-ui/react`, `assistant-cloud`, `react`) keeps a wide floor, raised only when the code requires a newer API.
