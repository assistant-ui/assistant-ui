...
interface MastraEventHandler {
  (event: MastraEvent): void | Promise<void>;
}

interface MastraHealthCheck {
...
declare enum MastraKnownEventTypes {
  Message = "message",
...
declare class MastraMessageAccumulator<TMessage extends {
  id?: string;
}> {
  private messagesMap;
  ...
  constructor(_param0?: MastraStateAccumulatorConfig<TMessage> & {
    maxMessages?: number;
  });
...
declare const MastraMessageConverter: {
  callback: useExternalMessageConverter.Callback<MastraMessage>;
};

type MastraMessageStatus = "complete" | "incomplete" | "requires-action" | "running";

type MastraRole = "assistant" | "human" | "system" | "tool";

type MastraRuntimeConfig = ExternalStoreSharedOptions & {
  agentId: string;
  api?: string;
...
type MastraStateAccumulatorConfig<TMessage> = {
  initialMessages?: TMessage[];
  appendMessage?: (prev: TMessage | undefined, curr: TMessage) => TMessage;
  onMessageUpdate?: (message: TMessage) => void;
};

type MastraStreamCallback = (messages: MastraMessage[], config: MastraSendMessageConfig & {
  abortSignal: AbortSignal;
}) => Promise<AsyncIterable<MastraEvent>> | AsyncIterable<MastraEvent>;

type MastraThreadState = {
  id: string;
...
type MastraToolCallStatus = "complete" | "input-available" | "input-streaming" | "output-available" | "output-error";

type MastraToolResult = {
  tool_call_id: string;
  name?: string;
  result?: unknown;
  error?: string;
...
interface MastraWorkflowCommand {
  resume?: unknown;
  transition?: string;
...
interface MastraWorkflowInterrupt {
  id: string;
  state: string;
  context: Record<string, unknown>;
...
interface MastraWorkflowState {
  id: string;
  current: string;
  status: "completed" | "error" | "running" | "suspended";
  context: Record<string, unknown>;
...
interface MastraWorkflowTransition {
  from: string;
  to: string;
  event: string;
  timestamp: string;
...
declare const appendMastraChunk: (prev: MastraMessage | undefined, curr: MastraMessage) => MastraMessage;

declare const checkHealthThresholds: (health: MastraHealthCheck) => {
  isHealthy: boolean;
  warnings: string[];
  errors: string[];
};

declare const convertMastraMessage: useExternalMessageConverter.Callback<MastraMessage>;

declare const createHealthMonitor: (monitorOptions?: {
  onUnhealthy?: (health: MastraHealthCheck, thresholds: ReturnType<typeof checkHealthThresholds>) => void;
  onError?: (error: unknown) => void;
}) => {
  check: (options?: HealthCheckOptions) => Promise<MastraHealthCheck | {
    status: string;
    timestamp: string;
    metrics: {
      memoryUsage: number;
  ...
  getLastCheck: () => MastraHealthCheck | null;
  getStats: () => {
    checkCount: number;
    errorCount: number;
    errorRate: number;
...
declare const createMastraFetchStream: (params: {
  api: string;
  agentId: string;
}) => MastraStreamCallback;

declare const extractMastraToolCalls: (message: MastraMessage) => MastraToolCall[];

declare const extractMastraToolResults: (message: MastraMessage) => MastraToolCall[];

declare const getMastraMessageStatus: (message: MastraMessage) => MastraMessageStatus;
...
declare namespace entry_health_exports {
  export { HealthCheckOptions, MastraHealthCheck, checkHealthThresholds, createHealthMonitor, performHealthCheck };
}

declare namespace entry_root_exports {
  export { HealthCheckOptions, MastraContent, MastraEvent, MastraEventHandler, MastraHealthCheck, MastraInterruptState, MastraKnownEventTypes, MastraMemoryConfig, MastraMemoryQuery, MastraMemoryResult, MastraMessage, MastraMessageAccumulator, MastraMessageConverter, MastraMessageStatus, MastraRuntimeConfig, MastraRuntimeExtras, MastraSendMessageConfig, MastraStreamCallback, MastraThreadState, MastraToolCall, MastraToolCallStatus, MastraToolResult, MastraWorkflowCommand, MastraWorkflowConfig, MastraWorkflowInterrupt, MastraWorkflowState, MastraWorkflowTransition, appendMastraChunk, checkHealthThresholds, convertMastraMessage, createHealthMonitor, createMastraFetchStream, extractMastraToolCalls, extractMastraToolResults, getMastraMessageStatus, isMastraPartialMessage, parsePartialToolCall, performHealthCheck, useMastraExtras, useMastraMemory, useMastraMessages, useMastraRuntime, useMastraWorkflowInterrupt, useMastraWorkflows };
}

declare const isMastraPartialMessage: (message: Partial<MastraMessage>) => boolean;

declare const parsePartialToolCall: (chunk: Partial<MastraToolCall>, existing?: MastraToolCall) => MastraToolCall;

declare const performHealthCheck: (options?: HealthCheckOptions) => Promise<MastraHealthCheck>;
...
declare const useMastraExtras: {
  (): MastraRuntimeExtras;
  <S>(select: (extras: MastraRuntimeExtras) => S): S;
  <S>(select: (extras: MastraRuntimeExtras) => S, fallback: S): S;
};

declare const useMastraMemory: (config: MastraMemoryConfig) => {
  threads: Map<string, MastraThreadState>;
  currentThread: string | null;
  isSearching: boolean;
  searchMemory: (query: MastraMemoryQuery) => Promise<MastraMemoryResult[]>;
  getThreadContext: (threadId: string) => Promise<MastraThreadState>;
  createThread: (metadata?: Record<string, any>) => Promise<string>;
  deleteThread: (threadId: string) => Promise<void>;
  updateThreadMetadata: (threadId: string, metadata: Record<string, any>) => Promise<void>;
  setCurrentThread: import("react").Dispatch<import("react").SetStateAction<string | null>>;
};

declare namespace entry_memory_exports {
  export { useMastraMemory };
}

declare const useMastraMessages: <TMessage extends MastraMessage = MastraMessage>(_param4: {
  stream: MastraStreamCallback;
  appendMessage?: (prev: TMessage | undefined, curr: TMessage) => TMessage;
  eventHandlers?: {
    onMetadata?: (metadata: Record<string, unknown>) => void;
  ...
  isRunning: boolean;
  interrupt: MastraInterruptState | undefined;
  sendMessage: (newMessages: TMessage[], config?: MastraSendMessageConfig) => Promise<void>;
  cancel: () => void;
  clearMessages: () => void;
  replaceMessages: (nextMessages: TMessage[]) => void;
  resetInterrupt: () => void;
};

declare const useMastraRuntime: (config: MastraRuntimeConfig) => AssistantRuntime;

declare const useMastraWorkflowInterrupt: () => {
  interrupt: MastraWorkflowInterrupt | null;
  setWorkflowInterrupt: import("react").Dispatch<import("react").SetStateAction<MastraWorkflowInterrupt | null>>;
};

declare const useMastraWorkflows: (config: MastraWorkflowConfig) => {
  workflowState: MastraWorkflowState | null;
  isRunning: boolean;
  isSuspended: boolean;
  startWorkflow: (initialContext?: Record<string, unknown>) => Promise<MastraWorkflowState>;
  suspendWorkflow: () => Promise<MastraWorkflowState | undefined>;
  resumeWorkflow: (input?: unknown) => Promise<MastraWorkflowState | undefined>;
  sendCommand: (command: MastraWorkflowCommand) => Promise<MastraWorkflowState | undefined>;
  transitionTo: (targetState: string, context?: Record<string, unknown>) => Promise<MastraWorkflowState | undefined>;
};

declare namespace entry_workflows_exports {
  export { useMastraWorkflowInterrupt, useMastraWorkflows };
}

export { entry_health_exports as entry_health, entry_memory_exports as entry_memory, entry_root_exports as entry_root, entry_workflows_exports as entry_workflows };
