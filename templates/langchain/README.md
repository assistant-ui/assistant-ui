...
The bundled proxy rejects browser requests marked cross-site so the local starter works without exposing `LANGCHAIN_API_KEY` to the client. Request-context checks are not user authentication. Before deploying with a cloud API key, require your application session in `app/api/[..._path]/route.ts` and apply a durable rate limit.
